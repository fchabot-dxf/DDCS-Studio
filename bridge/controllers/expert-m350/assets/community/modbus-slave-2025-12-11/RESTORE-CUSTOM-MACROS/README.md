...
- **sysstart.nc** — boot: **per-axis** homing (`M98P501X2/X0/X1`, not `M115`), then the A→Y sync, and **zero B in place** (`#884=0` + `#1519=1`, no homing move). Per-axis is required: `M98P501X<n>` sets `#1` (the axis index) via the `X`-arg, so factory O501's `#[1920+#1]` resolves. `M115` at boot leaves `#1` dirty → "Macro address does not exist" error. Ends with `M30`.

...
| Live inbound channel to a running program | ✅ via the **program file** — `M47` self-loop re-reads disk each cycle, PC overwrites it `[CONFIRMED]`; `uservar` vars do NOT | Modbus serial (`MGETDATA`) `[CONFIRMED via docs]` — ⚠️⚠️ **`mgetdata.nc` live test HARD-WEDGED → required a REBOOT** `[CONFIRMED 2026-06-06]` (slave must answer the exact reg/func/slave-id or controller hangs forever; `#1630`-class wedge). **`MSETDATA` (outbound push) is the proven-safe direction** |
| Software dispatcher (1 bootstrap Start, then SMB-fed jobs) | ✅ `[CONFIRMED]` `M47` loop + overwrite file; no per-job trigger | ❌ **file-overwrite/self-loop dispatcher does NOT port** `[RESOLVED 2026-06-06]`: Expert `M47`=count/pause; `IF/GOTO` & `M98` loop in RAM (don't re-read disk). Use `sysstart` boot-bootstrap + **Modbus `MGETDATA`** for live inbound instead |

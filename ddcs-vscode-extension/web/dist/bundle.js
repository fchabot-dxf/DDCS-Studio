...
function fillLevelMoves(rows, ctx) {
...
function contourLevel(contours, ctx) {
  const { z, clr, feed, plunge } = ctx;
  const L2 = [];
  for (const c2 of contours) {
...
function concentricRect(x0, y0, x1, y1, step, ctx) {
  const { z, clr, feed, plunge } = ctx;
  const L2 = [];
...
function concentricCircle(cx, cy, Rc, step, ctx) {
  const { z, clr, feed, plunge } = ctx;
  const L2 = [];
...
function zigzagFill(contours, step, ctx, opts = {}) {
  const { z, clr, feed, plunge } = ctx;
...
function concentricFill(rg, step, ctx, opts = {}) {
  const { z, clr, feed, plunge } = ctx;
...
function fillStrategy(p, z) {
  ...
  if (p.strategy === "concentric")
    return rg.kind === "circle" ? concentricCircle(rg.cx, rg.cy, rg.r, step, ctx) : concentricRect(rg.x, rg.y, rg.x + rg.w, rg.y + rg.h, step, ctx);
  ...
  return fillLevelMoves(rows, ctx);
}
...
var catSlug = (c2) => (c2 || "Ops").toLowerCase().replace(/\s+/g, "");
var FN = (field2) => field2.toUpperCase();
var REPORTER_CHECK = { boolean: "Boolean", region: "Region" };
var outputCheck = (def) => REPORTER_CHECK[def.returns] || "Number";
var isWrap = (def) => ["container", "path", "loop", "cond", "depth", "fill"].includes(def.kind);
var fieldsOf = (def, params) => (def.fieldsFor ? def.fieldsFor(params || def.defaults) : def.fields) || [];
var DESCRIPTIONS = {
...
var getDesc = (f) => DESCRIPTIONS[f.toLowerCase()] || `The ${f} parameter`;
var optionsFor = (def, field2) => {
  if (field2 === "op") return def.type === "compare" ? ["<", ">", "<=", ">=", "==", "!="] : ["+", "-", "*", "/", "%"];
  if (field2 === "mode") {
  ...
  return SELECTS[field2] || null;
};
function fieldKind(def, field2) {
  if (optionsFor(def, field2)) return "dropdown";
  const sock = def.sockets && def.sockets[field2];
  if (sock === "region") return "region";
  if (sock === "boolean") return "boolean";
  const d = def.defaults[field2];
...
var makeOpDef = (type, label, msgAdd = "", argsAdd = []) => ({
  type,
  message0: `\u2B21 %1 ${msgAdd}`,
  args0: [
...
var cornerView = {
  ...
  update(ctx) {
    ...
    ctx.preview3D(gcode, "cornerVizContainer", wizard3.inferStart(params, stock));
...
var middleView = {
  ...
  async update(ctx) {
    ...
    const stock = window.ddcsGetSettings && window.ddcsGetSettings().stock || {};
    ctx.preview3D(gcode, "middleVizContainer", wizard4.inferStart(params, stock));
...
var circularView = {
  ...
  update(ctx) {
    ...
    const stock = window.ddcsGetSettings && window.ddcsGetSettings().stock || {};
    ctx.preview3D(gcode, "circularVizContainer", wizard5.inferStart(params, stock));
...
var rotaryCenterView = {
  ...
  update(ctx) {
    ...
    const stock = window.ddcsGetSettings && window.ddcsGetSettings().stock || {};
    ctx.preview3D(gcode, "rotaryCenterVizContainer", wizard6.inferStart(params, stock));
...
var rotaryClockView = {
  ...
  update(ctx) {
    ...
    const stock = window.ddcsGetSettings && window.ddcsGetSettings().stock || {};
    ctx.preview3D(gcode, "rotaryClockVizContainer", wizard7.inferStart(params, stock));
...
var edgeView = {
  ...
  update(ctx) {
    ...
    const stock = window.ddcsGetSettings && window.ddcsGetSettings().stock || {};
    ctx.preview3D(gcode, "probeVizContainer", wizard8.inferStart(params, stock));
...
function applyState(input, btn, field2) {
  const src = probeSrc(field2);
...
function decorateInput(inputId, field2) {
  const input = el(inputId);
  if (!input || !probeSrcAvailable(field2)) return;
  ...
  if (!btn) {
    ...
    btn.addEventListener("click", () => {
      setProbeSrc(field2, probeSrc(field2) ? "studio" : "ctrl");
      applyState(input, btn, field2);
    });
  }
  applyState(input, btn, field2);
}
function decorateProbeSrc(view) {
  ...
  if (!map) return;
  for (const [inputId, field2] of Object.entries(map)) decorateInput(inputId, field2);
}
...
var GcodeExecutionEngine = class _GcodeExecutionEngine {
  ...
  _executeStep(step) {
    ...
    const setAxis = (key, field2) => {
      ...
      target[field2] = this.absolute || g53 ? value * this.unitScale : this.pos[field2] + value * this.unitScale;
    };
    setAxis("X", "x");
    setAxis("Y", "y");
    setAxis("Z", "z");
...
var _pop = null;
var _anchor = null;
var esc = (v6) => String(v6).replace(/[<>&]/g, (c2) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" })[c2]);
function tplLabel(t) {

...
1. **[M] One diff at 3 surfaces** — re-base `isOpBlockEdited`/glow on a per-op reverse-synced reconstruction (`RECONCILERS[op]`→`BUILDERS`→diff) so glow ⟺ chip ⟺ Merge/Replace notice all mean exactly "Replace would lose something." Today a *surfaced* param edit still trips the notice (forward-only over-reports). Precision rides on RECONCILERS coverage; fails safe. `opGlow.js`, `opSession.js`, `editorOpHover.js`.
...
3. **[M] Region editor v1.x — poly/freeform point editing** — runtime/bake already render polygons; the authoring UI is rect-only. Add a poly/freeform create mode + per-vertex handles in `shapeStage.stageSvg` + a freehand trace+simplify. `ui/regionEditor.js`, `ui/shapeStage.js`.

...
3. **[XL] Wizards-as-data Stage 5 — IN PROGRESS** — port the rest of the built-ins to data, one-by-one, each gated by output-equivalence + binding-wiring (reuse `dataOps/equivalence.js`); never batch. Shipped: **`atc_warmup`** (static-shape; the `stripAnnotations` FUNCTIONAL-equivalence normalizer) and the **placement-bbox fix** (below).
   - **⚑ Frontier-coverage map (all 19 remaining built-ins classified, 2026-06-26):** `atc_warmup` was the ONLY "free" (pure/cosmetic-only) port — **every other op hits a FUNCTIONAL blocker.** Closest are `surfacing`/`contour` (blocked by *nothing but* placement-bbox). Unportable (conditional structure dominates): `pocket`, `middle`, `corner`, `circular`, `rotary_center`, `comm`, `homing`, `atc_change/test/table`. Blocker leverage (ops gated): conditional-STRUCTURE 13 · computed-ADDRESS/value 14 · computed-TEXT 15 (cosmetic) · ifgoto-var 8 · fan-out 6 · placement-bbox 5 · loop 4 · conditional-TYPE 2.

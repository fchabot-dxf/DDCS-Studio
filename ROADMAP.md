...
4. **Wizards-as-Data Architecture & Layout Splitters:**
   - ⭐⭐ **✅ SHIPPED — the "future session" this section deferred to HAPPENED (2026-09-04→06, turns t2595–t2665, releases V2026.09.04.1 → V2026.09.06.6, all merged to `main`/live).** **All 32 built-in-equivalent ops now declare the `split_horizontal` tree layout** (BACKLOG #72, Phase 1 complete) — corner included, WITH its REDIVIDE concern-sections composed inside the split (t2635) and all four of its mouths holding real content (t2655). **Authoring is PROVEN, not just expressed:** a wizard was built from scratch through the real UI to full built-in parity — form, preview3d, feature_canvas, a drag handle writing back to the field AND the emit — in BOTH binding modes (t2663/t2665), and the 10-gap authoring list that test produced is closed 8 fixed / 1 deliberate-documented / 1 owner-parked (drag feel). Standing cross-op guards now hold the parity: section-order, fold-threshold (`sectionizeFor`), the emit lint (32 ops × 7 posts), canvas-mount.
   - **⏳ IN FLIGHT — Phase 2, gestures-as-blocks (the owner's original "step 3"):** each migrated op's canvas HANDLES expressed as blocks in its `feature_canvas` mouth. Piloting on corner (t2675); measured board: 1 ready · 7 blocked on a live-anchor/conditional-target primitive (declaration being designed, owner rules the shape) · slot on the deliberately-unbuilt `translate` kind · 23 ops have no interactive handle. The Phase 2 board lives in BACKLOG — read it there, not here (this section goes stale; that one is maintained).
   - **Parity Principle (unchanged, now enforced):** every block-rendered visual container preserves 100% feature parity with the built-in 2-pane HTML — `viz-split`, drag handles, `makePanesCollapsible` — and the "corner stays baseline-protected on its native renderer" caveat is RETIRED: corner runs the tree renderer like everything else.
...
One new surface + one standing law, detailed in **[PARAM-WRITE-STRATEGY.md](docs/specs/PARAM-WRITE-STRATEGY.md)**:
...
⭐ **2026-09-06 UPDATE: Stages 4–5 are DONE** — all 32 ops are data defs on the declared tree (see section 4
above for the shipped state and BACKLOG #72 for the record). What genuinely remains of this list is **Stage 6
(self-host / reset-to-factory)** and Phase 2 (gestures-as-blocks, in flight).

...
2. **✅ SHIPPED (Stage 4, 2026-06-26) — Wizards-as-data Stage 4 `[L]`** — **drill** is now expressed as a pure `{template, bindings}` data def (`blocks/dataOps/drillData.js`) consumed by `registerUserOp`/`instantiate` via the federated user layer, and the reusable **equivalence harness** exists (`blocks/dataOps/equivalence.js` `emitEquivalence(builderA, builderB, sweep)`). `drill-as-data.spec.js` proves it two ways: **emit-equivalence** (byte-identical to `drillStack` across a grid-at-origin/cut/skip/wcs sweep) + **structural binding-wiring** (every binding routes its param to the same socket `drillStack` uses — the only way to validate the params emit can't reach). **Key finding — drill is ~90% data-expressible** (the per-hole loop lives in the `array` emit fold, so the template is a static 4-block tree); the **3 frontier blockers** the pure format can't yet express, each a *failing-on-purpose* tripwire test: **(2, PRIMARY) live bbox** — `placeOnStock` bakes `pointsBBox(patternPoints(params))`, and the default placement shifts by `-bbox.min`, so ANY pattern that moves the bbox (x0/y0 offset, circle/line/rect) diverges from the frozen snapshot; **(1) method swap** — `helical` needs a `bore` child (block-type change, not a value); **(3) fan-out param** — `clearance` feeds 2 sockets (1 binding = 1 socket). `blocks/dataOps/*`, `blocks/userOps.js`.
3. **[XL] Wizards-as-data Stage 5** — port the rest of the built-ins to data, one-by-one, each gated by output-equivalence + binding-wiring (reuse `dataOps/equivalence.js`); never batch. **The Stage-4 port sharpened the hard frontier into 3 concrete format extensions Stage 5 must grow** (see Stage 4): **(a) computed/derived bindings OR a live-bbox `placeOnStock`** — the *primary* blocker; the baked bbox snapshot is fundamentally incompatible with a parametric data-def (affects EVERY placed op, not just ATC), so either bindings must derive values from other params, or the place atom must compute its shift from a live param-derived bbox; **(b) conditional block-TYPE** (the drill↔bore / probe IF-GOTO-retry family — instantiate substitutes values, never a block type); **(c) one-param-→-many-sockets** fan-out. `blocks/opBuilders.js`, `wizards/probeBlocks.js`, `wizards/atcChangeWizard.js`, `wizards/ops/placement.js`.

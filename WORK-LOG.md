...
## 🔨 turn 107 (cycle 11) — MACHINE-FAITHFUL SIM (the VISIBLE dog-leg fix, human priority; full REBASE per human t101 "i need it exactly on it"). The wall1→wall2 dog-leg now draws from the RUNTIME wall-1 END + marker② relocates to END+delta + the probe fires there + the drag is END-relative. PREVIEW/SIM ONLY — emit BYTE-IDENTICAL at default. Full suite 497 passed / 0 failed (500 total, 1 known middle-animator flake retried green). COMMITTED (not pushed). ⏸ PASS BACK for advisor+human live-verify.

**THE MECHANISM (one-source: the trace publishes the runtime END; all consumers read it via ONE resolver):**
- **passAnchor.js — NEW `passAnchorFor(starts, ends, pass)`** supersedes drawAnchorFor. A flagged (anchorsAtPrev) pass → `ends[pass-1]` (the RUNTIME world-END, post probe+retract+lift, collision-clamped) if present, else the t94 static `starts[pass-1]`, else self. **`drawAnchorFor = passAnchorFor(starts, null, pass)`** → byte-identical to before for every ends-less caller (the all-ops fallback lives in ONE place still).
- **GcodeExecutionEngine.js** — records each pass's runtime world-END `_passEnds[N] = O + local pos` at the reposition (BEFORE the `pos={0,0,0}` reset) + a final capture in `_buildTraceResult`; **publishes `parsed.passEnds`**; collision O (:985) + DRO O (:502) now `passAnchorFor(_passStarts, _passEnds, _pass)`. Inert for non-corner ops (only a flagged pass READS ends; recording is harmless).
- **gcodeViz3d.js** — route (`off`) + tool-head (setToolPosition) anchor via `passAnchorFor(this.starts, this._passEnds, p)`; **`_markerWorld(p)`** relocates a flagged marker SPRITE to `passEnds[p-1] + (starts[p]−starts[p-1])` = the machine-correct wall approach (display-only VIEW; `this.starts` stays the declared/dragged source). `setPassEnds(ends)`.
- **toolpath2d.js** — passOff via `passAnchorFor(starts, passEnds, i)`; `markerWorld(i)` relocates the drawn handle; the hit-test + drag are DRIFT-AWARE (pass-0 sim-only = drift 0, fully unchanged; a flagged marker's drag subtracts the relocation drift so the displayed marker follows the cursor). `setPassEnds`.
- **createPreviewPanel.js** — captures `parsed.passEnds` → `v.setPassEnds` + `t2.setPassEnds` (BEFORE setSegments so the route rebuild reads it) + `getPassEnds()` for the Layout drag.
- **panelTypes.js + userOpView.js (P2)** — `layoutSpecFromOp(def, params, simStart, sources, passEnds)`: the relTo:'wall1' anchor SHIFTS to `passEnds[ri]` (the runtime END), so the Layout handle RELOCATES to the same machine-correct ② the top panel shows AND the drag writes **#23/#24 = target − passEnds[wall1]** (END-relative). Coherent: passEnds[wall1] is STABLE while dragging wall-2 (wall-1 is upstream of #23/#24). Absent passEnds (pre-trace) → the static start (graceful, byte-identical).
- **cornerWizard.js inferStart** — the flagged inferStart-z consequence: probeZ-OFF now returns **z = −scanDepth** (was safeZ) — the operator jogs to the PROBE height (below the top-at-0), not hovering ABOVE it, so the horizontal wall probe fires AT the wall (Z-trust removed the plunge that used to reach it). probeZ-ON keeps safeZ (hover OVER, then measure Z down).

**VERIFIED (FL/YX, probeZ-off, stock 100×80, tipR 2, retract 5, safeZ 10, scanDepth 5 — hand-derived, INDEPENDENT of the code):**
- passEnds[0] (wall-1 runtime END) = **(7, −7, 5)**: X unmoved (Y-probe), Y = contact(0−tipR=−2) − retract(5) = −7 (**+36 above the jog start ① = the "~probe distance" the t94 start-based frame was short by**), Z = start(−5) + safeZ lift(#19=10) = 5.
- relocated ② = passEnds[0] + off.wall2(−50,+50) = **(−43, 43, 5)** = the dog-leg END = the wall-2 probe fire.
- Engine **#1925=−2** (contact X = front-face 0 − tipR), **#1926=43** (= relocated ②.y; was 7 start-based), **#1927=−5** (drop height). MANUAL: ② is NOT flagged → self-anchored, unchanged.
- The preview now MATCHES the real machine motion (the emitted dog-leg is incremental from the real wall-1 end; the old preview showed ② at a point the machine never visits).

**SPEC (corner-draw-anchor rebased Fix-A → machine-faithful; corner-data-drag + repos-handle + disc-on-surface):**
- corner-draw-anchor: (4) route-first == passEnds[0] · route-end == relocated ② == probe-fire (one runtime value); (1) engine #1926 == relocated ②.y (re-derived via an INDEPENDENT trace, NOT ==7 start-based) + #1925=−2 + #1927=−5; (7) live head rides passEnds[0] (+ ends=null → t94 static fallback pinned); (8) NEW marker-relocation (flagged ② → passEnds[0]+cross; non-flagged/ends-absent → declared row). Kept (2) fallback/NaN, (3) manual self-anchor, (5) drawAnchorFor live-mutation, (6) byte-parity (twin==cornerStack, no anchor leak).
- corner-data-drag: END-relative — anchor re-derived as the RUNTIME END (passEnds[0]) via an independent trace; the real pointer drag writes #23/#24 = world − runtime-END. corner-data-repos-handle: NEW machine-faithful case (handle relocates to passEnds[0]+cross when passEnds threaded; static fallback kept). disc-on-surface: DROPPED the test-only `z:−5` override → now drives the sim from inferStart's z directly (real-symptom of the inferStart-z fix).

**ADVERSARIAL FAN-OUT (wbp2or4ck, 4 refutation lenses) — CAUGHT A REAL COHERENCE BUG (fixed):** the coherence skeptic found that panelTypes' Layout-handle passEnds-shift was gated only on `end && a`, NOT on the destination's `anchorsAtPrev` (unlike the 3D/2D `_markerWorld`). So for MANUAL travel (anchorsAtPrev false — the operator jogs, no programmed dog-leg) the Layout handle RELOCATED (to passEnds[wall1]+cross) while the 3D/2D marker stayed at the DECLARED wall-2 → a 36-unit cross-surface DIVERGENCE. **FIX:** gate the panelTypes shift on `dest && dest.anchorsAtPrev` (exactly like `_markerWorld`) → manual keeps the declared wall on ALL surfaces; auto still relocates. NEW `corner-layout-coherence.spec.js` asserts the Layout handle == the 3D/2D marker for BOTH auto (relocated) AND manual (declared), probeZ-on, via the REAL layoutSpecFromOp (mutation-proven MutE: ungate → RED). [NB: the fan-out agents run with Edit access in the working tree (I forgot `isolation:'worktree'`) → they mutated passAnchor/cornerWizard to test + dropped 2 `_`-diagnostics; all reverted/removed, tree re-verified clean, full suite re-run green. LESSON: set `isolation:'worktree'` for any workflow whose agents might Edit.]

**MUTATION-PROVEN (each property bites):** MutA engine collision → `passAnchorFor(_passStarts, null,…)` → corner-draw-anchor(1) RED (probe back to start-based). MutB `_markerWorld → return row` → (8) RED (marker not relocated). MutC inferStart z→safeZ → disc-on-surface RED (probe fires above the stock → discs stay raw, nDiscs 4→1). MutD panelTypes end-shift off → corner-data-drag RED (#24 back to start-relative). MutE panelTypes gate ungated → corner-layout-coherence RED (manual diverges). All reverted byte-exact.

**BYTE-PARITY / ALL-OPS SAFETY:** emit untouched (passEnds/anchor/inferStart are preview-sim only). Non-corner ops: no pass is flagged → passAnchorFor never reads ends → byte-identical to drawAnchorFor (the fallback is the SAME single helper). anchorsAtPrev hard-coerced bool. Combos dump confirmed coherence (marker==probe-fire-start) across FL/BR/XY + probeZ + manual. **⚠ The per-corner PREFILL bug (non-FL markers FL-canonical → the BR/etc. sim probe fires into empty space, deferred t133 "not urgent, just prefill") is UNCHANGED — orthogonal to this frame fix; my relocation is coherent WITHIN that (separate, still-wrong) non-FL frame.**

**P2 was BUILDABLE, not a coherence blocker** — the Layout drag reaches passEnds via getPassEnds (same path as getPassSources), the indexing is by-pass consistent, and the END-relative delta is coherent because the anchor pass is upstream of the dragged socket. So the FULL rebase (route + marker + probe-fire + END-relative drag, both surfaces) landed — nothing flagged.

**Surgical:** 8 source + 5 test files (passAnchor · engine · gcodeViz3d · toolpath2d · createPreviewPanel · panelTypes · userOpView · cornerWizard; corner-draw-anchor · corner-data-drag · corner-data-repos-handle · disc-on-surface · NEW corner-layout-coherence). Left HANDOFF/NEXT-SESSION + _*.png unstaged. Full suite 497 passed / 0 real fails (project-drawer-smoke + middle-animator = known flakes, pass on retry/isolation). COMMITTED, NOT pushed. ⏸ PASS BACK for advisor+human live-verify (the user will finally SEE the dog-leg from the END).

## 🔨 turn 109 (cycle 11) — PREFILL FIX (per-corner sim-start markers flip) + a HUMAN-SURFACED follow-on (the LIVE-form dog-leg now flips per corner INSTANTLY). SIM/PREVIEW + form-emit correctness; build() BYTE-IDENTICAL. Full suite 501 passed / 0 real fails (middle-animator known flake retries green). COMMITTED (not pushed). ⏸ PASS BACK for advisor+human live-verify.

**PART A — the dispatched PREFILL FIX (cornerData.js + cornerWizard.js):** the sim-start marker CHAIN was canonical-FL for non-FL corners → the BR sim probe fired into EMPTY SPACE (x=-947). Root: cornerReposOffsets ALREADY flips the offsets per corner (axesOf), but the frac BASE (zsurf, fx=0.07/fy=0.0875 = FL-relative) did NOT. FIX: `cornerSimStartsProvider` mirrors the base per corner via `dirsOf` (EXPORTED from cornerWizard — the SAME sign source the emit's axesOf/cornerReposOffsets use → one geometry, can't desync): `base = { x: xd==='−'? sx−zsurf.x : zsurf.x, y: yd==='−'? sy−zsurf.y : zsurf.y }`. FL (xd/yd '+') → no flip → BYTE-IDENTICAL. Verified all 4 corners (independent geometry): FL[(7,-43),(-43,7)] · FR[(93,-43),(143,7)] · BL[(7,123),(-43,73)] · BR[(93,123),(143,73)]; the wall-2 X-probe now contacts the corner's X-wall (FL/BL x≈-2 [x=0−tipR], FR/BR x≈102 [x=100+tipR]) — ON the corner, not empty space. COHERENT with the t107 passEnds frame (the relocated ② rides these flipped starts).

**PART B — HUMAN LIVE (3 msgs: "make sure the dogleg goes the right way" → "it just needs to use the marker pos" → "drag the second marker a little bit and the toolpath refreshes correctly, needs to be instant"):** after Part A the MARKERS flipped ("great") but the drawn DOG-LEG stayed FL until a marker nudge. ROOT (root-caused via 6 diagnostics + instrumentation, verify-real-symptom): the LIVE FORM renders cross1_x/cross1_y (the reposition #23/#24 sockets, which DEFAULT to a PER-CORNER signed-travelDist EXPRESSION). The numeric form widget (`formWidgets.js:53`) returned `b.default` when the field is empty — and that derived default is the CANONICAL-FL expression `#16`. So `'cross1_x' in params` was TRUE with the FL value → `instantiate` OVERWROTE the pruned per-corner socket (#15 for BR) with the stale FL `#16` → the emitted dog-leg stayed FL for EVERY corner (build() was correct only because CORNER_DEFAULTS deliberately OMITS cross1_x; the form defeated that). A marker nudge "fixed" it by writing a literal cross1_x. FIX: the numeric widget OMITS an untouched field whose default is NON-NUMERIC (an expression string) → `param in params` false → the per-corner socket expression holds → the dog-leg flips per corner INSTANTLY. A typed number / drag literal still overrides. Verified LIVE (openWiz + corner change): FL #23/#24=#16/#15, BR=#15/#16, FR=#15/#15 — flips + tracks (incl. back to FL); the wall-2 probe now HITS the corner (passEnds[1] near the corner, not -947).

**⚠ RE: the advisor's "EMIT UNTOUCHED, do NOT touch it":** the emit LOGIC is untouched (build(corner) byte-identical, all combos). Part B fixes the FORM from INJECTING a stale FL default over the correct per-corner socket — so it CHANGES what the LIVE FORM emits for NON-FL corners (from a WRONG FL dog-leg to the CORRECT per-corner dog-leg). This is a real emit-output CORRECTNESS fix (the LIVE form was emitting the wrong dog-leg for FR/BL/BR — a latent bug the 8-combo build() verification missed because it never included cross1_x). The HUMAN directly directed it (3 msgs). `formWidgets.js` is SHARED machinery, but the change is scoped (only bindings with a NON-numeric/expression default behave differently; the full suite 501 passed = no blast radius) + principled (realizes the LOCKED-MODEL "expression-holding socket": unset → expression, drag → literal).

**VERIFY (mutation-proven):** NEW `corner-prefill.spec` — (1) all-4-corner markers vs INDEPENDENT geometry + non-FL ≠ canonical-FL [MutF canonical-FL base → RED]; (2) the sim probe contacts the corner wall (±tipR), not an empty-space miss [MutF → RED]; (3) LIVE-FORM real-symptom: the emitted #23/#24 flips per corner INSTANTLY across FL→BR→FR→BL→BR→FL (tracks, not sticks) [MutG revert the omit → RED]. Updated corner-data-sim-marker-track (BR/XY: (57,7)/(7,57)→(143,73)/(93,123), the correct BR chain) + corner-data-repos-handle (BR handle (7,57)→(93,123)). Full suite 501 passed / 0 real fails.

**Surgical:** 3 source (cornerData base-flip + dirsOf export · cornerWizard export · formWidgets omit-expr-default) + 3 test (corner-data-sim-marker-track · corner-data-repos-handle · NEW corner-prefill). Left HANDOFF/NEXT-SESSION + _*.png unstaged. COMMITTED, NOT pushed. ⏸ PASS BACK. Remaining corner: item-2 corner-selector-on-canvas GUI.

## 🔨 turn 112 (cycle 11) — ITEM-2 CORNER-SELECTOR GUI (the LAST corner task). Click a stock CORNER on the Layout FeatureCanvas → sets the `corner` param → the emit + the per-corner markers/sim all follow (prefill t109 done, they track). A spatial GUI pick augmenting the Corner dropdown ([[prefer-gui-over-fields]] / [[spatial-gui-form-vs-canvas]]). SIM/PREVIEW + param (emit follows the corner param, already correct per corner). Full suite 503 passed / 0 real fails. COMMITTED (not pushed). ⏸ PASS BACK for advisor+human live-verify. **After this, CORNER IS DONE.**

**MECHANISM (mirror the existing stock-attach picker — the FeatureCanvas already has one, `_drawStockAttach`/`_hitAttach`/spec.onStockAttach; the corner op has no stock-attach → no conflict):**
- **featureCanvas.js:** `_drawCornerPick(spec)` renders 4 ring targets on the stock corners (FL=min-XY · FR=max-X,min-Y · BL=min-X,max-Y · BR=max-XY — matches cornerXY/the emit); the CURRENT corner is filled (`.fc-corner-pick-cur`), hover brightens (CSS). `_hitCorner(w)` hit-tests (12px world tol). The pointerdown handler, after the handle + stock-attach hit-tests, calls `spec.onCornerPick(code)`. Opt-in: only fires when the spec carries `onCornerPick` (other ops draw nothing → unchanged).
- **panelTypes.js (layoutSpecFromOp):** an op that declares a `corner` enum binding gets `corner: params.corner` + `onCornerPick(code)` on its spec. onCornerPick SETS the corner `<select>` (`#wiz_user_form [data-param="corner"]`) + dispatches `change` → the form's OWN delegated listener → update() re-emits (already correct per corner) + re-derives the per-corner markers/sim. REUSES the dropdown's change seam — no new param path.
- **formWidgets.js:** the enum `<select>` now carries `data-param` (parity with the numeric fields) so the picker can target + set it. (No handle/`_writable` side-effect: `corner` has no `group` → no canvas handle.)
- **styles.css:** `.fc-corner-pick` = hollow red ring (the human's "red circles"); `:hover` brightens; `-cur` = filled. Distinct from the gold drag-handles + blue stock-attach markers.

**VERIFY (mutation-proven, REAL-SYMPTOM via a genuine canvas click through the FeatureCanvas pointerdown → onCornerPick → change → update):** NEW `corner-selector.spec` — (1) all 4 corner targets render + the current (FL) is highlighted; (2) clicking each corner FL/FR/BL/BR (looped AROUND incl. back to FL, proving it TRACKS not sticks) SETS the `<select>` = that corner + highlights it + the EMITTED dog-leg #23/#24 follows (independent truth: FL #16/#15 · FR #15/#15 · BL #16/#16 · BR #15/#16) + the sim markers follow to the per-corner chain. **[Mut H: onCornerPick sets the select value but SKIPS the change dispatch → the emit/markers do NOT follow → RED]** — proves the click is wired to the real re-render seam, not a cosmetic value-set. Full suite 503 passed / 0 real fails (middle-animator known flake); the existing stock-attach picker (wizard-pathdatum) still green = no blast radius from the shared featureCanvas/select changes.

**Surgical:** 4 source (featureCanvas corner-pick · panelTypes onCornerPick · formWidgets select data-param · styles.css) + 1 test (NEW corner-selector). Left HANDOFF/NEXT-SESSION + _*.png unstaged. COMMITTED, NOT pushed. ⏸ PASS BACK. **CORNER CAMPAIGN COMPLETE** (emit · Z-trust · machine-faithful sim · prefill · corner-selector) — pending advisor+human final live-verify + the release decision.

## 🔨 turn 114 (cycle 11) — PROBESEQ START-POS FIX (human t113: the 'x then y' XY start positions wrong). REFINED DIAGNOSIS + a deeper fix than the advisor's framing. SIM markers already flip per probeSeq; the real bug was the LIVE-FORM EMIT of the Z→wall1 start #21/#22 (a socket-held gap the t109 fix missed). Emit-output correctness; build() BYTE-IDENTICAL. Full suite 507 passed / 0 fails. COMMITTED (not pushed). ⏸ PASS BACK for advisor+human live-verify. BLOCKS the release.

**DIAGNOSIS (traced empirically, NOT the advisor's "markers canonical-YX" framing):**
- The sim-start MARKERS **already flip per probeSeq** — cornerReposOffsets/axesOf swaps which axis is wall-1/wall-2, so wall1/wall2 marker positions swap XY↔YX (FL/YX [(7,-43),(-43,7)] vs FL/XY [(-43,7),(7,-43)]; probe fires on the RIGHT wall per order). No marker fix needed.
- The **LIVE-FORM EMIT** of #21/#22 (the Z→wall1 start, probeZ-ON only) was WRONG for XY: build(FL/XY/probeZ-on) #21=#16 (X-travel, wall-1 is X) but the LIVE FORM emitted #21=0. ROOT: `startX`'s CANONICAL (FL/YX) socket default is the PERP-axis `'0'` — a FINITE value. The t109 omit-fix only omitted NON-numeric (expression) defaults, so it did NOT omit startX=0 → the form injected startX=0 → instantiate overwrote the pruned per-probeSeq #21 socket (which for XY should be the X-travel #16). #22/#23/#24 default to EXPRESSIONS in the canonical (already omitted by t109), so only #21 (the perp-'0' axis) leaked.

**FIX (extends t109 — the analogous emit-output fix the advisor anticipated):**
- **deriveBindings.js:** a spec with NO explicit `default` reads the socket's CANONICAL baked value (line 64; per-structural-combo — corner #21-#24 change with corner×probeSeq). Flag it `socketHeld = (s.default === undefined)` — that canonical default must NEVER be injected when the field is untouched (it overwrites the pruned per-combo socket, even a FINITE canonical like #21='0' that's WRONG for XY).
- **formWidgets.js:** a SOCKET-HELD numeric field renders EMPTY (placeholder "auto") + the reader OMITS it unless the user TYPED a finite number → the template's per-combo socket expression holds. (t109's non-numeric-default omit stays for any other case.) A typed value / drag literal still overrides. Only corner's derived cross1_x/cross1_y/startX/startY are socketHeld today → corner-scoped effect (full suite 507 passed, no blast radius; the existing corner drag/start-live/repos-handle tests still green — a drag writes a literal → included).

**⚠ EMIT-OUTPUT CHANGE (flagged, as t109 was):** the LIVE FORM now emits #21=#16 for XY (was 0) — CORRECTS the Z→wall1 start for the non-YX probe order. build() is byte-identical (build omits the sockets already, so it was always right — the 8-combo verification never exercised the FORM path with startX present). Shared machinery (deriveBindings + formWidgets) but scoped to socket-held bindings.

**VERIFY (mutation-proven):** NEW corner-probeseq.spec — (1) markers flip per probeSeq (FL+BR, YX vs XY) vs INDEPENDENT geometry + non-canonical; (2) the sim probe fires on the RIGHT wall per order (XY last-probe = Y-wall, YX = X-wall); (3) LIVE-form real-symptom: #21/#22 (probeZ-ON) + #23/#24 flip per probeSeq (FL/YX 0/#16/#16/#15 · FL/XY #16/0/#15/#16 — the SWAP), tracks back. **[Mut J: revert the socketHeld flag → the LIVE-form #21 reverts to 0 for XY → RED; #23/#24 stay correct (t109 branch) → the mutation isolates the finite-default gap]**. Full suite 507 passed / 0 fails.

**Surgical:** 2 source (deriveBindings socketHeld flag · formWidgets socket-held omit) + 1 test (NEW corner-probeseq). Left HANDOFF/NEXT-SESSION + _*.png unstaged. COMMITTED, NOT pushed. ⏸ PASS BACK. After this: corner truly complete + release-ready pending your review of the two emit-output form fixes (t109 + t114).

## 🔨 turn 116 (cycle 11) — MARKER-DRAG SCOUT/OBJECTION (no code). Human t116b OVERRODE the dispatch's framing: dispatch said "drag marker1 → marker2 rides = CORRECT"; the human said **"i dont want any coupling"** (all markers independent). Per protocol (plan/requirement contradiction → log + pass to advisor, don't re-architect unilaterally), passing back for a coherent re-plan. Suite untouched.

**EMPIRICAL FINDINGS (real pointer drags on the Layout, probeZ-ON — reproduced, then diags removed):**
- **drag marker2 (wall-2 reposition #23/#24) → marker1 STAYS PUT** (g21/g22 + its sprite byte-unchanged). So the advisor's "bug (2)" (marker2 drag moves marker1) does **NOT reproduce** — that direction is ALREADY independent.
- **drag marker1 (wall-1 start #21/#22) → marker2 FOLLOWS it** (rides along). This is the coupling the human is actually seeing (they said they'd confused the direction). The dispatch called this "correct"; the human wants it GONE.
- The coupling is INHERENT to the G91 INCREMENTAL emit: `marker2_world = passEnds[wall1] + (#23/#24)`, so moving wall-1 moves its runtime end → marker2 rides, UNLESS #23/#24 recomputes. probeZ-ON is a CHAIN: zsurf → wall1 → wall2 (each upstream drag rides the downstream). probeZ-OFF: sim-only(wall1) → wall2, same mechanism.
...
## 🔨 turn 120 (cycle 11) — CORNER-MARKER INDEPENDENCE (Option A) BUILT (the large highest-risk diff — advisor fan-out queued). Suite 510 pass / 0 fail / 1 known flake (middle-animator). One-source refinement APPLIED (panel-derive off the trace's passEnds; emit builder UNCHANGED).

**WHAT SHIPPED (3 files, surgical):**
- **panelTypes.js** — the spot mechanism. `layoutSpecFromOp(def, params, simStart, sources, passEnds, spots, setSpots)`. Datum = `cornerXY` (per-corner, from `params.corner`). `spotStore` active ONLY for a corner op with the store wired. DERIVE (in the relTo block): a group WITH a stored spot → world = cornerXY + spot (FIXED) → offX/offY = world − (current planned anchor `ax/ay`) → `_writeParam` the form field (GUARDED: only when it changed → no re-render loop; acyclic — the anchor is upstream, never reads this field). UNSET → the socket increment holds (byte-identical). CAPTURE (`spotOnDrag`): dragging any relTo handle freezes every OTHER un-set relTo group at its CURRENT displayed world (no visual jump), sets the dragged group's spot = drop − cornerXY. `repoGroups` stashes each group's live world+anchor for the capture.
- **userOpView.js** — `_layoutSpots` / `_layoutSpotsOp` module store (reset when the op changes → a fresh op is undragged → byte-identical); `setSpots` re-renders; passed into `renderLayout2D`.
- **corner-marker-independence.spec.js** (NEW, 4 tests, all green): (1) drag A → B stays (<3px); (2) drag B → A stays; (3) default undragged #23/#24 keeps the #15/#16 socket EXPRESSION (no baked literal); (4) drag → change corner FL→BR → BOTH handles re-anchor (>30px, datum-relative).

**KEY CORRECTION mid-build (machine-faithfulness):** I first made the sim-only ◇ drag ALSO capture the emitting handles. REVERTED — that is machine-WRONG. The sim-only ◇ is the CHAIN ROOT (the operator's MANUAL jog: probeZ-on zsurf / probeZ-off wall-1, NEVER emitted); the #-handles are G91 INCREMENTS relative to it. Freezing a handle against a PREVIEW root would bake that preview position into the emitted increment — but the operator's real jog is wherever they physically go. So the sim-only ◇ stays a PURE preview move: undragged handles ride the root (the relative chain, emit byte-identical — this is NOT the coupling the human meant), dragged handles hold their datum-relative spot. The coupling the human wants gone is strictly BETWEEN emitting handles (drag M1 → M2 rode) → that is `spotOnDrag`. This restored `corner-layout-sim-drag` (sim-only drag → emit byte-identical) which the over-capture had broken.

**VERIFIED REAL SYMPTOM (live drags + screen trace):** probeZ-ON drag M1(#21/#22)→M2(#23/#24) stays {dx0,dy0}; drag M2→M1 stays; probeZ-off drag sim-only ◇ → M2 stays; re-anchor on corner change (both to BR); NO re-render loop (tests fast, derive guarded+acyclic); no capture jump (freeze writes the CURRENT world). Byte-parity: 21 corner specs pass incl. corner-draw-anchor (twin==cornerStack byte-for-byte); default emit keeps the socket expression.

**PASSED BACK for advisor FAN-OUT review (the large highest-risk diff) + human live-verify. syncA move-removal still queued after (NOT touched). Did NOT start the Blockly reorg (being finalized with the human).**

## 🔨 turn 122 (cycle 11) — HARDEN Option A: fixed the fan-out's TWO real gaps in one coherent task. Suite 512 pass / 0 fail. Value-assert spec goes RED under the _writeParam mutation (verified). 3 files.

**PART 1 — CROSS-INSTANCE SPOT LEAK (byte-parity blocker), userOpView.js:** `_layoutSpots` was reset per `_def.opType` — but every corner instance SHARES the opType, so drag corner-#1's marker → open corner-#2 (same type) → #2 inherited #1's spot → #2 (undragged) derived a BAKED literal for #23/#24 instead of the socket expression → NOT byte-identical. FIX: clear `_layoutSpots` per wizard OPEN in `onShow` (wizardManager.js:258 fires it on EVERY open — new AND edit); removed the buggy per-opType guard + the `_layoutSpotsOp` var (my own t120 code). A fresh editing session starts undragged → socket holds → byte-identical.

**PART 2 — ASSERT-THE-VALUE spec gap (the logged lesson), corner-marker-independence.spec.js (rewritten) + featureCanvas.js:**
- Root of the gap: the DRAW uses a LOCAL offset (`x = ax + offX`), so removing the panel-derive `_writeParam` (panelTypes.js:177-178) leaves the handle VISUALLY frozen but emits a STALE #23/#24 (wrong machine target). The old 4 specs only checked the visual freeze → all stayed GREEN under that mutation.
- **(d) STABLE handle id:** added `data-hid` (the decl id: `start_pos` / `reposition_pos` / `__simstart0`) to the FeatureCanvas handle SVGs → tests key on the bound source, NOT sort-by-screen-position (which misattributes when a drag crosses the other handle).
- **VALUE truth:** the reposition's machine world = `passEnds[wall1] + (#23,#24)`, wall1 = the pass BEFORE wall2 = `passEnds[len-2]` (robust in BOTH probeZ states). Verified numerically: default `(7,-7)+(-50,+50)=(-43,43)`; after a start-drag wall1's end moves to `(33.19,-7)` and #23 re-derives to `-76.19` → `(33.19-76.19,-7+50)=(-43,43)` PRESERVED.
- **(a/b) test (1):** drag the START (the formerly-coupled direction, was tautological before), assert wall1's end MOVED yet the reposition's machine world is PRESERVED (value). **(c) test (4):** drag + change corner FL→BR, assert the datum-relative spot `(dx,dy)=world−cornerXY` is PRESERVED (not just ">30px moved"). **(e) test (5):** drag a marker then RE-OPEN the wizard → emit byte-identical to the pristine default (locks PART 1). test (2) = the other direction value-asserted (naturally independent — start is upstream). test (3) = default socket expression.
- **MUTATION-VERIFIED:** commented out panelTypes.js:177-178 → tests (1) + (4) went RED (they assert the emitted world); (2)/(3)/(5) green (mutation-orthogonal). Reverted panelTypes.js (untouched this turn).
...
## 🔨 turn 130 (cycle 12) — BLOCKLY REDIVIDE BUILD (Option B, corner-piloted, labeled-grouping first). Suite 513 pass / 0 fail. All 32 corner combos byte-identical; 7 labeled sections render; 5 other ops unchanged; round-trip intact. 7 files (+ new spec).

**THE `section` BLOCK (generalized param_group):** added `sectionBlock` (userRoot.js) — kind 'section', a `title` field, transparent emit `(p,children)=>children` (the param_group model). Registered in the PALETTE (ops/index.js). Wired 'section' into the 4 shared kind-check sites ALONGSIDE param_group (additive, so the 5 other ops are untouched): bridge.js:210 (the DO mouth), stackBridge.js:113 (block→json) + :243 (json→block round-trip), blockEmitter.js:117 (transparent emit). All the walkers (flattenBlocks, pruneGuards whenGuard.js:24, the emit) ALREADY recurse generically through `.children`, so nesting atoms in sections is transparent to bindings/sim-intent/panel-detection/guard-prune.

**CORNER TEMPLATE reorganized (cornerData.js cornerDataStack):** user_root KEEPS its 2 mouths (5 other ops unchanged — my structure call, flagged option "2-mouths+nest"). PRESENTATION mouth (uiChildren) → 4 view-peer sections FORM[panel,param_group] · LAYOUT-2D[] · 3D-SIM[sim,simstarts] · PROJECTED-GCODE[] (all emit ∅). EXECUTION mouth (children) → STRUCTURAL[] · VARIABLES[leading slice] · G-CODE[rest].

**BYTE-PARITY (hard gate A) — the SEAM:** the VARIABLES/G-CODE split falls on the CONTIGUOUS seam computed at build = the first top-level `guard`/motion block AFTER the first ungated `assign` (superset index 24: after the corner-header guards + config comments + #1..#18 ungated assigns, before the guard-probeZFirst reposition #21/#22). VARS = exec.slice(0,seam), G-CODE = exec.slice(seam) → VARS ++ G-CODE reproduces cornerStack order EXACTLY; STRUCTURAL emits ∅. Robust fallback: no seam → all in VARIABLES (still byte-exact). VERIFIED: all 32 combos (probeZ on/off × 4 corners × probeSeq × syncA) twin==cornerStack byte-for-byte (0 fails); corner-draw-anchor's byte-parity spec + all corner goldens UNCHANGED.

**ONE-SOURCE (gate B):** the view-peer sections are labeled render-rigs; the per-pass sim-start markers stay the SHARED anchor source (cornerSimStartsProvider feeds BOTH 2D+3D — ONE source, NOT re-declared per view; ride 3D-SIM). LAYOUT-2D + PROJECTED-GCODE + STRUCTURAL are LABELED PLACEHOLDERS this pass (2D/3D driven by the ONE trace + the form3d+2d panel; structural knobs = deferred item-d; per-view rig blocks + PROJECTED-GCODE view = follow-ups). Color-per-concern (item h) = follow-up, not now.

**BLAST-RADIUS (gate C) + ROUND-TRIP (gate D):** the 5 other ops (drill/slot/surfacing/text/atc) render+emit UNCHANGED (their specs + the full suite pass); corner is the ONLY op declaring sections. Round-trip VERIFIED: opening corner in Blocks → 7 `section` blocks in the correct mouths (PRESENTATION→FORM/LAYOUT-2D/3D-SIM/PROJECTED-GCODE; EXECUTION→STRUCTURAL/VARIABLES/G-CODE). NEW corner-redivide.spec (3): section transparency unit + all-32-combos byte-parity + the 7-sections-in-right-mouths structure.

**PASSED BACK for advisor fan-out (byte-parity + blast-radius) + human eyes. FLAGGED sub-decisions: (i) kept user_root 2-mouths+nest (cleanest, 5 ops untouched); (ii) LAYOUT-2D/PROJECTED-GCODE/STRUCTURAL are labeled placeholders (real per-view rigs + structural knobs are the named follow-ups). Corner RELEASED V10.51, disjoint.**

## 🔨 turn 132 (cycle 12) — CONCERN-COLOUR (item h). Data-driven per-section colours. Suite 517 pass / 0 fail. Emit byte-identical (all 32 combos); 5 other ops unchanged; colour round-trips. 3 files.

**DATA-DRIVEN COLOUR:** the generic `section` block carries an OPTIONAL declared colour — `params.color` (NOT a rendered field; it rides in the block's `data` via recToJson's non-field-params path, so it round-trips via toRecord→params and NEVER touches the transparent emit). A new Blockly extension `ddcs_seccolor` (bridge.js, added to every `section` block in jsonDef + registered in installBlockly) reads `data.color` and calls `setColour`. Timing: JSON load sets `data` AFTER init, so it applies on init + a `setTimeout(0)` re-apply + a setOnChange fallback-until-landed. A section with NO declared colour keeps its category style → the 5 other user ops (which declare none) render UNCHANGED.

**CORNER declares its 7 colours (cornerData.js cornerDataStack):** the `sec(title, color, children)` helper. Palette (easily tweakable): STRUCTURAL #f59e0b (amber) · VARIABLES #06b6d4 (cyan) · FORM #d946ef (magenta) · the VIEW FAMILY as a blue set LAYOUT-2D #3b82f6 / 3D-SIM #6366f1 / PROJECTED-GCODE #0ea5e9 (read as related views) · G-CODE #22c55e (green). Matches the existing --cat-* semantic palette.

**VERIFIED REAL SYMPTOM (eyeballed screenshot + readout):** the 7 sections render in their concern colours (getColour == the declared hex for all 7); the VIEW family reads blue, the others distinct. Byte-parity: all 32 combos (probeZ on/off × 4 corners × probeSeq × syncA) twin==cornerStack byte-for-byte (0 fails). Colour survives a workspace serialize→load round-trip (rides in `data`). Full suite 517 pass. NEW corner-redivide.spec test (4): colour value + round-trip persistence.

**PASSED BACK for advisor review + human eyes. Palette is easily adjustable. Remaining redivide follow-ups (unchanged): per-view rig blocks for LAYOUT-2D/PROJECTED-GCODE + the PROJECTED-GCODE view + structural knobs (item-d). Corner RELEASED V10.51, disjoint.**

## 🔨 turn 134 (cycle 12) — MMB-ALWAYS-PANS (Blocks-tab item f). Suite 520 pass / 0 fail. Middle-button pans even over a block; LMB drag + RMB menu unchanged. 2 files.

**THE FIX (blocksApp.js):** Blockly v13 decides block-drag vs canvas-pan by WHAT is under the pointer (a movable block → drag), NOT by which button — so a middle-drag starting over a block would try to grab the block, not pan. Added a CAPTURE-PHASE `pointerdown` on the Blockly injection host (`.blk-bk-host`) that, for `button===1` ONLY, `preventDefault`+`stopPropagation` (preempts Blockly's per-block gesture BEFORE it claims the pointer, + kills the browser's autoscroll puck) and drives `ws.scroll(ox+dx, oy+dy)` from window pointermove/up. Capture on the host (ancestor of every block) fires before any block-level listener, so it works identically whether the pointer is over a block or empty canvas. The guard returns immediately for any non-middle button → LMB (button 0, block drag) + RMB (button 2, context menu) are completely untouched.

**VERIFIED REAL SYMPTOM (live pointer drags, workspace-coord assertions):** middle-drag STARTING over a block → the workspace pans (scroll delta ~the drag) AND the block does NOT move in workspace coords (not grabbed); middle-drag over empty canvas → pans; LEFT-drag over a block → the block moves (drag unchanged); RMB untouched by construction (context menu intact). NEW blocks-mmb-pan.spec (3): middle-over-block pans+no-move · left-over-block moves · empty-canvas middle pans. Full suite 520 pass.

**PASSED BACK for advisor review + human eyes. Scoped to the Blocks-tab workspace only. This item is mechanical/independent (not design-gated). Corner RELEASED V10.51, disjoint.**

## 🔨 turn 136 (cycle 12) — HEADER-COMMENT SCOUT (verify-first, NO code). Finding + recommendation for advisor+human. Suite untouched.

### Confirmed MECHANISM (how the twin emits — static template + bindings, NOT a live re-run):
`instantiate(def, params)` (userOps.js): clones `def.template` (built ONCE at CORNER_DEFAULTS via `userOpFromStack`) → `pruneGuards` → `deriveBindings(flat, def.bindingSpecs)` → for each binding `flat[blockIndex].params[b.key] = params[b.param] ?? default`. CORNER_BINDING_SPECS all `match:{type:'assign', var:'#N'}` + `key:'value'` → they ONLY overwrite ASSIGN VALUES. Then `postInstantiate(stack, resolved)` (userOps.js:359 — note it DOES receive the resolved params) runs `applyProbeSources` (rewrites #5/#3/#2 assign value+note from live settings). Nowhere is `cornerDataStack(params)` re-run. So the 2 header comments (cornerWizard.js:182-183, `Probe dist: ${dist}mm…` / `Fast: ${fFast}…` — 6 scalars interpolated at BUILD time) are FROZEN at CORNER_DEFAULTS; for non-default scalars the twin's comment lags (shows 500mm) while #1 carries the correct value. COSMETIC (comment, not motion) + PRE-EXISTING.

### OPTIONS + effort:
- **(a) BIND the comment scalars (computed-text binding).** No existing binding form drives COMPOSED/multi-param text — bindingSpecs are single-param→single-socket-`value`. Would need a NEW spec type (a computed/template-text binding: declare a template + its params + a matcher) + instantiate handling. **Effort: MODERATE.** Most declarative/general, but a new engine for (currently) ONE case → rule-of-three says defer until a 2nd/3rd wizard needs composed-comment binding.
- **(b) reference the REGISTERS in the comment (`Probe dist: #1 | Retract: #2 …`).** Makes the comment STATIC in cornerStack → the twin's frozen template == the built-in → byte-identical twin==built-in, trivially. **Effort: TINY (2 lines).** BUT it CHANGES the built-in's operator-facing .nc for EVERYONE (the operator reads `#1` instead of `500mm` — a human-readability regression). **BYTE-AFFECTING → needs the human's sign-off. NOT recommended.**
- **(c) re-derive the 2 comments at emit (postInstantiate) — RECOMMENDED.** The hook ALREADY receives `(stack, resolved)` + ALREADY rewrites blocks from live state (applyProbeSources is the exact precedent). Add: recompose the 2 header comments from `resolved`; ONE-SOURCE the format by extracting a shared `cornerHeaderComments(params)→[l1,l2]` helper that cornerStack ALSO uses (format defined once); match the target comments by their default-composed text (robust — exact string). **Effort: SMALL.** Reuses the existing hook + pattern, keeps twin==built-in byte-identical, NO operator-facing regression, one-sources the format. Con: an imperative per-op rewrite (but consistent with applyProbeSources + declared-once format).
- **(d) accept + document (defer).** **Effort: ZERO.** Legitimate — it is genuinely cosmetic (the #1-#6 assign VALUES that drive probing are correct; only the summary comment lags, only for non-default scalars).
...
## 🔨 turn 142 (cycle 12) — GLOW MERGE + LIVE-VERIFY. Suite 521 pass / 0 fail. The exec-line glow works on port/corner-clean, LIVE-VERIFIED (screenshot). 3 files.

**MERGE-VS-REAPPLY DECISION: clean CHERRY-PICK.** `git cherry-pick -n 5d72c76` applied CLEANLY (exit 0, no conflict markers — git auto-merged the line-shifts) despite blocksApp.js having changed since via t126 (sidebar-collapse) + t134 (MMB-pan). Verified coexistence: the glow's `onLine: (i)=>setExecLine(i)` (line 156, panel options) + `setExecLine`/`execTrail`/`clearExec` (lines 407-414) + `clearExec()` in renderCode landed, AND the t134 middlePan capture handler (line 185) + t126 `setToolsOpen(!mobile)` (line 738) are intact. styles.css got the `#blk-gcode .gl.active-line` + `[data-exec-age]` comet-tail rules. node --check clean.

**LIVE-VERIFIED THE REAL SYMPTOM (the gate — never verified before) — SEEN in a real sim + screenshot:** opened the Blocks tab with a corner op, STEPPED the sim (`.pp-step`); the PROJECTED G-code panel lights the currently-executing line + a 5-element FADING COMET-TAIL. Trace readout: after 9 steps activeIndex=8, activeText=`#5=3 ( Probe port )`, tailAges=[5,4,3,2,1], total 66 lines — the sim's line index maps to the correct `.gl` span. Screenshot: the sim header shows "Running line 9/66", `#5=3` glows solid blue, the lines above fade upward (comet-tail). SAVED: scratchpad/glow.png.

**BYTE-PARITY:** the glow is a Blocks-tab DISPLAY feature — emits NOTHING → .nc byte-identical; corner + all wizards unaffected; full suite 521 pass. NEW blocks-exec-glow.spec: steps the sim, asserts the exec line glows, ADVANCES, and grows a comet-tail (index-mapped) — locks the feature (it had zero coverage). Skipped the empty per-view rig blocks (YAGNI per the t140 scout, human-confirmed).

**PASSED BACK for advisor review + human eyes. Screenshot: scratchpad/glow.png. Last queued follow-up = the STRUCTURAL knobs (item d). Corner RELEASED V10.51, disjoint.**

## 🔨 turn 144 (cycle 12) — STRUCTURAL-KNOBS SCOUT (item d, the LAST Blockly follow-up; NO code). Finding + proposal + forks for advisor+human. Suite untouched.

### MECHANISM (how structural differs from value):
- The 5 structural params (probeZFirst bool · corner/probeSeq/wcs enum · syncA bool) are ALREADY `def.bindings` (CORNER_STRUCT_BINDINGS: {param, type:'bool'/'enum', default, label, widgetConfig:{options}}). They render in the RUNTIME form via renderOpForm (toggleWidget/dropdownWidget) and DRIVE the guards: at instantiate `pruneGuards(clone, params)` → `whenOk(guard.when, params)` reads `params[param]` → the arm survives/drops. They EMIT nothing themselves — they reshape WHICH blocks survive.
- The VALUE-knob path: `deriveAuthoredDef` derives bindings from `EXPOSE_` fields ticked on blocks → `{param, blockIndex, key}` — anchored to a NUMERIC SOCKET (an assign field). The `param` GUI block is likewise SOCKET-BOUND ("every widget commits a NUMBER"; non-numeric widgets are form-only) → it CANNOT carry an enum-string structural param (corner=FL/…) NOR a param with no socket.
- **So the structural params genuinely need a SEPARATE authoring anchor** — they are GUARD CONDITIONS, not socket values (the value-vs-structural split the human validated, NEXT-SESSION:190/193/195). BUT the FORM OUTPUT is already unified: both value + structural are `def.bindings` → ONE `renderOpForm`.

### PROPOSAL — a "structural control" block (the STRUCTURAL section's authoring anchor):
- A new block declaring a structural param: `{param, kind:'toggle'|'select', options, default}`. EMITS NOTHING. Renders in the form as a toggle (bool) / dropdown (enum) — the RUNTIME form already handles bool/enum bindings. DRIVES the guards: its value is the structural param pruneGuards branches on → the preview repruned live. It is the authoring anchor for a "guard-driver" binding (a def.bindings entry with NO match/socket — just param+type+options+default), the structural parallel to the value knob's socket-anchored binding.
- **UNIFICATION (item d) — the honest picture:** ONE form-derivation OUTPUT (all bindings → renderOpForm, already true), TWO authoring ANCHORS by nature: value knobs anchor on numeric SOCKETS (the `param` block / EXPOSE_ on assigns); structural knobs anchor on the new STRUCTURAL-CONTROL block (no socket). The redivide already SEPARATED them (VARIABLES/FORM mouth vs STRUCTURAL mouth), so this split is BY CONSTRUCTION, not a divergence — the value-vs-structural distinction the human asked for.

### (3) ROUND-TRIP + BYTE-PARITY: the control block emits NOTHING → at any FIXED structural value the emit is byte-identical (the guards prune identically); authoring it changes ONLY the selected structural value (the intended reshape). Its {param, kind, options, value} are fields → round-trips.

### (4) EFFORT + DESIGN FORKS (the meaty one — for the human):
- **FORK 1 — block shape:** (A) ONE GENERIC `structparam`/`wizctrl` block parameterized by param+kind+options (matches the wizards-as-data generic-block family: section/param/panel/sim) — RECOMMEND; vs (B) per-param blocks (more self-documenting, but N types + corner-specific).
- **FORK 2 — unification depth (the big one):** (MINIMAL) populate corner's STRUCTURAL section with control blocks that MIRROR the existing code-declared CORNER_STRUCT_BINDINGS — the section becomes non-empty + editable, but the form still derives from the code-declared bindings (two derivation paths persist). SMALL. vs (FULL, item-d north-star) make the structural control blocks the SOURCE: `deriveAuthoredDef` derives the structural bindings FROM them, replacing CORNER_STRUCT_BINDINGS → ONE derivation path (declare-never-infer, wizards-as-data). LARGER (touches the corner def's binding derivation + deriveAuthoredDef).
- **FORK 3 — runtime vs authoring-only:** (i) the control block DECLARES the structural param (the form sets the runtime value; pruneGuards reads params; the block's value = the default) — RECOMMEND, parallels the value-knob binding model; vs (ii) the block's value IS the runtime source (instantiate reads the structural blocks).
- **EFFORT:** MINIMAL (Fork-2 minimal + Fork-1 generic + Fork-3 declare) = MODERATE (new block def + register + populate corner's STRUCTURAL section + confirm form/prune already handle it). FULL unification = LARGE (+ rewire deriveAuthoredDef to derive structural bindings from the control blocks, replacing the code-declared path).

### RECOMMENDATION: generic block (Fork-1 A) + declare-model (Fork-3 i) + START with Fork-2 MINIMAL (fill the STRUCTURAL section with the 5 controls now — visible, byte-safe, ships the vision), and treat FULL unification as a follow-up once the control block is proven (rule-of-three: corner is the pilot; the one derivation path is the north-star but the bigger lift). The human picks the unification depth.

**PASSED BACK the finding + proposal + 3 forks BEFORE building (SCOUT ONLY). This is the LAST queued Blockly follow-up. Corner RELEASED V10.51, disjoint.**

## 🔨 turn 146 (cycle 12) — C-MOUTH HEADER CLEANUP (shared jsonDef, generalized to ALL C-mouth blocks). Suite 522 pass / 0 fail. Emit byte-identical; round-trip + colors intact across wizards. 1 file (bridge.js).

**(A) LABEL ABOVE THE MOUTH (all C-mouth kinds):** refactored jsonDef so the block's label + inline fields build message0 (the HEADER ROW) and each statement-input MOUTH goes on its OWN row below (message1/message2 via an `addMouth` helper) — the same pattern makeOpDef already uses for op containers. Generalized to user_root's 2 mouths (Presentation/Execution, each on its own row with its sub-label), param_group + section's DO, and every isWrap kind's DO (container/path/loop/cond/depth/fill/place/rotate). So the label no longer sits BESIDE the mouth (which widened the block + shoved nested blocks right) → blocks collapse to CONTENT WIDTH, nested content starts hard-left. Dropped the user_root input_dummy (the row break replaces it). Non-C-mouth blocks (plain atoms, reporters) are UNCHANGED (no statement input → single message0 row as before).
**(B) DROP the 'Section title' prefix:** for the `section` block, message0 omits the label ('Section') + the field-name prefix ('title') → it shows JUST its title value (FORM / LAYOUT-2D / STRUCTURAL / …). ddcs_seccolor still applies.

**VERIFIED REAL SYMPTOM (screenshots + specs):** corner in Blocks — "Presentation (UI & Sim)" / "Execution (G-code)" render as HEADER ROWS above their mouths, the 7 sections nest hard-left, each showing just its title (FORM/LAYOUT-2D/3D-SIM/PROJECTED-GCODE + STRUCTURAL/VARIABLES/G-CODE), concern colors intact, blocks narrow. Drill (another wizard) renders + emits correctly. BYTE-PARITY: block-def/render change only → emit untouched; corner-redivide 32-combo parity + all goldens green; round-trip (studio-to-blocks, blocks-edit-reconcile, blocks-open-seeded, blocks-render) all pass; full suite 522. Screenshots: scratchpad/hdr-corner2.png (corner, sections collapsed) + hdr-drill.png (drill). NOTE (pre-existing, out of scope): the devMode knob affordance still augments the section's title field (the "? knob false" collapsed-view noise) — not introduced here; a devMode question (should a structural container be knob-exposable) for later.

**BLAST RADIUS: the SHARED jsonDef → every wizard's C-mouth blocks. VERIFIED corner + drill + the full suite. PASSED BACK for advisor review (render + round-trip across wizards). STRUCTURAL-KNOBS build queued behind, awaiting the human's MINIMAL-vs-FULL decision. Corner RELEASED, disjoint.**

## 🔨 turn 148 (cycle 12) — SECTION-TITLE KNOB CLEANUP. Suite 522 pass / 0 fail. Section headers show JUST their title (no knob kit); the value-knob path is intact. 1 file (devMode.js).

**FIX:** the devMode "expose as knob" augmentation (augment() → gated by isAtom) was augmenting the `section` block's TITLE field (an inline text field) with the knob kit (XMARK 'knob' + EXPOSE checkbox + PNAME 'title') → the "? knob false title" noise on the concern headers. A section's title is AUTHORING METADATA (its name), not an exposable wizard value, so excluded `section` blocks from `isAtom` (devMode.js): `const def = BLOCKS[blk.type]; return !(def && def.kind === 'section');`. augment() now skips sections entirely (their only field is the metadata title).

**VERIFIED REAL SYMPTOM (assertions + screenshot):** open corner in Blocks → all 7 sections have NO knob kit (getField('EXPOSE_TITLE')/getInput('DECL_TITLE') absent on every section; the headers read just STRUCTURAL / LAYOUT-2D / PROJECTED-GCODE / FORM / 3D-SIM / VARIABLES / G-CODE). NOT over-excluded: 93 non-section blocks STILL carry their DECL_ knob affordance (the value-knob expose path — deriveAuthoredDef / EXPOSE_ — stands; a real value atom's `value` field keeps its knob). BYTE-PARITY: authoring-only (no emit path touched) → .nc byte-identical; corner-redivide 32-combo parity + round-trip green; full suite 522. Screenshot: scratchpad/knob-clean.png (collapsed section headers, clean). NOTE (the lone "?" on an EMPTY collapsed section = Blockly's standard placeholder for an empty statement mouth, NOT knob noise — only in the collapsed view). RELATED (out of scope, same pattern): the `panel` + `param_group` metadata blocks also still get the knob kit on their panel/group field — a follow-up if the human wants the exclusion generalized to all structural-metadata blocks (I scoped to `section` per the dispatch).

**PASSED BACK for advisor review + human eyes. STRUCTURAL-KNOBS MINIMAL build queued next (the 5 structural controls). Corner RELEASED, disjoint.**

## 🔨 turn 150 (cycle 12) — STRUCTURAL-KNOBS BUILD: GATE (build-research surfaced 2 real constraints the MODERATE/generic estimate missed; passing back a shape decision + recommendation BEFORE the large diff). No code — did not ship a large shared-change-listener diff on a just-discovered mechanism/shape. Suite untouched.

### BUILD-RESEARCH FINDINGS (verified in the live app):
**(1) DRIVES-GUARDS is deeper than "set the param":** the inserted user op is CONCRETE — `guardCount:0` (guards already pruned at instantiate), `op.params` carries the structural values, STRUCTURAL is empty. The generic `op` block emits TRANSPARENTLY (blockEmitter.js:104 — emits `op.children`, no re-instantiate on edit; the reverse-sync + replaceOp path at blocksApp.js:580 is `if (blk.type !== 'op')` → EXCLUDES the generic op). So a structctl edit does NOT reprune by itself. To make it drive the guards LIVE needs a NEW hookup: structctl field-change → find the parent op → gather the structctl {param:value} → merge into op.params → `replaceOp(opId, params)` (verified: it re-instantiates a user op via the builder → reprunes → reloads). PLUS a structctl-VALUE-sync at (post)instantiate — else the re-instantiated structctl blocks reset to the TEMPLATE DEFAULT (the toggle visibly flips back). This is a new branch in the SHARED ws.addChangeListener (scoped to structctl → additive, but shared-surface) + a new derive step (structctl.value ← params[param], parallel to bindingSpecs).
**(2) The GENERIC block hits a Blockly STATIC-FIELD wall:** jsonDef renders STATIC fields (`optionsFor` is per-DEF, not per-instance). A single generic `structctl` block CANNOT carry per-INSTANCE enum options (corner FL/FR/BL/BR, wcs 7-way) NOR switch toggle-vs-dropdown per instance without a CUSTOM FIELD / dynamic extension (like field_cornergrid/ddcs_dynfields) — that is larger than MODERATE.

### SHAPE OPTIONS (the decision — all share the same reprune hookup + isAtom-exclude + byte-parity-emits-nothing):
- **(A) PER-PARAM static blocks (RECOMMEND):** 5 tiny native blocks — `sc_probezfirst` + `sc_synca` (checkbox), `sc_corner`/`sc_probeseq`/`sc_wcs` (static dropdowns with their own options). Real toggles/dropdowns, NATIVE static rendering (LOW Blockly risk), fill the STRUCTURAL section. Cost: 5 corner-specific block types (deviates from "generic" — but sidesteps the static-field wall the human's generic choice didn't know about). Effort: MODERATE.
- **(B) GENERIC block, custom value field:** one `structctl` + a custom field that renders checkbox/dropdown-with-per-instance-options from the block's kind/options data. Matches "generic" + real UX. Effort: LARGER (a new Blockly custom field).
- **(C) GENERIC block, plain text value:** one `structctl` with a static TEXT `value` field ("0"/"1"/"FL"). Simplest/generic, but an editable text box — NOT the toggle/dropdown UX the dispatch wants.
...
## 🔨 turn 154 (cycle 12) — STRUCTURAL-KNOBS BUILD (Option A + REFLECT THE FORM). Suite 524 pass (1 parallel-run flake passes in isolation; middle-animator known flake). The STRUCTURAL section now holds 6 working controls that reflect the wizard form + drive the guards LIVE. 7 files (+ new block file + new spec).

**SHAPE = per-param blocks (A)** — a single generic block can't carry per-INSTANCE dropdown options (Blockly renders STATIC fields), so per-param `sc_<param>` types sidestep it: a bool → a checkbox `value`; an enum → a dropdown `value` (options via `def._options`, read by bridge.optionsFor). **REFLECT THE FORM:** corner's STRUCTURAL section is FILLED by iterating CORNER_STRUCT_BINDINGS (the SAME source the runtime form renders from) → the controls' PRESENCE + defaults are DRIVEN from the bindings; the block DEFS mirror the bindings 1:1, asserted EXACTLY by corner-structctl.spec (one-source by assertion). [Residual: the defs are hand-declared in structCtl.js, NOT imported from cornerData — importing CORNER_STRUCT_BINDINGS into the PALETTE's early module load pulled cornerData's eval-time cornerDataStack ahead of userOps.js init → an ANCHORS load-order crash; the assert-match test is the one-source guard.]

**PIECES:** (1) NEW `wizards/ops/structCtl.js` — the 6 `sc_*` defs + SC_PARAM map + isStructCtlType. (2) ops/index.js registers `...STRUCT_CTL_BLOCKS` in the PALETTE. (3) bridge.js — optionsFor reads `def._options` (enum dropdown), jsonDef keeps the structctl label + drops the field-name prefix ("Probe Z First [x]"). (4) cornerData.js — MOVED CORNER_STRUCT_BINDINGS above cornerDataStack (so it can DERIVE the STRUCTURAL controls at eval); the STRUCTURAL section is filled from the bindings; `applyStructCtl` re-syncs each control's value ← the resolved op param in postInstantiate. (5) devMode.js — isAtom excludes structctl (no knob kit, like section). (6) blocksApp.js — the LIVE-REPRUNE hook: an sc_* field-change → walk to the enclosing user op → gather ALL sc_* values into op.params → `replaceOp` (re-instantiate → pruneGuards reprunes → reload). ADDITIVE + SCOPED to sc_* (isStructCtlType) — no other block's change handling touched.

**GATES VERIFIED (specs + screenshot):** (A) BYTE-PARITY — the controls emit nothing → corner-redivide 32-combo + corner-scalar-parity + twin==cornerStack (probeZ on/off) all byte-identical. (B) DRIVES-GUARDS + REFLECTS FORM — toggling `sc_probezfirst` reprunes the preview LIVE (the Z-Surface arm appears, emit 1707→2053) AND the control KEEPS its value (applyStructCtl, no snap-back); the 6 controls match CORNER_STRUCT_BINDINGS exactly (param/kind/options/label). (C) ROUND-TRIP — the controls round-trip (blocks-live-form / studio-to-blocks / blocks-edit-reconcile / blocks-authoring-always-on all green). (D) BLAST-RADIUS — ONLY corner declares them; the 5 other ops unaffected (full suite); the change-listener branch additive/scoped. Screenshot: scratchpad/structctl.png — STRUCTURAL shows Probe Z First ☐ · Travel[Auto] · WCS[Active] · Dual-Gantry Sync A ☐ · Corner[Front-Left] · Probe Order[Y then X].

**PASSED BACK for advisor FAN-OUT review (the shared change-listener) + human eyes. This is the LAST Blockly follow-up — the reorg thread is complete. Corner RELEASED, disjoint.**

## 🔨 turn 156 (cycle 12) — STRUCTURAL-KNOBS FIX: value-edit clobber guard (fan-out defect). Suite 525 pass (blocks-live-form:89 a parallel-run flake — passes in isolation; middle-animator known flake). 2 files.

**THE DEFECT (fan-out found):** the t154 sc_* live-reprune branch called `_ops.replaceOp` UNCONDITIONALLY — missing the `isOpBlockEdited/mergeOpBlocks` guard its SIBLING op-header branch has. Data-loss scenario: insert corner → edit a VALUE socket in Blocks (e.g. #1 dist 500→250; the edit is live + marks the op edited, but a value-socket edit reprojects with origin==='blockly' and does NOT refresh opBlk.data) → toggle ANY structural control → the hook seeds params from STALE opBlk.data (dist:500) + the structural override → replaceOp rebuilds ALL children from stale params → the #1 socket regenerates at 500 → the 250 edit is LOST.

**FIX (blocksApp.js):** mirror the sibling guard in the sc_* branch — `if (isOpBlockEdited(opBlk.id) && _ops.mergeOpBlocks) _ops.mergeOpBlocks(opBlk.id, params); else _ops.replaceOp(opBlk.id, params);`. mergeOpBlocks reconciles the structural reprune WITH the live value edits (it already supports the generic `op` — it's why corner_op's structural fields route through it), so a structural toggle on an EDITED op MERGES (preserves the edits) instead of wholesale-replacing. When the op is NOT edited → the plain replaceOp path (unchanged).
...
## 🔨 turn 161 (cycle 13) — PANEL + PARAM_GROUP KNOB-NOISE (interleaved easy fix, same class as t148 section-title). Suite 528 pass / 0 fail / 2 skip (no flakes this run). AUTHORING-ONLY -> byte-identical. 3 files (+ new spec).

**THE NOISE:** devMode.augment (the Blocks-tab "expose as knob" kit) grows a `DECL_<FIELD>` row (a dim `knob` marker + a FALSE checkbox + a name) on every isAtom block's numeric + inline fields. isAtom excluded only `section`/`structctl`, so the PANEL block's panel-type field + the PARAM_GROUP block's group-name field still got the kit — reading as "? knob false" noise on AUTHORING METADATA (the panel layout / the group label), NOT exposable wizard VALUES.

**FIX (extend the metadata exclusion, uniform + declared):**
- **devMode.js isAtom** — added `def.kind === 'panel' || def.kind === 'param_group'` to the exclusion (was section/structctl only). So section + structctl + panel + param_group all render clean; value atoms (assign/param/move/sim/comment...) are UNTOUCHED -> the value-knob expose path (deriveAuthoredDef/EXPOSE_) still works.
- **panel.js** — the dispatch assumed `def.kind === 'panel'` but the panel block had NO `kind` (the odd one out; section/param_group/structctl all carry an explicit metadata kind). Rather than special-case it by TYPE in isAtom, I DECLARED `kind: 'panel'` on the block — so isAtom stays uniform (kind-based) and panel is now self-describing as metadata. [declare-liberally: a near-free inert declaration over a hand-rolled type special-case.]

**WHY panel.js is safe (audited every block-def `.kind` consumer):** `.kind` is only ever matched against explicit values — stackBridge (user_root/param_group/section), lint (var/loop/container/path), bridge.js (isWrap list / isSection / isStructctl / `inputsInline: kind !== 'user_root'` / addMouth), blockEmitter (wrap+var/loop/.../container/path lists). A NEW `'panel'` value matches NONE of them (panel is in no wrap/emit list; `inputsInline` is unchanged since undefined and 'panel' both differ from 'user_root'); it ONLY becomes reachable in isAtom. Inert everywhere else. No emit path touched -> byte-parity by construction.

**VERIFIED REAL SYMPTOM (spec + mutation + screenshot):** NEW `blocks-knob-metadata.spec.js` — opens corner in Blocks, asserts (a) the panel block has ZERO DECL_ rows, (b) the param_group block ZERO, (c) sections ZERO (t148 regression guard), and (d) POSITIVE CONTROL — a real value atom (not a metadata/guard kind, not the op) STILL carries a DECL_ knob (proves NOT over-excluded, asserts the value not just the change). MUTATION-verified: reverting isAtom to the pre-t161 (section/structctl only) turns it RED — the panel block regrows `DECL_PANEL`. Screenshot scratchpad/knob-clean.png: FORM shows "panel panel [form3d]" + "Parameter Group group [Corner]" CLEAN, while the 3D-SIM "preview rig" block + the VARIABLES "Comment text" block still show their `knob` rows (value atoms keep the affordance). BYTE-PARITY: authoring-only, no emit path -> corner-redivide / corner-scalar-parity / round-trip all green; full suite 528.

**PASSED BACK for advisor review. INTERLEAVED easy fix while the human reviews the t159 machine-frame/ATC roadmap; machine-frame P-A (reconcile ATC/G53 onto the fixed frame + the magazine-drift bug) is queued next pending the human's fork-2 decision. Corner RELEASED, disjoint.**

## 🔨 turn 163 (cycle 13) — MACHINE-FRAME P-A: reconcile the ATC magazine + G53 path onto the FIXED machine frame. Suite 530 pass / 0 fail / 2 skip. SIM/VISUAL only -> emit byte-identical. 2 files (gcodeViz3d.js + new spec).

**DROVE THE BUG (step 1, not code-read):** a throwaway repro drove the REAL atc_change wizard with a non-zero WCS (G54 y=-666, workOrigin synced) + a magazine + measured scene positions. CONFIRMED: pocket 1 (machine y=100) rendered at world **y=766** (drift +666 = -workOrigin.y); the G53 path world bbox ALSO reached **y=766** — both slid off the fixed envelope by the WCS offset, TOGETHER. (Default zero-WCS hid it — the common case.)

**TWO ROOT CAUSES (different):**
- MAGAZINE: `setMagazine` drew pockets at `machine - workOrigin` added STRAIGHT to this.scene (a STALE 'same frame as the envelope' comment) — never carried across the sceneFrame.js reframe (which fixed home @ scene 0 + moved op/stock/tool/markers to a part frame @ +workOrigin, but left the magazine behind).
- G53 PATH: the toolpath rides the PART frame (gcodeViz3d:88), whose shift `_partShift` derives from the STOCK's WCS pin (0 when no pin) — while the ENGINE subtracts the ACTIVE wcsOffset from G53 moves (GcodeExecutionEngine:948). With no stock pin (an ATC preview), part-frame xy=0 but the engine subtracted the WCS -> UNCOMPENSATED -> the G53 path renders at machine-wcsOffset (drifts). KEY CONSTRAINT: the magazine + path currently drift TOGETHER, so fixing ONLY the magazine would MISALIGN it from the still-drifting path -> both must move together.

**FIX (2 pieces, gcodeViz3d.js):**
- **Magazine -> FIXED frame, RAW machine coords.** Dropped the `- ox/oy/oz` (workOrigin) subtraction in setMagazine; pockets now draw at raw machine coords in this.scene, exactly like the envelope box + home. The magazine is a FIXED machine object (pockets are absolute machine positions) -> workOrigin-INDEPENDENT.
- **Part frame carries the ACTIVE WCS when there's no stock pin.** `_partShift`: when the stock has no WCS pin (pin missing/origin), fall back to the ACTIVE WCS row (was: 0). So a G53/machine move — which the engine offsets by the active wcsOffset — CANCELS to raw machine coords on the fixed envelope, ALIGNED with the magazine. XY only (Z stays per the datum; absolute-machine-Z is P-B). Zero-WCS / no table -> 0 (the common case is byte-identical).

**DESIGN NUANCE (flag for the fan-out review):** the dispatch's literal ask was "put the G53 path on the FIXED frame, not the moving part frame." I implemented the engine-CONSISTENT minimal fix instead: the path stays on the part frame but the frame now sits at the ACTIVE WCS so the engine's existing G53 `- wcsOffset` CANCELS -> the NET rendered position is raw machine (the dispatch's VISIBLE goal — path on the envelope, aligned with the magazine). The literal alternative (raw-machine G53 coords on a fixed group) would require an ENGINE rewrite that BREAKS the with-stock case (the engine subtracts wcsOffset precisely so the path @ part-frame renders at raw machine) — i.e. per-segment frame splitting, a much larger + riskier change for the SAME visible result. So I eliminated it. This completes the reframe correctly (the part frame's job IS to carry the WCS origin; it just defaulted to 0 with no stock).

**VERIFIED REAL SYMPTOM (driven repro + spec + mutation + before/after screenshots):**
- AFTER the fix, the driven repro shows the magazine pocket + the G53 path bbox both at world **y=100** (was 766) — the TRUE machine position, aligned, on the envelope; the part frame at the active WCS (0,-666).
- NEW `atc-magazine-frame.spec.js` (2 tests): (1) the magazine anchor is INDEPENDENT of workOrigin (raw machine coords across 3 different WCS values) AND the part frame rides the active WCS when there's no stock pin (so G53 cancels); (2) REGRESSION GUARD — a stock pinned to G54 still rides the STOCK pin even when the active WCS is G55 (the fallback is ONLY for the no-pin case -> mill ops untouched). MUTATION-verified: stashing the gcodeViz3d fix turns test (1) RED (magazine drifts) while test (2) stays green.
- SCREENSHOTS scratchpad/atc-before.png vs atc-after.png (same non-zero WCS): BEFORE — magazine + spindle + G53 path shoved to the front-left, HANGING OFF the envelope edge; AFTER — all inside the fixed envelope, pockets on the floor, the yellow G53 path clean within the box. Opened in a VS Code tab for the human.
...
## 🔨 turn 167 (cycle 13) — P-C.0: ATC DIALECT FIX (declared, SIM-side, emit BYTE-IDENTICAL). Suite 532 pass / 0 fail / 2 skip. 4 files (engine + virtualIO + new spec + verification script).

**THE FIX — one DECLARED table replaces the hand-rolled dialect.** The sim ENGINE carried a stale/wrong ATC dialect while the wizard EMIT is already correct (advisor ground-truthed t166; I re-confirmed against DDCS-ATC-WORKFLOW.md §Sensors/§Outputs — lines 28-39). Declared `export const ATC_DIALECT` in GcodeExecutionEngine.js (module-level, next to DRO_BASES): M-code -> {kind: wait|output, pin, state, label}, covering the FULL ground truth — OUTPUTS M154/M155 drawbar release/lock, M162/M163 dust open/close, M150/M151 gripper open/close; WAITS M300 spindle-stopped, M301/M302 drawbar released/clamped, M303/M304 magazine open/closed, M305/M306 gripper open/closed. The engine's ATC branch now READS the table (one `else if (ATC_DIALECT[m])` -> wait|output), REPLACING the split local ATC_WAITS + the hand-rolled M154/155 + M305/306 if-else.

**WHAT WAS WRONG (all fixed, asserted):** (1) M301 (drawbar-released wait) was ABSENT from ATC_WAITS -> a silent no-op; now waits IN_DRAWBAR_OPEN. (2) M302 was IN_TOOL_LOCKED; now IN_DRAWBAR_CLOSED (drawbar-clamped). (3) M303/M304 were mislabelled tool-open/closed; now IN_MAG_OPEN/IN_MAG_CLOSED (magazine). (4) M305/M306 were hand-rolled as a dust-cover OUTPUT; now gripper WAITS (IN_GRIPPER_OPEN/CLOSED). (5) M162/M163 (dust cover) were UNIMPLEMENTED; now the dust-cover OUTPUT (OUT_DUST_COVER on/off). (6) M154/M155 fired the hand-rolled OUT_TOOL_RELEASE family; now the drawbar clamp-cycle pins (OUT_SPINDLE_UNCLAMP -> IN_DRAWBAR_OPEN [M301], OUT_SPINDLE_CLAMP -> IN_DRAWBAR_CLOSED [M302]).

**REAL PINS (do-not-invent, grounded):** every table entry maps to a REAL virtualIO pin. The EMITTED codes (M300/301/302/154/155/162/163) all mapped to EXISTING pins. For completeness the table also declares the (not-currently-emitted) gripper + magazine codes, which needed grounded pins: added OUT_GRIPPER_OPEN/CLOSE outputs + handshakes (-> IN_GRIPPER_OPEN/CLOSED) to virtualIO's truth table (M150/M151 fire them; M305/M306 wait them); IN_MAG_OPEN/IN_MAG_CLOSED are position/firmware sensors with no program output (M303/M304 wait, satisfied by auto-answer, like any un-driven sensor). REMOVED the now-orphaned hand-rolled OUT_TOOL_RELEASE / OUT_TOOL_RELEASE_OFF truth-table entries (superseded by the drawbar pins) + fixed the OUT_DUST_COVER descriptions (were labelled M305/M306, are M162/M163).

**CROSS-CHECK emit vs the doc (dispatch: flag residuals, don't silently pick a side):** the wizard EMIT (atcChangeWizard.js autoStack/diskAutoStack/firmwareStack + atcTestWizard drawbarStack) uses M300/M301/M302 waits + M154/M155 drawbar + M162/M163 dust — ALL match the doc exactly (WORKFLOW.md:28,30,38 + the real T.nc line 86). NO residual mismatch on the emitted codes. RESIDUAL FLAGGED (not a mismatch, a completeness gap): the gripper (M150/M151/M305/M306) + magazine-sensor (M303/M304) dialect is DECLARED (ground-truthed) but NOT emitted by the current ASSUMED generic/disk stacks — the real M350 change sequence uses drawbar + pneumatics (M156-M161), not the gripper/mag path. So those table entries are forward-looking (they'll matter when the swap/collet visual + a completed emit exercise them). The firmware pneumatics (M156-M161/M159/M19) are timed by a G04 dwell, not sensor waits, so they're outside the sensor/output dialect (unchanged, no-op in the sim).

**VERIFIED (spec + mutation + byte-parity):** NEW `atc-dialect.spec.js` (2 tests) asserts the SEMANTICS, not just green: (1) the ATC_DIALECT declaration matches the M350 ground truth (each M-code's kind/pin/state); (2) the engine HONORS it — M162 fires the dust OUTPUT (was unimplemented), M163 fires it OFF, M305 is a gripper WAIT that does NOT fire the dust output (the OLD WRONG behavior, explicitly asserted absent), M301 parks on the drawbar-released sensor (was a no-op), M154 -> drawbar-release output (NOT the removed OUT_TOOL_RELEASE), M303 -> magazine wait (NOT the old tool-open). MUTATION-verified: stashing the engine/virtualIO fix turns BOTH tests RED. BYTE-PARITY: no emit/wizard file touched -> the emitted .nc is byte-identical; atc-roundtrip + all emit specs green; FULL suite 532 pass / 0 fail.

**Also updated `verification/atc-gen-test.mjs`** (a standalone, NOT-in-CI node script) — its sensor-pin assertions asserted the OLD WRONG pins (IN_TOOL_LOCKED/IN_TOOL_OPEN); updated to the ground truth (IN_DRAWBAR_CLOSED/IN_DRAWBAR_OPEN). NOTE: the script has a PRE-EXISTING, unrelated env failure (case 1 `#1300 = undefined` — no settings/magazine stub in node); confirmed pre-existing by stashing my change and re-running (same failure on the original engine). Out of scope for the dialect fix; flagged.

**PASSED BACK for advisor review; next is P-C.1 (the swap visual). BACKLOG (still open, non-blocking): the _partShift asymmetric-fallback one-liner. SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 169 (cycle 13) — P-C.1 SWAP VISUAL: GATE (drove the firmware push first; the dispatch's #1300-change swap trigger is DRIVEN-FALSE for the firmware method — passing back the finding + a tool-identity fork before building the shared-core swap). No code. Suite untouched.

**DROVE IT FIRST (real symptom, headless firmware trace + station vars + a current tool #1300=5):**
- The firmware push REACHES the taught station in MACHINE coords — segments (0,0,-10) → (200,150,-10)[push-start #1320/1321] → (250,150,-10)[push-end #1323/1324] → (300,150,-10)[retreat #1325/1326]. So VERIFY (b) spindle-reaches-station + (d) cross-frame (via the P-A part-frame-at-WCS map) are ACHIEVABLE + grounded.
- BUT **#1300 stays 5 (UNCHANGED) and #1504 is undefined** — the firmware stack (atcChangeWizard.js:245-266) emits NO #1300 assignment and carries NO target tool. Confirmed the firmware UI (fwRow) has ONLY the M19 toggle — no "change to tool" target (the target selector is shown for m6/generic/disk, NOT firmware — atcViews.js:272). firmwareStack(params) uses only params.orient.

**THE GAP (why this is a GATE):** the dispatch's core swap trigger — "at the tool-identity #1300 change (during the push) swap old→new" — DOES NOT HOLD for the firmware method: there is no #1300 change and no NEW-tool identity in the emitted firmware push. So a standalone firmware atc_change PREVIEW (which runs exactly the op's emitted code) has NOTHING to swap TO and no change to trigger on — the swap visual would never fire in the real wizard. Building it on this premise would require INVENTING a target tool or a synthetic #1300 change (violates only-present-legitimate-options / dont-declare-away-user-responsibility). The push MOTION + station HIGHLIGHT are groundable NOW; the SWAP's tool-identity source is the open decision.

**WHAT IS UNAMBIGUOUS + READY TO BUILD (on confirm):** (1) the DECLARED per-method seam (SWAP_CHOREOGRAPHY keyed on resolveMethod; firmware entry, others no-op) — the mechanism the dispatch wants; (2) the STATION HIGHLIGHT of the taught #1320-1326 push region in the fixed MACHINE frame (the coords are in the traced G53 positions); (3) reuse of the already-animated G53 push travel (post-P-A, fixed frame); (4) the swap MECHANISM itself (setSimTool rebuilds the anim tool → old→new geometry; _animParts.tool was pre-split for exactly this). The only open piece is the SWAP's TRIGGER + the NEW-tool identity.

**THE FORK (tool-identity source for the firmware swap):**
- **(A) ADD A TARGET to the firmware op** — a "change to tool" selector on the firmware UI (like m6/generic/disk), SIM-ONLY (does NOT change the emitted push → still byte-identical). The swap = current→target at the push stroke (positional trigger). Concrete + demoable in the standalone preview. Cost: a small UI/param addition + a positional swap trigger.
- **(B) VALID-BY-CONSTRUCTION (matches the human's stated principle)** — the swap fires ONLY on a REAL #1300 change in the traced program (a `T# M6` before/with the change sets #1300 via the engine). The standalone firmware op shows the push + station highlight but NO swap (there's no tool change to show); the swap appears when the op runs in a real program context. No target invented. Cost: hook #1300 during play (poll in onPositionChange — no engine change); the VERIFY test drives `T7 M6` + firmware push to demo the swap.
- **(C) SIM-ONLY EXCHANGE placeholder** — at the push stroke the current tool retracts + a generic "new tool" emerges (no specific identity). Conveys an exchange without a target. Cheapest visual, least faithful.
- **(D) PREVIEW SYNTHESIZES a target** — the firmware preview prepends a synthetic `T<next> M6` so #1300 changes + the swap demos. A preview-harness-only decision (emit untouched).

**RECOMMENDATION:** build (1)-(4) [seam + station highlight + push reuse + swap mechanism] regardless — they're unambiguous. For the trigger, **(A) add a sim-only target to the firmware op** — it makes the swap concrete + demoable in the actual wizard preview (what the user sees), stays byte-identical (sim-only param), and is still valid-by-construction (the swap follows the op's DECLARED target). (B) is the purest but the firmware wizard preview alone then shows no swap — a UX call the human should make. All options keep the seam + station highlight + emit-byte-identical.

**GATED per worker step-5 (a shared-sim-core change whose PREMISE the driving proved false; a genuine tool-identity fork). NO code. PASSED BACK the finding + fork + recommendation for advisor synthesis + human eyes. Ready to build immediately on confirm. Backlog _partShift one-liner still open. Emit UNTOUCHED throughout; authoring axis stays done + disjoint.**

## 🔨 turn 171 (cycle 13) — P-C.1a: FIRMWARE push-station preview (declared per-method seam + station highlight, NO swap yet). Suite 535 pass / 0 fail / 2 skip. SIM/VISUAL only -> emit BYTE-IDENTICAL. 6 files (+ new spec).

**THE DECLARED SEAM (ATC_CHOREOGRAPHY, atcChangeWizard.js):** a registry keyed on resolveMethod that says HOW each change method animates in the sim — so the visual follows the op's DECLARED method (valid-by-construction), NO hard-coded animation. `firmware` = { kind:'push', stationVars:[1306,1320-1326], region(vars) } — a real INLINE pneumatic push at a taught fixed station; `m6` = { kind:'macro-call' } (controller runs T.nc — nothing inline); generic/disk/manual = null (no-op, P-C.3). Exported resolveMethod + ATC_CHOREOGRAPHY + `atcChoreography(params)`. The tool-SWAP (trigger + identity) is DELIBERATELY absent (P-C.1b, pending the human's swap-identity call).

**THE STATION HIGHLIGHT (gcodeViz3d.highlightStation):** marks the taught push region — push-start #1320/1321 -> push-end #1323/1324 (the swap stroke) -> retreat #1325/1326 — on the FIXED machine frame at RAW machine coords, EXACTLY like the magazine + envelope (post-P-A). The #1320-1326 are MACHINE/G53 coords, so raw-machine on the fixed frame means the highlight ALIGNS with the already-animated G53 push travel (which lands at raw machine via the P-A cancellation) — cross-frame correct. Distinct teal-green (0x35ff9e) vs the amber magazine / cyan path / yellow stock: a bold start->end->retreat line + sphere markers (push-end emphasised) + a "PUSH STATION" text sprite. Stored `_stationGroup`, cleared on re-call (like `_magGroup`).

**THE WIRING (additive, isolated):** (1) `traceToolpath` gained an OPT-IN `opts.captureVars` -> `result.vars = {n: resolved value}` (reads the taught #1320-1326 after the trace, before disposing the transient engine) — zero impact on other callers. (2) `wizardManager.previewAtcStation(vizId, region)` — a thin pass-through -> viz.highlightStation (mirrors previewMagazine). (3) the atc_change VIEW resolves `atcChoreography(params)`; FIRMWARE -> traceToolpath(gcode, {captureVars: choreo.stationVars}) -> choreo.region(vars) -> previewAtcStation; non-firmware -> previewAtcStation(null) (clears). So ONLY a firmware atc_change op highlights a station; every other op/method + every non-ATC preview is untouched.

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-station-highlight.spec.js` (3 tests, asserts the VALUE): (1) the seam DECLARATION — firmware=push with stationVars [1306,1320-1326] + region() builds {z,start,end,retreat}; m6=macro-call; generic/disk/manual=null. (2) the highlight sits at the taught RAW machine coords (200,150)/(250,150)/(300,150) AND is WCS-INDEPENDENT (same across two different non-zero WCS -> the fixed-frame guarantee, aligning with the G53 push post-P-A). (3) REAL-SYMPTOM — the emitted firmware push (with taught station vars) TRAVELS start->end->retreat reaching the station machine coords, choreo.region(capturedVars) = the taught #1320-1326; AND opening a real atc_change op in FIRMWARE method resolves firmware (not generic) + the view wires the highlight (a `_stationGroup` exists). Screenshot scratchpad/station-highlight.png (envelope + magazine + the teal-green push station on the fixed frame) opened in a VS Code tab. BYTE-PARITY: no emit stack touched -> atc-roundtrip + goldens green; FULL suite 535. BLAST: additive -> non-firmware + non-ATC previews unaffected (mill/corner + full suite green).

**KNOWN LIMITATION (noted, not a bug):** the taught station coords #1320-1326 come from the TRACED code -> in a live wizard preview WITHOUT a controller pull they resolve to 0 (highlight at machine origin — a "teach/pull me" signal). The station-coords SOURCE (a profile pull, like the WCS table) is a separate concern; the highlight MECHANISM is correct + frame-faithful. The screenshot uses injected coords to show the highlight at a real station.

**PASSED BACK for advisor review. NEXT: P-C.1b (the swap trigger + tool identity) per the human's pending call; then P-C.2 (articulation: M156-161 pneumatics + pusher/pin). Backlog _partShift one-liner still open. SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 173 (cycle 13) — EASY FIX: the _partShift asymmetric-fallback one-liner (fan-out t164). Suite 536 pass / 0 fail / 2 skip. SIM/VISUAL only -> emit BYTE-IDENTICAL. 2 files (gcodeViz3d + spec).

**THE MISMATCH:** P-A's `_partShift` no-pin fallback read the active WCS TABLE row (guarded on `Array.isArray(wt)`), else 0 — but `wcsForViz` (createPreviewPanel, the engine's G53 wcsOffset source) falls back to `m.workOrigin` when the table is null. So a PARTIAL/legacy profile (wcs.workOrigin set, wcs.table null): the engine offsets G53 by workOrigin while `_partShift` returned 0 -> the G53 path renders at machine−workOrigin (drifts) while the magazine sits at raw machine -> MISALIGN.

**FIX (one-liner, gcodeViz3d _partShift):** changed the `else if (Array.isArray(wt))` no-pin branch to a plain `else` with the SAME fallback chain as wcsForViz: `(Array.isArray(wt) && wt[active-1]) || m.workOrigin`. So the chain is now stock-pin row -> active WCS table row -> m.workOrigin -> 0, mirroring wcsForViz EXACTLY (one shared fallback). With table null + workOrigin set, the part frame now rides workOrigin -> the engine's G53 −wcsOffset CANCELS -> the path lands at raw machine, ALIGNED with the raw-machine magazine.

**VERIFIED (spec + mutation):** extended `atc-magazine-frame.spec.js` (+1 test) — a partial profile (workOrigin {220,130}, table null, no stock): assert the part frame rides workOrigin {220,130} (so G53 cancels) AND the magazine sits at raw machine {100,100} -> the two ALIGN (both resolve to raw machine, not drifted apart). Asserts the VALUE (positions), not just animated. MUTATION-verified: reverting to the old `else if (Array.isArray(wt))` turns the new test RED (part frame = 0), while the existing tests 1-2 stay green (they don't exercise the table-null fallback). ZERO-WCS unchanged (no table + no workOrigin -> a=undefined||undefined -> 0). BYTE-PARITY: SIM-only (no emit path) -> full suite 536 pass; goldens green.

**PASSED BACK for advisor review. NEXT: P-C.1b (the swap trigger + tool identity) pending the human's identity call. Backlog item now CLEARED. Emit UNTOUCHED; authoring axis stays done + disjoint.**

## 🔨 turn 175 (cycle 13) — P-C.1b: FIRMWARE tool-SWAP visual (valid-by-construction, purist — swap fires ONLY on a real #1300 change). Suite 539 pass / 0 fail / 2 skip. SIM/VISUAL only -> emit BYTE-IDENTICAL. 5 files (+ new spec).

**DROVE THE TRIGGER FIRST (dispatch step 1 — verify real symptom):** headless repro — `T5 M6` FLIPS #1300 2->5 (the engine models T->#1300 via the M6 handler); an isolated firmware push (no tool change) leaves #1300 UNCHANGED (7->7); a bare `T5` (no M6) does NOT flip it (2->2, correct DDCS: T selects, M6 executes). SO the trigger EXISTS -> built the swap on the #1300 change (no invented trigger, no sim-only target — the human's purist choice).

**THE SWAP (method-aware via the ATC_CHOREOGRAPHY seam; firmware=push only):**
- **Detection** — poll #1300 in the play loop's `onPositionChange` (`checkToolSwap`, createPreviewPanel). The FIRST value seen is the STARTING tool (no swap); a subsequent change old->new IS a real tool change (a program T#/M6) -> fire. onPositionChange fires as the push MOVES execute, so the swap lands ≈ at the push stroke. This is CORRECT for the real FULL-program flow: a move before the T#/M6 sets lastTool = the old tool, then the push moves see the new #1300 -> swap. The ISOLATED wizard preview (the op alone, no T#/M6) never flips #1300 -> NO swap (valid-by-construction; the op has no target — the human accepted this).
- **Effect** (`doToolSwap`): retire the OLD tool to the push STATION + put the NEW tool on the spindle. Tool specs looked up in the tool table (toolsForViz by #num). CROSS-FRAME (reuses P-A): the retired OLD tool (`viz.showRetiredTool`) sits at the push-END #1323/1324 on the FIXED machine frame (raw machine coords, dimmed grey, reuses the magazine tool builder); the NEW tool goes on the spindle via `setSimTool` (rides the PART frame @ WCS).

**WIRING (additive, isolated):** (1) `gcodeViz3d.showRetiredTool(tool, region)` — the grey retired tool at the station, null clears. (2) createPreviewPanel: `setAtcSwap(choreo, region)` arms it (only kind==='push'); `checkToolSwap`/`doToolSwap` fire during play; play() resets lastTool + clears the retired tool (fresh run re-arms); returned in the API. (3) `wizardManager.previewAtcSwap` thin pass-through -> panel.setAtcSwap. (4) the atc_change VIEW arms BOTH the station highlight AND the swap (choreo + region computed once). So ONLY a firmware atc_change op arms the swap; every other op/method + non-ATC preview is untouched.

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-tool-swap.spec.js` (3 tests, assert the VALUE): (1) `showRetiredTool` places the OLD tool at the push-END station coords (250,150), WCS-INDEPENDENT (fixed frame) + null clears it. (2) a REAL FULL-program play through the atc_change firmware panel — `#1300=2` -> a move (sets old=2) -> `T5 M6` (2->5) -> the firmware push — RETIRES tool 2 to the station (250,150) AND puts tool 5 on the spindle (`_simTool.num===5`). (3) the ISOLATED firmware op (its own code, no tool change) plays to completion with NO retired tool (no swap). Screenshot scratchpad/tool-swap.png (the push + green station + spindle tool + the dim retired tool in the machine frame) opened in a VS Code tab. BYTE-PARITY: no emit stack touched -> atc-roundtrip + goldens green; FULL suite 539. BLAST: additive event-overlay -> non-firmware + non-ATC previews unaffected (mill/corner + full suite green).

**NOTES:** the swap shows in a FULL PROGRAM (a tool change + the firmware push), NOT the isolated wizard preview (the op alone has no #1300 change) — correct valid-by-construction. The station coords still come from the traced code (untaught/unpulled -> 0), same P-C.1a limitation (the coords SOURCE = a pull, a separate concern). A tweened old-tool-leaving animation (vs the discrete retire) is a future polish; the swap identity + positions are correct + asserted now.

**PASSED BACK for advisor review. NEXT: P-C.2 (IO articulation: M156-161 pneumatics + model/animate the pusher/pin/dust/vacuum + surface the live IO). SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 177 (cycle 13) — P-C.2a: WIRE the M350 fixed-station pneumatic dialect (M156-161 + M19). Suite 539 pass / 0 fail (1 known middle-animator flake, passed on retry). SIM-side, emit BYTE-IDENTICAL. 3 files (engine + virtualIO + spec). On top of V10.54 (4d632a5).

**THE WIRE:** the firmwareStack RAW lines (atcChangeWizard.js 245-266) emit M19, M159, M157, M160, M163, M156, M161 — the engine RECOGNIZED only M163 (dust, from P-C.0); the rest were silent no-ops. Extended the DECLARED ATC_DIALECT (from P-C.0) with the pneumatics as OPEN-LOOP OUTPUTS — the push is timed by G04 dwells, NOT sensor waits, so they have NO paired sensor handshake (they set the output + fire io_change). Ground truth = slib-m.nc O10102 + firmwareStack:
- M156 → OUT_LOCATING_PIN true (open) · M157 → OUT_LOCATING_PIN false (close)
- M160 → OUT_PUSHER true (open) · M161 → OUT_PUSHER false (close)
- M159 → OUT_VACUUM false (off) · **M158 → OUT_VACUUM true (on)** — M159's pair-half, grounded in the WORKFLOW doc (M158/M159 blow/vacuum on/off) but NOT emitted by the current firmwareStack (declared for completeness; FLAGGED)
- M19 → OUT_SPINDLE_ORIENT true — RECOGNIZED (was a no-op; the engine has no M19 handler); noted for the P-C.2b spindle-orient anim, NOT over-modelled here.
The pins are plain output names (setVirtualOutput sets ioState.outputs) — open-loop, so NO virtualIO truth-table entries (no fake sensors).

**io_change FOR OUTPUTS (virtualIO.setVirtualOutput):** an open-loop output previously fired NO io_change (triggerVirtualHandshake returns when there's no paired-sensor rule). Made setVirtualOutput dispatch io_change for the OUTPUT pin itself on a real change — so the pneumatics are observable LIVE (the P-C.2b device animation subscribes to these). SAFE: the engine's io_change listener (_mirrorInputVar) only mirrors INPUT pins 1-24, so an OUT_* event is a harmless no-op there; the I/O panel already polls + refreshes on io_change. Uniform (all outputs now fire io_change on change) — the existing drawbar/gripper/dust outputs fire an OUT_ event now too (harmless; their paired-sensor io_change still fires after the delay).

**VERIFIED (spec + mutation):** extended `atc-dialect.spec.js` — (1) the DECLARATION covers the pneumatics (M156/157→OUT_LOCATING_PIN open/close, M160/161→OUT_PUSHER, M158/159→OUT_VACUUM on/off, M19→OUT_SPINDLE_ORIENT); (2) NEW test — the engine HONORS them: M160→OUT_PUSHER true, M161→false, M156/157→OUT_LOCATING_PIN open/close, M159→OUT_VACUUM off, M19→OUT_SPINDLE_ORIENT true, AND each open-loop output FIRES io_change (asserts an OUT_ pin appeared in the io_change stream — was a no-op). MUTATION-verified: stashing the engine+virtualIO turns the declaration + pneumatics tests RED (the P-C.0 dialect test stays green). BYTE-PARITY: no emit/wizard file touched — the firmwareStack RAW lines are unchanged → the emitted .nc is byte-identical; atc-roundtrip + goldens green. BLAST: additive (new codes recognized; nothing else uses M156-161) → non-ATC unaffected; FULL suite 539.

**PASSED BACK for advisor review. NEXT: P-C.2b (model + animate the pusher / locating pin / dust cover / vacuum / spindle-orient devices on these io_change events), then P-C.2c (surface the live IO). SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 179 (cycle 13) — P-C.2b: MODEL + ANIMATE the M350 station devices (PUSHER + LOCATING PIN). Suite 543 pass / 0 fail (1 known middle-animator flake, passed on retry). SIM/VISUAL only -> emit BYTE-IDENTICAL. 3 files (gcodeViz3d + createPreviewPanel + new spec).

**MODEL (gcodeViz3d.setStationDevices(region)):** at the push station, on the FIXED machine frame at RAW machine coords (like the highlight/magazine/retired-tool, WCS-independent):
- **PUSHER** — a steel-blue piston (body box + a lighter rod), built in a LOCAL frame where +X = the push direction (start->end), then rotated by atan2 into place at the push-start; the rod extends toward the push-END.
- **LOCATING PIN** — an orange vertical pin at the push-END that rises to engage / drops to release.
Distinct colors from the tool/magazine (amber), highlight (teal-green), retired tool (grey). `_deviceGroup` cleared on re-call; `_pusherRod`/`_locatingPin` + their rest/deployed positions stored for animation.

**ANIMATE (gcodeViz3d.setStationDevice(name, on)):** 'pusher' true->rod extends (x: rest->out) / false->retracts; 'pin' true->rises (z up) / false->drops. Driven by the pneumatic io_change.

**WIRE (createPreviewPanel, reuses the P-C.1b seam):** extended `setAtcSwap` — a firmware 'push' choreography now ALSO builds the devices (`viz.setStationDevices(region)`) + subscribes a window `io_change` listener that maps OUT_PUSHER -> setStationDevice('pusher', state) [M160 extend / M161 retract] and OUT_LOCATING_PIN -> setStationDevice('pin', state) [M156 engage / M157 release]; non-firmware clears the devices + UNSUBSCRIBES (the listener is added/removed per arm, no leak). play() resets the devices to rest before the firmware sequence re-animates them. So the devices sync to the firmwareStack M156-161 + the G04 dwells NATURALLY (they fire io_change as they execute, P-C.2a). atcViews UNCHANGED — it already arms via previewAtcSwap -> setAtcSwap.

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-station-devices.spec.js` (4 tests, assert the VALUE): (1) the devices are modelled at the station coords WCS-INDEPENDENT (fixed frame) + setStationDevice transforms them (pusher rod extends/retracts, pin rises/drops) + null clears. (2) a firmware push ARMS the devices + they TRANSFORM on the pneumatic io_change — M160->pusher extends, M161->retracts, M156->pin engages, M157->releases (the real panel io_change listener). (3) END-TO-END — a real firmware push PLAY animates them via the ENGINE io_change (the firmware sequence ends with M156 -> the locating pin ENGAGES, asserted after play). (4) a NON-firmware method (m6) arms NO devices (the seam disarms). Screenshot scratchpad/station-devices.png (green station + blue pusher extended + orange pin engaged) opened in a VS Code tab. BYTE-PARITY: no emit stack touched -> atc-roundtrip + goldens green; FULL suite 543. BLAST: additive (viz + panel; armed only for firmware push) -> non-firmware + non-ATC previews unaffected.
...
## 🔨 turn 183 (cycle 13) — P-C.2c-REVISED: LABEL the ATC IO in the existing numbered io tab from the Settings config (one-source) + join the sim outputs via the M-code + fix the dust bug. Suite 546 pass / 0 fail (1 known middle-animator flake, retry-passed). SIM/UI only -> emit BYTE-IDENTICAL. 4 files (ioTable + ioTab + styles + new spec). NO new panel (human t182 reframe).

**THE DUST BUG (ioTable.js OUTPUT_TYPES):** dustcover onCode/offCode was the STALE **M305/M306** -> fixed to **M162/M163** (P-C.0 ground truth: dust=M162/163, M305/306=gripper). CHECKED for other stale config ATC M-codes: settingsPanel seeds only drawbar (M154/M155, correct) + rotate (blank); ioTable OUTPUT_TYPES: drawbar M154/M155 (correct), dustcover (was the only stale one). No others.

**THE JOIN (config -> sim -> io tab, keyed on the M-CODE):** the io tab now READS the Settings IO config (`settings.outputs` rows: {label, onCode, offCode, pin}) and, for each ASSIGNED output pin whose onCode is an ATC_DIALECT output M-code, maps `pin -> { label, semanticPin }` where semanticPin = ATC_DIALECT[mcode(onCode)].pin. So: config supplies the LABEL + the assigned numbered PIN; the engine ATC_DIALECT supplies the SEMANTIC pin the sim actually drives; the M-CODE is the join. => a config `Pusher` pin (onCode M160) reflects `OUT_PUSHER` LIVE. ONE SOURCE (declare-from-config).
- `ioTab._deriveAtcMap()` builds the map from `window.ddcsGetSettings().outputs` + ATC_DIALECT (imported).
- `ioTab._applyAtcLabels()` (on show) puts the config function name beside each assigned ATC output pin (a `.io-atc-label` span) + the semantic pin in the title + an `.io-atc` border.
- `ioTab.refresh()` — an ATC-config pin reflects `ioState.outputs.get(semanticPin)`; any other pin keeps the numbered/HARDWARE_PIN_MAP state. So during a firmware push the labeled pins LIGHT/flip live in the existing numbered tab -> you watch the handshake fire IN ORDER (the human's sequence ask, no new panel).
- styles.css: `.io-atc-label` + `.io-atc` (subtle border, label lights amber when active).

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-io-labeling.spec.js` (3 tests, assert the VALUE): (0) the dustcover output TYPE uses M162/M163 (not M305/M306); (1) assign ATC pins (Pusher M160/p5, Locating M156/p6, Dust M162/p7) -> the pins are LABELED from the config AND a real firmware push PLAY lights them via the sim semantic output (the push ends with M156 -> the LABELED Locating pin lights; M161 -> the Pusher off) — the config->sim join flows; (2) the dust pin reflects OUT_DUST_COVER (M162) + does NOT light on M305 (that's the gripper) — the fix. Screenshot scratchpad/io-labeled.png: the OUTPUTS row shows `3 Spindle orient` / `5 Pusher` / `6 Locating pin` (LIT amber) · `7 Dust cover` · `8 Vacuum` labeled ON THE NUMBERED PINS (no new section). Opened in a VS Code tab. BYTE-PARITY: no emit touched -> goldens green; FULL suite 546. BLAST: UI labeling additive -> numbered-pin rendering + other IO unaffected.

**FLAG (per the dispatch — the JOIN is trickier for the DRAWBAR than the pneumatics):** the PNEUMATICS are CLEAN single-pin toggles — onCode + offCode drive the SAME semantic pin (M160/M161 both -> OUT_PUSHER true/false; M162/M163 -> OUT_DUST_COVER), so `ioState.get(semanticPin)` is the exact live state. But the DRAWBAR is a TWO-pin output: M154 -> OUT_SPINDLE_UNCLAMP (true), M155 -> OUT_SPINDLE_CLAMP (a DIFFERENT pin, true) — the on/off don't share one pin. So a config drawbar pin (onCode M154) reflects only OUT_SPINDLE_UNCLAMP (the RELEASE); after M155 (clamp) OUT_SPINDLE_UNCLAMP is not cleared, so the pin stays lit (doesn't reflect the clamp). This is OUT OF the firmware-push scope (the push uses the pneumatics, not the drawbar; the drawbar is the generic/disk methods), and the drawbar's two pins are ALSO in HARDWARE_PIN_MAP (4/5) — so I left the drawbar on the clean single-pin behavior + FLAG it for the human: a two-pin output needs either a last-fired-wins state (track which of on/off fired last) or a config convention. NOT built (design call).

**NOTES:** ATC INPUTS (sensors M300-306) are NOT labeled this turn — the firmware push is open-loop OUTPUTS, and sensor input rows have no onCode (the join key); the inputs are a follow-up. Dependency (noted, not built): ATC pins default BLANK -> labels appear once a pin is ASSIGNED (manual/controller-pull is separate config work); the spec assigns TEST pins.

**PASSED BACK for advisor review + the drawbar-join FLAG for the human. SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 185 (cycle 13) — P-C.2d: IO-LABELING COMPLETE (inputs too) + EDITABLE labels (config-driven, ONE SOURCE). Suite 549 pass / 0 fail. SIM/UI only -> emit BYTE-IDENTICAL. 3 files (ioTab + ioTable + spec).

**(a) INPUT LABELS (mirror the P-C.2c output labeling for inputs):** `_deriveAtcMap` now builds `_atcInMap` too — the ATC sensor rows (settings.inputs with group='atc': drawbar-released/clamped/spindle-stopped/pocket-index) → label each ASSIGNED input pin with row.label. `_applyAtcLabels` labels BOTH grids (a shared `apply(sel, map, mode)` for OUT + IN). LIVE-LIGHTING (best-effort per the dispatch): the ATC sensor inputs have NO onCode, so the join parses the WAIT M-code from the label (e.g. "Drawbar released (M301)" → 301) → ATC_DIALECT[301] (kind wait) → IN_DRAWBAR_OPEN → `ioState.inputs.get(IN_pin)`. So a generic/disk change's sensor waits light the labeled input pin; the open-loop firmware push doesn't fire sensors (labeled-but-static for firmware — as the dispatch noted). refresh() lights an ATC input via its semantic sensor when determinable, else the numbered state.

**(b) EDITABLE LABELS (ioTable.js config UI):** the row label was a STATIC span showing the TYPE label (not editable, ignored row.label). Now it's an editable `<input>` BOUND to row.label (default = the type label; on change → `row.label = value; onChange()` saves). ONE SOURCE: row.label shows in BOTH the config UI AND the io tab (which already reads r.label). The group badge sits beside the field. So renaming a pin ("Pusher", "Locating pin") in the config flows straight to the io panel.

**VERIFIED REAL SYMPTOM (spec + screenshot):** extended `atc-io-labeling.spec.js` (+2 tests, now 5): (3) an assigned ATC INPUT pin is LABELED from the config sensor row AND lights when its sensor fires (inject IN_DRAWBAR_OPEN → the labeled pin lights — the best-effort M301 join); (4) EDITABLE — render the config output table, edit the label field to "Big Pusher" → row.label updates + onChange saved AND the io tab reflects the NEW label ("Big Pusher" on pin 5). Screenshot scratchpad/io-labeled-inout.png: INPUTS "3 Drawbar released (M301)" (LIT green) / "4 Drawbar clamped" / "5 Spindle stopped"; OUTPUTS "5 Pusher" + "6 Locating pin" (LIT amber) / "7 Dust cover" — all labeled from the config, lighting live. Opened in a VS Code tab. BYTE-PARITY: no emit touched -> goldens green; FULL suite 549. BLAST: UI (config table + io tab), additive.

**NOTES / FLAGS:** (1) the INPUT live-light join parses the M-code from the LABEL (best-effort) — since (b) makes labels editable, a user who strips the "(M301)" from the label loses the live-light (labeling still works from row.label). A robust join would want a stable `waitCode`/semantic field on the sensor row (schema add, NOT built — flag). (2) A configured semantic-sensor pin's CLICK still injects the numbered pin (resolveVirtualPin), NOT the semantic IN_ pin, so a manual click may not satisfy the wait — a pre-existing separate concern, out of scope. (3) OPTIONAL (dispatch, not built): inline label-edit in the io tab writing the same row.label — the config UI edit already covers the one-source path; flag if wanted.

**PASSED BACK for advisor review. NEXT: P-C.2e (io modal resize by drag, mirror .viz3d-resize). SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 187 (cycle 13) — P-C.2e: io-panel UI polish (legible labels + drag-resize grip). Suite 551 pass / 0 fail. UI/CSS only -> emit BYTE-IDENTICAL. 3 files (styles + ioTab + ioTable + new spec).

**(a) LEGIBLE FONT (picked via the screenshot):** the ATC pin label `.io-atc-label` was 10px dim (#9fb4c9) -> **11px, bold, brighter (#dbe8f5)**; per-state active-label colours split (in=green, out=amber). Bumped the LED base `.io-led` 9px -> 10px. The editable config label INPUT (P-C.2d) 12px -> **13px** (the field the human flagged as possibly cramped). Also generalised `.io-output.io-atc` -> `.io-atc` so the border/label styling applies to the INPUT pins too (P-C.2d added input labels).

**(b) DRAG-RESIZE the floating panel (mirror .viz3d-resize):** added a visible corner grip element `.io-resize` (bottom-right, a 135deg gradient triangle, cursor nwse-resize, touch-action none) + `IOPanel._makeResizable(el)` — mirrors the gcodePreviewTab `.viz3d-resize` drag (pointer-capture, track the start size, set width/height from the delta clamped to [220,92vw]×[170,85vh], persist on release via `_savePos`; the existing `_sizeRo` ResizeObserver also persists any size change). REMOVED the old native `resize: both` (the subtle browser corner grip) — replaced by the visible mirrored grip. Hidden when docked (`.io-panel.embedded .io-resize { display:none }`).

**VERIFIED (spec + screenshot):** NEW `io-panel-resize.spec.js` (2 tests): (a) the `.io-atc-label` computed font-size >= 11px + weight >= 600 (legible + bold); (b) the `.io-resize` grip exists AND a pointer-drag on it (pointerdown/move/up) GROWS the panel width + height (assert after > before). Screenshot scratchpad/io-polish.png: the labels ("3 Drawbar released (M301)" green / "5 Pusher" · "6 Locating pin" amber / "7 Dust cover") now render clearly larger + bolder + brighter; the resize grip sits at the bottom-right. Opened in a VS Code tab. BYTE-PARITY: no emit touched -> goldens green; FULL suite 551. BLAST: UI/CSS (io panel only).

**PASSED BACK for advisor review. NEXT: P-C.2f (the declare-fix — replace the input-label M-code PARSE with a DECLARED waitCode field on the ATC sensor rows, join on that not the label; addresses my t185 flag). NOTE: snippets.js deletion resolved (not mine). SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 189 (cycle 13) — P-C.2f: the DECLARE-FIX (io-tab input live-light joins a DECLARED waitCode, not a parsed label). Suite 552 pass / 0 fail (blocks-live-form:10 a known parallel-run flake — passes 6/6 in isolation, unrelated to this io/settings change). SIM/UI only -> emit BYTE-IDENTICAL. 3 files (settingsPanel + ioTab + spec).

**THE VIOLATION (my t185 flag):** the io-tab INPUT live-light INFERRED the wait M-code by PARSING row.label text (`ATC_DIALECT[ M-number matched out of the label ]`) — a declare-never-infer violation that SELF-CONFLICTED with the P-C.2d editable labels: rename the sensor / strip the "(M301)" and the live-light silently broke.

**THE FIX (declare, one-source, rename-proof):** the seeded ATC SENSOR rows now carry a DECLARED `waitCode` field (the wait M-code they answer): drawbar-released=M301, drawbar-clamped=M302, spindle-stopped=M300 (pocket-index = none, no wait). The io-tab input join reads THAT (`ATC_DIALECT[parseInt(r.waitCode)].pin`), NOT the label. So renaming/stripping the label can't break the live-light. `addIn(id, label, waitCode)` seeds it (settingsPanel).
- BACKWARD-COMPAT (clean, no user re-config): `migrateIO` backfills `waitCode` on legacy ATC sensor rows one-time, keyed on the stable seeded id (drawbar_released_atc→M301 etc.) — NOT the label. Absent/blank waitCode (e.g. pocket-index, or an unknown pulled row) → label-only, live-light off (graceful degrade).
...
## 🔨 turn 193 (cycle 13) — P-C.3a: the pick-place SEAM kind + the COLLET device + input-sensor-verify (first pick-place increment). Suite 557 pass / 0 fail. SIM/VISUAL only -> emit BYTE-IDENTICAL (generic/disk ASSUMED, flagged-until-hardware). 3 files (atcChangeWizard + gcodeViz3d + createPreviewPanel + new spec; + 1 test updated). NOTE: the human DROPPED the multi-state label — NOT built; the drawbar stays two plain pins.

**(1) THE pick-place KIND (ATC_CHOREOGRAPHY seam):** generic -> { kind:'pick-place', variant:'magazine' }; disk -> { kind:'pick-place', variant:'carousel' } (was null). MINIMAL — just enough to ARM the collet + the swap; the per-pocket swap MOTION is P-C.3b. firmware stays 'push', m6 'macro-call', manual null.

**(2) THE COLLET DEVICE (mirror P-C.2b, but on the SPINDLE):** `gcodeViz3d.setStationDevice('collet', on)` + `_applyColletState()` animate `_animParts.collet` (the grey holder cylinder that was pre-split OUT of the tool "so ATC can move the tool independently of the collet"): OPEN (M154 drawbar release) -> retract up + a CYAN "released" tint; CLOSE (M155 lock) -> rest + grey. Re-applied at the end of `_buildAnimTool` so a tool-swap REBUILD keeps the collet state. CROSS-FRAME: the collet rides the PART frame with the anim tool (reuses P-A + the swap's cross-frame — no new frame work). WIRING (createPreviewPanel.setAtcSwap): generalised to arm for 'push' OR 'pick-place'; station devices (pusher/pin) are FIRMWARE-only (setStationDevices(kind==='push'?region:null)); the io_change listener adds OUT_SPINDLE_UNCLAMP&state -> collet OPEN (M154), OUT_SPINDLE_CLAMP&state -> collet CLOSE (M155); the collet resets to closed on (dis)arm + on fresh play. atcViews UNCHANGED (region=null for non-push -> previewAtcSwap(choreo, null) arms the collet).

**(3) INPUT SENSOR live-lighting ACTIVATES (verified, no new code):** generic/disk WAIT on M300/M301/M302 -> the io-tab INPUT pins configured with waitCode M300/301/302 (P-C.2f) LIGHT live as the waits fire. (Unlike the open-loop firmware push, which fires no sensors.)

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-collet.spec.js` (4 tests, assert the VALUE): (1) the seam resolves generic/disk -> pick-place (magazine/carousel), firmware still push, m6 macro-call; (2) setStationDevice('collet', on/off) flips `_colletOpen` + the collet position (lifts on open) + tint (cyan open / grey closed, distinct); (3) a GENERIC op ARMS the collet + it flips on the drawbar io_change — OUT_SPINDLE_UNCLAMP (M154) OPENS, OUT_SPINDLE_CLAMP (M155) CLOSES (the panel listener); (4) INPUT-SENSOR — a generic-change TRACE lights the assigned M300/M301/M302 input pins (the P-C.2f waitCode join). Screenshot scratchpad/collet.png: the CYAN open collet on the spindle assembly (io panel floated over; the lit input pins are test-verified + shown in the P-C.2d/2f shots). Opened in a VS Code tab. UPDATED `atc-station-highlight.spec` test (1): generic/disk now pick-place (were asserted null pre-P-C.3a). BYTE-PARITY: no emit stack touched -> atc-roundtrip + goldens green; FULL suite 557. BLAST: additive (the pick-place kind + the collet, armed only for generic/disk) -> firmware + non-ATC previews unaffected.

**PASSED BACK for advisor review. NEXT: P-C.3b (the pocket-swap: the tool swapped AT the magazine pockets — the swap-richness fork [full per-pocket occupancy vs minimal] goes to the human). Deferred: disk carousel-rotate. SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 195 (cycle 13) — P-C.3b: the PICK-PLACE POCKET-SWAP (FULL per-pocket occupancy). Suite 558 pass / 0 fail. SIM/VISUAL only -> emit BYTE-IDENTICAL (generic/disk ASSUMED). 2 files (gcodeViz3d + createPreviewPanel + new spec).

**THE SWAP (keyed on #1300, SHARED detection; EFFECT branches on the choreography KIND):** checkToolSwap (P-C.1b) is UNCHANGED — the SHARED #1300 old→new detector. doToolSwap now BRANCHES on `atcChoreo.kind`:
- **push (firmware)** -> retire the OLD tool to the push station + setSimTool(new) [P-C.1b — UNCHANGED].
- **pick-place (generic/disk)** -> swap AT THE MAGAZINE POCKETS.

**THE PICK-PLACE POCKET-SWAP (on a real #1300 old→new during a generic op):** (a) the OLD tool RETURNS to ITS magazine pocket (re-appears there); (b) the NEW tool LEAVES its pocket (disappears) -> onto the SPINDLE (setSimTool); (c) so the MAGAZINE OCCUPANCY visibly changes. Modelled as an OCCUPANCY set: the panel re-renders `viz.setMagazine(magPocketList(occupied))` from settings.atc.magazine —
- at the FIRST #1300 poll (initial spindle tool = old): occupancy = all magazine tools MINUS old (its pocket EMPTY — the tool is in the spindle);
- at the swap (old->new): occupancy = all MINUS new (the OLD tool's pocket FILLS, the NEW tool's pocket EMPTIES).
HIGHLIGHT the involved pockets: reused `highlightStation(region, label)` (added a `label` param) with start=old pocket / end=new pocket + a 'TOOL CHANGE' label. The COLLET (P-C.3a) opens/closes during (M154/M155, already wired). CROSS-FRAME: the pockets are MACHINE coords on the FIXED frame (t163 setMagazine raw-machine); the spindle tool rides the PART frame (setSimTool) — reuses P-A + the swap cross-frame. Panel helpers: `atcCfg / magToolNums / magPocketList(occupied) / pocketOf(tool)` read the config directly (the view draws the FULL magazine; the swap re-renders the OCCUPIED subset during play).

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-pocket-swap.spec.js` — a generic op + a 2-tool magazine (T2 pocket1, T5 pocket2) + a real change (`#1300=2` -> move -> `T5 M6` -> move) -> asserts (c) the spindle now holds T5 (`_simTool.num===5`), (a) the OLD tool T2 is back in its pocket (in `_magazine`), (b) the NEW tool T5's pocket is EMPTY (removed from `_magazine`), (e) cross-frame — T2's returned pocket sits at its MACHINE coords (100,100) on the fixed frame. (d) the collet flip is covered by atc-collet. Screenshot scratchpad/pocket-swap.png: the NEW tool on the spindle + the magazine with the swapped pocket EMPTIED (2 of 3 tools shown) + the TOOL CHANGE highlight. Opened in a VS Code tab. BLAST: the pick-place BRANCH is additive -> the FIRMWARE swap (push branch) + non-ATC UNCHANGED — atc-tool-swap / atc-collet / atc-station-* all STAY GREEN (15 ATC specs). BYTE-PARITY: no emit touched -> goldens green; FULL suite 558.

**DEFERRED:** the DISK carousel-rotate + its ring-occupancy (the STRAIGHT/generic magazine occupancy is done; disk needs the ring layout + the rotate anim — a later increment). **PASSED BACK for advisor review. SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 197 (cycle 13) — P-C.3c: the DISK carousel-rotate + ring-occupancy (the LAST ATC method — ATC campaign COMPLETE: firmware + generic + disk). Suite 558 pass / 0 fail (1 known middle-animator flake, retry-passed). SIM/VISUAL only -> emit BYTE-IDENTICAL (disk ASSUMED). 2 files (atcViews + createPreviewPanel + new spec).

**THE DISK METHOD** = a RING of pockets that ROTATES to a FIXED PICKUP (a single point), vs the generic's per-tool XY pockets. Grounded in diskAutoStack (#103-105 pickup + the flagged rotate-to-#106).

**(1) RING LAYOUT (reuse):** exported `magazinePockets(a, theta)` from atcViews (the SAME function the view already uses to draw the disk ring) + added the tool `num` to each pocket + a `theta` rotation param. The panel imports it (one source, no duplication).

**(2) THE ROTATE (discrete, keyed on the target pocket):** the ring angle `theta = -(i/n)·2π` brings pocket i to the fixed pickup (pocket 0's spot). On the tool change, the ring rotates so the just-picked pocket sits at the pickup. Tracked as `viz._diskTheta` (for tests). This is a DISCRETE index (the ring jumps to the target) — see FLAG.

**(3) THE OCCUPANCY SWAP at the pickup (reuse P-C.3b):** `renderPickPlaceMag(spindleTool)` re-renders `viz.setMagazine(magPocketList(all − spindleTool, theta))` — the spindle tool removed (on the spindle), the ring rotated so its pocket is at the pickup. `magPocketList` now uses `magazinePockets(atcCfg(), theta).filter(occupied)` — DISK = ring-laid, STRAIGHT = per-tool XY (the generic path is unchanged). `doToolSwap` (pick-place branch): occupancy = all − new; theta = diskTheta(new); the OLD tool returns to its ring pocket, the NEW leaves → the spindle; highlight old→new pockets (pocketPos is disk-aware via magazinePockets). The COLLET + input sensors (P-C.3a) apply. CROSS-FRAME: the ring on the FIXED machine frame (magazinePockets machine coords); the spindle at the pickup (part frame) — reuses P-A.

**VERIFIED REAL SYMPTOM (spec + screenshot):** NEW `atc-disk-swap.spec.js` — a DISK op (pickup 150/100, Ø120, +Y ring of T2/T5/T7) + a real change (T2→T5) -> asserts (a) the ring ROTATED (theta != 0) so tool 5's pocket sits at the FIXED PICKUP (computed via magazinePockets(atc, theta) = the pickup 150,100), (b) the occupancy swapped — the OLD (2) is back in the ring, the NEW (5) removed, spindle = 5. Screenshot scratchpad/disk-carousel.png: the RING of pockets (distinct from the generic's line) + the swapped pocket rotated to the pickup + the new tool on the spindle. Opened in a VS Code tab. BLAST: additive (the disk variant + the magPocketList rework) -> GENERIC + FIRMWARE + non-ATC UNAFFECTED — 22 ATC specs (pocket-swap / tool-swap / collet / station / disk / envelope) all STAY GREEN. BYTE-PARITY: no emit touched -> goldens green; FULL suite 558.

**FLAG (per the dispatch — the rotate-anim):** the ring rotate is DISCRETE (theta is set so the target pocket jumps to the pickup) — it conveys the carousel INDEX + is fully verifiable. A SMOOTH SPIN (a raf tween of the ring rotation over time, the physical carousel motion) is a design-heavy POLISH I did NOT build (it needs a play-timed animation loop) — flagged for the human if the physical spin is wanted; the index/occupancy/pickup are correct now.

**PASSED BACK for advisor review. THE ATC CAMPAIGN IS COMPLETE (firmware push + generic pick-place + disk carousel). SIM-FRAME axis; authoring axis stays done + disjoint.**

## 🔨 turn 439 (cycle 223) — WCS-EMIT BUG: REPRODUCED THE OPPOSITE — the dispatch premise is DISPROVEN (GATE, no fix built). Added tests/wcs-emit-resolved.spec.js (diagnostic→guard). Suite: the guard + rotary emit specs green (5 pass).

**TASK:** the red dispatch — rotary-centreline + rotary-clock allegedly emit the WCS work-origin write UNRESOLVED (`#[805+[active-1]*5+1]=#54`), the DDCS check rejecting `active` as a stray word; the dispatch flagged the value-swap twins + said the goldens are false-green + asked to REPRODUCE-AND-TRACE (do NOT code-guess — "the advisor's static model already contradicts the live output"), then root-cause a valid-by-construction atom fix.

**I DID EXACTLY THAT — REPRODUCE FIRST — AND THE SYMPTOM DOES NOT OCCUR ON HEAD (== V10.90, no rotary source modified).** The advisor's static model (should be #578) is CORRECT and MATCHES the live output. The reported `active` symptom does not reproduce anywhere I drove it.

**HOW I TRACED (verify-real-symptom — drove the REAL surface end-to-end, not a proxy):**
- Built `tests/wcs-emit-resolved.spec.js`: for each twin it does `clearCode()` → `openWiz(user_*_data)` → set the WCS on the form → `wizardManager.insert()` → reads `document.getElementById('editor').value` (the EXACT text headerPost.js:176 lints) → runs `validate()` (the DDCS check, shared/js/validate/validate.js) → collects any finding mentioning `active` + the SWO lines. This is the literal human path (insert → DDCS check).
- SWEPT 13 combos: rotary-centre (active / G54 / G59+fit), rotary-clock (active-set / G54-rotate / G57-report), + corner (active/G55), edge (active/G58), middle (active/G56), alignment.
- **RESULT: every SWO index RESOLVES; the DDCS check is CLEAN (ok=true, 0 errors, 0 stray-`active`) on ALL 13.**
  - centre active → `#[805+[#578-1]*5+1]=#54`; G54 → `#[805+[1-1]*5+1]`; G59 → `#[805+[6-1]*5+1]`.
  - clock active/set → `#[805+[#578-1]*5+3]=[#54-#53]`; G54/rotate → `#[805+[1-1]*5+3]`; report → no SWO.
  - corner/edge/middle (STRUCTURAL-GUARD wcs, valid-by-construction like middleData:10) → `#70=[805+[#72*5]]` (active, #72←#578) or the literal base for a G-word; alignment → NO wcs (measurement).

**TWO DISPATCH PREMISES DISPROVEN:**
1. **"the app emits `active`"** → FALSE. The editor textarea is CLEAN (no markers — markers are export-only; editor stays clean per programModel.js:215) and the SWO index is always resolved. The ONLY `active` anywhere is INSIDE the `( @DDCS:1 {…"wcs":"active"…} )` EXPORT marker — a `(...)` comment that `scanComments` (validate.js:43) blanks, so E-STRAYWORD never fires on it (the JSON has no `(` to break nesting). Confirmed: I also linted the marker-bearing buildProgram() text → still 0 `active` findings.
2. **"the goldens are FALSE-GREEN (captured broken output)"** → FALSE. rotary-center-data-emit.spec compares the twin to the BUILT-IN `rotaryCenterStack` (an INDEPENDENT path), and mkSWO (rotaryCenterWizard.js:59) has ALWAYS baked `wcsArg` (`active→#578` since the original port f532419). So byte-diff ZERO means the twin matches the CORRECTLY-resolving built-in — the goldens assert a real independent truth, not a captured broken output.

**THE ONE-AND-ONLY MECHANISM that CAN emit `active` (traced to the atom):** `setWorkOffset(wcsExpr,axis,value) => #[805+[${wcsExpr}-1]*5+ax]=value` (ddcs-expert-m350.js:31) interpolates `wcsExpr` RAW — no self-resolution. So `active` appears IFF a `setworkoffset` BLOCK carries raw `wcs:'active'` and reaches the atom. RESOLUTION currently lives UPSTREAM (mkSWO bakes wcsArg · rotary twins' applyDatumWcs/applyReferenceWcs re-set it · corner/edge/middle resolve by structural guard). NO current builder produces a raw-`active` block — the marker RE-IMPORT path also resolves (opFromMarker → _builderAtoms → builderOf → postInstantiate, programModel.js:113). So on HEAD there is no reachable raw-`active`.

**THIS IS A REAL FRAGILITY, not a manifesting bug:** the WCS resolution is NOT valid-by-construction in the atom — it depends on every upstream path pre-resolving. The advisor's recommended fix (make the `setworkoffset` atom resolve `active→#578` / `Gnn→index` itself, idempotent) WOULD harden this so a raw-`active` block (a stale stored child, a hand-authored Blocks op, a future twin, an edit-flow that bypasses postInstantiate) can never leak. But it corrects NO reproducing symptom on HEAD.

**GATE — NOT building past it (worker step 5 + [[confirm-premise-before-regression-blocker]] + [[verify-real-symptom-not-just-test]] + [[pause-when-user-questions]]):** (a) the reproduction contradicts the dispatch's core premise, and (b) the recommended fix is a SHARED-ATOM + ALL-5-DIALECTS + retire-the-twins'-postInstantiate contract change. Both say stop + pass back options rather than build on a disproven premise.

**OPTIONS for the advisor/human:**
- **A (recommended first): get the human's EXACT repro.** The human live-caught `active` — a fresh boot is clean, so their scenario is one I can't see: which op + which WCS, a screenshot of the DDCS-check finding, and crucially whether the op was FRESHLY inserted vs from a LOADED/older program or a manually-edited Blocks setworkoffset. That pins the real reachable path a targeted fix must cover.
- **B: build the SAFE idempotent atom-fix as HARDENING anyway (byte-identical).** Make the `setworkoffset` atom resolve `active`/`Gnn` ONLY when it sees those literal tokens, passing an already-resolved `#578`/index/expression through UNCHANGED → every current emit stays byte-for-byte identical (goldens unchanged), and any raw-`active` block becomes safe. Do NOT retire mkSWO/applyDatumWcs in the same step (that's the risky, non-byte-identical part — defer). This is the valid-by-construction, one-source move done with zero regression risk. Pre-authorized by the dispatch; I can build it next wake on the word.

**Files:** only ADDED `tests/wcs-emit-resolved.spec.js` (the reproduction, now a regression guard: asserts 0 stray-`active` + 0 DDCS errors + editor-stays-clean across all 6 twins × WCS). NO source changed — HEAD emit is already correct. Guard + rotary-center-data-emit + rotary-center-in-place = 5 pass.

## 🔨 turn 441 (cycle 223) — WCS-EMIT HARDENING (option B — the valid-by-construction idempotent atom fix). BYTE-IDENTICAL: full suite 723 pass / 0 fail / 2 skip. 2 files (setworkoffset.js + wcs-emit-resolved.spec.js).

**TASK (advisor blessed my t439 gate + the disproven premise, chose B):** make the `setworkoffset` ATOM self-resolve the WCS index IDEMPOTENTLY so ANY block carrying a raw `wcs=active`/`Gnn` (a Blocks edit / an older loaded program) is made safe — WITHOUT retiring mkSWO/applyDatumWcs (the atom is the safety net), byte-identical, all dialects unchanged. Add the raw-active-block case; confirm Expert + no corruption of grbl/rs274/v41/dm500.

**THE FIX (wizards/ops/setworkoffset.js) — resolution moved INTO the atom's emit, valid-by-construction:**
- Added `export function resolveWcsIndex(wcs)`: `'active'→'#578'` · `/^G(\d+)$/ → String(n−53)` (G54→1 … G59→6) · **everything else passes THROUGH UNCHANGED** (`#578`, a plain index, a `#var`, an expression). Mirrors mkSWO's `wcsArg` (rotaryCenterWizard.js:48) EXACTLY, made idempotent.
- The atom emit now calls `dialect.setWorkOffset(resolveWcsIndex(p.wcs || '#578'), …)`.
- **WHY byte-identical:** the wizard already bakes `wcsArg` (`#578`/`1..6`) and feeds THAT to every dialect. `resolveWcsIndex('#578')='#578'`, `resolveWcsIndex('1')='1'` → the already-resolved value passes through untouched → every existing emit is byte-for-byte the same. The resolution ONLY fires when the atom is handed a raw `active`/`Gnn` — which no current builder produces (so it's pure hardening).

**DIALECT AUDIT (the advisor's explicit ask — read all 5 setWorkOffset):** Expert `#[805+[${wcsExpr}-1]*5+ax]=value` (needs #578/index) · v41 + dm500 `G90 G92 <axis><value>` (IGNORE wcsExpr entirely — an `active` there could never leak anyway) · grbl + rs274 `G10 L20 P<wcsExpr> <axis>value` (needs the P-number). Since the wizard already passes the RESOLVED form to ALL of them today, producing that SAME resolved form in the atom cannot corrupt any dialect — it emits exactly what they already receive. (Pre-existing + OUT OF SCOPE: grbl/rs274 get `P#578` for the `active` selector because the wizard bakes the Expert active-var for every dialect; my fix reproduces that byte-for-byte, does NOT change it — a separate grbl-active question, flagged not touched.)

**VERIFY (assert-the-value, not just something-changed):**
- NEW test `atom fix: a raw wcs=active/Gnn setworkoffset block RESOLVES across every dialect; already-resolved passes through byte-identical`: builds a raw `{setworkoffset, wcs:'active'|'G55', axis:'Y', value:'#54'}` block, emits it through ALL 5 dialects, and asserts (a) helper idempotency (active→#578, G54→1, G59→6, and #578/`1`/`[#71+1]` unchanged); (b) **raw-active emit == baked-#578 emit for EVERY dialect** (the byte-identity proof, an independent equality — the raw block now emits exactly what the wizard's baked #578 does); (c) Expert = `#[805+[#578-1]*5+1]=#54` (active) / `#[805+[2-1]*5+1]` (G55); grbl P-arg = `P2` (G55); (d) NO dialect emits the literal word `active`.
- KEPT + still green: the t439 guard (6 twins × WCS → the DDCS check on `editor.value` is clean, 0 stray-active, editor stays marker-free).
- **FULL SUITE 723 pass / 0 fail / 2 skip** — every existing golden + every dialect emit BYTE-IDENTICAL (the shared-atom change regressed nothing).

**ONE-SOURCE NOTE (deferred per the advisor's "don't retire mkSWO in the same step"):** the resolution logic now lives in 3 places (the atom's `resolveWcsIndex` + mkSWO in rotaryCenter/rotaryClockWizard). A future consolidation could have mkSWO/applyDatumWcs/applyReferenceWcs DELEGATE to `resolveWcsIndex` (one-source, not retiring) — but it's harmless duplication today because the atom is idempotent (even if they drifted, the atom passes the wizard's already-resolved value through). Left surgical.

**PASSED BACK for advisor review → V10.91 release.** The human's EXACT repro is a SEPARATE advisor thread; if it reveals a save/load path that STORES a raw `active`, this atom-fix already hardens the regeneration (the block emits #578 regardless of how it got its wcs).

## 🔨 turn 443 (cycle 223) — CONTOUR feature-write-seam SCOUT (NO CODE — design/map only). Deliverable: the seam design + E-decomposition + forks. (Formatted copy: scratchpad/contour-feature-write-seam-scout.md, gitignored.)

**TASK:** map contour + design how getWorkpiece() PROJECTS a contour op's cut feature into features[] (derive-not-copy), resolve the outside-contour semantic fork, the E-decomposition, the region-pill reframe, forks+risks. Ground vs code + workpiece model. NO CODE; advisor rules the fork WITH the human, then build.

**1. CONTOUR SHAPE (ground truth):** contourWizard.js → contourStack = [makeStart, wcs, makePlace(bbox, StepDown{Contour(region,side,tool)}), makeEnd], emits via emitMapped — emit already declarative (no blocker). NO structural fork: side(outside|inside|on) + shape(rect|circle|polygon|ellipse) are pure VALUE-swaps in contourRegion/offsetRegion → NO {superset} guard prune (simplest twin yet). Bindings = POSITIONAL (blockIndex+key like slotData), not identity — cutting atoms have no #var assign blocks.

**1a. REGION-PILL → FLAT reframe (a real restructure, directive 1):** contourStack nests the region as a SOCKET block (contour.params.region = newBlock('region'), sockets:{region:'region'}). flattenBlocks walks only uiChildren/children — NOT block-valued params — so the region's shape/x/y/w/h/sides are INVISIBLE to positional binding. FIX = FLATTEN the region params onto the contour block (mirror slot's flat ax/ay/bx/by / surfacing's flat surfacefill atom). This is a PROVEN pattern: surfacing lifted geometry OUT of a Region pill into a flat surfacefill atom (surfacingData.js:3-8, surfaceFill.js:3, surfacing-as-data.spec.js:8); slot/text never had a pill. Contour is the LAST mill op still carrying the pill (contour.js:46). The FORM is already flat (ct_* fields) → only contourStack+contour atom change, must stay emit-byte-identical. (REJECT growing flattenBlocks to traverse socket blocks — directive 1: restructure source, don't grow the engine.)

**2. THE SEAM (derive-not-copy):** getWorkpiece() today projects ONLY settings.stock → {outer, features[]}; deriveLegacyFeatures SYNTHESIZES a pocket because NO op declares one (one-way bridge). The seam: getWorkpiece reads the PROGRAM via window.ddcsGetBlockProgram() (programModel.getStack():28 — the SAME global pattern it uses for settings.stock, no circular dep), projects each contour op via a pure featureFromContourOp(op) → {id,shape,side,pos,size,depth}, and UNIONS with the stock-modal features (provenance-keyed src:'program' vs 'stock' — NO copy into stock.features[], live-derived: op edits re-derive, op deleted → feature vanishes). Precedents: RECONCILERS.contour(prog) (opSession.js:129-147) + declaredOpParams(prog,opType) (opSession.js:331) already read a placed contour op's params; layoutSpecFromOp (panelTypes.js:88) is the pure-fn-of-(def,params)→spec projection to mirror. pos MUST reuse ops/placement.js math (one-source) so 2D/3D/feature agree. Param→feature: rect→rect / circle→round (polygon/ellipse → round-by-bounds or extend schema = minor fork); inside→inside, outside→outside, on→no distinct feature; size = the region (TRUE profile, pre-offset); depth = depth.

**3. THE KEY FORK (needs the human) — SHARPENED by the consumer contract.** ALL 5 feature readers consume ONLY side:'inside' (featureSize workpiece.js:34-38: outside inherits OUTER). So:
- INSIDE contour → {side:inside} cavity → consumed by all 5 readers FOR FREE (probe-stop walls probeGeometry:110, Middle centre-start opSimStarts:31, 3D cavity donut gcodeViz3d:1111, 2D backdrop panelTypes:115, modal). Zero reader cost, immediate win — THIS ALONE PROVES THE SEAM.
- OUTSIDE contour → a boss (the advisor's stated target), boss = region size (usually SUB-STOCK, a raised pad). But NO reader consumes an outside feature's own size today (outside ≡ whole stock). Fork A (outside≡outer): writes NOTHING new (redundant with the stock; probe reads outer, works without the seam). Fork B (activate the deferred size-override): boss = its own region size → requires (i) featureSize honor an explicit outside size (1-liner) AND (ii) the RENDERERS learn a RAISED sub-stock boss (gcodeViz3d boss mesh not cavity donut · probeGeometry probe the OUTSIDE walls · workpieceBackdrop a boss glyph, currently continues on non-inside) = a real multi-reader rendering slice.
- RECOMMEND: prove the seam INSIDE-first (E2, zero-cost); treat the OUTSIDE boss (Fork B) as E2b (the reader/renderer extension is the real content of "the outside/boss write", don't smuggle it into the first proof). [[workpiece-model-pivot]] deferred this size-override "DECLARE later if it comes up" — Contour is when it comes up, and it's a rendering slice, not a 1-liner.

**4. E-DECOMPOSITION:** E1 = the twin (contourDataDef mirroring slotData, positional bindings, opensAs in-place) + the region-pill→flat reframe (§1a); emit BYTE-IDENTICAL vs contourStack across shape×side×scalar; no superset. · E2 = the feature-write projection (featureFromContourOp + getWorkpiece reads program + union), INSIDE-first; VERIFY: a placed inside-contour op → getWorkpiece().features carries the cavity at the right pos/size (independent geometric truth), a Middle probe READS it (loop closes end-to-end), stock path byte-identical with no contour op. · E2b = the OUTSIDE boss (Fork B: featureSize + the 3 renderers), gated on the human's ruling. · E3 = workpiece-read/layout/datum (contour view reads workpieceBackdrop off flat s.stock; feature glyph; datum coherence via layoutSpecFromOp origin=datumXY).

**5. FORKS + RISKS:** (1) outside-contour semantics — Fork A redundant vs Fork B sub-stock-boss-needs-renderer-extension (§3, HUMAN call, recommend B as E2b). (2) region-pill→flat reframe — restructure source (proven surfacing pattern) vs grow engine; recommend flatten, risk = keep emit byte-identical. (3) shape mapping — polygon/ellipse → round-by-bounds (lossy) or extend schema. (4) side:'on' (wall-finish) → no distinct feature; confirm. (5) pos one-source — projector MUST reuse ops/placement.js or 2D/3D/feature diverge. (6) union keying — op feature vs modal feature at same spot: precedence/dedup rule before build. (7) multi-op ordering / nested boss-then-pocket → out of scope for Contour, flag for Pocket.

**PASSED BACK the design for advisor+human review.** NO CODE (scout). Recommend: rule Fork B (outside boss = E2b, the renderer extension) + the reframe (flatten, proven) with the human, then dispatch E1.

## 🔨 turn 445 (cycle 223) — CONTOUR E1: the flat twin + region-pill→flat reframe + in-place (BYTE-IDENTICAL, no feature-write yet). Full suite 725 pass / 0 fail / 2 skip / 1 known-flake. 8 src + 3 test files. Screenshot: scratchpad/contour_e1_inplace.png.

**TASK:** wrap contourStack as user_contour_data (mirror slotData — positional bindings, coarse atoms no #var); FLATTEN the region-pill socket (proven surfacefill pattern) byte-identical; opensAs in-place (seamless plain "Contour" title, retire the data-wiz entry, keep contourStack/BUILDERS.contour as the legacy shim). VERIFY byte-diff ZERO across side×4 shapes×scalar sweep + in-place form non-empty + sim + screenshot.

**THE REFRAME (the crux — a NEW flat atom, NOT a mutated shared one):** the `contour` atom is SHARED — pocketWizard.js:96 plugs a Region socket for the wall finish + RECONCILERS reads c.params.region — so flattening it in place would break pocket. Followed the surfacefill precedent: a NEW `wizards/ops/contourfill.js` FLAT leaf (shape/x/y/w/h/dia/sides on the block, no socket) that rebuilds the same region descriptor internally (regionDesc(regionFromFlat)) + reuses the EXACT kernels (contourRegion + circleTrace [now exported] + contourLevel) → emit BYTE-IDENTICAL. contourStack switched to contourfill; the shared `contour` atom is UNTOUCHED (pocket unaffected). Registered in ops/index.js.

**ORIGIN + BYTE-IDENTITY (the slot/surfacing pattern):** originX/originY ride the PLACEMENT (offX/offY), the geometry is local-0-based in the TEMPLATE — a SINGLE placement socket the twin binds (region-at-0 + shift originX == region-at-originX + shift 0; proven algebraically: both = P − bbox0.minX). The built-in keeps geometry-at-originX + offX=originX; the twin uses geometry-at-0 (template default) + offX bound — byte-identical because the bbox snapshot tracks the geometry position. `clearance` is INTENTIONALLY UNBOUND (the surfacing/drill frontier — fans out to progstart + the leaf; 1 param→1 socket) so the sweep doesn't vary it.

**THE ONE REAL BUG I HIT + FIXED (verify-real-symptom): 14 byte-diffs on the first run.** Root cause: the placement's baked bbox SNAPSHOT (bminX/bmaxX on placeonstock) is frozen in the twin template (rect-80×60) and NOT re-derived by a binding → changing shape/size in the twin used the STALE rect bbox → wrong placement for circle/other shapes. surfacing avoids this because surfacefill declares a LIVE `extent`; my contourfill didn't. FIX: added `extent: (p) => contourExtent(p)` (mirrors contourBBox from the flat params) → PlaceOnStock recomputes the placement from the CURRENT geometry. Byte-identical for BOTH paths (live extent == contourBBox). → byte-diff ZERO.

**RECONCILER + TEST updates (the reframe changed the block type contour→contourfill):** RECONCILERS.contour (opSession.js) now reads the flat contourfill params (was c.params.region.params); contour-wizard.spec's block-structure assertions updated to read the flat block (region → contour.params). Both green.

**THE TWIN (blocks/dataOps/contourData.js):** mirrors slotData/surfacingData — positional CONTOUR_BINDINGS over [progstart(0) wcs(1) placeonstock(2) stepdown(3) contourfill(4) progend(5)] + WRAP_PREFIX_COUNT 4. Binds wcs · placement(origin/attach/datum/stock/offZ) · depth/stepdown · shape/side/w/h/dia/sides/toolDia/feed/plunge. Shape-dim `when` conditionals (w/h→rect, dia→circle, sides→polygon). Panel form2d, group Contour. NO feature-write (E2). NO superset (contour has no structural fork).

**IN-PLACE:** wizardLibrary contour entry `opensAs: 'user_contour_data'` (ONE declaration → slot re-point + twin-hide + seamless plain "Contour" title via builtinLabelForTwin/OPENS_AS_TARGETS); seeded contourDataDef in app.js. Built-in contourStack/BUILDERS.contour stays as the byte-identical legacy shim.

**VERIFY (assert-the-value + real-symptom):** NEW contour-data-emit.spec — byte-diff ZERO across side(outside/inside/on)×4 shapes(rect/circle/polygon/ellipse)×a scalar/placement/wcs sweep, ASSERTED vs the built-in contourStack (INDEPENDENT truth), PLUS cross-dialect ZERO (grbl+rs274); WIRING (outside rect w50 tool6 → X56). NEW contour-in-place.spec — opensAs=user_contour_data, seamless "Contour", twin retired, registered on boot; DRIVE: the in-place form renders 21 fields (shape/side/w/h/dia/sides/toolDia/depth/…) NON-EMPTY (the drill-flat-form lesson) + the 2D layout renders. Screenshot scratchpad/contour_e1_inplace.png: plain "Contour" title + full form + the contour toolpath in the viz ("Contour (data) · 31 lines"). FULL SUITE 725 pass / 0 fail (1 known middle-animator flake, retry-passed; contour-wizard/contour-op/fills/region-shapes/shape-types all green → pocket + region atom UNREGRESSED).

**PASSED BACK: byte-diff ZERO + the screenshot.** Then E2 (the inside-cavity feature-write — getWorkpiece projects featureFromContourOp, derive-not-copy).

## 🔨 turn 447 (cycle 223) — CONTOUR E2 feature-write: BUILT + VERIFIED, then STOOD DOWN per a human supersede-amendment. Reverted CLEAN. E1 (7926ad4) STANDS.

**TASK (as dispatched):** the inside-cavity feature-write seam — getWorkpiece() projects a contour op's cut feature (derive-not-copy), unioned with stock-modal features.

**WHAT I BUILT + VERIFIED (before the stand-down):** a PURE `featureFromContourOp(params, outer)` in contourData.js (INSIDE → a {shape,side,pos,size,depth} cavity; outside/on → null); a generic projector REGISTRY in engine/workpiece.js (`setFeatureProjector`, mirroring `setUserSimStock`) wired via registerUserOp; `programFeatures()` reads the program via `window.ddcsGetBlockProgram()` + getWorkpiece() UNIONS (provenance-keyed src:'program', NO copy; projectWorkpiece stays pure). VERIFIED END-TO-END (tests/contour-feature-write.spec, all green): the derived cavity pos {32,27} == the EMITTED toolpath centre {32,27} (independent G-code truth), size {20,30}, depth 6; workpieceBackdrop DREW it; UNION with a stock-modal feature held (both provenances, no dup); outside/on → none; emit BYTE-IDENTICAL; the PROBE path (projectWorkpiece) was NOT locked onto the nominal. Screenshot showed the stock modal drawing the cavity FROM the milled contour (off-centre 30×24 @ 32,27). The one-source pos derivation (region-centre + placementShift + datumXY) landed exactly on the real cut.

**FIRST amendment (human t446, incorporated):** re-anchor E2 to STOCK-MODEL ENRICHMENT (a milled cavity has a real BOTTOM at the cut depth) not the mill-to-probe loop; DEFER probe-consumption to E3 (overridable default + honest-nominal); don't lock a probe. My build already complied (no probe lock; added a guard asserting projectWorkpiece/probes are unaffected).

**SECOND amendment (human, SUPERSEDES the first): STAND DOWN on E2 entirely.** The human is reconsidering the feature-write direction: (a) the pocket BOTTOM should be a DECLARED depth FIELD in the STOCK MODAL (declare-not-derive — the user owns it), NOT derived from a mill op; (b) true stock-removal (watch the toolpath carve the stock) is a separate BACKLOG item; (c) the mill-to-probe loop is redundant. Returns to the ORIGINAL workpiece pivot (features DECLARED in the modal, ops REFERENCE them — the mill-derivation was an over-addition). [[workpiece-model-pivot]] [[dont-declare-away-user-responsibility]]

**ACTION TAKEN (per the supersede):** did NOT commit the E2 work. REVERTED the 4 modified files (contourData.js, userOps.js, engine/workpiece.js, contourWizard.js) to HEAD (E1) + removed the E2 test + screenshot. CONFIRMED clean: `grep featureFromContourOp|programFeatures|setFeatureProjector` → none remain; E1 contour tests (data-emit/in-place/wizard/op) all GREEN (7 pass). The working tree carries only E1 (7926ad4) + this WORK-LOG note.

**STATE: CLEAN. E1 stands (the Contour in-place port, byte-identical). E2 stood down (not built into the tree). Awaiting the human's next dispatch (likely a small stock-modal pocket-DEPTH field). Not proceeding on E2/E3 until dispatched.**

## 🔨 turn 449 (cycle 223) — POCKET-DEPTH FIELD (declare-not-derive): inside features get a real FLOOR at a DECLARED depth, rendered 3D+2D. Full suite 729 pass / 0 fail / 2 skip. 4 src + 1 test file. Screenshot: scratchpad/pocket_depth_floor.png.

**TASK:** the user OWNS the pocket-bottom depth (declare-not-derive — the E2 mill-derivation is DROPPED). Add an editable depth FIELD to the stock modal + render the inside cavity with a real floor at (top − depth), not a bottomless through-cut.

**STATE CONFIRMED (the human's report is right):** the feature schema already has `depth` ({shape,side,pos,size,depth}); deriveLegacyFeatures gives a legacy pocket `depth: z` (full-through); gcodeViz3d.setStock extruded the cavity by the FULL stock.z as a THROUGH-HOLE (gcodeViz3d.js:1134) → pockets looked bottomless. NO depth field existed in the modal (features were drag-only via canvas handles).

**(1) 3D FLOOR (gcodeViz3d.setStock) — a real bottom, not a through-cut:** keep the donut (outer + holes) extruded full-z for the WALLS, then for each rect cavity with a DECLARED `0 < depth < stock.z` add a FLOOR PLUG — a box (cavity size, height z−depth) as a CHILD of the stock mesh, filling mesh-local z [0, z−depth]; the pocket void stays open above the plug, whose TOP is the floor at (top − depth). A full/undeclared depth (≥ z) keeps the through-hole (no plug) — so legacy pockets + the workpiece-seam byte-identity are UNREGRESSED. Depth is a PHYSICAL cut from the top (datum-independent — computed in the mesh/pg frame, top = z/2). Exposed `_pocketFloors`/`_stockTopZ` for the assertion + disposal via a traverse (shared material → dispose only plug geometries). Round bores aren't 3D-rendered yet (existing filter) — flagged, out of scope.

**(2) DEPTH FIELD (ui/stockEditor.js):** a "POCKET DEPTH — cut from the top" section (`#se_features`) rendering a number input per INSIDE feature (Pocket/Bore N depth); editing persists to `feature.depth` via writeFeatures (materializing a legacy pocket like the drag handlers) → re-renders the 2D + the 3D floor LIVE. The field shows feature.depth (legacy = z, full-through — the honest undeclared default; the user declares a real depth to floor it).

**(3) 2D BACKDROP reflects it (workpiece.js + featureCanvas.js):** workpieceBackdrop tags the cavity item with `depth` ONLY for a DECLARED sub-through floor (`0 < depth < outer.z`) → a legacy full-through pocket + no-depth features are UNCHANGED (the exact-match backdrop tests stay green); FeatureCanvas draws a "↓Nmm" tag at the cavity centre when `it.depth` is present (rect + circle).

**DECISION (flagged for the human):** kept deriveLegacyFeatures `depth: z` (full-through = the honest undeclared default; the workpiece-seam.spec asserts it) — so a legacy pocket is a through-cut UNTIL the user declares a depth in the new field. If the human wants legacy pockets to DEFAULT to a floor (a sensible sub-z default), that is a one-line change + a seam-test update — surfaced, not assumed. Scoped to the INSIDE-pocket floor (the human ask); did NOT generalize to boss-height.

**VERIFY (real-symptom + assert-the-value):** NEW pocket-depth.spec (3 tests, all green): (1) a standalone GcodeViz3D.setStock with a declared depth-8 cavity → `_pocketFloors` has ONE floor, and `_stockTopZ − floorZ == 8` (== top − depth, an INDEPENDENT geometric truth — top 10, floorZ 2); a depth-20 (== Z) cavity → ZERO floors (through-cut). (2) the modal depth field ROUND-TRIPS — editing it → `feature.depth == 7` persisted (materialized). (3) workpieceBackdrop tags depth 6 on a sub-through cavity, undefined on a full-through one. FULL SUITE 729 pass / 0 fail (all shared-file changes — gcodeViz3d/backdrop/featureCanvas/stockEditor — regressed nothing; the conditional depth-tag kept the exact-match backdrop tests green). Screenshot scratchpad/pocket_depth_floor.png: the modal with a depth-7 pocket — the field, the 2D "↓7mm" tag, and the 3D pocket with a VISIBLE FLOOR (recessed, a real bottom).

**PASSED BACK + the screenshot.** Awaiting the human's eyeball + the legacy-default-depth call.

## 🔨 turn 451 (cycle 223) — POCKET-DEPTH VISUAL FIX (the human live-caught 2 bugs): (C) a TRUE recess (walls end at the floor, solid below) + (B) the 2D tag clear of the handle. Render-only, emit BYTE-IDENTICAL, legacy through-holes UNCHANGED. Full suite 729 pass / 0 fail. 2 src + 1 test file. Screenshot: scratchpad/pocket_depth_floor.png.

**TASK:** finish the just-shipped pocket-depth feature — the human drove it live + caught 2 visual bugs. (C) the cavity rendered as a FULL-DEPTH hole (walls to the stock bottom) + a floating floor PLUG → the walls continued PAST the floor (a through-hole with a floating floor, not a real pocket). (B) the 2D depth tag was OCCLUDED by the origin drag handle at the cavity centre.

**(C) THE RECESS (gcodeViz3d.setStock — replaced plug-below-a-full-Z-hole with a real recess):** the WALLS now terminate at (top − depth) with SOLID stock below. Built as CHILDREN of the one stock mesh (so the −C pivot offset + datum apply once): (1) the TOP layer = the outer frame minus EVERY cavity hole, extruded ONLY to the recess depth D (= the deepest declared floor) → the pocket walls stop at −D; (2) a SOLID BASE box below (world [−z, −D]) with holes ONLY for full/undeclared THROUGH cavities (they stay bored full-Z) → the floor + the material below; (3) a shallower recessed pocket gets a plug raising its floor from −D to −its depth. A full/undeclared depth (≥ Z) → D = Z → full-Z donut, NO base → the through-hole is UNCHANGED (legacy pockets byte-identical). Exposed `_pocketWallDepthZ` (= D = the wall Z-extent) for the assertion.

**(B) THE 2D TAG (featureCanvas._featureDepthLabel):** OFFSET the "↓Nmm" tag DOWN by a constant 15px SCREEN offset (zoom-independent) so it clears the origin handle at the cavity centre; added a dark text stroke (paint-order) so it reads against the cavity fill.

**VERIFY (assert-the-value + real-symptom):** pocket-depth.spec updated — the CORE new assertion: `_pocketWallDepthZ == depth` (8, NOT the full 20) → the walls terminate at the floor, solid below (a real pocket, not a through-hole); a full-depth (20) → `_pocketWallDepthZ == 20` (full-Z through-hole) + 0 floors. PLUS the retained floor-Z == top − depth (top 10, floorZ 2, depth 8). All 3 pocket-depth tests green. FULL SUITE 729 pass / 0 fail (render-only — no emit path touched; the non-pocket cylinder/box branches + legacy through-holes UNREGRESSED). Screenshot scratchpad/pocket_depth_floor.png: the 3D pocket is now a TRUE RECESS (walls to the floor + solid stock below), and the 2D "↓7mm" tag sits clear below the ⌖ handle.

**PASSED BACK + the screenshot.** (Next per the advisor: the direct-edit canvas fields — double-click a handle → a dual x/y or H/W input.)

## 🔨 turn 453 (cycle 223) — POCKET-DEPTH RENDER, 3rd time: a TRUE CONFINED RECESS (Fusion extrusion-cut model). The slab+base (e57dad3) made a stock-spanning plane through the translucent stock; NOW one clean MANIFOLD, footprint-confined. Render-only, emit BYTE-IDENTICAL, full suite 730 pass. 1 src + 1 test. Screenshots: pocket_recess_3q.png (3/4 opaque) + pocket_depth_floor.png (modal).

**TASK:** the C fix was STILL wrong (human live-caught): the pocket floor extended across the WHOLE STOCK — a full-width horizontal plane at −depth visible THROUGH the translucent stock (0.72 opacity, depthWrite:false → INTERNAL faces show). ROOT: my top-slab + solid-base-box split creates a full-width internal face at −D (base top / slab cap). MODEL (human t452): THINK FUSION EXTRUSION CUTS — the stock is a SOLID body; each feature is a box (footprint×depth at its XYZ) BOOLEAN-SUBTRACTED → a CONFINED recess, solid everywhere else, NO stock-spanning plane; honor per-feature XYZ + multiple/multi-Z.

**IMPLEMENTATION (my call — manual manifold, NOT a CSG lib):** the app vendors a MINIFIED THREE r137 as window.THREE (no CSG, no bundler) → adding a CSG lib is risky. Since ANY internal face shows through the translucent stock, the fix is ONE clean MANIFOLD SURFACE (the boundary of the solid−pockets, no internal faces). New `_pocketRecessGeometry(stock, cavities)` builds a single BufferGeometry: TOP cap (outer minus EVERY footprint, ShapeGeometry-triangulated) · full-height OUTER 4 walls · per-pocket 4 INNER walls (0→−depth) · a FLOOR quad AT THE POCKET FOOTPRINT ONLY at −depth (recessed) · BOTTOM cap (outer minus THROUGH footprints). Material side=DoubleSide (winding-agnostic under translucency). Honors per-feature XY footprint + depth + multiple/multi-Z (each cut its own box). A full/undeclared depth (≥ Z) → top+bottom open, no floor = through-cut (legacy unchanged). Exposes _pocketFloors/_stockTopZ/_pocketWallDepthZ. NO CSG lib added.

**VERIFY (assert-the-value + real-symptom — this render MISSED TWICE, so I inspected the GEOMETRY + captured the visual):** pocket-depth.spec — the KEY new assertion inspects the actual mesh geometry: the horizontal surface at −depth (z≈−8) spans the POCKET FOOTPRINT ONLY — floorExtent = {x∈[30,70], y∈[25,55]} = the 40×30 footprint @ (50,40), NOT the stock (0..100 × 0..80) → NO stock-spanning plane. Plus wallDepth==8 (walls end at the floor), floor-Z==top−depth, full-depth→through+0 floors. A NEW SCRUTINY screenshot: a standalone viz, opaque + depth-tested (screenshot-only; the app stays 0.72 translucent) at a 3/4 top-down angle → pocket_recess_3q.png CLEARLY shows a SOLID block with a CONFINED recessed pocket (walls to a floor, solid stock all around, no full-width band). FULL SUITE 730 pass / 0 fail (render-only; non-pocket cylinder/box + legacy through UNREGRESSED).

**PASSED BACK + both screenshots (scrutinized).** (Next: the Z-first-probe-floor bug, then the direct-edit fields.)

## 🔨 turn 455 (cycle 223) — Z-FIRST-PROBE-FLOOR: the Middle Probe-Z-First on a declared-depth pocket now touches the FLOOR (top−depth), not the stock top. SIM/SIMULATION-ONLY — emit BYTE-IDENTICAL. Full suite 732 pass. 1 src + 1 test. Screenshot: zfirst_probe_floor.png.

**TASK:** the Middle Probe-Z-First on a depth-enabled pocket touched the STOCK TOP, not the pocket bottom. REPRODUCE first + determine sim-only vs emit.

**REPRODUCED (verify-real-symptom, at the collision level):** the surface model is engine/probeGeometry.stockProbeStop — modelled the pocket cavity as FULL-THROUGH in Z (z [−Z, 0]), so a Z-down probe inside the footprint hit the OUTER BOX TOP (z=0) and never the floor. Confirmed: stockProbeStop(Z-down over a declared depth-8 pocket) returned z=0 (the top), diff 8 from the expected −8.

**SIM/SIMULATION-ONLY (determined, not assumed):** stockProbeStop is the SHARED collision used by the 3D sim (gcodeViz3d.js:825, the probe-path clamp) AND the execution-engine simulation (GcodeExecutionEngine.js:1045, the #1927 DRO) — so fixing it corrects BOTH the preview AND the simulated run. The G31 emit finds the real surface PHYSICALLY (surface-agnostic) → emit BYTE-IDENTICAL. [[ddcs-ground-truth-reference]]

**FIX (probeGeometry.stockProbeStop):** a DECLARED-DEPTH pocket is a REAL RECESS — the outer-box TOP is REMOVED over its footprint (down to −depth) and the FLOOR is at −depth. So: (1) the outer-box top hit is IGNORED when the entry point is inside a declared recess footprint (openTop); (2) a per-recess FLOOR box (the material below, top face at −depth) stops a Z-first probe over the footprint at −depth; (3) the pocket WALL (bore probe from inside) is UNCHANGED (byte-identical). A full/undeclared depth (≥ Z) has no floor → the through-hole is UNCHANGED. Reads the declared feature.depth (the field shipped this cycle) + tip-r comp preserved.

**VERIFY (assert-the-value + real-symptom):** NEW zfirst-probe-floor.spec — the collision returns z=−8 (== top − depth, an INDEPENDENT geometric truth) INSIDE a declared depth-8 pocket; z=0 over SOLID stock (unchanged); z=0 for a THROUGH/undeclared pocket (unchanged). DRIVE the REAL Middle probe in-place (Feature=Pocket + Probe-Z-First ON + a depth-8 pocket workpiece) → the 3D probe descends INTO the pocket; screenshot zfirst_probe_floor.png (the form with Probe-Z-First ON, the tool probing into the pocket, the 2D ↓8mm). FULL SUITE 732 pass / 0 fail (the existing bore-wall stop + the middle-emit goldens UNREGRESSED → emit byte-identical confirmed).

**PASSED BACK: SIM/SIMULATION-ONLY (emit byte-identical) + the screenshot.** (Next: the direct-edit canvas fields — double-click a handle → a dual x/y or H/W input.)

## 🔨 turn 457 (cycle 223) — DIRECT-EDIT CANVAS FIELDS (the LAST requested UI edit): double-click a stock-modal handle → an inline DUAL number field (W/H or X/Y) to TYPE the value. Full suite 736 pass. 2 src + 1 test. Screenshot: dual_edit_field.png.

**TASK:** the stock-modal 2D handles show values (stock 120×90, feature size, feature offset); let the user DIRECTLY EDIT them by DOUBLE-CLICKING a handle → an inline DUAL input (X/Y for a position handle, W/H for a size handle) to type instead of only dragging. Enter/blur commits + re-renders 2D+3D, Esc cancels.

**BUILT (reused + extended the existing on-canvas edit pattern):** FeatureCanvas ALREADY had a single-value inline editor (_editDim: click a label → type → spec.onEdit(id, value)) + a dblclick handler (empty space → re-frame). ADDED a DUAL variant: (1) the dblclick handler now, on a HANDLE hit with an `edit` descriptor, opens `_editDual(h)` — two labelled number inputs overlaid at the handle (pre-filled with the current vals), Enter/blur → `spec.onEditDual(id, [a,b])`, Esc cancels (focusout commits only when leaving the whole box, not tabbing between the 2 inputs); styled to match the modal (panel bg, accent border). A handle opts in via `h.edit = { labels, vals }` — GENERIC (any wizard's handles become dual-typeable). Non-`edit` handles + empty-space re-frame UNCHANGED.
- stockEditor: the 3 handles carry `edit` + a shared `spec.onEditDual`: OUTER-SIZE → W/H → stock.x/y (via the same setOuter → #se_x/#se_y + commit the drag uses); FEATURE-SIZE → W/H → f.size; FEATURE-OFFSET → X/Y → f.pos = the DATUM-RELATIVE offset + datumXY (matching the ⌖ label). Materializes a legacy pocket like the drag.

**VERIFY (assert-the-value + real-symptom):** NEW stock-modal-dual-edit.spec (4 tests): double-click the OUTER-SIZE handle → a 2-input dual field → type 50×40 + Enter → stock.x==50, stock.y==40; the FEATURE-SIZE handle → type 24×18 → feature.size=={24,18}; the FEATURE-OFFSET handle → type 30×20 → feature.pos=={30,20} (offset+datum, front-left datum); ESC → the editor closes + NO model change (toEqual the before). Screenshot dual_edit_field.png: the dual X/Y editor OPEN on the offset handle (2 inputs at the handle, modal-styled). FULL SUITE 736 pass / 0 fail (FeatureCanvas non-edit handles + empty-dblclick re-frame UNREGRESSED).

**PASSED BACK + the screenshot. This was the LAST requested UI edit → the porting goes AUTONOMOUS next (advisor scouts/ports the remaining wizards + Setup/IO, surfacing only real forks).**

## 🔨 turn 459 (cycle 223) — BORE HELICAL TWIN (first autonomous fan-out port): user_bore_data, byte-diff ZERO + in-place. Full suite 739 pass. 3 src + 2 new tests + 2 updated. Screenshot: bore_inplace.png. NO design fork — a clean port.

**TASK:** Bore is the Drill wizard's HELICAL variant (wizardLibrary `bore` = {type:'drill', variant:'bore'}) — drillStack builds a `bore` leaf (holeDia/toolDia/depth/pitch/ramp — a DIFFERENT block type + key set) when method='helical', so the drill twin (drillData, method='peck', block-4 keys depth/peck) CANNOT be reused. Build user_bore_data mirroring drillData; byte-identical emit + opensAs in-place.

**WHERE BORE LIVES (confirmed first, per the dispatch):** wizardLibrary.js:64 `{ id:'bore', type:'drill', variant:'bore', label:'Bore', group:'mill' }` — a drill-wizard variant, NO opensAs (was built-in). The built-in Bore = the drill VIEW with variant='bore' → drillStack(method='helical').

**BUILT (blocks/dataOps/boreData.js — mirrors drillData EXACTLY):** BORE_DEFAULTS = the drill pattern/placement half VERBATIM + method:'helical' (BAKED, not bound — Bore is always helical, like drillData bakes peck) + the bore cut params (holeDia/toolDia/depth/pitch/ramp/feed). The template is drillStack(BORE_DEFAULTS) → [progstart, wcs, placeonstock{array{bore}}, progend]; positional bindings by (blockIndex,key): blocks 1-3 (wcs/placement/pattern) IDENTICAL to drillData; block 4 = the bore leaf (holeDia/toolDia/depth/pitch/ramp[enum step/helix]/feed — NOT peck). `clearance` unbound (frontier #3 fan-out, same as drill). The bore atom (bore.js) NEVER emits G3-with-Z — step = plunge+full-G3-circle; helix = LINEARIZED to G1 chords — the twin inherits that byte-for-byte via drillStack. opensAs on the Bore entry + seeded in app.js.

**VERIFY (assert-the-value, INDEPENDENT truth = drillStack):** NEW bore-as-data.spec — byte-diff ZERO across pattern(grid/circle/line/rect) × ramp(step AND the linearized-helix hazard) × cut(incl. holeDia≤tool straight-plunge fallback) × off-origin+skip × placement × wcs, ASSERTED vs drillStack (a separate path) + cross-dialect ZERO (grbl/rs274); WIRING (holeDia=94 tool=6 → bore radius 44 → G0 X44). NEW bore-in-place.spec — Bore opensAs=user_bore_data, seamless "Bore", twin retired, seeded; DRIVE: the in-place form renders 32 fields NON-EMPTY (pattern/placement + holeDia/toolDia/pitch/ramp/depth) + the 3D renders the bore toolpath ("Bore (data) · 157 lines"). UPDATED drill-in-place.spec + wizard-bar.spec (the obsolete "Bore stays built-in — peck-only-twin gap" assertions → Bore now opens its own helical twin). FULL SUITE 739 pass / 0 fail (drill twin + drillStack goldens UNREGRESSED — the peck path is untouched; Bore is a SEPARATE twin). Screenshot bore_inplace.png: plain "Bore" title + the full form + the bore toolpath in the 3D.

**NO DESIGN FORK — a clean mirror port (the peck-only-twin gap is now closed).** PASSED BACK: byte-diff ZERO + the screenshot. Advisor releases + continues the autonomous fan-out.

## 🔨 turn 461 (cycle 223) — FLATTEN-MIGRATION (workpiece-completion): 11 sites now draw the DECLARED workpiece feature at its OFFSET (via workpieceBackdrop/getWorkpiece), not the legacy centered 25% inset. Byte-identical for boss/box + legacy. Full suite 741 pass. 10 src + 1 test. Screenshot: flatten_offcentre.png.

**TASK:** a declared OFF-CENTRE feature (pocket/boss) should render at its offset in EVERY 2D preview; ~11 sites still drew the LEGACY centered inset (a hardcoded 0.25·min(x,y) inset at the centre) instead of reading the declared feature via workpieceBackdrop/getWorkpiece.

**FOUND (the flatten sites — those NOT using workpieceBackdrop):** middleView.buildFeatureItems drew the WRONG centered 25% inset for a pocket; the other 7 wizard views (drill/edge/contour/pocket/slot/surfacing/text) + the 3 formWidgets canvases (xyPad/coordEditor/rectPad) built a stock backdrop but drew NO declared features. panelTypes.js was ALREADY migrated (t385, the precedent: `items.push(...workpieceBackdrop(getWorkpiece(),{ox,oy}).items)`).

**MIGRATED (one-source):** added ONE shared helper `workpieceFeatureItems(ox,oy)` in engine/workpiece.js = `workpieceBackdrop(getWorkpiece(),{ox,oy}).items || []` (safe []-on-none). Each site PREPENDS it to its spec `items` (behind the op glyphs, ox/oy matching its own `stock:{ox,oy}` — pl.stockOx for the placed views, 0 for edge/middle, bd.ox for formWidgets). middleView: the POCKET branch → workpieceBackdrop (keeps its boss/cylinder glyphs). BYTE-IDENTICAL for the legacy case BY CONSTRUCTION: an OUTER (boss/box/cylinder) stock → NO inside-cavity items → nothing added; a LEGACY pocket (no declared features) → deriveLegacyFeatures gives the SAME centered 25% inset. A DECLARED off-centre feature → draws at its real pos+size. (10 files: middleView by me + the 7 views + formWidgets by a delegated subagent, each = ONE import + ONE items-field prepend, nothing else touched.)

**11 MIGRATED SITES:** middleView · drillView · edgeView · contourView · pocketView · slotView · surfacingView · textView · formWidgets(xyPadWidget, buildCoordEditor, rectPadWidget). (+ panelTypes was already done.)

**VERIFY (assert-the-value + real-symptom):** NEW flatten-migration.spec — the shared truth: workpieceFeatureItems for a DECLARED off-centre pocket (pos 30,20 size 20×10) → the cavity centred AT (30,20) size 20×10, NOT the stock centre (50,40); a LEGACY pocket → centred (50,40) size 60×40 (the 25% inset, byte-identical); a BOSS/box → 0 items (byte-identical). DRIVE the Middle preview with a declared off-centre pocket (pos 30,25) → the 2D + 3D draw the cavity AT the offset (lower-left, NOT centred); screenshot flatten_offcentre.png. FULL SUITE 741 pass / 0 fail (every view + stock-modal + workpiece-seam + boss-pocket-reconcile UNREGRESSED — the boss/box byte-identity held everywhere).

**PASSED BACK: the 11-site list + the screenshot.** The workpiece 2D-preview flatten is COMPLETE (declared features render at their offset in every preview; panelTypes was the precedent, the other 11 now match).

## 🔨 turn 463 (cycle 223) — RETIRE-DEAD-MUTATORS: CONFIRMED NOT INERT → FLAGGED, did NOT remove (per the advisor's "flag if a caller depends"). No code change (empirically probed + restored). The premise is disproven.

**TASK:** retire 2 "dead" stock-shape mutators — (1) syncStockShape (middleView); (2) activateCylinderStock/restoreBoxStock (rotary force-cylinder) — believed INERT now the workpiece model replaced them. CONFIRM inert FIRST; if a caller still DEPENDS, FLAG + do NOT remove.

**CONFIRMED (empirically + by tracing): BOTH are STILL LIVE — the built-in legacy VIEWS depend on them, and those views are STILL TEST-EXERCISED. The advisor's "workpiece replaced them" premise is DISPROVEN.** [[confirm-premise-before-regression-blocker]]

**(1) syncStockShape (middleView:60) — NOT inert.** I REMOVED it (def + the :192 call) and ran the middle tests → `middle-feature-draw.spec` FAILED: it opens the BUILT-IN middle view (`wizardManager.open('middle')`, #wiz_middle) and asserts the op-type PRESELECTS the stock shape (pick pocket → `stock.shape==='pocket'` → a cavity in 2D+3D; boss → boss; circular → cylinder). syncStockShape IS that preselect (it WRITES stock.shape, which getWorkpiece READS). The TWIN (user_middle_data, the menu path) does NOT have this behavior — picking featureType does NOT preselect the shape; the "op references a DECLARED feature" replacement is the DEFERRED slice (middleData.js:16 "retire syncStockShape ... LATER slices") that was NOT built. So the workpiece read did NOT replace it. RESTORED (net-zero; the test is green again).

**(2) activateCylinderStock + restoreBoxStock (rotaryCenterView:15/31) — NOT inert.** `rotary-center-roundbar.spec:50-54` DIRECTLY imports + tests them (asserts "restoreBoxStock the defensive edge/middle/clock revert still works"); the BUILT-IN rotary/edge views that CALL them (rotaryCenterView:54 activate; edgeView:77 / middleView:110 / rotaryClockView:23 restore) are exercised by rotary-a-jog, rotary-center-rig-decouple, blocks-rotary-rig, edge-probe-vector, edge-start-position (all `open('rotary_center'|'rotary_clock'|'edge')` → #wiz_rotary*/#wiz_edge). The rotary-center-roundbar spec CONFIRMS the twin path retired activateCylinderStock (uses def.simStock, NO global mutation) — but the LEGACY VIEWS still force+revert the cylinder. NOT touched (no removal attempted).

**ROOT CAUSE the premise is wrong:** the built-in wizard VIEWS (middleView/edgeView/rotaryCenterView/rotaryClockView) are NOT fully retired — they are legacy shims that are STILL TEST-EXERCISED (via `wizardManager.open(type)` → #wiz_<type>), and they DEPEND on these mutators. The workpiece model replaced the mutators for the TWINS (getWorkpiece / def.simStock, non-mutating), but the LEGACY VIEWS + their tests still use them. Retiring the mutators BREAKS those views + tests.

**GATE — the advisor decides the path (do NOT guess):** to truly retire these mutators, the PREREQUISITE is one of: (A) RETIRE the legacy built-in views entirely (delete middleView/edgeView/rotaryCenterView/rotaryClockView + their now-obsolete specs — a real removal of the shims the twins replaced); or (B) BUILD the twin-side replacement for the middle preselect (the deferred "middle op references a DECLARED feature by featureType" slice) so the twin drives the feature, then drop syncStockShape. Neither is a dead-code delete — each is a real change. RECOMMEND clarifying whether the legacy views are being kept (as shims) or retired; if retired, that (A) is the actual cleanup and the mutators go WITH them.

**PASSED BACK: FLAGGED — neither mutator is inert; both serve the still-test-exercised legacy built-in views. No code changed. Awaiting the advisor's call on retiring the legacy views vs building the twin replacement.**

## 🔨 turn 465 (cycle 223) — ROTARY 2D DATUM-GLYPH: opt-in visual glyph for the rotary twins (centreline / span-segment), mirroring the edge wall-glyph. Sim-only → emit BYTE-IDENTICAL. Full suite 743 pass. 2 src + 1 test. Screenshot: rotary_center_glyph.png. FLAGGED: the clock's A0-reference-direction rendering.

**TASK:** the rotary wizard 2D layout gets an opt-in DATUM GLYPH — the bar CENTRELINE for rotaryCenter, the A-clock reference for rotaryClock — OPT-IN off the rotary bindings (like edge's axis+dir glyph) so other ops are untouched. Sim-only, no emit path. PROPOSE the viz; if it's a real design choice, FLAG it.

**OPT-IN GATE (mirrors the edge axis+dir detection):** the glyph fires off a GLOBALLY-UNIQUE binding param — `diameter` (only rotaryCenter declares it) → the centreline; `span` (only rotaryClock) → the span-segment. Confirmed unique by grep across all of web/ (no built-in or data op shares either name), so the gate CANNOT misfire onto a non-rotary op. `(def.bindings||[]).find(b => b.param==='diameter'|'span')` — the same shape as edgeAxisBind/edgeDirBind. A built-in wizard (no bindings) → [] → no glyph (unchanged).

**BUILT (panelTypes.layoutSpecFromOp, right after the EDGE DATUM VIZ block — PURELY VISUAL items[], no handle/emit/drag):**
- rotaryCenter → the bar CENTRELINE: a dash-dot line at the stock Y-centre (Y=stock.h/2) running the full X (0→stock.w) — the bar is a cylinder along X, so its axis IS the datum reference — + a small circle dot at the bar centre (stock.w/2, Y-centre) = the found Z0 datum point. Classes fc-rotary-axis / fc-rotary-datum.
- rotaryClock → the SPAN SEGMENT: a solid line at X=centre from cy-span/2 to cy+span/2 (the 2 Z-down touches A→B, span apart in Y — EXACTLY the rotary_clock sim-start geometry) + a touch dot at each end. Classes fc-rotary-span / fc-rotary-touch. span read from params.span (fallback 20, matching the sim-start).
- CSS (styles.css, next to fc-edge): same cyan=datum / amber=probe language as the edge glyph — cyan dash-dot centreline, cyan datum dot, cyan span segment, amber touch dots.
- The glyph items sit INSIDE the stock bounds → the FeatureCanvas auto-fit bbox is unchanged (no zoom shift).

**DESIGN-CHOICE JUDGEMENT (per "propose; flag if a real design choice; do not guess"):**
- rotaryCenter centreline — NOT ambiguous. The bar (cylinder along X) has its axis at the stock Y-centre; that axis is unambiguously the datum reference (the found centre + Z0 sit on it). Built it, no flag.
- rotaryClock — I drew the CONCRETE PROBE GEOMETRY (the span segment + the 2 touch points, straight from the sim-start), which is unambiguous. I did NOT draw an A0-REFERENCE-DIRECTION indicator (top +Z vs +Y side), and I FLAG that as the genuine design choice: A is a ROTATION about X, which is EDGE-ON in a 2D top-view (the flat rotates in the Y-Z plane, invisible from above), so "which orientation reads A0" has no honest 2D-top rendering. Options if the advisor wants it shown: (a) leave it to the 3D preview (current — the 3D shows the 4-jaw rig + the A rotation); (b) a small out-of-page mark / label ("A0 ▲+Z" or "A0 →+Y") near the span; (c) a side-elevation mini-view. RECOMMEND (a) — the top-view shows WHERE it probes (the span), the 3D shows the rotation; adding a rotation glyph to a top-view risks implying a position that isn't one.

**VERIFY (assert-the-value vs INDEPENDENT truth):** NEW rotary-datum-glyph.spec (3 tests): (1) layoutSpecFromOp for a KNOWN 200×100 stock → the centreline is a line at Y=50 (the exact stock Y-centre, x1=0 x2=200), the datum dot at (100,50); the clock (span=30) span-segment at X=100 from Y=35 to Y=65 (cy±span/2), the 2 touch dots at Y=35 & 65; a NON-rotary op (drill) → ZERO fc-rotary-* items (opt-in confirmed). (2) emit BYTE-IDENTICAL — the twin builderOf emit == the built-in rotaryCenterStack/rotaryClockStack across a method/datum/action/reference sweep (diffs=0 both) → the glyph is provably sim-only. (3) DRIVE the real app: openWiz('user_rotary_center_data') → the 2D layout pane renders .fc-rotary-axis + .fc-rotary-datum (real SVG, not just spec) → screenshot rotary_center_glyph.png shows the cyan dash-dot centreline through the round bar at the Y-centre, the datum dot on it. FULL SUITE 743 pass / 2 skip / 1 flaky (middle-animator stroke-dashoffset — a CSS-animation timing flake, passed on retry, unrelated) / 0 hard fail — the edge/corner/middle glyphs + every rotary emit golden UNREGRESSED.

**PASSED BACK: SIM-ONLY (emit byte-identical, proven by the sweep) + the screenshot. FLAGGED the clock's A0-reference-direction rendering (recommend leaving it to the 3D; the top-view shows the span, not the rotation) — advisor decides whether to add an indicator.**

## 🔨 turn 467 (cycle 223) — POCKET E0: the superset gate + the region-pill→flat reframe. Byte-diff ZERO (flatten + gate). Full suite 746 pass. 3 src + 1 new leaf-file + 1 new spec + 3 updated (opSession + 2 specs). FLAGGED: 2 leaves (not 1) + the tooSmall DERIVED-guard-key (E1 needs a derive-hook).

**TASK:** Pocket E0 — (1) flatten the region SOCKET to a flat leaf (positionally bindable, contourfill precedent); (2) add opts.superset guarding the 2 structural forks (strategy, tooSmall). VERIFY prune(superset,S) === concrete pocketStack(S) byte-identical across strategy × tooSmall × 4-shapes × scalar; superset:false === golden.

**BUILT:**
- NEW web/wizards/ops/pocketfill.js — TWO flat leaves (region-pill→flat reframe, mirroring contourfill), sharing pocketInsetRegion (ONE-SOURCE): pocketFillBlock (kind:'fill') = the stepover clearing; pocketWallBlock (kind:'leaf') = the raster wall finish (= Contour(on) on the inset region). Both hold TYPED geometry (shape/originX/originY/w/h/dia/sides) + wallOffset + toolDia + cut params FLAT; recompute the inset region + the absolute stepover (%·tool) INTERNALLY; reuse the SAME kernels (fillStrategy/fillSegments from stepover; circleTrace/contourLevel from contour). The shared stepover/contour/region atoms are UNTOUCHED (do NOT mutate a shared atom). Registered in ops/index.js (Toolpaths).
- pocketStack rewritten (opts.superset): the 2 forks GUARDED — strategy (raster → pocketfill(parallel)+pocketwall / spiral → pocketfill(concentric)) on the REAL param; tooSmall (drill vs stepdown-clearing) on the DERIVED `_tooSmall` key. Concrete (superset:false) selects the arms directly. Exported pocketTooSmall(params) (the ONE-SOURCE geometry predicate; used by the concrete build + the guard inject). Removed the now-orphaned offsetRegion import.
- opSession.pocket reverse-sync (blocks→form) updated to read the FLAT pocketfill block (mirroring opSession.contour) instead of un-insetting the stepover's region socket; now round-trips toolDia/wallOffset (the region socket COULDN'T) + reads stepoverPct directly (no un-deriving). Removed 3 orphaned imports (regionParamsFromDesc/regionDesc/offsetRegion — all only fed the removed un-inset).

**WHY 2 LEAVES (the dispatch said "a NEW flat pocketfill leaf" — singular):** the emitter wraps a `fill` block (leading `( <strategy> fill z= )` tag + trailing `G0 Z<clr> retract`) DIFFERENTLY from a `leaf` (bare emit); and the wall must emit AFTER the fill's retract (byte order). So ONE block can't be both kinds. Split into pocketfill (fill) + pocketwall (leaf), sharing pocketInsetRegion. Byte-diff ZERO confirms.

**WHY the tooSmall DERIVED-guard-key (a NEW concept — no precedent):** every prior E0 fork (corner/edge/middle/rotary/alignment) keys its guard on a REAL user param (probeZFirst/method/corner/action/…). Pocket's tooSmall is GEOMETRY-DERIVED (insetTooSmall(region, toolR−wallOffset)) — NOT any single param. So the guard keys on `_tooSmall`, INJECTED into the prune params (the E0 verify passes `{...S, _tooSmall: pocketTooSmall(S)}`). E1 IMPLICATION (flagged): the twin's instantiate (userOps.js:415 `pruneGuards(clone, p)`, p = withGuardDefaults — fills DECLARED struct-binding DEFAULTS only) has NO hook to inject a derived guard param → the twin would never emit the drill arm for a tiny pocket. RECOMMEND for E1: a `setUserDeriveGuards(opType, fn)` registry (mirroring setUserSimStock/setUserSimStarts — a live fn re-attached from the seed) that instantiate calls to augment p before prune; pocketData registers `p => ({ _tooSmall: pocketTooSmall(p) })`. (E0 does NOT need it — the verify injects directly.)

**VERIFY (assert-the-value, byte-diff ZERO):** NEW pocket-e0-superset.spec across the 96-combo sweep (4 shapes × 2 strategy × 2 size × 6 scalar): (1) FLATTEN byte-identity — new concrete (flat leaves) === the GOLDEN SNAPSHOT of the pre-E0 pocketStack (the stepover+contour region-socket emit — the INDEPENDENT truth, captured to tests/fixtures/pocket-golden.json before the restructure); goldenDiffs=0. (2) E0 GATE — prune(superset) === concrete; gateDiffs=0, leftoverGuards=0, guards=4 (tooSmall×2 + strategy×2), tooSmall-arm exercised 40×, wall-arm 28×. superset:false===default. UPDATED pocket-offset.spec + shape-types.spec (both inspected the removed stepover region socket → now inspect the pocketfill leaf via pocketInsetRegion/regionParamsFromDesc; the ASSERTED VALUES are unchanged — 74/3·78/1·70/5, poly Ø43.07, ell 74×54, + the round-trip recovers typed dia/sides/w/h via the new flat reverse-sync). FULL SUITE 746 pass / 0 fail (the initial run flagged only these 2 now-fixed specs; the 744 others held).

**NOTES:** regionParamsFromDesc stays EXPORTED in pocketWizard (its opSession consumer went, but the 2 specs + the inverse-of-regionDesc utility keep it live). NO shared atom mutated. NO emit change (the E0 concrete is byte-identical to the pre-E0 pocket).

**PASSED BACK: byte-diff ZERO (flatten + gate). FLAGGED (a) 2 leaves not 1 (the fill/leaf z-wrapper forces it); (b) the tooSmall DERIVED-guard-key + the E1 setUserDeriveGuards recommendation. Then E1 (twin + positional bindings + opensAs in-place).**

## 🔨 turn 469 (cycle 223) — POCKET E1: the twin + bindingSpecs + opensAs in-place + the DERIVE-GUARDS hook. Byte-diff ZERO (96 combos + cross-dialect); the tooSmall derive selects the right arm (armWrong=0). Full suite 750 pass. 5 src + 1 new twin + 2 new tests. Screenshot: pocket_inplace.png.

**TASK:** Pocket E1 — build user_pocket_data (superset twin) + the setUserDeriveGuards hook (approved t468) + opensAs in-place. VERIFY byte-identical across strategy × tooSmall (BOTH states) × 4-shapes × scalar + cross-dialect; tooSmall selects the drill arm; in-place; form non-empty; sim renders.

**THE DERIVE-GUARDS HOOK (whenGuard.js + userOps.js — approved):** setUserDeriveGuards(opType, fn) — a per-op LIVE fn (registered on the def like setUserSimStock, re-attached from the seed). instantiate runs it after withGuardDefaults + injects the result into the prune params BEFORE pruneGuards (NO hook → pruneGuards(clone,p) exactly as before → byte-identical for every existing op). ONE-SOURCE: the derive lives on the def, computed once. def.deriveGuards = p => ({ _tooSmall: pocketTooSmall(p) }) — the GEOMETRY-DERIVED guard key.

**POCKET IS THE FIRST COARSE-atom SUPERSET twin:** unlike drill/slot/surfacing/contour (no fork → frozen positional bindings), pocket has 2 structural forks → the template is pocketStack(DEFAULTS, superset:true) with GUARDs, and the value sockets bind BY IDENTITY (bindingSpecs, re-derived over the PRUNED stack — like corner/middle) with heavy FAN-OUT: a param → N specs across progstart/placeonstock/stepdown/pocketfill/pocketwall/drill, each `optional` with a {type}-match (UNIQUE per pruned state — each block type appears ≤1×). def.bindings (form) = deriveBindingsFor(canonical raster+big) DEDUPED by param (24 fields, no fan-out dupes); def.bindingSpecs = the full fan-out. strategy is a STRUCTURAL (bindingless) binding (guard key); _tooSmall is the derive-hook (not a binding).

**2 DERIVED SOCKETS via postInstantiate (kept the derive-hook scoped to the guard key only):**
- the DRILL arm x/y = the geometry-derived pocket CENTRE (pocketDrillCentre, exported one-source with pocketStack) — the frozen superset bakes them at DEFAULT geometry; postInstantiate rewrites from resolved.
- the placeonstock BBOX (bminX/bmaxX/bminY/bmaxY) — baked at DEFAULT geometry, NOT bound → placementShift computed offX−cornerX against a STALE bbox → a PHANTOM shift (THE 42-diff bug: every non-default-origin case + every tiny non-rect shape double-shifted by the frozen corner). postInstantiate rewrites the bbox from pocketBBox(resolved) → both arms track the actual footprint. ONE-SOURCE (pocketBBox). This is the coarse-superset analogue of contour's local-0 origin: pocket keeps the geometry AT its origin (E0-locked, not local-0), so the placement corner must track it.

**VERIFY (assert-the-value, INDEPENDENT truth = pocketStack):** NEW pocket-data-emit.spec — byte-diff ZERO across the 96-combo sweep (4 shapes × 2 strategy × 2 size × 6 scalar); armWrong=0 (the tooSmall derive selects the SAME arm as the built-in EVERY case — 40 drill / 56 stepover); cross-dialect ZERO (grbl + rs274ngc). NEW pocket-in-place.spec — Pocket opensAs=user_pocket_data, seamless title "Pocket", twin hidden; DRIVE: the in-place form renders 24 params NON-EMPTY (dedup worked — shape/strategy/depth/feed/stepoverPct + placement) + the 3D sim renders the toolpath ("Pocket (data) · 199 lines"). Screenshot pocket_inplace.png (title Pocket, full form, the concentric clearing in 2D+3D). FULL SUITE 750 pass / 0 fail (a prior run showed 9 FLAKY — the authoring/group tests under parallel contention + middle-animator; ALL passed in isolation; a cache-clear + re-run → 750/0, confirming transient).

**PASSED BACK: byte-diff ZERO (96 combos + cross-dialect) + the tooSmall derive selects the right arm (armWrong=0). The setUserDeriveGuards hook is LIVE + general (any geometry-derived fork can adopt it). Pocket fan-out + the 2 postInstantiate derived sockets are the coarse-superset-first learnings. Then WCS (assess + port if clean).**

## 🔨 turn 471 (cycle 223) — WCS PORT ASSESSMENT → NEEDS-DESIGN (flagged, did NOT force-port). No code change. The built-in wcsStack is HAND-ROLLED + M350-specific, bypassing the dialect-aware atoms → a frozen twin can't be cross-dialect byte-identical without a source restructure. Advisor decides.

**TASK:** assess WCS (wcsWizard.js, wizardLibrary id 'wcs') — (1) emit path declarative/portable? (2) structural forks? (3) live-machine hazard? VERDICT: clean → port (static or superset twin); needs-design → FLAG + pass the assessment.

**(1) EMIT PATH — declarative BUT dialect-baked-at-BUILD + M350-HARDCODED (the blocker).** wcsStack builds Comment + Set# (assign) atoms + emits via emitMapped → declarative in FORM. BUT: it reads the DIALECT at BUILD via a GLOBAL (`getDialect()` → getActiveProfile) and BAKES the WCS base into the assign var literal (`#${wcsBase+(sys-54)*stride+off}` → `#805` for G54/M350). AND it HARDCODES M350 registers regardless of dialect: the DRO source `#880/#881/#882` (axisX/Y/Z), the active-WCS `#578` (auto branch), the slave DRO `#883/#884` (sync). Only the WCS BASE is dialect-computed — the rest is M350. So the built-in is M350-CENTRIC with a half-done dialect-base computation (on V4.1/DM500/RS274 it emits a BROKEN mix: dialect base + M350 DRO). CRUCIALLY it does NOT use the DIALECT-AWARE atoms that exist for exactly this: `setWorkOffset` (dialect.setWorkOffset — the G10-equiv, Expert indirect / V4.1 `G90 G92` / RS274 `G10 L20 P`) + `readMachine` (dialect.readMachine — the per-dialect DRO). It hand-rolls raw assigns instead.

**(2) STRUCTURAL FORKS — a superset (pocket-style), + a dialect+sys-derived base.** `sys` (auto '0' vs FIXED G54..G59): AUTO adds #150=#578 + #151=base+[#150-1]*stride + INDIRECT targets `#[#151+off]`; FIXED uses DIRECT `#base+off` — a genuine STRUCTURAL fork (extra blocks + different targets). `axisX/Y/Z`: each toggles an assign (block-adds → structural). `sync` (dual gantry): adds slave writes, auto/fixed variant (structural). The 6 FIXED G-codes are the SAME shape with a different BASE (dialect+sys-derived → postInstantiate territory, like pocket's centre/bbox). So: a superset (auto|fixed × 3 axes × sync) with a dialect-derived base value.

**(3) LIVE-MACHINE — emit-only, but the MACRO writes offsets.** No direct Studio→controller push (like setWorkOffset). The EMITTED macro is #805+ register WRITES — when the operator RUNS it, it changes the work offsets (real machine state). Standard for emit; noted per the dispatch.

**VERDICT — NEEDS-DESIGN. The BLOCKER is the dialect-baked-at-BUILD var (§1):** a data-op twin's template is FROZEN (built once at seed), so it bakes ONE dialect's registers → a frozen WCS twin CANNOT be cross-dialect byte-identical (every other twin uses dialect-AGNOSTIC atoms resolved per-post at emit; wcsStack resolves the dialect at build + hardcodes M350). Force-porting would either (a) bake M350 into the frozen twin (wrong on other controllers, breaks the cross-dialect requirement) or (b) need a postInstantiate re-deriving the base per active-dialect AND recomposing the hardcoded DRO/active/slave per dialect — i.e. re-implementing dialect-awareness that the setWorkOffset/readMachine atoms ALREADY provide.

**RECOMMENDATION (ddcs directive #1 — restructure the SOURCE, don't force the twin):**
- **Option A (RECOMMENDED, but a CORRECTNESS decision the advisor must own):** REBUILD wcsStack on the dialect-aware atoms — `setWorkOffset` (the WCS write) + `readMachine` (the DRO read) — so the emit is declarative + dialect-aware AT EMIT. THEN the twin is a clean superset port (auto|fixed × axes × sync). SIDE EFFECT: this FIXES the built-in's M350-centrism → WCS would emit CORRECTLY on V4.1/DM500/RS274 (today it's a broken hardcoded mix). So the emit STAYS byte-identical on M350 but CHANGES (to correct) on the other dialects — a real G-code behavior change to verify per-dialect + ground vs the dumps (ddcs-expert skill). Not a silent port.
...
## 🔨 turn 475 (cycle 223) — WCS DIALECT-AWARE (Option A, BUILT): the wcsZeroAtCurrent atom (5 dialects, M350 dump-grounded) + wcsStack restructured onto it + caps-driven post-gating. M350 byte-diff ZERO; non-M350 the correct WCS-set (G10 L20 / G92). Full suite 751 pass. Screenshot: wcs_gating.png. PASS BACK for review BEFORE the twin. Key flag: G92 is a TEMP offset — the register-based DDCS variants may want persistent #1506+/#804+ writes.

**TASK:** build Option A — a new dialect-owned wcsZeroAtCurrent atom (M350 byte-identical, rs274/grbl G10 L20, v41/dm500 G92) + restructure wcsStack + caps-driven post-gating (auto M350-only; WCS-number gated on v41/dm500 G92; sync M350-only; grbl best-effort + flag). VERIFY + PASS BACK for review BEFORE the twin.

**BUILT — the wcsZeroAtCurrent atom (dialect-OWNED, resolved PER-POST at emit):** M350 = the DIRECT #805+ register writes, dump-grounded (SAVE_WCS_XY_AUTO.nc / COPY_WCS.nc — the INTERMEDIATE-VAR indirect form, NOT setWorkOffset's inline; setWorkOffset can't reproduce it byte-identical + can't change). rs274/grbl = G10 L20 P<n> <axes>0. v41/dm500 = G90 G92 <axes>0 (dm500 keeps its VERIFY note). NEW ops/wcszero.js — the `wcszero` leaf (emit = dialect.wcsZeroAtCurrent(p)); the auto|fixed × axes × sync forks are COMPUTED inside the atom (value params → a STATIC-shape op, NO superset — the E1 twin binds values, no guards). wcsStack rewritten to ONE wcszero block (removed the hand-rolled M350-hardcoded assigns + build-time getDialect). WCSWizard.generate emits with resolveActivePost(active) so the PREVIEW reflects the active post (else defaults to M350). ⚠ resolveParams coerces '0'→0/'3'→3 → normalized to strings in the methods (the auto/slave checks).

**POST-GATING (caps-driven, the probePort pattern):** 3 caps (wcsAuto/wcsFixed/wcsSync) — M350 all true; v41/dm500 all false; rs274/grbl wcsFixed only. postGating: w_sync/w_slave field-level (wcsSync); w_sys OPTION-level — auto gated on !wcsAuto, fixed G54-59 on !wcsFixed, the whole picker inert when no option applies (v41/dm500). Tooltips per the ruling.

**VERIFY (assert-the-value):** NEW wcs-dialect-emit.spec — M350 BYTE-IDENTICAL to the golden across the 48-combo sweep (sys × axes × sync) — ZERO diff; non-M350 emits the correct WCS-set + NO #880 leak. NEW wcs-gating.spec — M350 all-on; rs274 auto+sync gated / fixed G54 live; v41 whole picker + sync gated; screenshot wcs_gating.png (rs274 → the G10 L20 preview + auto/sync greyed). FULL SUITE 751 pass / 1 flaky (middle-animator) / 0 fail. (emitMapped takes `{ dialect }` settings, NOT a bare post; getDialect(id)=DIALECTS[id]; rs274/grbl are POSTS not PROFILES → set via setActivePostId.)

**THE non-M350 CHANGE (BEFORE→AFTER, fixed G54 zero X+Y — FOR REVIEW):** v41 `#1512=#880 · #1513=#881` → `G90 G92 X0 Y0`. dm500 `#804=#880 · #805=#881` → `G90 G92 X0 Y0   ( set datum - VERIFY on hardware )`. rs274 `#805=#880 · #806=#881` → `G10 L20 P1 X0 Y0`. grbl (couldn't target) → `G10 L20 P1 X0 Y0`.

**FLAGS:** (1) grbl HOST-STREAMED — a WCS macro is best-effort; the macro-vs-stream question is a broader porting concern (flagged, not solved). (2) ⚠ NEW GROUNDING REFINEMENT — G92 (v41/dm500) is a TEMPORARY offset, NOT a persistent WCS zero. V4.1's OWN zeroxy/zeroz macros write #1506-1509 DIRECTLY (persistent, consistent with M350); dm500 IS register-based (#804+). So a persistent #1506+/#804+ direct write may be MORE correct than G92 for the register-based DDCS variants. Built G92 per the ruling; FLAG the register-write alternative — advisor decides before the twin (no v41/dm500 dumps → best-effort).

**PASSED BACK: M350 byte-diff ZERO + the non-M350 before/after + the gating (screenshot) — FOR REVIEW, BEFORE the twin (did NOT chain the twin). Key flag: G92 (v41/dm500) is a TEMP offset; register-based DDCS variants may want #1506+/#804+ persistent writes.**

## 🔨 turn 477 (cycle 223) — WCS port COMPLETE: REVISED v41/dm500 to PERSISTENT register-writes (grounded) + BUILT the twin (user_wcs_data, static-shape) + opensAs in-place. M350 byte-diff ZERO (unchanged); v41 #1506, dm500 #804; twin == built-in ALL dialects. Full suite 754 pass. Screenshots: wcs_gating.png, wcs_inplace.png.

**TASK:** revise v41/dm500 G92 → PERSISTENT register-write (grounded); re-check the WCS-number gating; build the twin (static-shape, value bindings, opensAs). Pass back the full WCS port.

**REVISED (grounded vs the dumps):** V4.1 → `#1506=0 #1507=0` (Z→#1508) — GROUNDED in V4.1's OWN zeroxy.nc (`#1506=0 #1507=0`) + zeroall.nc (+#1508/#1509); writes the ACTIVE work registers to 0 (persistent). ACTIVE-WCS-ONLY (activeWcs null) → the WCS-number gate STAYS. dm500 → `#804=#864 #805=#865` (per-WCS, offset=DRO), M350-analogous via dm500's registers (#804 stride 4, DRO #864, active #455). PER-WCS → number MEANINGFUL → gate DROPPED. ⚠ FLAG: the dm500 #804 form is INFERRED (dm500's dumps show G92 in defprobe.nc + a #400-series in probe.nc; NO #804-write macro) — VERIFY on hardware. M350/rs274/grbl UNCHANGED (M350 ZERO re-verified).

**GATING RE-CHECK:** dm500 wcsFixed false→TRUE (per-WCS #804 → the number picker is meaningful; now gates like rs274: auto+sync off, fixed live); v41 stays wcsFixed false (active-only #1506, whole picker off).

**THE TWIN (user_wcs_data — STATIC-shape):** the wcszero atom COMPUTES the auto|fixed × axes × sync forks from value params → NO guards/superset. Frozen POSITIONAL bindings (blockIndex 4) over the ONE wcszero block: sys (enum) + axisX/Y/Z/sync (bool toggles) + slave (enum). panel 'form' (no motion). opensAs on the WCS slot (seamless "WCS / work offsets"; built-in wcsView = the shim). Emit dialect-aware → the twin == the built-in byte-identical on EVERY dialect for free.

**FORM FIX (general, additive):** toggleWidget didn't set data-param (unlike every other widget) → bool bindings rendered but weren't [data-param]-targetable. Added `cb.dataset.param = b.param` (parity) — now the WCS twin form renders all 6 fields incl. the 4 toggles.

**VERIFY (assert-the-value):** UPDATED wcs-dialect-emit.spec — M350 byte-diff ZERO (48-combo sweep) + v41 `#1506=0` (NOT G92/#880) + dm500 `#804=#864` (NOT G92). UPDATED wcs-gating.spec (+dm500: fixed G54 ENABLED, auto+sync gated). NEW wcs-in-place.spec — the twin == wcsStack byte-identical across ALL 5 dialects × a full-param sweep (ZERO) + opensAs + 6-field form. UPDATED wizard-bar.spec (WCS onclick → user_wcs_data). Screenshots wcs_gating.png + wcs_inplace.png. FULL SUITE 754 pass / 0 fail.
...
## 🔨 turn 481 (cycle 223) — HOMING H1: the HOME-END RECONCILIATION (SIM-ONLY, emit byte-identical). Declared the home end = MACHINE-0 (ONE source: axisHomeMotion), aligned homingSimProxy + the M98-P501 engine handler + axisSpan → the axes drive TO home (Z→top/0, NOT -120 — the fix for the human's DOWN+LEFT plunge). Full suite 755 pass. Screenshot: homing_at_home.png.

**TASK:** the home-end bug — axisSpan says home=machine-0, but homingSimProxy + the M98-P501 engine handler drove to the OPPOSITE (signed-travel/far) end → the axes drove AWAY from home. FIX: declare a homeDir FACT (home=machine-0, grounded in geometry.homeDir) as the ONE source; align all consumers; emit byte-identical. AMENDMENT (human LIVE-confirmed): the axes PLUNGE DOWN+LEFT (to the far end); the envelope is correct — only fix the home DIRECTION; reproduce the symptom then confirm the fix + screenshot at home.

**GROUNDED (the convention):** home = MACHINE-0 (per limit-switches.spec + FINDINGS: the home end reads ~0, the far/limit end is the signed travel). Z (travel -120): home=0 (TOP), far=-120 (bottom). X (travel +300): home=0, far=+300. axisSpan was ALREADY correct. The BUG: homingSimProxy.homeEnd + the engine handler drove to `t` (the signed-travel FAR end) — even X dir− → -300, OUTSIDE the [0,300] envelope.

**FIX (SIM-ONLY, ONE SOURCE):** NEW axisHomeMotion(travel, {dir, offset, backoff}) in engine/limitSwitches.js (next to axisSpan) → { seek, back }: seek = the machine-0 end (Auto); dir '+'/'−' = the max/min end (a Homing-block override); back = off the switch INTO the reachable travel (toward the span centre). homingSimProxy + the M98-P501 engine handler both use it → they + axisSpan AGREE on the home end. The NATIVE emit (M98 P501, reads the controller's #612) is UNCHANGED (byte-identical).

**VERIFY (assert-the-value + real-symptom):** homing-engine.spec (Z homes to -5 [machine-0/top backed off], NOT -115; X to +5, NOT 295) + homing-dir-override.spec (sim Auto → machine-0; the SEEK EMIT #102 + the NATIVE emit UNCHANGED) + limit-switches.spec (unchanged — home=machine-0). NEW homing-drive.spec — with the human's DOWN+LEFT envelope (x:-300 left, z:-120 down): the OLD far end = {x:-300, z:-120} (the plunge symptom) → the fix seeks X:0 Y:0 Z:0 (machine-0/home); sim + axisSpan AGREE. Screenshot homing_at_home.png (DRO Mach = home backed off ±5, "Execution complete", the native M98 P501 in the CODE PREVIEW — the tool AT home/top, NOT plunged down+left). FULL SUITE 755 pass / 0 fail.

**FLAG (out of H1 scope — emit byte-identical):** the `seek` method's EMIT (the G31 #102 seek-distance sign) still follows the signed-travel (far) direction — now INCONSISTENT with the fixed sim (homes to machine-0). Fixing it is a REAL emit change (byte-identity-breaking) → deferred. The NATIVE method (M98 P501, the default + verified) is fully consistent. `seek` is advanced/unverified.

**PASSED BACK: H1 done — the axes drive TO the TRUE home end (machine-0), ONE source (axisHomeMotion) shared by homingSimProxy + the M98-P501 engine handler + axisSpan; the human's down+left plunge CORRECTED (reproduced → fixed → screenshot at home). Emit BYTE-IDENTICAL (native M98 P501 unchanged). Full suite 755 pass. FLAG: the `seek`-method emit still seeks the far end (out of the sim-only H1 scope — a deferred emit fix).**

## 🔨 turn 483 (cycle 223) — HOMING H2: the SWITCH-TYPE CATALOG (mechanical vs proximity; per-EDGE; SIM-CONFIG only, emit byte-identical). NEW engine/switchTypes.js + the per-edge picker (row.switchType) + fed the standoff into limitSwitchTrips (proximity trips Sn BEFORE the edge). Full suite 757 pass. Screenshot: switch_type_picker.png.

**TASK:** (1) a pure engine/switchTypes.js exporting SWITCH_TYPES (mechanical standoff 0 render plunger / proximity standoff Sn render sensor-face), mirroring INPUT_TYPES' {type,label,help}; (2) a per-EDGE "Switch type" picker on the limit row (row.switchType, default mechanical); (3) feed the standoff into limitSwitchTrips (proximity trips at edgePos∓Sn). Sim-config only, emit byte-identical.

**BUILT:** NEW engine/switchTypes.js (pure, DOM-free, parallels limitSwitches.js): SWITCH_TYPES = [mechanical {standoff:0, render:plunger}, proximity {standoff:3, render:sensor-face}], each {type,label,help} (INPUT_TYPES shape) + switchTypeOf(type) (default mechanical) + switchStandoff(type) (0 unless proximity). limitSwitches.js: added switchTypeKey per edge to LIMIT_EDGES; limitSwitchTrips reads ioConfig[switchTypeKey] → switchStandoff → the per-edge trip shifts INWARD by Sn (minTrip = lo+Sn, maxTrip = hi−Sn; mechanical Sn=0 → unchanged AT the edge); edgePos = the standoff-adjusted trip position. ioTable.js: the per-EDGE "Switch type" select (SWITCH_TYPES) on the limit row, next to the Axis picker (default mechanical, add-row + migrate). settingsPanel.js: LIMIT_AXES + SETTINGS_DEFAULTS.limits + migrateIO + syncFlatFromIO extended with the switchType key (row.switchType ↔ settings.limits.<edge>SwitchType).

**VERIFY (assert-the-value):** NEW switch-types.spec — (1) the catalog (mechanical standoff 0/plunger, proximity standoff 3/sensor-face, each with help; unknown→mechanical/0); (2) limitSwitchTrips standoff — a proximity x_min (Sn 3) TRIPS with the tool 2mm from the edge (edgePos 3 = lo+Sn), a mechanical one does NOT (trips only AT the edge, edgePos 0) — the trip positions differ by exactly Sn; (3) round-trip (row.switchType=proximity → syncIO → settings.limits.xMinSwitchType=proximity; unset edges → mechanical); (4) emit BYTE-IDENTICAL (the homing macro references no switchType/proximity/standoff). Screenshot switch_type_picker.png (the limit row: Axis · Switch type [Proximity] · Pin · Level). FULL SUITE 757 pass / 0 fail.

**PASSED BACK: H2 done — the declared SWITCH_TYPES catalog (mechanical/proximity, per-edge) + the picker (row.switchType, round-trips) + the standoff fed into limitSwitchTrips (proximity trips Sn before the edge, mechanical at it — the trip position differs by Sn). SIM-CONFIG only, emit BYTE-IDENTICAL. Full suite 757 pass. Screenshot. Then H3 (wire the trip model into the engine).**

## 🔨 turn 485 (cycle 223) — HOMING H3: WIRED the live trip model into the engine (SIM-ONLY, emit byte-identical). setLimitSwitches(limitSwitchTrips(...)) at EVERY position-commit point (trace + real-time; linear/arc/homing) so IN_LIMIT_*/IN_HOME_* trip LIVE; the M98-P501 native handler now SEEKS the switch then BACKS OFF (trips→releases). Fixed a latent numbered-pin release bug in setLimitSwitches. Full suite 762 pass. Screenshot: homing_h3_switch_lit.png.

**TASK:** the trip model (limitSwitchTrips + the H2 standoff) existed + was unit-tested but GcodeExecutionEngine never CALLED it (SIM-ONLY, dead). WIRE setLimitSwitches(limitSwitchTrips(machinePos, machine, settings.limits)) at the engine position-commit points so the switches trip LIVE during any run (incl. the homing proxy + the M98-P501 handler). FRAME: this.pos is PART-frame; add _wcsOffset back for MACHINE coords. VERIFY: the home switch TRIPS during homing (respecting the standoff) + the back-off RELEASES it; a non-homing limit ALSO trips; emit byte-identical; suite green + drive-the-app + screenshot.

**FRAME (grounded):** the engine's G53/homing map (line ~918) is `part = machine·unitScale − wcsOffset`; the inverse (the ONE conversion the helper uses) is `machine = (part + wcsOffset) / unitScale`. `_wcsOffset` = the machine coord of part-zero (constructor line 98); default {0,0,0}, unitScale default 1 → machine==part when unknown.

**BUILT (GcodeExecutionEngine.js, SIM-ONLY):** (1) a `_updateLimitSwitches(pos = this.pos)` helper — maps PART→MACHINE via the inverse G53 map + calls `setLimitSwitches(limitSwitchTrips(mp, s.machine, s.limits))` (reads settings via window.ddcsGetSettings; early-returns with no window/machine). (2) CALLED at EVERY commit point — the intermediate animated tick (live during travel), _finishMove (animated landing), the linear TRACE commit, the linear real-time sub-frame commit, the arc TRACE commit, the arc real-time sub-frame commit, + the homing paths. Coverage is trace AND real-time for linear/arc/homing (I FOUND + fixed a gap: the linear TRACE commit at ~1117 + the arc TRACE commit at ~1169 were separate from the real-time commits I first wired — the app's trace() uses those, so a non-homing trace limit would not have tripped without them).

**M98-P501 NATIVE HANDLER — seek→back (the honest model, in-scope):** H1 made the native handler drive STRAIGHT to `back` (the rest spot), so it never passed THROUGH the switch → its home switch would not trip. The task explicitly names the M98 handler as needing to trip. Fixed by modeling the real O501 motion (what homingSimProxy's G53 seek/back already does): carry BOTH seek+back (homedMove = {axis, seek, back}); the motion goes current → SEEK (machine-0 → TRIP) → BACK (→ RELEASE). Trace = two rapid segments; real-time animated = a polyline [from, seek, back] (the tick already interpolates mv.path like an arc) so the switch flips LIVE mid-move; instant = trip at seek then release at back. Sim-only → emit BYTE-IDENTICAL. Test-safe: homing-engine.spec has no limits pins (setLimitSwitches([]) = no-op there) + the last segment still lands at `back` (lastZ=-5, lastRapid=true, segCount>1, no X=index all hold).

**LATENT BUG FIXED (virtualIO.setLimitSwitches):** its release loop cleared only the CONSTANT semantic names (IN_LIMIT_*/IN_HOME_*), NOT the config-driven NUMBERED IN_<pin> — so on back-off the semantic aliases released but the numbered pin (the wire the operator sees on the I/O panel) stayed orphaned-ON (violated its own "the others are cleared" docstring). This was dead until H3 exercised live release. Fixed: track `_assertedNumberedPins` (the numbered pins driven true last call) + include them in the release set; cleared in resetVirtualIO. First surfaced by the H3 spec (IN_7 stuck true after the back-off).

**VERIFY (assert-the-value + real-symptom):** NEW homing-limit-trip.spec — (1) native M98-P501 homing Z: capture IN_HOME_Z on every io_change → it TRIPS at the seek (machine-0) then RELEASES on the back-off; final home/limit/pin7 all released; #1517 set; (2) a NON-homing plain G0 to the +X envelope edge (300) trips IN_LIMIT_X_MAX + the numbered pin (9), but NOT IN_HOME_X (the +X max is a limit only — home is the machine-0/min end); (3) the proximity standoff THROUGH the engine — at X=298 (2mm from the 300 edge) a proximity switch TRIPS (within Sn=3), a mechanical one does NOT; (4) emit byte-identical (the homing macro references no limit/home IO); (5) DRIVE THE APP — drive Z to its home edge, show the live Virtual I/O panel → assert pin-7 + IN_HOME_Z + IN_LIMIT_Z_MAX tripped + the pin-7 LED renders .active (lit); screenshot homing_h3_switch_lit.png (pin 7 glowing green). FULL SUITE 762 pass / 0 fail (757→762 = the 5 new tests, no regressions).

**FILES:** engine/GcodeExecutionEngine.js (the helper + the commit-point calls + the M98 seek→back) · engine/virtualIO.js (the numbered-pin release fix + import) · tests/homing-limit-trip.spec.js (new).

**PASSED BACK: H3 done — the trip model is WIRED live at every commit point (trace + real-time, linear/arc/homing); the home switch TRIPS during homing (M98 native now seeks→backs off) + RELEASES on the back-off; a non-homing limit trips; the proximity standoff flows through; emit BYTE-IDENTICAL; a latent numbered-pin release bug fixed. Full suite 762 pass. Screenshot homing_h3_switch_lit.png (pin 7 lit). Then H4 (the switch-device MESH).**

## 🔨 turn 487 (cycle 223) — HOMING H4: the SWITCH-DEVICE MESH (the FINAL Homing slice; SIM/VISUAL only, emit byte-identical). A 3D switch device at each fitted home-end edge that LIGHTS/PLUNGES on the H3 trip (mechanical plunger / proximity sensor + standoff GAP). Homing H1-H4 COMPLETE. Full suite 768 pass. Screenshot: homing_h4_switch_devices.png.

**TASK:** render the home-end switch per axis (proximity = sensor face + a visible standoff GAP never contacted; mechanical = a plunger/lever). Mirror the ATC sim-devices: (1) viz.setLimitSwitchDevices(edges) — a body at each FITTED home-end edge's MACHINE coord (axisSpan/limitSwitches), styled per switchType; (2) viz.setLimitSwitchDevice(edge, made) — animate 'made' on trip, wired to io_change (the setAtcSwap listener pattern; reset on play, remove on setActive(false)); (3) wizardManager.previewLimitSwitches(containerId, edges) → viz.setLimitSwitchDevices; call it from homingView.update after previewMachine. VERIFY: device renders AT the home edge; lights/plunges on trip; proximity shows the gap (never contacted), mechanical contacts; emit byte-identical; drive-the-app + screenshot the axis AT home with the switch LIT.

**BUILT (SIM/VISUAL only — NO engine/emit touched):**
- **gcodeViz3d.js** — setLimitSwitchDevices(edges): a device per edge on the FIXED machine frame (raw machine coords, home at scene 0, like setForkDock/setStationDevices; teardown+dispose the prior group). Mechanical = a grey body OUTSIDE the edge + an amber PLUNGER reaching to the edge (contact). Proximity = a cyan sensor FACE set back outside the edge + a translucent standoff GAP box reaching inward by Sn (the non-contact trigger zone, NEVER crosses the edge). setLimitSwitchDevice(edge, made): both types LIGHT (vivid red 0xff2e2e); the mechanical plunger DEPRESSES toward the body (proximity never moves). getLimitSwitch(edge) → {x,y,z,axis,made,kind} for assertion. Box dims/offsets derived along the switch axis (dims/shift helpers), so one code path handles X/Y/Z.
- **createPreviewPanel.js** — setLimitSwitches(edges): calls viz.setLimitSwitchDevices + arms an io_change listener (like setAtcSwap's deviceIoListener) keyed by IN_HOME_<axis> / IN_LIMIT_<axis>_<side> (the H3 pins) → viz.setLimitSwitchDevice(edge, made). Reset the devices to rest in play(); drop the listener in setActive(false). Exposed on the panel API. (Routed via the PANEL — not viz-direct like previewToolSetter — because the trip animation needs the io_change lifecycle the panel owns; the task's "mirror previewToolSetter" is the manager-method shape, the io_change part explicitly refs setAtcSwap.)
- **wizardManager.js** — previewLimitSwitches(containerId, edges) → panel.setLimitSwitches (mirror previewAtcSwap).
- **homingView.js** — homingEdges(settings): ONE per linear axis whose HOME-end limit pin is fitted; positioned at the home edge (machine-0, from axisSpan) on its axis + the span mid on the other two, styled per its switchType + the H2 standoff. Called via ctx.previewLimitSwitches right after previewMachine. ONE-SOURCE: axisSpan + settings.limits — the SAME the H3 trip model reads, so the device sits exactly where its switch trips.

**VISUAL POLISH:** bumped the device sizes (proximity face 5×28, gap 20; mechanical body 9×26, plunger 10×9) + the lit colour to a vivid red so the switches read clearly at envelope scale (300×200×120) in the deliverable shot.

**VERIFY (assert-the-value + real-symptom):** NEW homing-switch-devices.spec (6 tests) — (1) a device renders AT each fitted home edge (X mech at x=0, Z prox at z=0/top, Y mech at y=0); (2) opt-in: an axis with NO home-end pin gets NO device; (3) io_change trip → the device is MADE; the mechanical plunger MOVES (depresses) + returns to rest; the proximity face LIGHTS (colour change) but its position NEVER moves (non-contact); (4) end-to-end — a real homing sim PLAY (the panel engine, H3-wired) lights the devices via the engine io_change (recorded the transient 'made' across the seek→back-off); (5) emit byte-identical (no switch/limit IO in the macro); (6) DRIVE THE APP — drive the seek to each home edge + hold, screenshot homing_h4_switch_devices.png (the tool at home, Mach 0,0,0, "Execution complete", the fitted home switches lit red). FULL SUITE 768 pass / 0 fail (762→768 = the 6 new tests, no regressions).

**FILES:** viz/gcodeViz3d.js (the device mesh + animate + getter) · viz/createPreviewPanel.js (setLimitSwitches + the io_change listener + reset/teardown) · wizardManager.js (previewLimitSwitches) · wizards/views/homingView.js (homingEdges + the call) · tests/homing-switch-devices.spec.js (new).
...
## 🔨 turn 491 (cycle 223) — HOMING DIRECTION: the DECLARED ENVELOPE SIGN is the single source (the human's principle). FIXED the Z=-120 plunge = a per-axis homing.z.dir='-' DIVERGING from the declared sign; axisHomeMotion now ALWAYS seeks machine-0 so a stale dir can't diverge. Emit byte-identical (sim-only). Full suite 772 pass. Screenshot: homing_declared_dir_top.png.

**SYNTHESIS (advisor + human):** human confirmed machine.z=-120 (declared on the envelope; X=600 Y=-600 Z=-120) + the decisive PRINCIPLE — "the home direction can't be hand-rolled or hardcoded, it is DECLARED by the user" = the direction IS the DECLARED ENVELOPE SIGN (one-source, declare-not-infer). Z=-120 MUST home to the TOP; the plunge = something OVERRIDING the declared sign. FIX: derive the home end from the envelope sign so homing.z.dir CANNOT diverge. REJECT Option B (hardcode Z-up) + Option C (a decoupled homeDir field) — the human wants it DERIVED from the envelope.

**DIVERGENCE FOUND (reproduced t489 + pinned t491):** axisHomeMotion's old `dir` param let '+'/'-' pick the max/min END: `seek = dir==='+' ? hi : dir==='-' ? lo : 0`. For Z travel -120, axisSpan={lo:-120, hi:0}, so a stale `dir='-'` seeked `lo`=-120 = the far/BOTTOM end → the PLUNGE (repro C). The dir-override.spec (t481) even ASSERTED this (dir='-' → the min end) — that WAS the hand-rolled divergence the human is ruling out. (The no-limit -120 default was never the cause — dir='' Auto already homes to 0; axisSpan/axisHomeMotion never read a limit pin.)

**FIX (SIM-ONLY, byte-identical):** engine/limitSwitches.js axisHomeMotion — `const seek = 0 + offset` (machine-0 = the DECLARED home, always lo or hi per axisSpan); DROPPED the `dir` param entirely so no override can diverge it. Both callers drop the `dir:` pass: the M98-P501 engine handler (GcodeExecutionEngine ~896) + homingSimProxy (homingWizard ~254). So Z=-120 ALWAYS homes UP to 0 regardless of any stale homing.z.dir; a +120 axis homes to its declared 0 end. The `dir` field still EXISTS (round-trips) + still signs the seek-METHOD G31 emit (homingWizard ~137) — that emit-direction reconciliation is the NEXT task (an emit change, not byte-identical), noted.

**VERIFY (assert-the-value + real-symptom):** NEW homing-declared-direction.spec — (1) machine.z=-120: the sim seeks Z to the TOP (0) for EVERY dir ('', '-', '+'); the M98 handler homes to -5 (top backed off), NOT -115 (the -120 plunge) — for dir='-' too (the human's exact divergence, corrected); (2) a +120 axis homes to its declared 0 end regardless of dir; (3) emit byte-identical (native M98 unchanged by dir); (4) DRIVE THE APP — open Homing with machine.z=-120 + a STALE dir='-', run the sim → DRO Mach Z=-5 (the TOP), NOT -120; screenshot homing_declared_dir_top.png. UPDATED homing-dir-override.spec: the SIM now homes to machine-0 REGARDLESS of dir (was: dir picks the far end) — the seek-EMIT #102 assertions kept (the next task). FULL SUITE 772 pass / 0 fail (768→772 = the 4 new tests, no regressions).

**ANSWER TO THE HUMAN (delivered via the advisor):** homing direction needs NO limit input — it's DECLARED by the signed Z travel (a limit only ADDS the H4 switch device). The plunge was a stale per-axis dir override diverging from your declared -120; now the ENVELOPE SIGN WINS and Z=-120 always homes UP.

**NEXT (noted, not this turn):** the G31-HOMING-OUTPUT — reconcile the seek-method emit direction (homingWizard ~137, still signs by dir/tSign) to seek toward this same declared home end.
...
## 🔨 turn 497 (cycle 223) — HOMING PREVIEW RENDER FIX: the homing tool renders in the MACHINE frame (not the stock-shifted part frame), so it draws at the envelope TOP even with a stock shown — the watched "plunge" is GONE. Verified IN THE REAL APP (rendered worldZ tracks engine.pos). Full suite 776 pass. Screenshot: homing_tool_at_top_with_stock.png.

**TASK:** you root-caused it (t495) — gcodeViz3d._partShift shifts the part frame by (tableFloor − _stockFloorZ) when a stock is shown, and the homing tool RIDES it → renders ~95mm too low (worldZ ~-100, near the bottom) = the plunge the human WATCHES, while the logic/engine/emit are correct (home to the top -5). FIX: render the HOMING tool at its MACHINE position (raw machine coords), NOT the stock-shifted part frame; do NOT touch the direction NOR the stock-shift for CUTTING ops. VERIFY IN THE REAL APP + screenshot.

**BUILT (VISUAL only — a machine-frame TOOL flag; the stock + cutting ops untouched):**
- **gcodeViz3d.setToolPosition** — subtract `this.partFrame.shift` from the tool's local position when `_toolMachineFrame` is set, so the animTool's WORLD position lands at the raw machine coord (= engine.pos). Off (default) → the normal stock-placed part-frame tool. + `setToolMachineFrame(on)` (builds + re-places) + `_reapplyMachineTool()` (called after each `partFrame.update` — setStock/setMachine — so the tool re-compensates when the shift changes) + `_lastToolPos` cache.
- **createPreviewPanel** — a panel-stored `machineFrameTool` hint (like `rotaryFixture`): `setToolMachineFrame(on)` + re-applied in `ensureViz` (survives lazy viz creation) + exposed on the API. In `play()`, SEAT the machine-frame tool at the start BEFORE the run — else a seek-to-home (a no-move) never fires onPositionChange, leaving the tool at the shifted build spot for the first frames (the transient I chased down + killed).
- **wizardManager.previewToolMachineFrame(containerId, on)** → panel.setToolMachineFrame. **homingView.update** calls it (true) right after previewMachine.

**WHY the layered plumbing (each piece fixed a real gap found by driving the app):** the flag alone left a pre-play transient because (a) the lazy viz didn't exist when the hint was first sent → panel-store + ensureViz re-apply; (b) the compensation ran while partShift was still 0 (before setMachine) → the `_reapplyMachineTool` hook on partFrame.update; (c) a no-move seek never fires onPositionChange → the play() seat. Each verified by re-driving the real app + reading the rendered world Z.

**VERIFY (REAL APP — drove the actual wizard, per the whole lesson; asserted the RENDERED viz._animTool world Z, not a function):** NEW homing-preview-machine-frame.spec — (1) native, machine.z=-120, STOCK SHOWN → the rendered tool reaches the TOP (worldZ ~-5, matching engine.pos.z), NOT -100; (2) STOCK HIDDEN → still the top; (3) a CUTTING op (drill) with the same stock → the machine-frame flag stays OFF (tool rides the stock-placed part frame, UNCHANGED); (4) DRIVE THE APP — the emitted native homing PLAYED with a workpiece shown → the tool rests at the TOP; screenshot homing_tool_at_top_with_stock.png (the spindle at the envelope top, the green stock on the table below, DRO Mach Z=-5). FULL SUITE 776 pass / 0 fail (the flag is gated off for every non-homing op → no cutting-op regression).

**FILES:** viz/gcodeViz3d.js (the machine-frame tool compensation + setter + re-apply hook) · viz/createPreviewPanel.js (the panel-stored hint + ensureViz re-apply + play() seat + API) · wizardManager.js (previewToolMachineFrame) · wizards/views/homingView.js (the call) · tests/homing-preview-machine-frame.spec.js (new).

**PASSED BACK: the homing preview render bug is FIXED — the homing tool renders at its MACHINE position (raw machine coords), so it draws at the envelope TOP even with a stock shown (rendered worldZ ~-5 = engine.pos, NOT -100). Direction untouched (correct); the stock-shift + cutting ops untouched (verified). Real-app before/after: STOCK SHOWN worldZ -100 → -5. Full suite 776 pass. Screenshot homing_tool_at_top_with_stock.png. (The seek-G31 emit fix stays PARKED for re-application per the G31-output want.)**

## 🔨 turn 499 (cycle 223) — G31 HOMING OUTPUT: the homing WIZARD now emits explicit G31 seek (params VISIBLE), NOT M98 P501. Re-applied the parked seek-direction fix (Auto → toward machine-0/home) + made seek the DEFAULT method + the engine envelope clamp. Native (M98) stays a SEPARATE setup option. Verified in the REAL app (code panel = G31, rendered tool at the switch/top). Full suite 781 pass / 2 skip / 1 flaky (middle-animator, unrelated). Screenshot: homing_g31_tool_at_switch.png.

**TASK (advisor re-frame, from the human, emphatic):** "i dont like that the homing param are in macro" (M98 P501 hides feed/dir/port inside the controller's O501); "the wizard is for g31"; "your screenshot still uses m98 not g31". So the homing WIZARD's OUTPUT = explicit G31 seek (params VISIBLE in the G-code), NOT M98. M98/native = the SEPARATE Homing Setup (controller built-in) — do NOT merge, do NOT delete native, but the WIZARD emits G31. The insight: M98 homes INTERNALLY on the controller (sim can only GUESS the destination); G31 seeks to the ACTUAL home switch (a real, defined place). BUILD: (1) re-apply the parked seek-G31 direction fix (dir=-tSign toward machine-0, #612 fallback, envelope clamp); (2) make the wizard emit G31 by default, params explicit, native separate; (3) the machine-frame RENDER fix (2826f74) applies to the G31 preview too.

**BUILT — 3 coupled changes + copy alignment:**
1. **Seek DIRECTION reconciled (homingWizard.js ~137)** — the Auto branch `(tSign||0)` → `(-tSign||0)`: the seek goes TOWARD machine-0 (the DECLARED home end, the ⬤ marker), OPPOSITE the signed travel that runs AWAY from home. For Z=-120 (home at the TOP) this seeks UP (`#102=10000`), not DOWN toward the -120 far end (the old +tSign "plunge"). Kept the explicit `c.dir` '+'/'-' override; `#612` fallback when tSign=0 (unknown envelope) — the exact O501 expression `20000*#[612+N]-10000`. Grounded vs the REAL O501 dump (slib-g.nc): line 184 `#[2+#1]=20000*#[612+#1]-10000` (seek dir/dist); line 190 `G91G31…P#[1045+#1*3] L#[1047+#1*3]` (P=limit port, L=level) — our emit matches.
2. **Wizard emits G31 by DEFAULT (native → seek)** — changed the per-axis default method in ALL coupled places (the twin-default rule): settingsPanel DEFAULTS x/y/z (172-174), homingWizard fallback (`c.method||'seek'`), homingView label fallback, macrosApp modal method-select fallback (862). Native is STILL selectable (HOMING_METHODS keeps it; the modal dropdown offers "Native (M98 P501)"). a/b stay `setzero` (rotary).
3. **Engine G31 envelope clamp (GcodeExecutionEngine.js ~1073)** — a G31 with NO stock to trip on and NOT the setter is a homing seek; clamp the seek target to the machine envelope span (part = machine·unitScale − wcsOffset, `axisSpan`) so a PLAYED single G31 seek stops AT the home switch (machine-0 edge), not the full ±10000 to the far corner (the -20000 plunge). `te>=0` so an on-edge seek is a no-op. Gated `!this.stock && probePort!==setterPin` → CANNOT touch a real probe (they always have a stock).
4. **Copy** — settingsPanel + homingWizard header + macrosApp hint + index.html wiz-usage blurb: all now say switch-seek (G31, visible params) is the default, native = the controller built-in (separate).

**DECISIONS / WHY:**
- **Kept the `c.dir` override on the seek** (only flipped the Auto branch to -tSign) — this is the parked fix AS DOCUMENTED (t495: "dir=-tSign … was +tSign" = flip the auto branch); it fixes the human's real symptom (Auto/default = dir='') and doesn't touch the modal. **FLAG (residual, advisor's call):** the explicit override CAN still emit a seek toward the FAR end (e.g. '+' on a +envelope), inconsistent with the now-declared SIM (t491 made the sim ALWAYS home to machine-0 regardless of dir). The clean one-source follow-up = make the seek emit fully declared (drop the override) like `axisHomeMotion`. Not done this turn (scope + it touches the modal dir field).
- **The preview KEEPS `simProxy` (NOT the full emitted G31).** The full G31 macro (multi-pass debounce/re-seek) relies on controller registers the Studio sim doesn't seed (#1045/#1910/#1520 = 0) → it doesn't cleanly replay. simProxy homes to machine-0 = EXACTLY where the G31 seeks (the home switch), and the render fix (t497) draws the tool at the top. So "the tool seeks to the switch/top" is TRUE via simProxy + render. The engine clamp is verified on a SINGLE representative G31 line, not a full-macro replay — honest about the limit.
- **Updated homing-dir-override.spec** — line 92 there PRE-ANNOUNCED "the seek-EMIT is reconciled to this same home end in the NEXT task" (this task): flipped the seekAuto assertion (+envelope) `#102=10000` → `#102=-10000` (toward machine-0), updated the narrative. Override + #612-fallback + round-trip assertions unchanged.

**VERIFY (REAL APP — drove the ACTUAL wizard, asserted BOTH the emitted code AND the rendered tool):** NEW homing-g31-output.spec (5 tests) — (1) UNIT: default emit contains G31, NOT M98P501; Z=-120 → `#102=10000` (toward machine-0/top); X=+300 → `#102=-10000` (toward machine-0/min); params visible + grounded vs O501 (P#[1045+#100*3], L#[1047+#100*3], F#22). (2) REAL APP: the wizard code panel shows G31, NOT M98P501, port param visible. (3) REAL APP with a STOCK SHOWN: the rendered tool seeks to the switch/top (worldZ ~-5), never plunges to -100; screenshot homing_g31_tool_at_switch.png. (4) a PLAYED single G31 seek (no stock) clamps to machine-0 (z≈0), not +10000. **18 homing tests pass; full suite 781 pass / 2 skip / 1 flaky (middle-animator stroke-dashoffset — pre-existing animation flake, passed on retry, unrelated). NO probe regression from the shared-handler clamp.** Screenshot confirms: axis chips read "switch-seek", code panel = the G31 seek block (`#102=10000`, `G91 G31 Z[#102] F#22 P#[1045+#100*3] L#[1047+#100*3]`), tool at the envelope TOP with the green stock below, DRO Mach Z=-5.

**BEFORE/AFTER emit (Z, DDCS Expert):** BEFORE (native) = `M98P501X2` (params HIDDEN in O501). AFTER (default seek) = the explicit G31 block — `#102=10000 ( seek dist signed by dir + )`, `G91 G31 Z[#102] F#22 P#[1045+#100*3] L#[1047+#100*3] K0 Q0 ( seek switch )`, … `G91 G01 Z-5 ( clearance back-off )`. Params VISIBLE.

**FILES:** wizards/homingWizard.js (seek dir -tSign + default seek + header) · ui/settingsPanel.js (DEFAULTS x/y/z seek + comment) · wizards/views/homingView.js (label fallback) · ui/macrosApp.js (modal method fallback + hint) · web/index.html (wiz-usage blurb) · engine/GcodeExecutionEngine.js (the homing-seek envelope clamp + axisSpan import) · tests/homing-dir-override.spec.js (seekAuto reconciled) · tests/homing-g31-output.spec.js (new).

**NOT released** — version.json still 10.97 (the advisor said they release after the human confirms; "do NOT claim fixed, I show the human"). **PASSED BACK: the wizard emits explicit G31 (params visible) not M98; seek reconciled toward machine-0/home; native kept as a separate setup option; real-app verified (code = G31, tool at the switch/top); screenshot homing_g31_tool_at_switch.png; before/after M98→G31 above. FLAGGED: the explicit dir override can still diverge the seek from the declared home (a clean follow-up to make it fully declared like the sim).**

## 🔨 turn 502 (cycle 223) — IO SECTION: the DECLARATION surface for the homing-plunge fix (increment 1 of 2 — homing NOT touched). Added a per-row HOME flag to the limit input row (declares WHICH switch is the home reference, replacing the machine-sign guess) + extended the switch-type catalog 2→4 (mechanical/proximity/optical/hall). Persists to settings.limits.<edge>Home; one-home-per-axis; all 4 types in the picker. Full suite 782 pass (3 unrelated flakes pass in isolation). Screenshot: io_home_flag_and_types.png.

**TASK (supersedes the obsolete turn-501 delete-toggle task):** CONFIRMED ROOT CAUSE (homing-switch-gap investigation, 4 agents): the I/O Input table already has a 'limit' switch type (axis-end + pin + level + switchType → settings.limits via syncFlatFromIO), BUT (a) NO way to declare WHICH switch is the HOME reference — home is GUESSED from the settings.machine travel SIGN (axisSpan.homeSide), which IS the positive-Z plunge; (b) homing never reads settings.limits (that's increment 2). THIS INCREMENT = the DECLARATION side only. (A) HOME flag on the limit row (store row.home, mirror to settings.limits.<edge>Home; ≤1 home per axis) — declares the home end but does NOT rewire homing. (B) switch types 2→4: add optical (photo-interrupter, non-contact, standoff ~1, render glyph) + hall (hall-effect, non-contact, standoff ~1, render glyph), keeping the {type,label,help,standoff,render} shape so the picker auto-offers them. Do NOT modify homingWizard/homingSimProxy/axisHomeMotion. Do NOT claim the plunge is fixed.

**BUILT — DECLARATION only, valid-by-construction, one-source:**
1. **engine/switchTypes.js** — SWITCH_TYPES 2→4: added `optical` (standoff 1, render 'sensor-face') + `hall` (standoff 1, render 'sensor-face'), same {type,label,help,standoff,render} shape. The sim trip model (limitSwitchTrips/switchStandoff, already wired H2/H3) automatically trips these 1mm before the edge (non-contact) — so the standoff is MEANINGFUL now without any homing rewire.
2. **ui/ioTable.js** — a per-row **Home** checkbox on the limit row (after the Switch-type picker). On check: `row.home=true` + CLEAR the other end(s) of the SAME axis (axis prefix, e.g. z_min↔z_max) → at most ONE home per axis. `.io-home-cb` class for the test. The Switch-type `<select>` already iterates SWITCH_TYPES.forEach → all 4 auto-appear.
3. **ui/settingsPanel.js** — LIMIT_AXES gains a 6th element per edge (the `<edge>Home` key, e.g. 'zMaxHome'); SETTINGS_DEFAULTS.limits gains `<edge>Home:false` for all 6 edges; **syncFlatFromIO** mirrors `row.home → s.limits.<edge>Home` (cleared to false each sync, then set from the fitted rows); **migrateIO** seeds `home:!!L[<edge>Home]` when legacy flat→inputs (complete round-trip both directions).

**DECISIONS / WHY:**
- **DECLARE-not-infer, exactly the north-star fix:** the home end is TODAY inferred from the machine travel sign (axisSpan.homeSide) → the positive-Z plunge. The Home flag DECLARES it (one-source with the fitted switch). This increment builds the declaration; increment 2 rewires homing to READ it (that's where the plunge is actually fixed).
- **Kept the 3D device glyph (gcodeViz3d) UNTOUCHED** — strictly "declaration only" per the dispatch, and the H4 device render is homing-preview-adjacent. **FLAG:** the viz's non-contact glyph currently keys on `switchType === 'proximity'` (gcodeViz3d ~1656), so optical/hall would draw the mechanical plunger glyph even though the sim trips them non-contact (standoff 1) — a small glyph/trip inconsistency. The one-source follow-up = drive that branch off the DECLARED standoff (>0) not a hardcoded type string; NOT done here (out of the declaration scope, and touches render).
- **One-home enforcement lives in the toggle handler** (as dispatched); a later axis-CHANGE on an already-home row isn't re-enforced (a minor edge) — noted, not fixed (out of scope).

**VERIFY (real app + a test):** NEW io-home-flag.spec (3 tests) — (1) SWITCH_TYPES is the 4 declared types with the {standoff>0,render,help} shape for optical/hall; (2) a fitted limit input marked Home → settings.limits.zMaxHome=true, un-marked ends=false, and all 4 switch types round-trip (zMinSwitchType 'optical', zMaxSwitchType 'hall') + pin/level persist; (3) DRIVE the ioTable: two Z rows, click Home on z_max then z_min → z_max clears (ONE home per axis), and the picker offers Mechanical/Proximity/Optical/Hall effect; screenshot io_home_flag_and_types.png (Z− Home checked + Z+ cleared + Optical selected). The existing switch-types.spec (mechanical/proximity standoff + emit byte-identical) still PASSES. **Full suite 782 pass / 2 skip / 3 FAILED-then-PASS-in-isolation (blocks-roundtrip, mill-atc-in-place, op-sim-starts-registry — all unrelated to the IO change, flaky under parallel load; re-ran the 3 alone → 8/8 pass).** Emit byte-identical (no macro reads settings.limits — increment 2 wiring hasn't happened).

**FILES:** engine/switchTypes.js (optical+hall) · ui/ioTable.js (Home toggle + import comment) · ui/settingsPanel.js (LIMIT_AXES 6th elem + DEFAULTS.limits Home keys + syncFlatFromIO mirror + migrateIO seed) · tests/io-home-flag.spec.js (new).

**PASSED BACK: increment 1 (the I/O declaration surface) is done — a per-row Home flag (persists to settings.limits.<edge>Home, ≤1 per axis) + 2 new switch types (optical/hall), all 4 in the picker; verified in the real app + a test; screenshot io_home_flag_and_types.png. Homing untouched — the plunge is NOT claimed fixed (that's increment 2: rewire homing to READ settings.limits.<edge>Home instead of the travel-sign guess). FLAGGED: the 3D device glyph still keys on switchType==='proximity' (optical/hall draw the mechanical glyph) — a one-source follow-up.**

### 🔨 turn 502 — AMENDMENT (2 refinements, absorbed pre-commit; still increment 1, declaration only):
**(1) SEED default home switches (do NOT ship an empty limits table).** `migrateIO` (settingsPanel.js) now seeds a HOME switch per LINEAR axis on a FRESH machine (empty inputs + no legacy flat-limit pins): X→x_min, Y→y_min, **Z→z_max (top — routers home Z UP; a physical direction, sign-agnostic, NOT the human's number)**, each `home:true`, `switchType:'mechanical'`, **pin UNASSIGNED** (the user sets their controller pin), all user-editable. Then `syncFlatFromIO(s)` mirrors the seed (incl. the per-edge Home flags) into the flat `settings.limits` on first load → increment 2 reads the DECLARED top instead of the axisSpan sign-guess. Only fires when no legacy flat-limit rows exist (returning users keep their config; no duplicates). One home per axis by construction (one seeded row each).
**(2) MODAL not dropdown.** The `+ Add input/output` entry-point (ui/ioTable.js) now opens a self-contained **modal type-picker** — a 2-col grid of TYPE cards (each showing its label + help) + a Name field + Cancel/Add — replacing the cramped inline `<select>`. Picking a card selects it (highlight) + fills the Name; Add creates the row (pre-expanded for the SAME in-row config, which is unchanged). Esc / backdrop / Cancel close. Selectors `.io-add-modal` / `.io-add-type[data-type]` / `.io-add-confirm` for the test.
**VERIFY:** io-home-flag.spec grew to 5 tests (added: fresh-seed → 3 home switches X/Y/Z with Z=z_max, pin unassigned, mirrored to flat; + the modal opens/offers types/adds a limit row, no inline dropdown). **Full suite 787 pass / 2 skip / 0 fail / 0 flaky.** Screenshots: io_home_flag_and_types.png (Home flag + 4-type picker) + io_add_input_modal.png (the type-picker modal). **FILES (+ this amendment):** ui/ioTable.js (the modal + openAddIoModal) · ui/settingsPanel.js (migrateIO seed 3 home switches + syncFlatFromIO on seed). Homing STILL untouched; plunge NOT claimed fixed (increment 2).

## 🔨 turn 504 (cycle 223) — HOMING increment 2: WIRE homing to READ the DECLARED home switch — THE PLUNGE FIX. Homing now homes to the edge whose settings.limits.<edge>Home===true (seeded Z=z_max/top), NOT the machine travel-sign guess. SIGN-AGNOSTIC verified in the REAL app at BOTH signs: z=-120 → Z Mach -5 (top); z=+500 → Z Mach 495 (TOP, not machine-0/0 the plunge). Full suite 790 pass / 0 fail. Screenshots both signs.

**TASK:** increment 1 (268641e) added settings.limits.<edge>Home = the DECLARED home end per axis (seeded X x_min, Y y_min, Z z_max/top). NOW make homing READ it instead of the axisSpan travel-SIGN guess (the positive-Z plunge). The home END = the declared edge's coordinate; seek TOWARD it, sim STOPS there. (1) axisHomeMotion → return the declared edge coord not machine-0 (Z zMaxHome → hi/top for + AND −). Do NOT break axisSpan (the pure span for limitSwitchTrips/homingEdges). No declared home → sign-derived fallback (no regression). (2) homingSimProxy → seek the declared edge + stop at the switch; pass settings.limits in. (3) the emitted G31 seek DIRECTION → toward the declared edge (NOT the emit STRUCTURE — that's increment 3). Do NOT touch the I/O UI (increment 1) or the emit structure (increment 3). Do NOT claim fixed.

**BUILT — the declared-home read at the home-TARGET layer (axisSpan stays the pure span):**
1. **engine/limitSwitches.js** — NEW `declaredHomeEdgeSide(axis, limits)` → 'min'|'max'|null from `settings.limits.<axis>MinHome/MaxHome` (the ONE source for WHICH end is home; ≤1 enforced by the UI, max wins if both). `axisHomeMotion(travel, {offset, backoff, axis, limits})` now seeks `declaredHomeEdgeSide() || axisSpan.homeSide` → the edge COORD (`lo`|`hi`), NOT always machine-0. Z zMaxHome → `hi` (TOP) for machine.z + OR − (SIGN-AGNOSTIC). `axisSpan` UNCHANGED (limitSwitchTrips/homingEdges still read the pure span).
2. **homingWizard.js** — `homingSimProxy` passes `{axis, limits}` → the G53 G0 rapid targets the declared edge coord (the rapid ENDS at the switch → the tool STOPS there), then backs off. `homingStack` seek DIRECTION: `homeSide==='max'?+1 : 'min'?-1 : (-tSign fallback ⇒ #612)` — the emitted G31 seeks toward the declared edge (Z zMaxHome → +10000/up for BOTH signs). Emit STRUCTURE untouched. Import `declaredHomeEdgeSide`.
3. **wizards/views/homingView.js** — threads `limits: settings.limits` into the wizard params (both generate + simProxy read it).
4. **engine/GcodeExecutionEngine.js** — the M98 P501 handler passes `{axis: AX, limits: s.limits}` to axisHomeMotion (a played native home also uses the declared edge).

**WHY / DECISIONS:**
- **The declared read lives ONLY at the home-TARGET layer (axisHomeMotion + the seek dir), NOT in axisSpan** — axisSpan is the pure geometric [lo,hi] span that limitSwitchTrips + homingEdges + the H4 device placement depend on; changing it would move the SWITCH GEOMETRY, not just the home target. Directive-1 clean: the span stays dumb, the target reads the declaration.
- **Fallback preserves every no-limits caller** (homing-declared-direction, homing-engine, homing-dir-override, homing-g31-output all pass NO limits → `declaredHomeEdgeSide→null` → `axisSpan.homeSide` = machine-0 = the pre-t504 behavior). 32 homing/switch tests green, incl. the byte-identical emit guards.
- **The observed z=+500 [0,0] engineZ was the SAMPLE WINDOW, not a bug** — the emit was already correct (`G53 G0 Z500`, `#102=10000`); the 500mm rapid was mid-flight (engine.pos commits at the move END; the render animated to 338). The real-app test waits for `!engine.running` (+ bumps simSpeed) then reads the SETTLED pos → 495. Honest: the fix is at the emit/target layer (deterministic), the real-app confirms the tool follows.

**VERIFY (deterministic + REAL app, BOTH signs):** NEW homing-declared-home.spec (3 tests) — (1) DETERMINISTIC: `axisHomeMotion(-120,zMaxHome).seek=0`, `(+500,zMaxHome).seek=500` (TOP for both), `(+500,zMinHome).seek=0` (reads the FLAG not the sign), `(+500,no-decl).seek=0` (sign fallback); simProxy `G53 G0 Z0`/`Z500`; G31 `#102=10000` (up) for BOTH; (2) REAL app z=-120 → settled Z -5 (top); (3) REAL app **z=+500 → settled Z 495 (the declared TOP), NOT 0 (the plunge is GONE)**. **Full suite 790 pass / 2 skip / 0 fail.** Screenshots homing_declared_z_neg120.png (DRO Z Mach -5) + homing_declared_z_pos500.png (DRO Z Mach 495) — BOTH tool-at-the-top.

**FILES:** engine/limitSwitches.js (declaredHomeEdgeSide + axisHomeMotion reads it) · wizards/homingWizard.js (simProxy + seek-dir + import) · wizards/views/homingView.js (limits in params) · engine/GcodeExecutionEngine.js (M98 handler passes axis+limits) · tests/homing-declared-home.spec.js (new).

### 🔨 turn 504 — AMENDMENT (a 1-line UI z-index fix, disjoint from homing, folded into this commit; the human hit it live):
The `+ Add input` MODAL rendered BEHIND the settings panel — `openAddIoModal` (ui/ioTable.js) set the overlay `z-index:10080` but the settings-overlay is `z-index:12000` (styles.css), so 10080 < 12000 buried it. **FIX:** bumped the io-add-modal overlay to `z-index:13100` (above the settings-overlay 12000 + the 13000 suggestion dropdown, below blockly's 20000). One line in ioTable.js; homing untouched. **VERIFY (in context):** drove the REAL settings panel (openSettings hardware/Input) → the Inputs list shows the seeded **Home X / Home Y / Home Z** rows (the increment-1 seed working live) → clicked `+ Add input` → the modal sits ON TOP (measured modal z=13100 > settings-overlay z=12000); screenshot io_add_modal_over_settings.png. io-home-flag modal test also asserts z > 12000. FILE: ui/ioTable.js (z-index only).

**PASSED BACK: increment 2 done — homing READS the DECLARED home switch (settings.limits.<edge>Home), sign-agnostic. z=+500 now homes UP to the declared top (Z Mach 495), NOT machine-0/bottom (0) = the plunge is fixed at the logic layer; z=-120 still Z Mach -5. axisSpan untouched (pure span); no-declaration fallback = no regression; emit STRUCTURE untouched (increment 3). PLUS the amendment: the + Add input modal now stacks ABOVE the settings panel (z 13100 > 12000), verified in context (+ the seeded Home X/Y/Z rows show live). Verified deterministic + real app both signs; screenshots homing_declared_z_neg120.png / homing_declared_z_pos500.png / io_add_modal_over_settings.png. NOT claimed fixed — the human confirms on their screen. Still open: the optical/hall glyph follow-up.**

## 🔨 turn 506 (cycle 223) — ALIGNMENT one-source probe points + AUTO/MANUAL travel — FOUNDATION (the DECLARE side), byte-safe. Built the ONE source (alignPoints) + wired the SIM markers + the AUTO/MANUAL emit LOGIC to read it; declared the params for the Blockly round-trip. Full suite 792 pass. **SURFACED 2 architecture forks** (the 2D-canvas surface + the AUTO-through-the-static-twin emit flow) — this feature is deep in data-op/twin territory; the advisor should steer the next increment.

**TASK (autonomous — advisor reviews + drives to done):** (A) declare the 2 alignment probe points as ONE draggable source, FRACTIONS of the stock, persisted on the op; EVERY surface reads it — the 2D layout canvas (2 handles), the sim (replace the fixed 0.3/0.7 opSimStarts), the emit. (B) AUTO/MANUAL travel toggle (default AUTO): AUTO rapids to the 2 declared positions at safe-Z + probes (no jog/Confirm); MANUAL = today (jog + Confirm, byte-identical); grey AUTO with no stock. + Blockly round-trip; verify VALUES; screenshots.

**BUILT (the DECLARE-side foundation — byte-safe, tested):**
1. **wizards/ops/alignPoints.js (NEW)** — the ONE source: `alignPoints(params)` → the 2 points as FRACTIONS (params.ax/ay/bx/by; empty → the checkAxis default spread 0.3/0.7 @ 0.85, mirroring the old inference); `alignPointsXY(params, stock)` → stock-frame mm. Neutral module (no wizard/viz imports) so the wizard + opSimStarts + the view all read it without a cycle.
2. **viz/opSimStarts.js** — `alignment()` now reads `alignPointsXY` — the 2 sim markers come from the declared source, REPLACING the fixed 0.3/0.7. Default = the same positions (E2 twin sim-starts test still byte-identical); a SET fraction MOVES the marker.
3. **wizards/alignmentWizard.js** — `travel` param (default AUTO, guarded by `canAuto` = a usable stock; no stock → MANUAL, byte-safe). AUTO `buildBody`: `travelTo(px,py)` rapids to the declared point at safe-Z (#19) then descends, REPLACING the operator jog + Confirm; MANUAL keeps the Confirm gates verbatim → byte-identical (the twin E1/E3 specs prove full identity; they pass no stock → MANUAL).
4. **blocks/opSchema.js** — declared `travel/ax/ay/bx/by` on the alignment shape → they round-trip through the op marker codec (the Blockly-param wiring; no form field yet → not in FIELD_BIND, no orphan).

**VERIFY (deterministic VALUES):** NEW alignment-onesource.spec (4 tests) — (1) opSimStarts reads the fraction source: default A=(45,85)/B=(105,85) for a 150×100 stock; a dragged ax=0.5 → marker A.x=75 (the source drives the sim); (2) AUTO emit (default + stock) rapids to X45/Y85/X105 at safe-Z (#19), a dragged ax=0.5 → X75, no Confirm prompt; MANUAL keeps 'Press Enter … point A', no AUTO travel; (3) AUTO needs a stock — no/0×0 stock → MANUAL (never an AUTO rapid to 0,0); (4) travel + the 4 fractions round-trip through markerLine/parseMarker. **Full suite 792 pass / 1 flaky / 0 fail; the 8 alignment twin (E0-E3) + protocol-validator specs still green (byte-identity + no orphan).**

**⚠️ 2 ARCHITECTURE FORKS surfaced (this feature is deep in data-op/twin territory — grounding revealed the standalone alignmentView (wiz_alignment) is legacy; Align opens the DATA-OP `user_alignment_data` IN-PLACE, E3):**
- **Fork 1 — the 2D-canvas 2 handles.** The "2D layout canvas" is the DATA-OP in-place form's canvas (generic userOpForm/formWidgets, driven by the sim-start markers), NOT a bespoke alignmentView canvas. So "2 draggable handles that write the fraction params" = extending the data-op form's canvas to render 2 sim-start handles whose drag writes params.ax/ay/bx/by (persisted). Recommend: wire the data-op form canvas → the source; confirm that IS the intended surface.
- **Fork 2 — the AUTO emit through the STATIC twin.** The alignment EMIT flows through the data-op twin (`alignmentStack(superset)` → static template + SCALAR bindings, #1-#5/#19). AUTO's coords are STOCK-DEPENDENT (frac × stock) — NOT user scalars — so they don't fit the static-template + scalar-binding model; the twin (built with no stock) would emit MANUAL even with a stock, DIVERGING from the JS builder's AUTO. Options: (a) rebuild the twin superset per-instance with the op's stock; (b) bind the 4 coords as computed #vars; (c) exempt alignment-AUTO from the twin (the AUTO path emits via the JS builder). The AUTO emit LOGIC is built + tested in `alignmentStack` (the source) and is twin-SAFE today (no-stock→MANUAL); it just doesn't yet FLOW through the data-op emit until this fork is decided.

**FILES:** wizards/ops/alignPoints.js (new) · viz/opSimStarts.js (reads the source) · wizards/alignmentWizard.js (travel + AUTO/MANUAL emit) · blocks/opSchema.js (params round-trip) · tests/alignment-onesource.spec.js (new).

**PASSED BACK: the DECLARE-side foundation for alignment one-source + AUTO/MANUAL is done + byte-safe (full suite 792 pass; twin byte-identity intact). The SIM markers now read the ONE source; the AUTO/MANUAL emit LOGIC reads it (twin-safe); the params round-trip. NEXT increment is blocked on 2 forks I surfaced (the data-op form's 2D-canvas 2-handle wiring + how AUTO flows through the static twin emit) — recommend the advisor steer those before I wire the visible canvas + the real-app AUTO emit + gating UI + screenshots.**

## 🔨 turn 508 (cycle 223) — ALIGNMENT increment 2, FORK 1 (the VISIBLE surface): the in-place data-op form's 2D Layout now renders BOTH sim-start markers (A, B) as DRAGGABLE handles, bound to the ONE source via a DECLARED marker→param binding. A real pointer drag writes the fraction param → recorded (persist + round-trip) + re-read by opSimStarts (the marker tracks it). Verified in the REAL in-place UI. Full suite 795 pass / 0 fail. Screenshot alignment_fork1_handles.png (2 handles A/B).

**TASK (both forks RESOLVED by the advisor; autonomous):** FORK 1 (canvas) — extend the DATA-OP in-place form's canvas (the legacy standalone alignmentView is dead; Align opens user_alignment_data in-place) to render the 2 sim-start markers as DRAGGABLE handles; a drag writes the DECLARED marker→param binding (A → params.ax/ay, B → params.bx/by, FRACTIONS), persisted + re-read by opSimStarts + the emit. FORK 2 (AUTO through the twin) — extend the superset with GUARDED travel arms + a postInstantiate applyAlignAutoTravel that BINDS the AUTO coords from the op's stock × the fractions (twin stays the SOURCE, no JS delegate).

**BUILT this turn — FORK 1 (the visible surface), a GENERIC declared seam scoped to alignment:**
1. **blocks/dataOps/alignmentData.js** — `def.simStartParams = [{x:'ax',y:'ay'},{x:'bx',y:'by'}]` — the DECLARED marker→param binding (index = pass; fractions). Declared on the def, NOT hardcoded in the view.
2. **wizards/views/userOpView.js** — a `_simStartFracs` store: seeded from the op's params on EDIT (setForm), cleared per fresh OPEN (onShow), MERGED into `params` in update() → recorded (persist + round-trip) + read by opSimStarts (marker moves) + the emit. Builds `simMarkers` from `def.simStartParams` × the sim-start positions; each marker's onDrag writes the fraction (`writeSimStartFrac` = world / stock, clamped 0..1) → mgr.update(). Ops WITHOUT the binding (corner/edge) keep the existing single sim-only ○ (unchanged).
3. **wizards/ops/panelTypes.js** — `renderLayout2D`/`layoutSpecFromOp` take a `simMarkers` array → render EACH as a draggable Layout handle (labelled A, B…, id `__simstart<i>`) + route its drag to the marker's onDrag. Generic seam; the single-simStart path (corner/edge) is untouched.

**WHY / DECISIONS:**
- **The binding is DECLARED on the def (simStartParams), read by a GENERIC view seam** — per the advisor's "keep it generic where cheap, scope to alignment now." A future op declares its own marker→param map; no per-op view code.
- **ZERO change to the twin EMIT** — Fork 1 is the drag→param→sim surface only. The drag writes the FRACTION params (the ONE source); opSimStarts reads them (the marker tracks); recordOp persists them (round-trip). The EMITTED-AUTO-coord surface is FORK 2 (below), coupled but separable — the sim + persist + round-trip are fully live now.
- **Low-risk to the shared layout** — the `simMarkers` block returns early only when the array is present (alignment); every other op falls through to the existing simStart path (corner-drag + the corner-start-datum flicker test both still 0-regression, 0px flicker).

**VERIFY (REAL in-place UI):** NEW alignment-fork1-handles.spec — open user_alignment_data with a 200×120 stock → the Layout shows BOTH `__simstart0`/`__simstart1` handles (was 1); the binding is declared on the def; a REAL pointer-drag of handle A to the stock centre → getLastOp().params.ax≈0.5, ay≈0.5 (the fraction, RECORDED → persist + round-trip), and opSimStarts(recorded params) marker A tracks it (x≈100). Screenshot alignment_fork1_handles.png (A dragged to centre, B at upper-right — 2 cyan handles). **Full suite 795 pass / 2 skip / 0 fail; the 31 in-place + corner-drag + alignment twin (E0-E3) specs green (no layout/byte-identity regression).**

**FORK 2 = the NEXT increment (the twin AUTO emit + the travel toggle/gating), plan worked out this turn:** extend `alignmentStack(superset)` to carry the travel arms GUARDED (a `capture()` of the Point-A/B arrival sub-sequences → `GUARD({param:'travel',is:'auto'/'manual'}, …)`, nested under the checkAxis×probeDir arms; Point-B's manual-only `MV Z#20` descend = its own `GUARD(manual)`). Add `applyAlignAutoTravel(stack, resolved)` to the twin postInstantiate (mirroring applyDatumWcs) — when resolved.travel==='auto', read `getWorkpiece()` stock × `alignPoints(resolved)` → rewrite the `travelTo` MV X/Y coords. RECONCILE the canAuto-vs-static-guard: the GATING (grey AUTO no stock) lives in the in-place FORM (a `travel` segmented binding, data-op-gated) so `params.travel` is valid → both the builder + the twin trust it → byte-identical (AUTO+stock) / MANUAL (E1/E3). Add the `travel` toggle binding (+ gating) WITH Fork 2 so it's not a dead control. The AUTO emit LOGIC already lives in alignmentStack (t506) + is tested.

**FILES:** blocks/dataOps/alignmentData.js (simStartParams) · wizards/views/userOpView.js (the fraction store + simMarkers) · wizards/ops/panelTypes.js (simMarkers handles) · tests/alignment-fork1-handles.spec.js (new).

**PASSED BACK: Fork 1 done + verified in the REAL in-place UI — the 2D Layout has 2 draggable handles (A/B) bound to the ONE source (a declared marker→param binding); dragging writes the fraction param → recorded (round-trip) + the sim marker tracks it. Screenshot alignment_fork1_handles.png. ZERO twin/layout regression (795 pass). Fork 2 (the twin AUTO emit + the travel toggle/gating) is the next increment — the plan (guarded travel arms + applyAlignAutoTravel + form-side gating) is in this entry; the AUTO logic already lives in alignmentStack.**

## 🔨 turn 510 (cycle 223) — ALIGNMENT increment 2, FORK 2 (the AUTO emit through the TWIN): COMPLETES Alignment. The data-op twin (user_alignment_data) now emits AUTO byte-identical to alignmentStack WITH a stock (binding the travel coords from the op's stock × the fraction params) and MANUAL without — keeping the twin the emit SOURCE (wizards-as-data M2, no JS delegate). + the travel toggle + gating in the in-place form. Full suite 797 pass / 0 fail. Screenshots: the twin AUTO emit code + the travel toggle greyed/enabled.

**TASK (advisor-confirmed plan, autonomous):** (1) extend the twin superset to carry the travel(auto/manual) arms GUARDED (like the checkAxis×probeDir E0 arms) → prune collapses to the concrete travel mode. (2) postInstantiate `applyAlignAutoTravel` (mirror applyDatumWcs) BINDS the AUTO travel-move coords from the op's stock (getWorkpiece) × the fractions → the twin emits AUTO BYTE-IDENTICAL to alignmentStack AUTO with a stock, MANUAL without. (3) the travel toggle + gating in the in-place form (grey AUTO no stock, data-op-gated).

**BUILT — the twin AUTO emit + the toggle/gating:**
1. **wizards/ops/alignPoints.js** — `alignStockUsable` + `alignEffectiveTravel(params, stock)`: the ONE source of the effective mode (AUTO needs a usable stock; else MANUAL). Shared by the builder, the twin's prune-derive, and applyAlignAutoTravel → all three agree.
2. **wizards/alignmentWizard.js** — the superset (`guardTravel`) wraps the Point-A/B arrival sub-sequences in `GUARD({param:'travel',is:…})` via a `capture()` helper (nested under checkAxis×probeDir; Point-B's manual-only `MV Z#20` = its own GUARD(manual)); the CONCRETE build calls the arm fn directly → byte-identical. The AUTO comment is coord-FREE (the coords ride the MOVES, recomposed by the twin — so the placeholder-superset comment == the concrete). `travel = alignEffectiveTravel`.
3. **blocks/dataOps/alignmentData.js** — `def.deriveGuards` injects the EFFECTIVE travel into the prune params (getWorkpiece stock) → the superset collapses to the right arm even when AUTO+no-stock (→ MANUAL, E1/E3 safe). `applyAlignAutoTravel(stack, resolved)` in postInstantiate — when effective=auto, rewrite the numeric X/Y move params IN ORDER [A.x,B.x]/[A.y,B.y] from `alignPointsXY(resolved, getWorkpiece().outer)`. + the `travel` segmented STRUCT binding (widgetConfig.gateAuto) + `ALIGNMENT_DEFAULTS.travel='auto'`.
4. **ui/formWidgets.js** — the segmented widget's `gateAuto`: GREY the 'auto' segment (disabled + `data-op-gated=on` so postGating won't re-enable + a 'place a stock/datum' tooltip) when no usable stock, and fall the value to 'manual'; re-enable dynamically on the `ddcs:settings-changed` event (self-removes the listener when detached).

**WHY / DECISIONS:**
- **The twin stays the SOURCE (M2), NOT a JS delegate** — the AUTO arm rides the SUPERSET (guarded) + the coords are RECOMPOSED from the op's stock in postInstantiate (the applyDatumWcs pattern). So `user_alignment_data` (what Align actually opens) emits AUTO itself.
- **The canAuto-vs-static-guard reconciliation = `def.deriveGuards`** (the pocket `_tooSmall` precedent): inject the effective travel BEFORE prune, so a stock-dependent mode collapses a static guard correctly. The FORM gating keeps `params.travel` valid (manual with no stock).
- **The AUTO comment dropped its coords** — the placeholder superset (built no-stock → X0Y0) must match the concrete comment; the coords live ONLY on the MOVES (which applyAlignAutoTravel rewrites) → byte-identity holds.
- **The E0-E3 MANUAL byte-identity STAYS the guarantee** — updated E0 to prune with the effective (manual) travel; E1/E2/E3 pin the MANUAL emit (`travel:'manual'`, stock-independent). AUTO byte-identity = its own new test.

**VERIFY (byte-identity + real app):** NEW alignment-fork2-twin-auto.spec (2) — (1) the twin AUTO emit == alignmentStack AUTO (byte-diff ZERO, STUDIO + EXPERT, across checkAxis/probeDir/fraction/safeZ combos; the twin binds `G0 X45 / G0 Y85` from the stock×fractions); a dragged fraction (ax 0.3→0.5) changes the TWIN emit (X45→X75); no usable stock → the twin emits MANUAL == alignmentStack MANUAL (byte-identical). (2) REAL app: the in-place form shows the Travel toggle; AUTO greys (disabled + data-op-gated + tooltip) with no stock, re-enables (dynamic) with a stock. **Updated E0 (prune with effective travel) + E1/E2/E3 (pin MANUAL) → all GREEN. Full suite 797 pass / 0 fail.** Screenshots alignment_twin_auto_emit.png (the code preview = the AUTO emit) + alignment_travel_gated.png (Auto greyed) + alignment_travel_enabled.png.

**FILES:** wizards/ops/alignPoints.js (alignStockUsable + alignEffectiveTravel) · wizards/alignmentWizard.js (guardTravel superset + coord-free AUTO comment) · blocks/dataOps/alignmentData.js (deriveGuards + applyAlignAutoTravel + travel binding) · ui/formWidgets.js (gateAuto) · tests/alignment-superset.spec.js + alignment-data-emit.spec.js + alignment-in-place.spec.js (pin MANUAL) · tests/alignment-fork2-twin-auto.spec.js (new).

**PASSED BACK: Fork 2 done — ALIGNMENT COMPLETE. The twin emits AUTO byte-identical to alignmentStack with a stock (coords bound from the op's stock × the draggable fractions), MANUAL without (E0-E3 green); a dragged handle changes the TWIN emit (one source → the twin, not just the JS builder); the in-place form has the Travel toggle with AUTO gated (grey + tooltip) when no stock, re-enabling with a stock. Full suite 797 pass. Screenshots: alignment_twin_auto_emit.png / alignment_travel_gated.png / alignment_travel_enabled.png. NOT claimed done — the advisor takes it to the human.**

## 🔨 turn 512 (cycle 223) — OPTICAL/HALL DEVICE GLYPH (the I/O-work loose end): the 3D home/limit switch device glyph is now DECLARED by the switch type's `render` field, so the new optical/hall types draw the non-contact SENSOR-FACE glyph (not the mechanical plunger). Full suite 798 pass. Screenshot switch_glyphs_4types.png (X− plunger; X+/Y−/Y+ sensor faces).

**TASK:** gcodeViz3d.setLimitSwitchDevices (~1656) keyed the non-contact glyph on `e.switchType === 'proximity'` → optical/hall (also non-contact, standoff 1) drew the MECHANICAL plunger. FIX (one-source): drive the glyph off the DECLARED data (switchTypes.js `render` 'sensor-face' vs 'plunger'), not the hardcoded type string.

**BUILT (one line of logic + the kind fix):** import `switchTypeOf`; `const nonContact = switchTypeOf(e.switchType).render === 'sensor-face'` → the branch (was `switchType==='proximity'`). The non-contact device `kind` = the actual type (`e.switchType`: proximity/optical/hall) not the literal 'proximity' → and the plunge animation stays gated on `kind === 'mechanical'` (the declared plunger glyph) so NO non-contact type ever moves. Comment on setLimitSwitchDevice updated (contact = kind==='mechanical', not per-type).

**VERIFY:** NEW switch-glyph-declared.spec — fit one device of EACH of the 4 types (mechanical/proximity/optical/hall); assert the DECLARED render map ({mechanical:'plunger', the other 3:'sensor-face'}) drives the selection; mechanical → a plunger (contact); proximity/optical/hall → NO plunger (the sensor face, non-contact), kind = the type (never 'mechanical' → never plunges). Screenshot switch_glyphs_4types.png (X− = the grey+orange plunger; X+/Y−/Y+ = cyan sensor faces). **The H3/H4 device specs (mechanical PLUNGES / proximity glows), switch-types, io-home-flag all still GREEN; full suite 798 pass / 0 fail.**

**FILES:** viz/gcodeViz3d.js (the render-declared glyph + kind) · tests/switch-glyph-declared.spec.js (new).

**PASSED BACK: the optical/hall glyph loose end is closed — the 3D switch-device glyph reads the DECLARED render field (mechanical→plunger; proximity/optical/hall→sensor-face), one-source, not the hardcoded 'proximity' string. Verified all 4 types + the H3/H4 specs green (798 pass). Screenshot switch_glyphs_4types.png.**

## 🔨 turn 514 (cycle 223) — SETUP/IO increment 1: FIXED the Comm/MDI MSG ReferenceError crash (Comm was BROKEN on every non-HMI post). The PORT to a data-op twin is SCOPED as its own decomposition (a large frontier-touching effort — surfaced for the advisor to dispatch as focused sub-increments, like the alignment E0→E3). Full suite 798 pass.

**TASK:** (1) FIX the MSG bug — `MSG(...)` is called at communicationWizard.js ~45/60/72 (the non-HMI fallbacks) but never defined (only C/A/IF/GO/LB/RAW) → a ReferenceError aborted commStack/generate on any post WITHOUT caps.hmi → Comm broken on V4.1/DM500/grbl/rs274. (2) PORT Comm/MDI to a user_comm_data twin (opensAs, byte-identical), in-place.

**DONE — the MSG fix (critical, the reason Comm was broken on non-HMI):** defined `const MSG = (t) => newBlock('message')` alongside the other helpers — the DIALECT-AWARE message atom (hmiToast on HMI, an operator comment on non-HMI), mirroring alignmentWizard's MSG. Additive (MSG is called ONLY in the `!caps.hmi` branches → the HMI emit is untouched). **VERIFY (comm-msg-fix.spec):** commStack emits (NO ReferenceError, non-empty G-code) on BOTH an HMI post (ddcs-expert-m350) AND a NON-HMI post (ddcs-v41), across all 6 type cases (popup×2/status/input/beep/dwell); the non-HMI fallbacks carry the message text (popup 'Load part', status 'Running', input 'Enter depth'). Full suite 798 pass / 0 fail — additive, no regression.

**PORT (piece 2) = SCOPED as its own decomposition, NOT done this turn — the honest reason:** Comm is a STRUCTURAL-BRANCH op (the atc_warmup port's own doc flags WCS's auto/fixed branch as "not data-expressible at all" without the superset machinery; Comm is worse — it branches on `type`(5) × `caps.hmi`(post-dependent) × `popupMode`(3)). PLUS the value-bearing `msg` is baked into POST-SPECIFIC RAW text (hmiPrompt/#1503/#2070/message) — exactly the "VALUE-BEARING message an operator reads before acting → a future GENERAL annotation-text atom / recompose-from-param" FRONTIER the atc_warmup port named. So the port needs: **(E0)** make `commStack(superset)` carry the type × hmi × popupMode arms GUARDED → prune==concrete gate; **(deriveGuards)** inject `_hmi = getCaps(active).hmi` so the hmi fork prunes per-post (the alignment effective-travel precedent); **(recompose)** a postInstantiate that rebuilds the value-bearing `msg` text from the resolved param (like applyHeaderComments, across the 5 type variants) + the slots/color/id/val value-swaps; **(twin)** userOpFromStack off the superset + bindings + wizardLibrary `opensAs 'user_comm_data'` + OPENS_AS_TARGETS + E3. That is a multi-sub-increment port (the alignment port took E0/E1/E2/E3 across turns). RECOMMEND the advisor dispatch it as focused sub-increments (E0 superset gate → the msg recompose → the twin/opensAs), so each is a complete, byte-identically-verified piece.

**FILES:** wizards/communicationWizard.js (define MSG) · tests/comm-msg-fix.spec.js (new).

**PASSED BACK: the Comm MSG crash is FIXED (Comm now emits on every post, HMI + non-HMI, all types — it was broken on non-HMI by an undefined MSG). Verified both posts × all types (comm-msg-fix.spec); full suite 798 pass. The PORT to a user_comm_data twin is a large STRUCTURAL-BRANCH + value-bearing-msg frontier port (the atc_warmup doc's named frontier) — SCOPED with a concrete E0→recompose→twin decomposition; recommend dispatching it as focused sub-increments (like alignment E0-E3) rather than one turn. NOT claimed done — the port is scoped, not built.**

## 🔨 turn 516 (cycle 223) — COMM PORT E0: the commStack SUPERSET gate (the twin's foundation), byte-identical. The advisor dispatched the twin port (1b). I built + verified E0 (the hardest structural piece); the twin def + value-bearing recompose + opensAs is the next sub-increment (1b-ii). Full suite green.

**TASK (1b):** PORT Comm/MDI to a user_comm_data twin (opensAs, byte-identical), in-place; the twin byte-identity must hold across the type sweep × HMI + non-HMI posts. I decomposed it (as the alignment port was E0/E1/E2/E3): E0 (superset gate) THIS turn; the recompose + twin def + opensAs next.

**BUILT — E0 (the commStack superset), byte-identical + non-regressive:** `commStack(P, {superset:true})` carries the STRUCTURAL forks GUARDED (the alignment/rotaryClock E0 pattern) — `type`(popup/status/input/beep/dwell) × the HMI cap × `popupMode`(0/1/3) × the conditional slots(#1510-1513)/color(#2039)/status-dwell/dest/cyc. The forks are refactored into per-arm builder fns (popupArm/statusArm/inputArm/beepArm/dwellArm + the hmi/mode/conditional sub-forks); the CONCRETE (superset:false) path calls them imperatively — I verified it emits BYTE-FOR-BYTE the original (git-stash diff, full sweep × both posts = IDENTICAL). The value-typed forks guard on DERIVED keys (`_hmi/_popupMode/_useColor/_hasStatusDwell/_hasDest/_hasCyc`) so a value-fork (e.g. the popupMode "else"/toast, un-guardable by "not 1 and not 3") collapses cleanly — the twin's deriveGuards + the E0 test compute them.

**VERIFY:** NEW comm-e0-superset.spec — `pruneGuards(commStack superset, {P, ...derive(P)})` == concrete `commStack(P)`, byte-diff ZERO across 11 combos (each type + popupMode 0/1/3 + slots + color + status-dwell + persist mode + dest + beep cyc) × BOTH ddcs-expert-m350 (HMI) AND ddcs-v41 (non-HMI); prune leaves 0 guard blocks. PLUS the git-stash diff: the refactored CONCRETE emit == the committed original (byte-for-byte). comm-msg-fix.spec still green. Full suite green.

**NEXT (1b-ii, the plan):** blocks/dataOps/commData.js — `userOpFromStack(commStack(COMM_DEFAULTS, {superset:true}), bindings, …)` + `def.deriveGuards = (p) => ({ _hmi: getCaps(active).hmi, _popupMode: Number(p.popupMode), _useColor: …, _hasStatusDwell: …, _hasDest: …, _hasCyc: … })` + a postInstantiate `applyCommRecompose` for the VALUE-BEARING fields: the superset bakes COMM_DEFAULTS with SENTINELS for msg/slots/color/id/val (the msg baked into the post-specific RAW via hmiPrompt/hmiToast/hmiInput/#1503/#2070/MSG); the recompose swaps each sentinel → the resolved value with the right formatting (fmtCtrl for popup/input, fmtLine for status; the computed useId for input). Then wizardLibrary `opensAs 'user_comm_data'` on the comm entry + OPENS_AS_TARGETS (E3). Verify the TWIN byte-identity (== commStack across the type sweep × HMI+non-HMI) + the in-place form + screenshot.

**FILES:** wizards/communicationWizard.js (the superset — concrete byte-identical) · tests/comm-e0-superset.spec.js (new).

**PASSED BACK: Comm PORT E0 done — the commStack SUPERSET gate is byte-identical (prune(superset)==concrete across the full type×hmi×mode×conditional sweep × HMI+non-HMI; the refactored concrete == the committed original, git-stash-diff verified). This is the twin's foundation; the twin def + the value-bearing-msg recompose (sentinel-swap) + opensAs is 1b-ii (concrete plan in this entry). NOT claimed done — E0 is the verified structural gate, the twin is next.**

## 🔨 turn 518 (cycle 223) — COMM PORT 1b-ii COMPLETE: the user_comm_data TWIN (def + deriveGuards + value-bearing recompose + opensAs in-place). Byte-identical across the type sweep × HMI + non-HMI; opens IN-PLACE with a live DDCS-screen preview. Full suite green (802 pass).

**TASK (1b-ii):** Build the twin ON the E0 superset (388b4c8) — (1) blocks/dataOps/commData.js (COMM_DEFAULTS mirroring the FORM defaults, deriveBindings by #var identity, deriveGuards injecting the derived keys, postInstantiate recompose for the value-swaps the guards don't cover); (2) wizardLibrary opensAs + seed; (3) Comm opens the twin IN-PLACE (E3). Verify byte-identity (× type sweep × HMI+non-HMI, 0 diffs) + in-place form + DDCS-screen preview + screenshot.

**BUILT — the twin, byte-identical (0 diffs) across 8 combos × Expert(HMI) + V4.1(non-HMI):**
- `commData.js` — `commDataDef()` = `userOpFromStack(commDataStack(), bindings, 'commscreen', …)` where `commDataStack()` wraps `commStack(SUPERSET_PARAMS, {superset:true})`. `def.deriveGuards` injects `_hmi/_popupMode/_popupToast/_useColor/_hasStatusDwell/_hasDest/_hasCyc` into the prune params (the alignment/pocket precedent) → the superset collapses to the concrete shape. `def.bindingSpecs` re-derives the CLEAN #var scalar sockets BY IDENTITY over the pruned stack every build (slots #1510-1513, beep dur #2042 + cyc #2043, status colour #2039 — the color disambiguated by `params:{var,note:'Status bar color - BGR'}` since #2039 appears twice). `def.postInstantiate = applyCommRecompose`.
- **The value-bearing recompose (the frontier the atc_warmup doc named):** the superset bakes SENTINELS (msg='COMMMSG', dest='CDEST', slots='CS1..4', id='#479'→useId 479, and NUMERIC sentinels 444444=statusDwell / 222222=val for values baked verbatim into RAW G-code). `applyCommRecompose` global-swaps each sentinel → the resolved, dialect-formatted value across every raw/message/comment/assign block — so ONE swap covers all ~6 msg embeddings (hmiPrompt/hmiToast/hmiInput RAW · the #1503 status composite · the #2070 input · the message atom). fmtCtrl (popup/input) vs fmtLine (status). Composite #1503 = `${statusMode}(${line})` REBUILT from resolved. The computed useId (id 50-499 else 100), the dest VAR swap, the Persistent/Update status comment + the pulsed-beep comment REBUILT from resolved. String assign values swap (catches the dest-copy `#${useId}`); NUMBER-typed bound sockets are left typed (not stringified).
- `wizardLibrary.js` — `opensAs: 'user_comm_data'` on the comm entry (OPENS_AS_TARGETS auto-hides the twin's own entry, plain title "Comm / MDI"). `app.js` — import + seed `commDataDef()` on boot.
- **The DDCS-screen preview = a DECLARED new panel view (one-stack-many-views):** panelTypes.js `commscreen` (viz:true, mode:'commscreen'); userOpView renders `CommunicationWizard.generateScreenPreview(params)` (pure fn) into a self-cleaning `.comm-screen-host` sibling — isolated branch, removed at the top of every render so it never leaks into another op's 3D/2D pane.

**PARITY GAP the comm port surfaced + fixed one-source:** `textWidget` was the ONLY form widget missing `dataset.param` (number/slider/dropdown/segmented/toggle all set it, with comments citing "[data-param] parity … targetable for post-gating + form-read"). So string fields (msg/id/dest/slots) rendered + read but weren't targetable (post-gating / source-chips / detection). Added the one line → the in-place form now exposes all 14 fields.

**VERIFY (byte-identity + real app):** comm-twin.spec (NEW) — `user_comm_data` emit == `commStack` byte-for-byte across popup×3(toast/OK/binary-slots)/status×2/input/beep/dwell × Expert + V4.1 = 0 diffs. comm-in-place.spec (NEW) — opensAs re-points + plain title + twin retired + registered; DRIVE `openWiz('user_comm_data')` → 14 comm fields render (type/msg/popupMode/…), the `.comm-screen-host` shows the controller dialog mock (popup Message + Enter btn), the LEGACY #wiz_comm modal does NOT open. Screenshot comm_e3_inplace.png (the in-place form + the live DDCS screen + `#1505=-5000()` toast emit). comm-e0-superset + comm-msg-fix still green. wizard-bar.spec updated (the comm onclick → `openWiz('user_comm_data')`, mirroring the atc_warmup line). **Full suite 802 pass / 0 fail** (1 unrelated flaky retried green).

**ALSO shipped:** the E0 toast-guard FIX (uncommitted from t516) — the committed E0 (388b4c8) guarded the toast arm on `_popupMode:0`, which never matched the real form value -5000, so the toast would prune away; corrected to the derived boolean `_popupToast` (= mode !== 1 && mode !== 3) in communicationWizard.js + the E0 test derive + the toast combo.
...
## 🔨 turn 522 (cycle 223) — I/O-STEP wizard E2: the twin def + the mode-picker FORM + register + opensAs in-place + gating. Opens IN-PLACE with the 3 modes; declared-I/O pickers by name; Input greyed on non-Expert. Full suite 810 pass.

**TASK (E2):** the twin def (deriveGuards + declared-I/O recompose) + the mode-picker form (output/input/dwell + per-mode fields, declared-I/O pickers by name + raw-pin fallback) + register (setup group) + opensAs in-place + postGating (grey Input on non-Expert).

**BUILT:**
- **`blocks/dataOps/ioStepData.js` (NEW)** — `ioStepDataDef()` over the E0 superset. `deriveGuards` injects `_mode/_outDeclared/_inDeclared`; `applyIoRecompose` (postInstantiate) fills the pruned leaf params from the instance — the outpin/waitinput/dwell block params AND the DECLARED-I/O values RE-RESOLVED from settings (resolveIoParams: the selected output → its on/off M-code as a RAW line, the selected input → its poll pin), the row values riding SENTINELS (IO_SENT) the recompose swaps (the Comm-twin recompose pattern). IO_STEP_DEFAULTS mirror the form. No bindingSpecs needed (io values aren't #var assigns — all ride the recompose). Byte-identical to ioStepStackResolved.
- **the mode-picker FORM** (IO_STEP_BINDINGS) — mode (segmented) + per-mode fields: output(declared-output picker + On/Off), input(declared-input picker + rise/fall/high/low + timeout + var), dwell(seconds); the raw-pin fields gated by the NEW compound `whenAll` (mode AND ref).
- **register** — wizardLibrary BUILTINS `{ id:'io_step', opensAs:'user_io_step', group:'setup' }` (opensAs hides the twin's own entry + gives the plain "I/O Step" title, like comm); seeded in app.js. The raw atoms stay in the Blocks palette (unchanged).
- **postGating** — the Input segment greys (data-op-gated + tooltip) on a post without wait-on-input, falling to Output (E1's ioInputSupported() = caps.flow==='oword' || caps.inputRead).

**TWO reusable framework pieces the wizard surfaced (declare, one-source):**
- `declared-io` FORM widget (formWidgets) — a dropdown listing settings.outputs/inputs BY NAME (live) + a "Raw pin…" fallback, value = the row id (relabel-proof join, the ATC pin-picker pattern).
- COMPOUND `whenAll` gate — a binding declares `whenAll: [{param,is},…]` ANDed; the view evals all. The single `when` can't express two conditions (the raw-pin field needs mode=output AND outputRef=raw, else the input raw-pin leaked into output mode). Additive; the existing single `when` path is unchanged.
- generic `gateSeg` (segmentedWidget) — a declarative segment gate (value/pred/fallback/tip) generalising the t510 gateAuto; the mode picker uses `gateSeg:{value:'input',pred:'ioInput',...}`.

**VERIFY (real app + values):** io-step-twin.spec (NEW) — user_io_step emit == ioStepStackResolved byte-identical across mode × declared/raw × Expert+V4.1 (0 diffs). io-step-in-place.spec (NEW) — opensAs wiring (plain title, twin retired, registered); DRIVE Expert: the mode picker + per-mode fields render, the declared-output picker lists Coolant, the declared-input picker lists X min, the Input segment NOT gated, the compound whenAll hides the input raw-pin in output mode (+ dwell hidden); DRIVE V4.1: the Input segment is data-op-gated ON + disabled + tooltip. Screenshots: io_step_output/input/dwell + io_step_input_gated_v41 (the greyed Input segment). wizard-bar.spec updated (the io_step onclick). **Full suite 810 pass / 0 fail.**

**FILES:** web/blocks/dataOps/ioStepData.js (NEW) · web/wizards/ioStepWizard.js (inRaw → waitPin) · web/ui/formWidgets.js (declared-io widget + gateSeg + whenAll) · web/wizards/views/userOpView.js (whenAll eval) · web/blocks/wizardLibrary.js (io_step entry) · web/app.js (seed) · tests/io-step-twin.spec.js (NEW) · tests/io-step-in-place.spec.js (NEW) · tests/wizard-bar.spec.js (onclick).

**PASSED BACK: I/O-step wizard E2 done — the grouped wizard opens IN-PLACE (opensAs, plain title, twin hidden) with the mode picker + per-mode fields; the declared-I/O pickers list settings.outputs/inputs BY NAME (+ raw fallback); the twin emit == ioStepStackResolved byte-identical; the Input mode greys on a non-Expert post. Surfaced 3 reusable pieces: the declared-io widget, the compound whenAll gate, the generic gateSeg. Full suite 810 pass. NEXT (E3) = Blockly round-trip + the SETUP-menu I/O quick-insert rewiring (the 3 bar-special I/O buttons → open the wizard). NOT claimed done past E2.**

## 🔨 turn 524 (cycle 223) — I/O-STEP wizard E3: Blockly round-trip + the SETUP-menu rewiring. COMPLETES the grouped I/O-step wizard (and the Setup/IO set). Full suite green.

**TASK (E3):** (1) Blockly round-trip — the mode + per-mode params round-trip through the op marker codec (opSchema markerLine/parseMarker, no FIELD_BIND orphan); (2) SETUP-menu rewiring — the 3 bar-special I/O quick-insert buttons OPEN the grouped wizard PRE-SELECTING the mode (instead of the raw-text caret insert); keep the raw atoms in the Blocks palette.

**BUILT:**
- **Blockly round-trip = AUTOMATIC (no new code) + verified.** registerUserOp auto-registers a USER_SCHEMA spec from def.bindings (userOps.js ~503), so all 10 io_step params (mode/outputRef/state/pin/inputRef/mode2/timeout/var/waitPin/sec) are in the spec → markerLine/parseMarker round-trip them exactly. io_step is a USER op → NOT in FIELD_BIND → no orphan (protocol-validator green). The op surfaces in the Blocks tab as the "I/O Step" block + the Wait Input/Set Output/Dwell atom child with its params (screenshot io_step_blockly.png: the block + the projected `WHILE [#[1520+3]...]`).
- **SETUP-menu rewiring** — commandDeck WIZ_IO_SECTION: the 3 buttons now `openWiz('user_io_step','<mode>')` (output/input/dwell) instead of `ddcsInsertIo`. The VARIANT pre-selects the mode via a NEW `userOpView.applyVariant(variant)` that seeds `_seed = { mode: variant }` (only for ops with a `mode` binding; runs before onShow→render). No user op passed a variant before (opensAs short-circuits it in commandDeck), so this is safe/additive. The raw atoms (outPinBlock/waitInputBlock/dwellBlock) stay in the Blocks palette (Signals/Spindle&Feed — untouched); ddcsInsertIo stays defined (unused by the buttons now, but a public window fn — left in place).

**VERIFY (real app + round-trip):** io-step-roundtrip.spec (NEW) — (a) markerLine→parseMarker round-trips all 3 modes EXACTLY (the spec carries all 10 params); (b) openWiz('user_io_step',<mode>) pre-selects the mode (output/input/dwell) in the in-place form, and the bar's I/O onclicks re-point to openWiz('user_io_step',<mode>). io-step-shots.spec (NEW) — screenshots: io_step_setup_menu.png (the I/O buttons) + io_step_blockly.png (the op as a Blockly block: "I/O Step" + "Wait Input pin 3 mode rise timeout 0 var #5399" + the projected G-code). wizard-bar.spec updated (the I/O onclicks). protocol-validator green (no orphan). **Full suite 810 pass** (1 unrelated flaky retried green).

**FILES:** web/wizards/views/userOpView.js (applyVariant) · web/ui/commandDeck.js (WIZ_IO_SECTION → openWiz) · tests/io-step-roundtrip.spec.js (NEW) · tests/io-step-shots.spec.js (NEW) · tests/wizard-bar.spec.js (I/O onclicks).

**PASSED BACK: I/O-step wizard COMPLETE (E3) — the mode + per-mode params round-trip through the marker codec (markerLine/parseMarker, no FIELD_BIND orphan) and surface in the Blocks tab as the "I/O Step" block; the 3 SETUP-menu I/O buttons OPEN the grouped wizard pre-selecting the matching mode (applyVariant seeds mode); the raw atoms stay in the Blocks palette. Screenshots io_step_setup_menu + io_step_blockly. Full suite 810 pass. This COMPLETES the Setup/IO set (the Comm twin at 048e995 + the grouped I/O-step wizard E1-E3). Ready for the human.**

## 🔨 turn 526 (cycle 223) — I/O-STEP small fix: the Wait-Input EDGE field is DIALECT-AWARE (the human confirmed). DDCS shows High/Low (level); RS274/grblHAL show all 4. Emit unchanged. Full suite 813 pass.

**TASK:** the io_step Input 'Edge' field (mode2: Rise/Fall/High/Low) is meaningless on DDCS Expert (reads a LEVEL — cnc.js waitInput: high/rise→1, fall/low→0). Make it dialect-aware: DDCS (caps.flow!=='oword') → High/Low only; RS274/grblHAL (oword M66 L1-L4) → all 4. Keep the value valid across a post switch (no emit change; only the FORM options narrow).

**BUILT (form-only — no emit-logic change):**
- `ioStepWizard.js` — `ioEdgeOptions()` returns `{ options, alias }` for the active post (via the same resolveActivePost the E1 ioInputSupported/gate use): oword → all 4; else (DDCS level-only) → [High, Low] + `alias {rise:'high', fall:'low'}`.
- `formWidgets.js` — a NEW declarative `SEG_OPTS_PREDS` registry + `segmentedWidget` support for `widgetConfig.optionsBy` (a live `{options, alias}` set, static `options` as fallback). `alias` maps a stored value → a shown one for the HIGHLIGHT ONLY — read() still returns the raw value, so the emit is byte-identical (a hidden `rise` on a DDCS post stays 'rise' and emits level 1 = high, per the existing atom mapping).
- `ioStepData.js` — the mode2 binding gets `widgetConfig.optionsBy:'ioEdge'` (keeping the 4-option static list as the fallback) + a clearer help string.

**WHY alias, not a value rewrite:** the dispatch requires "no emit change; only the FORM options narrow." Rewriting rise→high would change the stored value (and the oword emit L1→L3). The alias narrows the DISPLAY only: on Expert a default 'rise' highlights High (correct — rise≡high there) while read()/emit keep 'rise'. Zero emit change.

**VERIFY:** io-step-edge-dialect.spec (NEW) — on ddcs-expert-m350 the Edge segmented shows exactly [High, Low] (a stored 'rise' highlights High via the alias); on the rs274ngc post (setActivePostId) it shows [Rise, Fall, High, Low] (Rise highlighted). io-step-emit.spec + io-step-twin.spec re-run GREEN (the emit is byte-identical — the fix never touches ioStepStack/the atoms). Screenshots io_step_edge_expert.png (High/Low) + io_step_edge_rs274.png (4). **Full suite 813 pass** (the shared segmentedWidget change is additive — all existing segmented fields, e.g. alignment travel, still green).

**OBSERVED (pre-existing, NOT this fix, flagged for the advisor):** with a POST OVERRIDE set (setActivePostId rs274ngc) the caps-driven FORM (Input-gate + edge-options, both via resolveActivePost) follows the override, but the wizard CODE-PREVIEW emit follows the PROFILE (emitMapped → getDialect(profileId); there is no rs274 profile), so the rs274 screenshot's preview shows the DDCS WHILE-poll. This divergence between resolveActivePost (caps UI) and getDialect (emit) predates this fix and affects all wizards; out of scope here. Normal use (profile == dialect, no override) is fully consistent — the Expert screenshot shows High/Low + the matching WHILE preview.

**FILES:** web/wizards/ioStepWizard.js (ioEdgeOptions) · web/ui/formWidgets.js (SEG_OPTS_PREDS + segmented optionsBy/alias) · web/blocks/dataOps/ioStepData.js (mode2 optionsBy) · tests/io-step-edge-dialect.spec.js (NEW).

**PASSED BACK: the Wait-Input EDGE field is now DIALECT-AWARE — DDCS Expert shows High/Low (level), RS274/grblHAL show all 4 (M66 L1-L4); a stored rise/fall stays valid + emits correctly (alias = display-only, emit byte-identical, confirmed by io-step-emit/twin). Screenshots io_step_edge_expert (High/Low) + io_step_edge_rs274 (4). Full suite 813 pass. FLAGGED a pre-existing resolveActivePost-vs-emit divergence under a post override (not this fix). Ready for the human.**

## 🔨 turn 528 (cycle 223) — UNCLAMP the sim-start handles from the STOCK to the machine ENVELOPE (the human, decisive: the alignment marker must be able to exit the stock contour). Full suite 815 pass.

**TASK (the human's principle):** the STOCK is a REFERENCE, never a hard bound — the only real limit is REACHABILITY = the machine ENVELOPE. The draggable sim-start handles clamped to [0,1] of the stock in two places; free them.

**FIXED (generic — benefits every simStartParams wizard):**
- **(b) alignPoints.js line 13** — dropped the `Math.max(0, Math.min(1, …))` frac clamp. A fraction beyond [0,1] now resolves to a probe point PAST the stock edge (alignPointsXY: 1.4 × 200mm = 280mm). Kept the numeric/empty-default handling (empty → the checkAxis default).
- **(a) userOpView.writeSimStartFrac** — the drag-write now clamps the WORLD to the machine ENVELOPE reach, not the stock. New `machineReachXY()` = the envelope in the WCS/stock frame (stock corner at the WCS origin): `w ∈ [-workOrigin, |span| - workOrigin]` from settings.machine.{x,y}/workOrigin. The FRACTION (world/stock) then resolves beyond [0,1] freely, bounded only by reachability. No envelope configured → unbounded (still never the stock).
- **(c) featureCanvas._fit** — a 15%-of-stock MARGIN band around the stock so a handle dragged just past the edge stays VISIBLE + grabbable (the fit already unions the handles; this guarantees breathing room even at the edge — no clip at the stock bounds).

**VERIFY (real app, the human's exact symptom):** alignment-envelope-unclamp.spec (NEW) — (1) alignPoints allows fx=1.4 / fy=-0.2 (not clamped) + the empty-default still works; (2) a REAL pointer drag of handle A far past the stock's right edge (stock 200×120, TIGHT envelope 220×220) → the recorded params.ax = 1.10 (>1 = EXITED the stock, clamped EXACTLY at the envelope 220/200 = 1.10, not runaway), and opSimStarts marker A followed to x=220mm (past the 200 edge, at the envelope). Screenshot alignment_handle_outside_stock.png: handle A sits OUTSIDE the stock contour, fully visible (the margin), the 3D DRO reads Work X 220, the code preview's AUTO travel follows it. **Full suite 815 pass** — corner/edge/middle/drill/pocket 2D layouts + the corner Start-drag ("0px flicker") all green (the _fit margin + the envelope-bound drag are non-regressive; corner/middle/drill handles use their own sim-only drag path, unaffected, still drag). NOTE: the full-suite run had a transient ATC-test flaky cluster (7 ATC + middle-animator) from parallelism — all 47 pass in isolation + the re-run was clean (815, 0 fail).

**FILES:** web/wizards/ops/alignPoints.js (drop [0,1]) · web/wizards/views/userOpView.js (machineReachXY + envelope-bound drag) · web/viz/featureCanvas.js (_fit stock margin) · tests/alignment-envelope-unclamp.spec.js (NEW).

**PASSED BACK: the sim-start handles UNCLAMP from the stock to the machine ENVELOPE — a real drag past the stock edge now persists a fraction >1 (or <0), the sim marker + the emitted AUTO coord follow, and it stops ONLY at the envelope reach (verified: dragged to ax=1.10, bounded exactly by a 220mm envelope on a 200mm stock; marker at x=220). The handle stays visible via the canvas margin (screenshot alignment_handle_outside_stock: A outside the contour). Generic — every simStartParams wizard inherits it; corner/middle/drill still drag (no regression). Full suite 815 pass. Ready for the human.**

## 🔨 turn 528 (amendment) — ROTARY CLOCK 2-draggable-handles: GATED to the advisor (a real design fork the "mirror alignment exactly" framing doesn't resolve; machine-critical rotary G-code). Unclamp shipped (93b0f05); this part flagged, not implemented.

**AMENDMENT:** add 2 draggable probe-point handles to rotary_clock like alignment got — (1) opSimStarts.rotary_clock → 2 starts (A + B = A + span in Y); (2) rotaryClockData def.simStartParams so both are draggable, marker A → the start, marker B → the second point (dragging B sets the Y span #6). "Mirror the alignment Fork 1 mechanism exactly."

**WHY IT'S A GATE (the models differ — this ISN'T a clean mirror):** alignment's A/B are TWO INDEPENDENT points (ax/ay + bx/by fractions ARE the one source; no separate distance field). Rotary clock's B is NOT independent — it's **A + span**, and the span is **#6, a #var-bound FORM field** that drives the emit (the step `MV('Y','#6')` AND the tilt `ATAN[[#52-#51]]/[#6]` — machine-critical). So "dragging B sets #6" forces a ONE-SOURCE decision the amendment doesn't make:
- **Option 1 (KEEP #6 as the one source; field + drag = two editors):** marker A = clkAx/clkAy (sim-only start), marker B's Y-drag writes `span` = (Bworld.y − Aworld.y) mm. Preserves #6's #var bind + the existing rotary-clock byte-identity tests + the type-a-precise-span field. COST: a small ADDITIVE extension to the generic writeSimStartFrac seam (a "relative-span" marker mode — B writes an mm offset from marker A, not an absolute [0,1] fraction). NOT "exactly alignment" (alignment has no relative marker).
- **Option 2 (LITERAL alignment mirror):** A + B both independent fraction params (clkAx/y + clkBx/y), #6 DERIVED = (clkBy−clkAy)·stock.y in the recompose, the span FIELD removed. This is "exactly alignment" — BUT it (a) REMOVES the type-a-precise-span field (UX regression), and (b) BREAKS the existing rotary-clock tests, which set the span via `params.span` (the field) — they'd need rewriting, and #6 moves from a #var bind to a derived recompose (machine-critical byte-identity risk).

**RECOMMENDATION → Option 1.** It preserves the #6 one-source + the existing byte-identity tests + the precise-type UX, and is the smaller machine-critical change (the emit/#6 binding is untouched; only a new additive drag-write path). Option 2's "exactly alignment" literalness costs the span field + a test/emit reframe on rotary probing G-code — not worth it. The fork is REAL (both legit) + it's the advisor's call on the span field's fate, so I did NOT implement (won't reframe rotary tilt-probe G-code or extend the generic drag seam on an underspecified amendment). Part (1) alone (return 2 static markers) is coupled to (2) + would perturb the existing single-start tests, so it's held with (2).

**PASSED BACK (amendment): ROTARY CLOCK 2-handles GATED — it's NOT a clean "mirror alignment" (rotary's B = A + span #6, a #var-bound machine-critical field, vs alignment's 2 independent fraction points). Making B set #6 forces a one-source call: Option 1 (KEEP the #6 field + a small additive relative-span drag extension — preserves the tests + precise-type UX, RECOMMENDED) vs Option 2 (literal alignment mirror — derives #6, REMOVES the span field, breaks the existing tests). Awaiting your pick before I touch rotary probing G-code. The UNCLAMP task (the turn's primary work) is DONE + committed (93b0f05, full suite 815 pass).**

## 🔨 turn 530 (cycle 223) — ROTARY CLOCK 2 draggable handles (Option 1: KEEP the #6 field + ADD the handles). Drag B sets the span, type #6 for precision — one source. Emit byte-identical. Full suite 816 pass.

**TASK (Option 1, advisor-chosen from my t528 gate):** KEEP the #6 span field (precise machine-critical input); ADD 2 draggable handles — marker A = the start, marker B = A + span, dragging B does a RELATIVE-SPAN write (B.y − A.y → #6). ADDITIVE: the #6 field AND the drag are two editors of the ONE source. Emit byte-identical.

**BUILT:**
- **opSimStarts.rotary_clock → 2 starts [A, B]:** A = the start (default the stock CENTRE, SPAN-INDEPENDENT — see the feedback fix; overridable by clkAx/clkAy fractions, sim-only), B = A + span. inferStart stays `[0]`=A so the built-in single-start path is unperturbed.
- **rotaryClockData:** `def.simStartParams = [{x:'clkAx',y:'clkAy'}, {y:'span', relSpanFrom:0}]` + clkAx/clkAy sim-only defaults. The span marker declares `relSpanFrom:0` — its Y-drag writes a span relative to marker A.
- **userOpView.writeSimStartFrac — a NEW `relSpanFrom` marker mode** (additive to the generic seam): span = (B.worldY − starts[relSpanFrom].worldY), and it writes the FORM FIELD `span` DIRECTLY (not `_simStartFracs`) so the drag + the numeric field are two editors of the ONE #6 (a parallel `_simStartFracs.span` would override a later typed value — the bug I hit). setForm skips seeding a relSpanFrom field-param into `_simStartFracs` (the field seeds it). `starts` is now passed to writeSimStartFrac (the relative reference).
- **THE FEEDBACK FIX (found via the real drag):** A's old default was centre − span/2 (SPAN-COUPLED). Dragging B (span = B − A) then re-rendered A downward each step → span ran away (a moderate drag ballooned to 155mm). A is now the stock CENTRE (span-INDEPENDENT) → A holds while B sets the span. Sim-only (the emit never uses A's absolute pos — the operator jogs there), so emit byte-identical.

**VERIFY (real app + tests):** rotary-clock-handles.spec (NEW) — the 2D layout shows 2 draggable handles (__simstart0/1) + the #6 field STAYS; dragging B UP grew #6 to 81.882 (A HELD at centre y=60, B = A+span = 141.882, emit `#6=81.882` == the field); then typing the #6 field = 15 → span=15 + marker B followed (B−A=15). rotary-clock-sim-starts.spec updated (2 starts: A=centre, B=A+span; inferStart[0]=A parity). rotary-clock-data-emit (byte-diff ZERO, 84 combos) + rotary-clock-superset (E0) + rotary-clock-in-place GREEN — **emit byte-identical**. alignment-fork1/fork2 + alignment-envelope-unclamp GREEN — the shared writeSimStartFrac change is additive (alignment's normal fraction path unaffected). Screenshot rotary_clock_2handles.png (A + B handles + the Y-Span 81.8 field + `#6=81.882`). **Full suite 816 pass.**

**FILES:** web/viz/opSimStarts.js (2 starts, A=centre) · web/wizards/views/userOpView.js (relSpanFrom mode + field-write + setForm guard + pass starts) · web/blocks/dataOps/rotaryClockData.js (clkAx/clkAy + simStartParams) · tests/rotary-clock-sim-starts.spec.js (2 starts) · tests/rotary-clock-handles.spec.js (NEW).

**PASSED BACK: ROTARY CLOCK now has 2 DRAGGABLE handles (Option 1) — drag B to set the #6 span OR type #6 for precision, ONE source (the drag writes the field directly, additive). Marker A = the start (centre, span-INDEPENDENT so no feedback), B = A + span. Verified: drag B → #6=81.882 + emit + marker B follow, A held at centre; type #6=15 → B follows. Emit BYTE-IDENTICAL (A's pos is sim-only; the 84-combo emit + E0 gate stay green). Screenshot rotary_clock_2handles. Full suite 816 pass. Completes the sim-start-handle work (unclamp 93b0f05 + rotary 2-handles).**

## 🔨 turn 532 (cycle 223) — ALIGNMENT handle CAN'T EXIT the perimeter (human's 3rd report): REPRODUCED the VISUAL symptom, found the REAL walls (my 93b0f05 value-unclamp was necessary but NOT sufficient), fixed both. Full suite 817 pass.

**THE LESSON (why 93b0f05 didn't fix it on the human's screen):** t528 verified a PARAM (ax=1.1) via a fake 220mm envelope, never a real canvas drag watching the VISIBLE marker. The value was already unclamped; the walls were in the CANVAS INTERACTION, not the value. [[verify-real-symptom-not-just-test]] — the symptom was VISUAL; the verification must be VISUAL.

**REPRODUCE (step 1, as instructed):** real config (envelope 600, stock 150×100), ACTUAL pointer drag of __simstart0 past the stock edge, measuring the VISIBLE marker's screen pos vs the drawn stock rect (not a param). Found: the marker exits into the canvas's extra space but is limited by the frozen viewBox; and near the stock edge it SNAPS back.

**ROOT CAUSE (step 2, two real walls — checked, not assumed):**
- **(c) THE SNAP** — the sim-start markers are created `kind:'move'` (panelTypes ~341), and featureCanvas snaps `kind==='move'` handles to stock corners/edges (~122, ~12px). So a probe marker dragged toward the edge SNAPS to the perimeter → "can't leave the stock". [dominant for a near-edge placement]
- **(b) THE FROZEN VIEWBOX** — the auto-fit is skipped while dragging (featureCanvas ~192, `!this.active`) to avoid swim, so a handle dragged past the visible content walks OFF the frozen viewBox = invisible/stuck. [dominant for a far drag]

**FIX (step 3):**
- **(c)** sim-start markers get `noSnap:true` (panelTypes) + featureCanvas skips `_snapToAnchor`/`_snapOffsets` for noSnap handles — a probe point is a FREE position, not a feature to seat on the stock.
- **(b)** a NEW `_followHandle()` auto-pans the (drag-frozen) viewBox to keep the dragged handle ≥80px inside the viewport — TRANSLATE only (scale fixed → NO swim), following the handle's ACTUAL clamped position (this.spec.handles, not the raw cursor world — the cursor is unclamped + would run the pan off past the envelope; that was my first-cut bug, hCx=-3600). SCOPED to `noSnap` markers only, so corner/path/ATC drags (exact coords + the anti-flicker "wall holds") are UNTOUCHED.

**VERIFY BY REPRODUCING THE GESTURE (step 4):** alignment-handle-visible-exit.spec (NEW) — a REAL pointer drag of __simstart0 hard right (past + beyond the canvas edge): DURING the drag the marker stays VISIBLE in the canvas (hCx=1264, inside 1344) and reaches ax=4 (the FULL 600/150 envelope reach); AFTER release the VISIBLE marker sits OUTSIDE the drawn stock rect (1264 vs stockRight=462). Screenshot alignment_exit_repro.png — handle A far right of the stock, DRO Work X=600. Regression: corner-start-datum-drag (incl. "0px flicker"), corner-marker-independence, atc-setup-canvas GREEN (the noSnap gate excludes them from the pan); rotary-clock-handles GREEN. **Full suite 817 pass.**

**FILES:** web/wizards/ops/panelTypes.js (sim markers noSnap) · web/viz/featureCanvas.js (noSnap skips snap + _followHandle auto-pan, gated to noSnap) · tests/alignment-handle-visible-exit.spec.js (NEW).

**PASSED BACK (do NOT claim fixed — the human confirms): REPRODUCED the visual symptom (real canvas drag, watching the marker) — my t528 value-unclamp was necessary but not sufficient; the real walls were (c) the sim markers SNAP to stock anchors (kind 'move') → stuck at the perimeter, and (b) the viewBox is FROZEN during a drag → a handle past the visible area walks off-canvas. FIXED both: noSnap on the sim markers + an auto-pan that follows the handle out (scoped to noSnap so corner/ATC/flicker are untouched). Verified by REPRODUCING the gesture: a real drag now moves the VISIBLE marker to ax=4 (full envelope) and it sits OUTSIDE the drawn stock rect on-screen; screenshot alignment_exit_repro. Full suite 817 pass. Please confirm on your screen.**

**AMENDMENT (t532, the human "maybe it's a gui or sim problem" — check BOTH surfaces):** verified 2D AND 3D by reproducing the drag. RESULT — the 2D LAYOUT was the failing surface (the two walls above: snap + frozen viewBox → FIXED). The 3D SIM ruby was ALREADY correct: it renders at the raw world coord from opSimStarts (gcodeViz3d `_positionMarkers`: `s.x − off.x`, NO stock clamp), so with the unclamped fraction it already sits past the stock. VERIFIED in the same test: after dragging to ax=4, the 3D `viz.starts[0].x = 600` AND the ruby `spindleMarkers[0].position.x = 600` — both past the 150mm stock (not render-clamped). So NO 3D fix needed; the only wall was the 2D canvas interaction (now fixed). Both surfaces now show the marker outside the stock.

## 🔨 turn 534 (cycle 223) — HOMING UN-PARK part 1: the home-switch BACKFILL for an EXISTING config + homingEdges reads the DECLARED home end. The human's +Z config now homes UP. Full suite 819 pass.

**THE CONFIRMED GAP (4-agent verified):** the t502 home-switch seed only ran inside `migrateIO`'s `inputs.length===0` block; the human's config already had inputs (probe/setter) → the seed was SKIPPED → NO `<edge>Home` → declaredHomeEdgeSide()=null → axisHomeMotion fell back to the travel-SIGN guess (axisSpan.homeSide) → their +Z envelope homed to machine-0 = the BOTTOM = the plunge. Nothing on disk could set `<edge>Home` for an existing config (syncFlatFromIO clears Home false each sync unless row.home).

**BUILT (2 pieces):**
- **(1) BACKFILL** — `backfillHomeSwitches(s)` in settingsPanel: ONE-TIME, FLAG-GUARDED (`s._ioHomeBackfill`) migration in migrateIO (runs for EXISTING configs too, after the seed block). For each linear axis with NO declared home end: if the axis has a limit row → mark home on it (prefer the default-end row, else the existing one — a fitted switch's end is where the home IS); if NO row → ADD the default row (X x_min / Y y_min / Z z_max/top, home:true, pin UNASSIGNED, mechanical, editable). ≤1 home per axis. Runs ONCE → a later user DELETION stays deleted (never re-add). The flag is carried through loadSettings' merge (`_ioHomeBackfill: p._ioHomeBackfill`) so it persists across loads.
- **(2) homingEdges** (homingView) — the 3D switch DEVICE now reads `declaredHomeEdgeSide(axis, limits)` FIRST (fallback to the sign) so it sits at the DECLARED end the MOTION drives to (kills the motion-vs-device divergence). Also draws a DECLARED home end even when its pin is UNASSIGNED (the backfilled home reference) — the pin gate now only skips a NON-declared, unpinned end.

**VERIFY (the config axis, not a fresh default) — io-home-backfill.spec (NEW):**
- (a) an EXISTING config {probe+setter, no home, machine.z=500} → migrateIO → z_max home row (pin unassigned) + x_min/y_min home + zMaxHome flag + the one-time flag set.
- (a/b) MOTION (axisHomeMotion): with the declared z_max home, Z seeks the TOP for BOTH signs — z=+500 → seek 500, z=-120 → seek 0; contrast: NO declared home → +500 seeks 0 = the OLD plunge.
- (c) delete the backfilled z_max + migrateIO again → NOT re-added (flag).
- (d) an EXISTING z_min row → home marked on IT (prefer-existing), no duplicate z_max.
- (e) homingEdges — device at the DECLARED max end (top) for BOTH signs (z=500→side max,z 500; z=-120→side max,z 0), even unpinned; sanity: no declared home + a fitted z_min pin → falls back to side min (the old divergence).
- REAL APP: pre-seed an existing-config localStorage (probe+setter, +Z, no home) → reload → z_max Home backfilled ON LOAD (zMaxHome=true, flag persisted, z_max row home:true) → open Homing → the preview homes UP (DRO Z climbs to the top) with the switch DEVICE at the top. Screenshot homing_backfill_top.png. **Full suite 819 pass** (1 unrelated flaky retried).

**FILES:** web/ui/settingsPanel.js (backfillHomeSwitches + migrateIO call + the flag carry) · web/wizards/views/homingView.js (import declaredHomeEdgeSide + homingEdges declared-end/pin + export) · tests/io-home-backfill.spec.js (NEW). [[homing-direction-declared-by-envelope-sign]]

**PASSED BACK (do NOT claim fixed — the human confirms): HOMING part 1 done — an EXISTING config now gets its missing per-axis home end BACKFILLED on load (one-time, flag-guarded so a deletion stays; prefer-existing, no duplicates), so declaredHomeEdgeSide drives homing to the DECLARED end (Z → the TOP, both signs) instead of the travel-sign plunge; and homingEdges draws the switch device at that declared end (even unpinned). Verified against the human's config shape (probe+setter, +Z=500): reload → z_max Home backfilled → homing preview homes UP with the device at the top (screenshot homing_backfill_top). Full suite 819 pass. Please confirm on your machine's config.**

## 🔨 turn 536 (cycle 223) — HOMING part 2, the EMIT side: wizard = G31-ONLY (ignore the saved method) · a SIMPLE readable per-axis block (kill the O501 soup) · drop the c.dir override. Full suite 821 pass.

**TASK (3 emit changes, all decisions locked):**
- **(1) WIZARD = G31 ONLY** — homeAxis resolved `c.method || 'seek'`; the human's SAVED `method:'native'` survived the merge → the wizard emitted M98. FIX: the wizard IGNORES the saved per-axis method — a LINEAR axis ALWAYS emits G31; a rotary A/B emits setzero (the sensible method). Native M98 stays reachable ONLY via the separate Homing Setup (its machinery is KEPT, just not wizard-routed). The human asked 4×: "wizard is g31 only".
- **(2) SIMPLE READABLE EMIT** — replaced the O501 re-derivation (the 7-read "debounce" that summed the SAME input 7× and debounced nothing; the limit-vs-home GOTO guard; the feed-halving multi-pass loop — "wtf is this code") with ~7 plain lines per axis: `G91` → fast `G31 <span+margin> F<seekFeed> P#<port> L#<level>` toward the declared home end → `G01` back off → ONE slow `G31` re-touch → `#[880+N]=0` datum + `#[1515+N]=1` homed flag → `G01` clearance → `G90`. Seek dist = the travel span + 20 (NOT ±10000). Feeds are plain literals (c.seekFeed / c.slowFeed; F is modal so it prints once). Kept the `#655` soft-limit re-enable tail.
- **(3) DROP c.dir** — the seek direction is the DECLARED home end ONLY (declaredHomeEdgeSide; fallback -tSign; unknown envelope → +1). No c.dir override → a stale saved dir can no longer steer the seek away (the t491 bug class). dir still rides the config Struct (stored/round-trips), just not read by the emit.

**VERIFY (the config axis) — homing-simple-emit.spec (NEW):** a SAVED `method:'native'` config + backfilled zMaxHome + machine.z=+500 → the wizard emits the SHORT G31 block (G31, no M98P501, no debounce, no GOTO, no ±10000, no #102; the ordered lines G91 / fast G31 Z520 F800 P#1051 L#1053 / G01 Z-5 F100 / slow G31 Z7 / #882=0 / #1517=1 / G01 clearance / G90; <15 lines/axis). Both signs seek UP toward the declared top. REAL-APP code preview (saved-native config) shows the G31 block, no toggle — screenshot homing_simple_g31.png (+ the 3D homes UP). UPDATED the emit-pinning tests to the NEW shape (intended change): homing-g31-output (new shape, no #102/#612/debounce/GOTO), homing-declared-home (G31 Z positive both signs), homing-declared-direction + homing-dir-override (dir IGNORED → emit identical; method:native on a linear axis → G31 not M98), homing-limit-trip + homing-switch-devices (byte-identity → G31 not M98). **Full suite 821 pass** (1 unrelated flaky).

**FILES:** web/wizards/homingWizard.js (import r3; method → linear-seek/rotary-setzero; the simple seek block; dropped c.dir + the O501 machinery) · tests: homing-simple-emit.spec.js (NEW) + homing-g31-output / homing-declared-home / homing-declared-direction / homing-dir-override / homing-limit-trip / homing-switch-devices (re-pinned to the new shape). [[homing-direction-declared-by-envelope-sign]]

**PASSED BACK (do NOT claim fixed — the human reviews the emitted code): HOMING part 2 done — the WIZARD is now G31-ONLY (a saved method:'native' is IGNORED → the code preview shows the simple G31, no toggle; native M98 stays a Setup option, machinery kept); the per-axis emit is ~7 READABLE lines (fast G31 to the declared home end → back off → ONE slow re-touch → datum + homed flag → clearance → G90) — no debounce vote, no GOTO soup, no ±10000; and the c.dir override is DROPPED (the declared home end is the only seek source). Both Z signs seek UP toward the declared top; the preview homes UP (simProxy unchanged). Screenshot homing_simple_g31 (the short block per axis). Emit-pins updated to the new shape; full suite 821 pass. Please review the emitted homing code.**

## 🔨 turn 538 (cycle 223) — SMALL UI CLEANUP ×3 (disjoint, one pass): homing stale labels · SETUP-menu I/O consolidation · I/O-step dead fields on Expert. Emit byte-identical (labels/visibility only). Full suite 823 pass.

**(1) HOMING stale labels** — the run-row chips rendered METHOD_LABEL[c.method] → a saved 'native' showed as the chip; the header hint still described per-axis method routing ('native uses M98 instead'). But the wizard is now G31-ONLY (b0a9791). FIX: homingView chip reads the EFFECTIVE wizard method (linear → 'switch-seek', rotary → 'set-zero'), not the saved c.method; index.html hint rewritten — "the wizard emits explicit G31 switch-seek homing (feed/port/level visible), each axis seeks its declared home switch; Native M98 homing lives in Homing Setup." The screen now SAYS what the wizard DOES.

**(2) SETUP-menu I/O consolidation** — the 3 pre-select bar buttons (Set Output / Wait Input / Dwell) collapsed into the SINGLE grouped 'I/O Step' entry (mode picked inside). commandDeck WIZ_IO_SECTION is now EMPTY; the single door is the existing wizardLibrary built-in `io_step` entry (opensAs user_io_step, already in the Setup dropdown). The raw outpin/waitinput/dwell atoms stay in the Blocks palette. (wizard-bar.spec updated: the Setup group = comm / io_step / atc_warmup, no 'I/O' divider, no 3 bar buttons.)

**(3) I/O-step DEAD FIELDS on Expert** — the Input mode still showed 'Result var (#5399)' (M66 result — writes NOTHING on the Expert WHILE-poll) + 'Timeout (ms)' (M66 Q — ignored). Both are RS274/grblHAL-ONLY. FIX (dialect-aware, like the Edge field 1812e1b): a generic `_oword` gate flag — userOpView injects `_oword` (the active post is o-word/M66) into the row-gate params; the var/timeout bindings gate `whenAll: [{mode:input},{_oword:true}]`. Shown on RS274/grblHAL, HIDDEN on a DDCS post. Visibility-only — the hidden fields still read (the widget renders+reads when hidden), so the emit is byte-identical.

**VERIFY — homing-io-ui-cleanup.spec (NEW):** (1) with a saved method:'native' config the chips read 'switch-seek' (x/y/z) — NO 'native' anywhere; screenshot homing_no_stale_native.png (chips + the rewritten hint + the G31 preview). (3) on ddcs-expert-m350 the Input mode shows Input + Edge but HIDES Result var + Timeout; on rs274ngc all four show; the DDCS emit is still the WHILE-poll `#[1520+4]` (unchanged); screenshots io_step_input_expert.png (no var/timeout) + io_step_input_rs274.png. wizard-bar.spec updated + green (change 2). **Full suite 823 pass** (1 unrelated flaky; a transient corner cluster confirmed clean in isolation, 12/12).

**FILES:** web/wizards/views/homingView.js (chip = effective method) · web/index.html (homing hint) · web/ui/commandDeck.js (WIZ_IO_SECTION empty) · web/wizards/views/userOpView.js (_oword gate flag + dialect imports) · web/blocks/dataOps/ioStepData.js (var/timeout whenAll _oword) · tests/wizard-bar.spec.js + tests/homing-io-ui-cleanup.spec.js (NEW).

**PASSED BACK: UI cleanup ×3 done — (1) the homing form no longer shows stale 'native' (chips read switch-seek/set-zero; the hint says the wizard emits G31, native lives in Setup); (2) the SETUP menu is ONE 'I/O Step' entry that opens the grouped wizard (the 3 pre-select bar buttons removed; raw atoms stay in Blocks); (3) on a DDCS post the I/O-step Input mode hides the dead Result-var + Timeout (M66-only), shows them on RS274/grblHAL — a generic `_oword` dialect gate. All labels/visibility only — emit byte-identical; full suite 823 pass. Screenshots homing_no_stale_native + io_step_input_expert/rs274.**

**AMENDMENTS (t538, both incorporated):**
- **(A1) REMOVE the 3D switch-device meshes** (the human circled the floating grey box in the homing preview). The H4 device render was homingView → ctx.previewLimitSwitches → gcodeViz3d.setLimitSwitchDevices. REMOVED the homingView caller (the only reachable path); `homingEdges` (the device-placement fn) + its `axisSpan`/`declaredHomeEdgeSide`/`switchStandoff` imports are now orphaned → removed; setLimitSwitchDevices is dormant (uninvoked, intact). KEPT layer 1 (the declared Home rows — homing direction still reads them via axisHomeMotion) + layer 2 (the trip model + the I/O-panel lights — a SEPARATE `setLimitSwitches` in the engine, untouched). Deleted the H4 device specs (tests/homing-switch-devices.spec.js) + the io-home-backfill (e) device-placement test. VERIFY: the homing preview draws NO device meshes (viz._limitGroup null, 0 devices) — screenshot homing_no_device_meshes.png (the floating box is gone).
- **(A2) AUDIT every homing emit door → all route through the NEW simple G31** (the human, live: "still emitting m98 or g31 with weird calls"). Grepped every homing-emit caller: the WIZARD (homingView → HomingWizard.generate), the Blocks/op re-open (opBuilders `homing: homingStack`), and the macrosApp Homing Setup + sysstart deploy (all `emitMapped(homingStack(h))`) — ALL call the ONE homingStack (b0a9791). No duplicate/cached/stale emit path exists; homingStack NEVER emits M98 now (method forced seek/setzero). The human's M98 was a pre-b0a9791 (stale) view. VERIFY: for a saved method:'native' config both homingStack (wizard/macrosApp/sysstart) AND builderOf('homing') (Blocks/op) emit the SIMPLE G31 — no M98P501, no debounce/GOTO, no ±10000 — and the op door == the shared emit (byte-identical, one source). Full suite 819 pass.
- **(A3) DEV BUILD STAMP** (systemic — ends the "is my page stale?" disputes, 3rd occurrence). app.js boot HEAD-fetches the served app.js's Last-Modified (no-store) → `window.__ddcsBuild` + a boot console log + the `.ver` chip TOOLTIP ("app.js build: <mtime> — served mtime; if older than your last edit, the tree is stale, hard-reload"). Dev-cheap (one HEAD, no build pipeline); silent no-op when packaged / no server. Tracks the ENTRY module (app.js) mtime — the honest floor without a build step. VERIFY: window.__ddcsBuild is the iso-ish served mtime + the chip tooltip shows it (edit app.js → reload → it changes). **Full suite 820 pass.**

**FILES (amendments):** web/wizards/views/homingView.js (remove the previewLimitSwitches call + the orphaned homingEdges + its imports) · web/viz/gcodeViz3d.js (setLimitSwitchDevices now dormant/uninvoked) · web/app.js (the dev build-stamp IIFE) · tests/homing-switch-devices.spec.js (DELETED — H4 device specs) · tests/io-home-backfill.spec.js (removed the (e) device test) · tests/homing-io-ui-cleanup.spec.js (added the device-removal + all-doors + build-stamp checks).

**PASSED BACK (amendments): (A1) the 3D switch-device MESHES are REMOVED from the homing preview (the floating grey box is gone — screenshot homing_no_device_meshes); homing direction still reads the declared Home rows, the trip model + I/O-panel lights are kept. (A2) AUDITED every homing emit door — wizard, macrosApp Homing Setup, sysstart deploy, Blocks/op — ALL route through the ONE homingStack (b0a9791), which NEVER emits M98 now; verified each emits the simple G31 (no M98/O501) for a saved method:'native'; no stale/duplicate path (the human's M98 was a pre-b0a9791 view). (A3) a DEV BUILD STAMP (window.__ddcsBuild = served app.js mtime, in the .ver chip tooltip) so "what am I running" is a visible fact. Full suite 820 pass.**

## 🔨 turn 540 (cycle 223) — HOMING PREVIEW package (the human's 3 asks) + close the STALL loop: (0) stall verify + the G31 clamp (with stock) · (1) draggable machine-frame START marker the sim runs FROM · (2) FORCE the envelope box · (3) NO silent travel fallbacks. + amendment (machine-envelope travel fields → a clean column). Full suite 829 pass.

**(0) STALL VERIFY + THE G31 CLAMP (with a stock shown)** — the human's line-8 "stall" was the OLD O501 GOTO wall (gone since b0a9791). PROVED the new simple-G31 emit PLAYS TO COMPLETION: homing-play-complete.spec (NEW) traces the emitted macro through the engine, BOTH Z signs, WITH + WITHOUT a stock — the trace RETURNS (an infinite loop would hang the test) with a finite end pos, and z=+500 ends <=505 (the top-backoff), not overshooting. The clamp fix: the homing-seek envelope clamp (t499) was gated `!this.stock`, so a homing G31 PLAYED inside a program with a stock shown wasn't clamped -> it ran the full seek past the switch. DROPPED `!this.stock` so the clamp fires whenever the STOCK-trip did not (tt==null). **REGRESSION caught by the full suite (boss-probe-collision) + ROOT-CAUSED:** the un-gated clamp also caught a TOUCH-PROBE that legitimately MISSES the stock (a boss-both first-pass miss must run the FULL ~130 -> the macro's error branch) -> it clamped that miss to ~0. FIX: gate the clamp on the PORT — `isTouchProbe = probePort === probes.probePin || probes.setterPin` -> the clamp fires only for a HOME/LIMIT seek (the homing G31's port = a limit register like #1051), NOT the touch probe (P#5 -> probePin=3). Verified: boss-probe runs full again AND homing clamps to the switch, both with a stock.

**(1) DRAGGABLE machine-frame START marker — the sim runs FROM it** — a sim-only Start (default MID-ENVELOPE), draggable anywhere in the envelope, the homing tool travels Start->switch, persists across updates, NEVER emitted. **The architecture tension:** the probe wizards' draggable-start works because they are INCREMENTAL (stats.absolute=false -> curAnchor=true -> the route emanates from the marker); homing is ABSOLUTE G53 (curAnchor=false -> the marker is independent, the route does not follow). Also the 3D route is normalized-from-0-deltas then positioned at `off` (0 for absolute), while the live tool rides engine.pos. **Chosen (least-invasive, no touch to the shared anchor/frame coupling — which has stock-datum `_probeXYOff` risk):** seed the engine's INITIAL pos from the Start (new `initialPos` ctor arg, DEFAULT null -> byte-identical for every existing sim; resetState seeds it), threaded via trace.js + createPreviewPanel (gated `machineFrameTool`, homing-only) for BOTH the trace and the play engine -> the absolute route's deltas compute relative to the Start, and a drag re-traces live through the existing `st = getStartPos()` chain (no regeneration hook). Then anchor the STATIC route's `off` to the Start IN THE MACHINE FRAME (gcodeViz3d, gated `_toolMachineFrame`: `rawOff = starts[p]`, minus the part-frame shift like the live tool ~L588) so the drawn seek EMANATES from the Start and coincides with the animated tool — WITHOUT flipping _anchorToStart (envelope stays). Seated the PRE-PLAY tool at the Start too (createPreviewPanel, machineFrameTool). homingView passes a stable mid-envelope default (machine spans /2); the panel's own curStart persists a drag. **VERIFY (homing-start-marker.spec NEW):** the Start seeds mid-envelope (300/200/250 for 600/400/500); the played tool STARTS at the mid Start (Z~250) and homes to the top switch (~495); a drag persists + the next run begins there (Z=60); the emit never contains the Start. Screenshots homing_start_midenvelope (tool rests at the mid Start, route up to the switch, envelope box) + homing_start_homed (tool at the top, route Start->switch).

**(2) FORCE the envelope BOX in the machine-frame homing pin** — homing motion IS machine-frame, so the box is its reference regardless of settings.machine.show (the `show` toggle only governs ordinary part-frame views). gcodeViz3d `setForceMachineBox(on)` -> `_forceMachineBox`; the box gate reads `machine.show || _forceMachineBox`; wired createPreviewPanel.setForceMachine -> viz.setForceMachineBox, reached via homingView's existing previewMachine(true). VERIFY (homing-preview-package.spec NEW): with show OFF the envelope box still draws (viz.machineBox truthy, _forceMachineBox true). Screenshot homing_box_forced_showoff.

**(3) NO SILENT TRAVEL FALLBACKS** — an unset/0 axis travel used to simulate a fictional -120 Z (homingSimProxy `num(machine.z,-120)`). Now it is SURFACED, not faked: homingSimProxy SKIPS an unset-travel axis (a visible `( SET Z TRAVEL ... )` comment, no fabricated G53 move); the emit skips it (`Home Z — SET Z TRAVEL ... first; homing SKIPPED`); `homingUnsetAxes(params)` (NEW export) lists them; homingView appends a `WARN Set Z travel in Machine settings — that axis is skipped (no envelope)` hint to both status lines. Configured axes (span>0) unaffected. VERIFY (homing-preview-package.spec): Z travel=0 -> the hint shows in both statuses + the code comment, the emit skips Z but still homes X/Y, the proxy has no `G53 G0 Z`. Screenshot homing_unset_travel_hint.

**AMENDMENT (t540, folded in — the earlier homing-2D-layout amendment was RETRACTED, "it can wait"):** the Settings->Machine envelope TRAVEL fields floated at the 3D box corners and OVERLAPPED (the human's repro: -120 stacked on 300; supersedes the corner-offset attempt 7458548). Replaced with a CLEAN COLUMN beside the box: the three inputs stacked vertically, each axis label COLOR-MATCHED (X #e0564f / Y #5fbf6a / Z #5a8fe0), no floating overlays. `.mach-col-field` (static, overrides the shared `.dim-edit` absolute); the Machine-tab HTML is a flex row [200x200 box cell][labeled column]; renderMachineGui W 260->200 (square, fits the cell), pad 46->26 (no field margin needed), the per-field `place()` positioning REMOVED. The box still redraws live as you type (listeners are by ID, unchanged). VERIFY (machine-envelope-travel-column.spec NEW, replaces machine-envelope-field-offset): the three fields are one aligned vertical column, no overlap at 600/-600/-120, the human's 756/-776/-150, and a thin 1200/-40/-600; every field is right of the box; typing redraws the box. Screenshots machine_travel_column + machine_travel_column_756.

**TEST UPDATES (intended — the mid-Start changes the sim START, not the logic):** homing-g31-output + homing-preview-machine-frame asserted the tool starts PINNED at the top (`wMin >= -8` / `ezMax===0` / `ezMin===-5`). The tool now STARTS at the mid Start and travels UP, legitimately occupying mid-Z. Re-expressed the t497 "no plunge" guard as the real property — the RENDERED tool TRACKS engine.pos.z (machine frame): `|wMin - ezMin| < 2/3` (a stock-floor plunge decouples them by ~95mm) + the settled state (ezMin=the mid Start, ezLast=-5 backoff; the switch-touch 0 is a transient the 110ms sampling skips).

**FILES:** web/engine/GcodeExecutionEngine.js (initialPos ctor+resetState; the clamp: drop !this.stock + the isTouchProbe port-gate) · web/engine/trace.js (thread initialPos) · web/viz/createPreviewPanel.js (setForceMachine->setForceMachineBox; initialPos for the trace+play, machineFrameTool; seat the pre-play tool at the Start) · web/viz/gcodeViz3d.js (setForceMachineBox; the machine-frame route `off` anchors to the Start minus the part shift) · web/wizards/homingWizard.js (homingSimProxy skips unset-travel axes; homingUnsetAxes export; the emit span-skip) · web/wizards/views/homingView.js (mid-envelope Start passed to preview3D; the unset-travel hint) · web/ui/settingsPanel.js (the travel-field column: CSS .mach-col-field + Machine-tab HTML + renderMachineGui W/pad/no-place) · tests: homing-play-complete.spec.js + homing-start-marker.spec.js + homing-preview-package.spec.js + machine-envelope-travel-column.spec.js (NEW) · homing-g31-output + homing-preview-machine-frame (re-expressed the tracking guard) · machine-envelope-field-offset.spec.js (DELETED — superseded). **Full suite 829 pass** (2 skipped; blocks-live-form + middle-animator flaked under parallelism, both green in isolation). [[g53-move-breaks-preview-start-anchor]] [[machine-frame-sim-spec]] [[verify-real-symptom-not-just-test]]

**PASSED BACK (do NOT claim fixed — the human confirms on their machine): HOMING PREVIEW package done. (0) The simple-G31 emit PLAYS TO M30 (no freeze) both signs, stock shown+hidden, tool at the top-backoff; the seek clamp now applies WITH a stock — port-gated so a touch-probe MISS still runs full (fixed a boss-probe regression). (1) A draggable machine-frame START marker (default mid-envelope) — the tool travels Start->switch, drag it anywhere and the sim re-runs from there (engine initialPos + a machine-frame route anchor; the 3D marker drag is the source; NOT emitted). (2) The envelope BOX is FORCED in the homing pin even with show OFF. (3) An unset axis travel is SURFACED ("WARN Set Z travel...", the axis skipped) instead of a fictional -120 span. + amendment: the Settings->Machine travel fields are now a clean color-matched X/Y/Z column beside the box (no more overlap; supersedes 7458548). Screenshots: homing_start_midenvelope/_homed, homing_box_forced_showoff, homing_unset_travel_hint, machine_travel_column(_756). Full suite 829 pass. Please play homing on your config + confirm the Start drag + the travel-field column.**

## 🔨 turn 542 (cycle 223) — KILL THE HOMING SIM PROXY: the wizard preview plays the REAL emitted G-code through the engine, the SAME execution the editor does (the human: "the homing wizard sim should match the editor sim"). One simulator, one truth. Full suite 831 pass.

**WHY** — homingSimProxy (a hand-made G53 motion model) existed ONLY because the pre-b0a9791 M98 emit wasn't engine-runnable. The wizard is G31-only now and t540 PROVED the emit plays to M30 with the port-gated seek stop. So the proxy was a DUPLICATE simulator that could DIVERGE from the editor (the human's line-8 "stall" class of bug — a proxy-vs-real mismatch). This is the north-star move: declare-never-infer / one-source — the emit IS the source; the preview plays it, not a re-derivation.

**THE CHANGE (web):**
- **homingView** — the 3D preview now feeds `ctx.preview3D(gcode, …)` the REAL emitted code (the same `gcode = wizard.generate(params)` already computed for the code panel + inserted into the editor), NOT `wizard.simProxy(params)`. The machine-frame pin (previewMachine/previewToolMachineFrame) + the t540 Start seeding (initialPos) all stay — the Start drag re-runs the REAL code from the new start.
- **homingWizard** — DELETED `homingSimProxy` (the whole hand-made G53 function), the `simProxy()` method, and `inferStart()`. Removed the now-orphaned `axisHomeMotion` import (only the proxy used it; the emit uses declaredHomeEdgeSide directly). Kept `homingUnsetAxes` + `homingStack` + `generate`.
- **GcodeExecutionEngine** — the M98 P501 native-home handler STAYS (Setup-inserted native code played in the editor still needs it). Only updated its comments that named the deleted homingSimProxy (no dead references).

**VERIFY (homing-preview-plays-real-emit.spec, NEW — the human's config: declared homes, z=+500 AND -120, stock shown):**
- (a) the preview plays the REAL emit: host.__gcode has G31 + is NOT the old "SIMULATION PROXY" text.
- (b) SAME MOTION as the editor: the wizard preview played to completion AND a fresh GcodeExecutionEngine (the editor's gcode-preview panel runs exactly this) tracing the SAME code from the SAME start settle at the SAME top-backoff (z=+500 → 495; z=-120 → -5). NOTE: seeded the editor trace with the same Start (initialPos) as the preview — from a DIFFERENT start the incremental seek can trip on the stock instead of the switch; that's a start-position difference, not a simulator one (the point of killing the proxy is ONE engine → identical inputs give identical output).
- (c) the preview LINE-HIGHLIGHTS the real emitted lines during play — polled a `.g-line.active-line` whose text is a G31; screenshot homing_preview_highlights_g31 (the code panel highlights `G31 Z520 F600 P#1051 L#1053` mid-run + the live G31 status + the tool traveling from the mid Start to the top switch).
- The Start drag still re-runs correctly (homing-start-marker green — it plays the real emit now).

**PROXY-TEST CLEANUP (removed/updated — the proxy is deleted; every home-end assertion re-pointed to the ONE source, axisHomeMotion + the emitted G31 + the M98 engine handler):**
- homing-declared-home: dropped the `homingSimProxy` proxySeek block; the declared-edge target is asserted via axisHomeMotion + the g31 emit (title updated).
- homing-declared-direction (3 tests): the "regardless of dir" proof is now axisHomeMotion (no dir input) + the M98 engine handler (real execution); the DRIVE-APP test PLAYS the real emit (simSpeed + wait-to-settle) and asserts the played engine homes Z to the TOP (~-5), not the -120 plunge.
- homing-dir-override: removed the `simEnd` proxy helper + its assertions (the emit's G31 direction is the home-end truth; kept emit/native/round-trip).
- homing-drive: the home-end each axis drives to is axisHomeMotion (was the proxy's G53 seek line).
- homing-g31-output + homing-preview-machine-frame: the preview-playing tests now play the REAL emit (slower — 3 axes × slow F100 re-touch ~12s vs the proxy's ~1s), so added simSpeed (20 for the 22×110ms samplers, 60 + a 20s wait for the play-to-completion ones). The t497 no-plunge guard is unchanged (worldZ tracks engine.pos).
- homing-preview-package (3): dropped the `homingSimProxy` check; the "no fictional span" intent is the EMIT skipping Z (a visible SET-TRAVEL comment, no G31 Z) — which is what actually plays now.
- homing-engine: comment-only (the M98 handler stays; the preview no longer uses a proxy).

**FILES:** web/wizards/views/homingView.js (play gcode, not simProxy) · web/wizards/homingWizard.js (delete homingSimProxy + simProxy + inferStart + the orphan import) · web/engine/GcodeExecutionEngine.js (M98-handler comments, no dead proxy refs) · tests: homing-preview-plays-real-emit.spec.js (NEW) · homing-declared-home / homing-declared-direction / homing-dir-override / homing-drive / homing-engine / homing-g31-output / homing-preview-machine-frame / homing-preview-package (proxy assertions removed / re-pointed to axisHomeMotion / real-emit playback + simSpeed). **Full suite 831 pass** (1 known middle-animator flaky, green on retry; 2 skipped). [[verify-real-symptom-not-just-test]] [[machine-frame-sim-spec]]

**PASSED BACK (do NOT claim fixed — the human confirms): the homing wizard preview now PLAYS THE REAL EMITTED G-CODE through the same engine the editor uses — the hand-made homingSimProxy is DELETED. One simulator, one truth: a preview-vs-editor divergence (the line-8 stall class) is impossible. Verified (z=+500 AND -120, stock shown): the wizard preview + the editor engine playing the same code from the same start settle at the SAME top-backoff (495 / -5); the preview line-highlights the real G31 lines as they execute (screenshot homing_preview_highlights_g31); the Start drag re-runs the real code. The native M98 engine handler stays for Setup-inserted native code played in the editor. All proxy tests removed/re-pointed to axisHomeMotion (the one source). Full suite 831 pass. Please play homing in the wizard AND insert+play it in the editor on your config — confirm they match.**

## 🔨 turn 544 (cycle 223) — ALIGNMENT AUTO REVISION (the human's redefinition, design-locked t539): the declared value becomes an A→B SPAN scalar; AUTO probes A IN PLACE + does ONE relative checkAxis jog of the span to B; handle A = a sim-only START anchor; AUTO no longer needs a stock. Full suite 832 pass.

**WHY** — the OLD AUTO rapided to TWO absolute stock-frame points (ax/ay/bx/by fractions × stock), needed a stock/datum, and greyed without one. The human's design: point A = WHEREVER the machine is when the program runs (no travel, no Confirm — the corner/edge contract: position first, run, it probes); point B = the ONLY auto-jog (lift safe-Z, step the DECLARED SPAN along the fence as a RELATIVE move, descend, probe). So the declared geometry is a SCALAR span (plain mm) + a sim-only A anchor — NOT two coords.

**THE CASCADE (all three):**
- **(1) the declared value = the A→B SPAN (mm along checkAxis)** — alignPoints.js redefined: `alignAnchor` (A's sim-only fraction {fx,fy}), `alignSpan` (params.span, signed), `alignMarkersXY` (A = frac×stock, B = A + span along checkAxis). opSimStarts.alignment = A anchor + B=A+span. Handle B's drag → the SPAN field (the rotary-clock relSpanFrom pattern: B−A → the field; signed; ALONG checkAxis). Added a `span` FORM FIELD so type-or-drag both edit the one source. Handle A = the sim/preview START anchor (ax/ay fractions, sim-only — NEVER emitted; the preview starts the tool AT A).
- **(2) AUTO no longer needs a stock** — `alignEffectiveTravel(params)` dropped the stock check (auto default | manual); removed the gateAuto stock-greying from the twin's travel binding + the deriveGuards stock read. The span is plain mm.
- **(3) the twin SIMPLIFIES** — DELETED applyAlignAutoTravel (the stock-dependent absolute-coord travel writer); AUTO now binds ONE span scalar via a `#73` value binding (optional — present only in the AUTO arm, so a MANUAL build has no #73 → byte-identical). simStartParams = [{x:ax,y:ay}, {y:span, relSpanFrom:0, spanAxisFrom:checkAxis, signed:true}].

**THE EMIT (alignmentStack):** AUTO point A = `#73=span` + a "probe point A in place" comment (NO travel, NO Confirm) → RM #70 → probe A. AUTO point B = a `REPOSITION: auto-traverse the declared span (#73) along the <axis> fence` comment (so the sim counts 2 passes + colours B's marker auto) → G91 / `G0 <checkAxis>#73` (the relative span jog) / G90 / RM #71 / #72=[#71-#70] / descend #20 → probe B. MANUAL = unchanged (jog + Confirm both points), byte-identical.

**A GENERIC EXTENSION (userOpView.writeSimStartFrac):** the relSpanFrom marker gained `spanAxisFrom` (read the checkAxis LIVE from the segmented form widget — via the `.seg-on` button, not `.value`) + `signed` (alignment: B either side of A). Rotary-clock's default (Y axis, floored ≥1mm) is unchanged (rotary-clock-handles green).

**VERIFY (values + real app):** the AUTO emit = probe A in place + ONE relative checkAxis jog of exactly the span + descend + probe B (screenshot alignment_auto_emit); drag handle B → the span field + emit updates (fork1); type the span 30→90 → marker B moves +60mm (alignment-span-verify); A drag moves ONLY the sim marker — emit unchanged (fork2 aAnchorSimOnly); AUTO no longer greys with no stock (fork2 real-app); twin==revised alignmentStack byte-diff ZERO studio+Expert (fork2 + E1); MANUAL byte-identical (E1/E3 UNCHANGED, still green); E0 prune==concrete across checkAxis×probeDir×travel. Screenshots: alignment_span_handles_form + alignment_fork1_handles (the 2 handles A/B + the span field), alignment_travel_no_gate (AUTO not greyed), alignment_auto_emit (the emit).

**TEST REWORK (old two-coord model → span):** alignment-onesource (opSimStarts A+span, AUTO probe-in-place+jog, AUTO-needs-no-stock, span round-trip), alignment-fork2-twin-auto (twin span byte-identity + span drives #73 + A sim-only; the greying test → AUTO-not-greyed), alignment-fork1-handles (A drag→ax/ay, B drag→span), alignment-superset (sweep checkAxis×probeDir×travel, prune with the effective travel), alignment-envelope-unclamp (alignAnchor/alignMarkersXY API), sim-starts-data (the alignment rows example = A+span), alignment-span-verify (NEW — type-span→B + the AUTO emit). engine-trace (align 2-passes) now passes via the REPOSITION comment (no test edit). alignment-data-emit (E1/E2) + alignment-in-place (E3) UNCHANGED + green (MANUAL byte-identity + the twin sweep hold for the new default).

**FILES:** web/wizards/ops/alignPoints.js (anchor+span model, drop the stock gate) · web/wizards/alignmentWizard.js (probe A in place + the span jog; #73) · web/viz/opSimStarts.js (A anchor + B=A+span) · web/wizards/views/userOpView.js (relSpanFrom spanAxisFrom+signed, segmented read) · web/blocks/dataOps/alignmentData.js (span binding #73 optional, delete applyAlignAutoTravel, no gateAuto, simStartParams span marker, deriveGuards no-stock) · tests: alignment-span-verify.spec.js (NEW) · alignment-onesource / -fork1-handles / -fork2-twin-auto / -superset / -envelope-unclamp / sim-starts-data (reworked to the span model). **Full suite 832 pass** (1 known middle-animator flaky; 2 skipped). [[alignment-one-source-probe-points]] [[spatial-gui-form-vs-canvas]] [[verify-real-symptom-not-just-test]]

**PASSED BACK (do NOT claim fixed — the human confirms): ALIGNMENT AUTO REVISION done (all 3 cascade parts). The declared value is now an A→B SPAN scalar (plain mm, signed along checkAxis) — NOT two absolute coords. AUTO probes point A WHERE THE MACHINE IS (no travel, no Confirm — the corner/edge contract) then does the ONLY jog: lift safe-Z, step the span relative along the fence, descend, probe B. Handle A = a sim-only START anchor (never emitted; the preview starts the tool at A); handle B's drag / typing the span both set the one span field. AUTO no longer needs a stock (removed the stock gating + the greyed-AUTO UI). The twin simplified — applyAlignAutoTravel DELETED, AUTO binds one #73 span scalar. MANUAL byte-identical (E1/E3 green). Verified values + real app; screenshots alignment_auto_emit (probe-A-in-place + the span jog), alignment_span_handles_form / alignment_fork1_handles (the 2 handles + span field), alignment_travel_no_gate. Full suite 832 pass. Please open Align, drag B / type the span, and run AUTO on your machine.**

## 🔨 turn 546 (cycle 223) — HOMING PORT increment E0: homingStack SUPERSET (per-axis GUARDED arms) + the prune==concrete gate. The first of the decomposed user_homing_data port (E0 → twin → in-place → 2D layout, one increment per pass-back). Full suite 835 pass.

**INCREMENT PLAN (I picked the cuts, like comm/alignment):**
- **E0 (THIS turn):** homingStack superset — every axis's home block GUARDED by its run-tick; prune==concrete byte-identical across the axis-SELECTION sweep.
- E1 (next): blocks/dataOps/homingData.js twin — DEFAULTS mirror the run-form; scalar bindings (#var identity: feeds/backoff); the SETTINGS-dependent recompose (seek dist = machine span + margin, direction = declared <edge>Home) via deriveGuards + postInstantiate reading settings.machine/limits (the alignment-fork2 precedent, simpler post-t544); the run-ORDER unroll (reorder the pruned arms into the run-order — the comm dynamic-shape precedent).
- E2 (next): opensAs 'user_homing_data' + OPENS_AS_TARGETS + registerUserOp + FORCE_MACHINE; homing opens IN-PLACE keeping ALL the t540-542 preview mechanics (machine-frame pin + FORCED envelope box + draggable Start + plays the REAL emit + previewToolMachineFrame).
- E3 (next): the machine-frame 2D layout canvas (envelope rect + declared HOME glyph + the Start draggable there, one source; no stock backdrop, no switch art).

**E0 — WHAT:** `homingStack(params, {superset:true})` now carries EVERY axis (x/y/z/a/b, canonical order) as a GUARDED arm — each axis's home block captured + wrapped in `GUARD({param:'_run<AX>', is:true}, …)`. pruneGuards collapses to the selected set → BYTE-IDENTICAL to the concrete `homingStack(params)` build. The CONCRETE path is UNCHANGED (opts absent → `axes.forEach(homeAxis)` exactly as before). The header/tail/label are built from params.axes (the selection) in BOTH, so they match under prune. rotary a/b = the setzero arm, guarded like the linear seek arms.

**SETTINGS-DEPENDENCE (noted for E1):** the seek distance (machine span + 20) + direction (declaredHomeEdgeSide → the declared <edge>Home) + the unset-travel SKIP ride params.machine + params.limits. In E0 those ride the test params (both superset + concrete read the same → byte-identical). E1's twin binds them from settings (deriveGuards + a postInstantiate recompose) — the alignment-fork2 precedent, now simpler since the t544 span redesign dropped the stock-coord recompose.

**DYNAMIC SHAPE (the reorder — E1's unroll):** homing's axis list is variable-length AND reorderable (the run-form's per-axis `order` field; the default run-order is Z,X,Y — NOT canonical). E0's superset is a FIXED canonical-order template, so prune==concrete holds for CANONICAL-order selections (the gate sweep). The RUN-ORDER reorder is the twin's UNROLL at instantiation (E1) — the comm variable-length precedent ("a variable-length loop → UNROLL at instantiation"). E0 deliberately gates the SELECTION mechanism only; the order lands with the twin.

**VERIFY (homing-superset.spec, NEW):** prune(superset)==concrete byte-diff ZERO across 48 combos = 8 canonical-order SELECTIONS (x / z / xz / xyz / a / ab / all5 / yz) × 2 Z signs (−120 / +500) × 3 declared-home variants (zMaxHome / zMinHome / none-fallback); the superset carries exactly 5 axis guards; prune leaves ZERO guards. + an E0 golden: the concrete build (no opts) has NO guard blocks + still emits the simple G31 seek (no M98). All existing homing emit tests (g31-output, simple-emit, declared-home/direction, dir-override, play-complete, plays-real-emit, io-ui-cleanup) UNCHANGED + green (the concrete is byte-identical). **Full suite 835 pass** (2 skipped).

**FILES:** web/wizards/homingWizard.js (the superset seam: opts + ALL_AXES/capture/GUARD + the guarded axis loop; concrete unchanged) · tests/homing-superset.spec.js (NEW). [[every-wizard-becomes-a-data-twin]] [[wizards-as-data-emit-is-template-not-delegate]]

**PASSED BACK (increment E0 of the homing port; do NOT claim fixed): homingStack now has the {superset:true} data-op seam — every axis's home block is a GUARDED arm (canonical x/y/z/a/b, guarded by its _run<AX> tick), and prune(superset)==concrete BYTE-IDENTICAL across the axis-selection sweep (8 selections × 2 Z signs × 3 declared-home variants = 48 combos, byte-diff ZERO). The concrete/default emit is UNCHANGED (opt-in only). E0 gates the SELECTION in canonical order; the run-ORDER reorder + the settings-dependent (span/direction) bind land with the twin (E1, via unroll + deriveGuards/postInstantiate — noted). Full suite 835 pass. Next increment on your dispatch: E1 the user_homing_data twin (homingData.js).**

## 🔨 turn 548 (cycle 223) — HOMING PORT E1: the user_homing_data TWIN def (blocks/dataOps/homingData.js) on the E0 superset. Unroll-at-instantiation (run-order) + deriveGuards + the settings-dependent recompose. Full suite 836 pass.

**E1 — WHAT.** blocks/dataOps/homingData.js — the twin def. Plus a shared refactor: homingWizard.js now exposes `homeAxisBlocks(ax, config, machine, limits)` (the per-axis home arm — seek/setzero/skip), the ONE source both homingStack AND the twin read (homeAxis is now a thin wrapper; byte-identical, the 16 existing homing tests + E0 stay green).

**The 4 advisor points:**
- **(1) RUN-ORDER unroll-at-instantiation** — the emitted axis order (default Z,X,Y; user-reorderable) is NOT canonical, so the twin UNROLLS the arms in the OP'S order at instantiation. Implemented in postInstantiate.
- **(2) deriveGuards** — `homingDeriveGuards(p)` maps the op's ordered `axes` → the `_run<AX>` ticks, so pruneGuards collapses the E0 template to the selection.
- **(3) the SETTINGS-dependent recompose (postInstantiate)** — `applyHomingUnroll` reads the CURRENT settings (settings.homing.axes = the per-axis config, settings.machine = the span, settings.limits = the declared <edge>Home) and rebuilds the emit in the op's run-order. NEVER a frozen snapshot (the ATC live-settings lesson) — verified by changing the Z travel / flipping a Home flag → the NEXT emit tracks it.
- **(4) DEFAULTS + params** — HOMING_DEFAULTS = { axes: ['z','x','y'] (the safe order), softLimits: true } — mirror the run-form (twin-default rule). The op stores ONLY the axis SELECTION+order (`axes`, a `list` binding) + the soft-limit re-enable flag (`bool`); the per-axis config lives in settings.homing.axes (Homing Setup), read at emit.

**THE MECHANISM (a design note for the advisor).** Homing's settings-dependence is STRUCTURE-CHANGING: an unset span turns an axis's whole arm into a single SKIP comment (t540). So a pure in-place value-swap on the pruned template (the alignment-fork2 style) is IMPOSSIBLE — the recompose MUST rebuild arms. So postInstantiate UNROLLS via the ONE shared builder: `homingStack({ axes: resolved.axes, config: settings.homing.axes, machine: settings.machine, limits: settings.limits, softLimits: resolved.softLimits }, {})` — header + ordered arms + tail, all from the one source, byte-identical, reorder + skip handled + live-settings tracked. The E0 superset template + deriveGuards drive the Blocks structure-view + the E0 prune gate + the schema; the EMIT unrolls (the sanctioned dynamic-shape pattern — every-wizard-becomes-a-data-twin: "a variable-length loop → UNROLL at instantiation"). The pruned template's arms are re-derived (homing arms are fully machine-derived G-code, no free-form content) — flagging so you can confirm this is the shape you want before E2/E3 build the in-place view on it.

**VERIFY (homing-data-emit.spec, NEW):** twin emit == homingStack byte-diff ZERO across 66 combos = 11 selections/PERMUTATIONS (incl. reordered Z,X,Y / Y,X,Z / B,A — the unroll) × 2 Z signs × 3 declared-home variants (zMaxHome/zMinHome/none). Live-settings: Z travel -120→600 → the seek tracks 140→620; flip zMaxHome→zMinHome → the direction flips (140 up → -620) — the NEXT emit reflects it, no snapshot. studio+Expert: homing has NO source-chips (only the Expert dialect gate, shared by the twin + homingStack), so both posts are byte-identical by construction. NOT registered/opened in-place yet (E2). **Full suite 836 pass** (1 known flaky; 2 skipped).

**FILES:** web/wizards/homingWizard.js (extract the shared `homeAxisBlocks`; homeAxis → a thin wrapper — byte-identical) · web/blocks/dataOps/homingData.js (NEW — the twin def) · tests/homing-data-emit.spec.js (NEW). [[every-wizard-becomes-a-data-twin]] [[wizards-as-data-emit-is-template-not-delegate]] [[machine-facts-vs-macro]]

**PASSED BACK (increment E1 of the homing port; do NOT claim fixed): the user_homing_data TWIN emits BYTE-IDENTICAL to homingStack across the axis-selection × run-ORDER permutations × Z-sign × declared-home sweep (66 combos, byte-diff ZERO), and TRACKS live settings (change the Z travel / flip a Home flag → the next emit reflects it — no frozen snapshot). deriveGuards maps `axes`→`_run` ticks; postInstantiate UNROLLS the ordered arms via the ONE shared per-axis builder (homeAxisBlocks) reading current settings. DESIGN NOTE: homing's settings-dependence is STRUCTURE-changing (unset span → skip comment), so the recompose is a REBUILD via the shared builder (not a value-swap) — the emit unrolls the ordered arms; the E0 template drives the Blocks structure/schema. Please confirm this shape before E2 builds the in-place view. NOT registered/opened in-place yet (E2). Full suite 836 pass. Ready for the E2 dispatch (opensAs + the t540-542 preview mechanics)."

## 🔨 turn 550 (cycle 223) — HOMING PORT E2 part 1: the M2 EMIT RESTRUCTURE (the advisor's ruling on my E1 design note) + the re-authoring acceptance test. The emit now operates on the OP'S OWN STORED STACK — a Blocks edit survives + the seek still tracks settings. Full suite 837 pass (1 known flaky).

**THE RULING (advisor).** My E1 emit path (postInstantiate returned homingStack(merged) — a REBUILD) was APPROVED CONDITIONALLY: the north-star gate is that USER RE-AUTHORING SURVIVES. If a user edits the op's stack in Blocks (a beep/comment inside the Z arm), the NEXT emit must KEEP that edit. The recompose may value-swap (seek dist/direction) + include/skip arms, but it must operate on the op's OWN STORED STACK — never regenerate arms in a way that discards edits. My E1 rebuild DID discard edits → restructured now, before the view (E2-proper) lands on it.

**THE FINDING (why this is subtle).** The emit walks a stored op's `children` (so edits survive) but the settings-values are BAKED at build (so they DON'T track settings). The existing twins (comm/alignment) bake at build too — they don't track settings on re-emit. Homing needs BOTH (edits survive AND the seek tracks live machine/limits). So the recompose must be an IN-PLACE transform of the stored stack: reorder + value-swap, preserving user blocks.

**THE RESTRUCTURE (homingData.js `applyHomingRecompose`).** postInstantiate now transforms the PASSED stack in place (never rebuilds):
- partitions the op-container children into header | per-axis ARMS (split on the "Home <L>" head comment) | tail;
- UNROLLS — reorders the STORED arms into the op's run-order (moving the arm's blocks, so a user edit inside an arm rides with it);
- VALUE-SWAPS each arm's SETTINGS-derived lines by their ROLE (the trailing "( … )" note — fast-seek/back-off/re-touch/clearance + the "__HEAD" direction comment), recomputed from the CURRENT settings via the ONE shared homeAxisBlocks (a FRESH homingStack supplies the rebuilt header/tail + the per-axis role→text map). Every NON-settings block (G91/G90/assigns + any USER-ADDED atom) is KEPT verbatim → the edit survives;
- a fresh arm whose SHAPE changed vs the stored (an unset span → a SKIP comment; a newly-ticked axis) is taken WHOLESALE from the fresh (the documented structure-toggle gap — analogous to opEdits' pure-deletion gap). No real-app skip toggle is silent: the fresh skip comment lands.
- Byte-identity is preserved: with NO edit, the swapped arm == the fresh arm (the non-settings blocks are settings-independent + identical), so the whole emit == homingStack(order+settings). The superset template is built with a default TEMPLATE_MACHINE (real spans) so every arm is a real seek to value-swap.

**VERIFY (homing-data-emit.spec):** (E1, still green) twin emit == homingStack byte-diff ZERO across 66 combos (11 selections/permutations × 2 Z signs × 3 declared-home) + live-settings tracking. (E2 ruling, NEW) instantiate → inject a USER BEEP comment inside the Z arm → change the Z travel -120→600 → the recompose ON THE STORED (EDITED) stack keeps 'USER BEEP - do not lose me' AND the seek tracks 140→620. So a Blocks edit survives the settings recompose. **Full suite 837 pass** (1 known middle-animator flaky, green in isolation; 2 skipped).

**REMAINING (E2-proper, next increment):** register (registerUserOp + opensAs 'user_homing_data' + OPENS_AS_TARGETS + FORCE_MACHINE), homing opens IN-PLACE with the run-form (axis ticks/order as def params + the Homing Setup button), + carry the t540-542 preview mechanics (machine-frame pin + FORCED envelope + draggable Start + plays the REAL emit + previewToolMachineFrame). I passed back the M2 restructure as its own increment: the emit shape is the load-bearing prerequisite the ruling emphasised ("restructure it now BEFORE the view lands on it") + it's now approved-condition-met + verified, so the in-place view can safely build on it.

**FILES:** web/blocks/dataOps/homingData.js (applyHomingRecompose — the in-place reorder + role value-swap; TEMPLATE_MACHINE; partitionArms/blockRole helpers; replaces the E1 rebuild) · tests/homing-data-emit.spec.js (+ the re-authoring acceptance test). [[track-edits-by-declaration-not-inference]] [[wizards-as-data-emit-is-template-not-delegate]] [[every-wizard-becomes-a-data-twin]]

**PASSED BACK (increment E2-part-1 of the homing port; do NOT claim fixed): the M2 RULING is met — the twin emit now operates on the OP'S OWN STORED STACK (postInstantiate reorders + VALUE-SWAPS in place, never rebuilds), so a Blocks edit SURVIVES the settings recompose (acceptance test green: a USER BEEP inside the Z arm is kept while the seek tracks 140→620). Byte-identity intact (E1 66-combo sweep still ZERO). The structure-toggle (unset span → skip; newly-ticked axis) is taken wholesale from the fresh — the documented gap (like opEdits' deletion gap). E2-PROPER (opensAs in-place + the t540-542 preview mechanics + FORCE_MACHINE + screenshot) is the next increment, now that the emit shape is approved + verified. Full suite 837 pass. Ready for the E2-proper dispatch."

## 🔨 turn 552 (cycle 223) — HOMING PORT E2-proper: the Homing slot opensAs the twin IN-PLACE, carrying the t540-542 machine-frame preview mechanics. Full suite 841 pass.

**REGISTER + OPENS-AS (the in-place redirect):**
- app.js — import + seed `homingDataDef()` in seedDefaultPortedUserOps (so the twin is registered on boot).
- wizardLibrary.js — `opensAs: 'user_homing_data'` on the `homing` BUILTINS entry. OPENS_AS_TARGETS (hides the twin's own menu slot) + builtinLabelForTwin (the plain 'Homing' title) are auto-derived; the click routes via commandDeck → `openWiz('user_homing_data')` → userOpView.

**THE RUN-FORM AS DEF PARAMS:** per-axis boolean RUN TICKS — `run_z/run_x/run_y/run_a/run_b` (bool widgets = the checkboxes) + `softLimits` (bool). The execution ORDER + per-axis feeds/back-off/declared-home stay in settings (Homing Setup), read at emit. `axesOf(p, config)` derives the ORDERED selection from an explicit `axes` list (tests/round-trip) OR the ticks sorted by the settings order — so the E1/E2 66-combo byte-identity + the re-authoring acceptance test STILL pass (they pass `axes` explicitly). deriveGuards + the recompose both read `axesOf`.

**THE t540-542 MACHINE-FRAME PREVIEW carried into the GENERIC view:** userOpView's rig block (which only did previewRotaryFixture) now also — GATED on the DECLARED machine-frame-tool intent — calls `previewMachine` (FORCE the envelope box regardless of settings.machine.show) + `previewToolMachineFrame` (the live tool in RAW machine coords, t497). The intent is declared on the twin's stack `sim { machine:true, toolMachine:true }` → a NEW `toolMachineFrame` flag threaded through simIntentFromStack → opSimContext. The draggable machine-frame START is a `def.simStartsProvider` returning the mid-envelope point (`homingMidStart` from settings.machine) → userOpView passes starts[0] as the start → the panel seeds it as the engine initialPos (machineFrameTool) → the sim runs FROM it, a drag re-runs (t540). The preview PLAYS THE REAL EMIT (userOpView already emits builderOf→emitMapped — the twin's real code, no proxy).

**THE CORNER REGRESSION (caught + fixed):** applying `previewMachine` for EVERY forceMachine op broke corner — corner DECLARES forceMachine (for the whole-program preview) but its IN-PLACE probe preview must stay LOCAL-frame (forcing the machine frame shifted the probe marker one stock-width = the phantom offset). FIX: gate the machine-frame preview block on `toolMachineFrame` (the homing-only intent), NOT plain forceMachine — so corner/ATC in-place previews are UNCHANGED (legacy behaviour), only homing forces the envelope + machine-frame tool.

**VERIFY (real app + sweeps):** homing opens IN-PLACE (plain 'Homing' title, twin hidden, seeded) with the run ticks + a real G31 code preview; the twin round-trips through the op marker (opSchema, no FIELD_BIND orphan — `( @DDCS:1 {run_z,run_x,…} )`); the 3D FORCES the envelope with settings.machine.show OFF + renders the tool in the machine frame + plays the REAL emit to the top-backoff (both Z signs, ~z-5); the draggable Start (mid-envelope) re-runs from a new spot (drag z=250→60 → the run begins at 60). E1/E2 byte-identity + re-authoring STILL green. Corner + op-sim-context/custom-op-sim-intent/gui-sim-block updated (the +toolMachineFrame key) + green. Screenshots homing_inplace_form + homing_inplace_running (the in-place homing running with the Start marker + forced envelope). **Full suite 841 pass** (2 skipped).

**REMAINING (small E2 polish, noted — NOT correctness): the 'Homing Setup...' BUTTON in the in-place form** (a form button isn't a param → needs a small action-widget; the order/config are still reachable via Settings→Machine→Homing Setup) + the **unset-travel HINT** (homingView's "⚠ Set Z travel …" status; homingUnsetAxes exists). Both are UX; fold into E3 or a quick follow-up.

**FILES:** web/app.js (import + seed homingDataDef) · web/blocks/wizardLibrary.js (opensAs homing) · web/wizards/views/userOpView.js (the machine-frame preview block, gated on toolMachineFrame) · web/blocks/userOps.js (simIntentFromStack toolMachine→toolMachineFrame) · web/viz/opSimContext.js (toolMachineFrame — declared + homing built-in) · web/blocks/dataOps/homingData.js (run-tick form params + axesOf + toolMachine sim + simStartsProvider/homingMidStart) · tests/homing-inplace.spec.js (NEW) · tests/op-sim-context + custom-op-sim-intent + gui-sim-block (the +toolMachineFrame key). [[every-wizard-becomes-a-data-twin]] [[prefer-gui-over-fields]]

**PASSED BACK (increment E2-proper of the homing port; do NOT claim fixed): the built-in Homing slot now opensAs the user_homing_data TWIN IN-PLACE (plain title, twin hidden, seeded), with the per-axis run ticks + a real G31 code preview + the FULL t540-542 machine-frame preview carried into the generic view — FORCED envelope (show OFF), the draggable mid-envelope Start (drag re-runs), the tool in the machine frame, plays the REAL emit to the top-backoff (both Z signs). Round-trip clean (no orphan). Caught + fixed a corner regression (gated the machine-frame preview on the homing-only toolMachineFrame intent, not plain forceMachine — corner/ATC in-place unchanged). E1/E2 byte-identity + re-authoring still green; full suite 841 pass. Screenshots homing_inplace_form/_running. REMAINING small polish (noted): the Homing Setup button (needs an action-widget) + the unset-travel hint. E3 (the machine-frame 2D layout canvas) is the LAST increment. Ready for E3 (and can fold the button/hint in)."

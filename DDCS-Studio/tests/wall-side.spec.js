...
// Wall `side` (software cutter-comp): `on` = tool centre on the boundary (the pocket-wall finish, default — pocket
// pre-insets its region so this is an inside finish); `outside`/`inside` = offset the toolpath by the tool radius
// (the contour/profile cut). rect/polygon/ellipse trace the offset polygon; circle uses a true G3 arc.
test.use({ viewport: { width: 1280, height: 900 } });

test('wall side offsets the traced boundary by the tool radius', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsStudio);
  const r = await page.evaluate(async () => {
    const bm = await import('/blocks/blockModel.js');
    const w = (side) => {
      const b = bm.newBlock('wall');

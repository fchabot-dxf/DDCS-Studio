 ...
 * always drawn), declared as blocks under the 2D canvas node's new DO mouth (`kind: 'uibox'`, the t1595 guard
 * precedent on the mouth axis). The spec builder (layoutSpecFromOp) CONSUMES them from the template — never
 * re-derives — and every coordinate field takes a number OR an expression over the wizard's live params,
 * evaluated by the ONE evaluator (ops/expr.js, params as the caller-populated scope).
 *
...
const HAND_BUILT = [{ type: 'user_root', params: {}, uiChildren: [
    ...
    { type: 'layout_2d_canvas', params: {}, children: SHAPES },
]}];

test('DECLARED → DRAWN: shapes on the canvas mouth round-trip the Blockly canvas and render in the REAL modal', async ({ page }) => {
    await boot(page);
    await page.evaluate((stack) => window.ddcsLoadBlockStack(stack), HAND_BUILT);
    await page.waitForTimeout(2000);

    // the mouth round-trips: canvas → record keeps all four shapes as the canvas node's children (kind 'uibox')
    const rt = await page.evaluate(() => {
        const rec = (window.ddcsGetBlockProgram() || [])[0];
        const canvas = rec && (rec.uiChildren || []).find((c) => c.type === 'layout_2d_canvas');
...
test('EXPRESSIONS over LIVE params — through the REAL registered path, and the value drives the drawing', async ({ page }) => {
    ...
    const r = await page.evaluate(async () => {
        ...
        const tpl = [{ type: 'user_root', params: {}, uiChildren: [
            ...
            { type: 'layout_2d_canvas', params: {}, children: [

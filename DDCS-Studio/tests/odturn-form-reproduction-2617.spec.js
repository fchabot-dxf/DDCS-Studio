...
/**
 * WIZARDS-AS-DATA — t2617 (BACKLOG #71/#72, Phase 1) migrated odTurnData.js onto the declared
 * `split_horizontal`/`group_box`/`field_ref` tree — the FIFTH of the seven lathe-family ops. Rule 17
 * (self-applied): grepped the whole `tests/` directory for `user_lathe_odturn`/`OD_DATA_OPTYPE`/`odTurnData`
 * before calling this migration verified.
 *
 * EXPECTED_ORDER declared in the canonical SECTION_RANK order from the start (IDENTITY/GEOMETRY/TOOL & CUT).
 * `endDiameter` is `when`-gated to `kind==='taper'` — rendered-but-hidden (t1303's own rule), so it still
 * appears as a `[data-param]` row at its declared position regardless of the default `kind`.
 */

const ROW_SELECTOR = '.form-row, .grid-2, .grid-3';
const DATA_MODULE = '/blocks/dataOps/odTurnData.js';
const DEF_FACTORY = 'odTurnDataDef';

const EXPECTED_ORDER = [
  // IDENTITY
  'kind',
  // GEOMETRY
  'barDiameter', 'targetDiameter', 'endDiameter', 'depth', 'doc', 'finish',
  // TOOL & CUT
  'feed', 'feedFinish', 'toolNum',
];

test('odturn-form-reproduction: declared tree places fields in the same structure as the flat form', async ({ page }) => {
...
test('odturn-form-reproduction: an edit in the declared tree reaches the op model and comes back', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsGetBlockProgram);
  ...
  const r = await page.evaluate(async (a) => {
    ...
    // odTurnData.js's own postInstantiate REBUILDS the macro (a build-time branch on `kind`) — the real
    // equivalent of emitEquivalence here is the built G-code itself, same as facing/faceProbe/odProbe.
    ...
    dataModule: DATA_MODULE, defFactory: DEF_FACTORY, rowSelector: ROW_SELECTOR,
    dataOptypeExport: 'OD_DATA_OPTYPE', defaultsExport: 'OD_DEFAULTS',
    editParam: 'depth', editValue: '40',
  });

  expect(r.afterVal).toBe(40);
  expect(r.text, 'the rebuilt macro carries the edited depth value').toContain('40');
});

test('odturn-form-reproduction: the lathe half-profile mounts as a REAL 2D canvas, no unwired placeholder', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsStudio && window.openWiz);
  await page.evaluate(async () => { const U = await import('/blocks/userOps.js'); const M = await import('/blocks/dataOps/odTurnData.js'); try { U.registerUserOp(M.odTurnDataDef()); } catch (_) {} });

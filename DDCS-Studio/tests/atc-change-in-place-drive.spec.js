...
/**
 * ATC CHANGE E2 (t566) — the in-place form's per-method FIELD GATING matches the legacy (z only manual; zClear greyed
 * for auto; fixedT greyed for auto+inline; callMacro only for auto; orient only firmware) · each method's emit · the
 * 3D sim.
 *
 * Split from atc-change-in-place.spec.js at the tier migration work package D; its sibling test (the pure opensAs
 * wiring + opSimContext + choreography-hook check) moved to tests/node/atc-change-in-place.test.mjs. This one stayed
 * because it opens the twin via window.openWiz, reads real DOM (getComputedStyle, data-op-gated attrs), dispatches
 * change events on real form elements, and screenshots the panel.
 */

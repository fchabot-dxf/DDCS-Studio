...
/**
 * WIZARDS-AS-DATA — t2617 (BACKLOG #71/#72, Phase 1) migrated partingData.js onto the declared
 * `split_horizontal`/`group_box`/`field_ref` tree — the SIXTH of the seven lathe-family ops. Rule 17
 * (self-applied): grepped the whole `tests/` directory for `user_lathe_parting`/`PART_DATA_OPTYPE`/
 * `partingData` before calling this migration verified.
 *
...
const DATA_MODULE = '/blocks/dataOps/partingData.js';
const DEF_FACTORY = 'partingDataDef';

const EXPECTED_ORDER = [
  // IDENTITY
  'kind',
  // GEOMETRY
  'zFace', 'floorDiameter', 'width', 'peck',
  // TOOL & CUT
  'feed', 'toolNum',
];

test('parting-form-reproduction: declared tree places fields in the same structure as the flat form', async ({ page }) => {
...
test('parting-form-reproduction: an edit in the declared tree reaches the op model and comes back', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsGetBlockProgram);
  ...
  const r = await page.evaluate(async (a) => {
    ...
    dataModule: DATA_MODULE, defFactory: DEF_FACTORY, rowSelector: ROW_SELECTOR,
    refStackModule: '/wizards/lathe/parting.js', refStackExport: 'partingStack',
    dataOptypeExport: 'PART_DATA_OPTYPE', defaultsExport: 'PART_DEFAULTS',
    editParam: 'width', editValue: '3.5',
  });

  expect(r.afterVal).toBe(3.5);
  expect(r.eqPass, r.firstDiff ? JSON.stringify(r.firstDiff) : '').toBe(true);
});

test('parting-form-reproduction: the lathe half-profile mounts as a REAL 2D canvas, no unwired placeholder', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsStudio && window.openWiz);
  await page.evaluate(async () => { const U = await import('/blocks/userOps.js'); const M = await import('/blocks/dataOps/partingData.js'); try { U.registerUserOp(M.partingDataDef()); } catch (_) {} });
  await page.evaluate(() => window.openWiz('user_lathe_parting'));

...
/**
 * t2695 — TIER MIGRATION: split from pocket-depth.spec.js. These three tests all depend on real DOM/canvas/
 * Three.js (a real `document.createElement` host, `viz.stockMesh.geometry` buffers, a real dispatched `change`
 * event, page screenshots) so they stay in the browser tier. The fourth original test ("2D backdrop reflects a
 * declared floor depth") was pure data (`projectWorkpiece`/`workpieceBackdrop`, no DOM) and moved to
 * tests/node/pocket-depth.test.mjs.
 *
 ...
 * feature.depth (round-trips); (3) a 3/4-angle scrutiny render.
 */

 ...
 * INDEPENDENT geometric truth (outset off the wall · perp axis centred · wall height) AND the built-in EdgeWizard.inferStart
 * (a second independent path) — not merely "a marker appears". Sim-only → the EMIT stays byte-identical (E1 goldens unchanged).
 */
test('edge-sim-starts: ONE edge-anchor pass at the edge line (== built-in inferStart + independent geometry); emit byte-identical', async ({ page }) => {
  ...
  const r = await page.evaluate(async () => {
    ...
    for (const axis of ['X', 'Y']) for (const dir of ['pos', 'neg']) for (const dist of [15, 300]) {
      ...
      const viaOp = opSimStarts(EDGE_DATA_OPTYPE, p, stock);   // the PREVIEW path (the registered provider)
      const viaBuiltIn = wiz.inferStarts(p, stock);            // the built-in edge's own marker (a second independent path)
      combos.push({ k: `${axis}/${dir}/${dist}`, count: Array.isArray(viaOp) ? viaOp.length : -1, op: viaOp && viaOp[0], builtIn: viaBuiltIn && viaBuiltIn[0], truth: truth(p), emits: viaOp && viaOp[0] && viaOp[0].emits });
    }
  ...
  for (const c of r.combos) {
    ...
    expect(near(c.op, c.builtIn), `${c.k}: the data-op marker == the built-in edge inferStart ${JSON.stringify(c.builtIn)}`).toBe(true);

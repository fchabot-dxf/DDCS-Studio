...
test('in-axis MANUAL: amber jog markers appear (sim-only drag, emit byte-identical); AUTO shows teal ↔ distance circles', async ({ page }) => {
  ...
  await page.waitForFunction(() => document.querySelector('[id*="userVizContainer"]:has([data-hid], .fc-handle) .fc-handle[data-hid="crossAimP"]'), null, { timeout: 8000 });

  // AUTO: the in-axis travel targets are TEAL DISTANCE CIRCLES (#39c0d8), the ② trans end a square, the Start amber
  const auto = await page.evaluate(() => {
    const hs = [...document.querySelectorAll('[id*="userVizContainer"]:has([data-hid], .fc-handle) .fc-handle')].map((h) => ({ tag: h.tagName.toLowerCase(), fill: h.style.fill, hid: h.getAttribute('data-hid') }));
    return hs;
  });
  ...
  await page.waitForFunction(() => document.querySelector('[id*="userVizContainer"]:has([data-hid], .fc-handle) .fc-handle[data-hid="__simstart1"]'), null, { timeout: 8000 });

  const st = await page.evaluate(() => {
    const host = document.querySelector('.wiz-viz3d'); const panel = host && host.__panel;
    const hs = [...document.querySelectorAll('[id*="userVizContainer"]:has([data-hid], .fc-handle) .fc-handle')].map((h) => ({ tag: h.tagName.toLowerCase(), fill: h.style.fill, stroke: h.style.stroke, hid: h.getAttribute('data-hid') }));
  ...
  const target = await page.$('[id*="userVizContainer"]:has([data-hid], .fc-handle) .fc-handle[data-hid="__simstart1"]');

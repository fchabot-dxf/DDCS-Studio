...
test('THE SWEEP + THE LOCK — every per-type generator, every build enum, both directions', async ({ page }) => {
    ...
    const r = await page.evaluate(async () => {
        const { seedFromOp, camTypeOf, buildEnumFields, GENERATOR_IGNORES, PARAM_ALIAS } = await import('/data/opCamMap.js');
        ...
        for (const o of OPS) {
            ...
            for (const e of enums) {
                ...
                rows.push({
                    ...
                    declaredIgnored: !!(GENERATOR_IGNORES[t.camType] && GENERATOR_IGNORES[t.camType][e.key]),
                    seedExposable: !!seedRow.exposable, seedTip: seedRow._exposeTip || '',
                    buildable: bodies.every((b) => typeof b === 'string' && b.length > 0),
                });
            }
        }
        return { rows, table: GENERATOR_IGNORES };
    });
 ...
 */
test('THE ROW SAYS WHAT THE MACRO DOES — a spiral pocket is told its slot cuts a raster', async ({ page }) => {
    await boot(page);
    const r = await page.evaluate(async () => {
    ...
    // … and now says, in words, that the macro does not carry it.
    expect(r.spiralStrategy._exposeTip, 'the spiral pocket is told its slot cuts a raster').toMatch(/cuts a RASTER/);
    expect(r.spiralStrategy._exposeTip, 'and that the pick is not carried').toMatch(/NOT carried/);
    expect(r.spiralStrategy.exposable, 'and it cannot be branched, because the macro has no second arm').toBe(false);
    ...
    // ⚠ `direction` has NO row on this table at all, and that is honest-by-omission rather than a gap: it binds to a
    // socket on the fill leaf, so it is a VALUE enum, not a build enum, and the rect pocket generator's own field
    // spec has no such field. Nothing displays it, so nothing can misstate it. (I declared a sentence for it before
    // measuring and removed it — a declaration nothing reads is worse than none, because it looks handled.)
    expect(r.direction, 'direction has no row on the rect pocket generator arm — nothing to lie with').toBe(null);
    // ⚠ `entry` is NOT declared ignored, and that is the control: it genuinely reaches the macro (the t1341 entry
    // gate + the ramp lead-in), so it keeps its own treatment. If this ever starts matching, the declaration has
    // over-reached.
    if (r.entryRow) expect(r.entryRow._exposeTip || '', 'entry genuinely reaches the macro — no not-carried sentence').not.toMatch(/NOT carried/);

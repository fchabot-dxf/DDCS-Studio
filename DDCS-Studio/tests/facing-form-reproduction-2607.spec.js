...
/**
 * WIZARDS-AS-DATA — t2607 (BACKLOG #71/#72, Phase 1, the sixth axis) migrated facingData.js onto the declared
 * `split_horizontal`/`group_box`/`field_ref` tree — the PILOT for the 7 lathe-family ops that declare
 * `panel:'form3d+2d'` as a POSITIONAL arg and carry a `layout{kind:'lathe_profile'}` node no prior screen ever
 * enumerated (centerDrill, faceProbe, facing, odProbe, odTurn, parting, polygon — facing goes first).
 *
 * MEASURED, not guessed: `layoutSpecFromOp` (panelTypes.js:296) already special-cases `def.layout.kind ===
 * 'lathe_profile'` via `latheLayoutSpec`, BEFORE the mill stock-footprint default — the SAME `feature_canvas`/
 * `renderLayout2D` pipeline every migrated MILL op's 2D pane already goes through. `def.layout` is self-healed
 * at registration (`resolveLayoutMeta`/`layoutFromStack`, userOps.js:363-373) by scanning `def.template` via
 * `flattenBlocks`, which recurses into `uiChildren` regardless of nesting — so the `layout` node is found
 * whether it sits at the top of a flat stack (the old shape) or nested inside a split/RIGHT tree (this one).
 * NO new formWidgets.js RENDER branch was needed for the 2D content itself — only a one-line addition (t2607,
 * formWidgets.js's `traverse()`) so a `layout` node is treated as consumed-elsewhere metadata (exactly like
 * `SHAPE_2D_TYPES`) instead of falling into the t1561 unwired-placeholder. Test 3 below proves that live.
 *
 * NOT built on `tests/support/formReproduction.js`'s shared suite: facing has no classic shell (born a data-op
 * twin, t1271) — same reason edge/corner/bore/rotary_clock/alignment stand alone. Tests 1/2 are copied from
 * formReproduction.js's own test 1/test 3 (unmodified logic), mirroring edge-form-reproduction-2599.spec.js.
 *
 * EXPECTED_ORDER is FACING_BINDING_SPECS' own array order, grouped by section (GEOMETRY, TOOL & CUT).
 */

const ROW_SELECTOR = '.form-row, .grid-2, .grid-3';
const DATA_MODULE = '/blocks/dataOps/facingData.js';
const DEF_FACTORY = 'facingDataDef';

const EXPECTED_ORDER = [
  // GEOMETRY
  'allowance', 'doc', 'xStart',
  // TOOL & CUT
  'feed',
];

test('facing-form-reproduction: declared tree places fields in the same structure as the flat form', async ({ page }) => {
...
test('facing-form-reproduction: an edit in the declared tree reaches the op model and comes back', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsGetBlockProgram);
  ...
  const r = await page.evaluate(async (a) => {
    ...
    dataModule: DATA_MODULE, defFactory: DEF_FACTORY, rowSelector: ROW_SELECTOR,
    refStackModule: '/wizards/lathe/facing.js', refStackExport: 'facingStack',
    dataOptypeExport: 'FACING_DATA_OPTYPE', defaultsExport: 'FACING_DEFAULTS',
    editParam: 'doc', editValue: '2.5',
  });

  expect(r.beforeVal).toBe(1);   // FACING_DEFAULTS.doc
  expect(r.afterVal).toBe(2.5);
  expect(r.eqPass, r.firstDiff ? JSON.stringify(r.firstDiff) : '').toBe(true);
});

test('facing-form-reproduction: the lathe half-profile mounts as a REAL 2D canvas, no unwired placeholder (t2607)', async ({ page }) => {
  await page.goto('http://localhost:3211');
  await page.waitForFunction(() => window.ddcsStudio && window.openWiz);
  await page.evaluate(async () => { const U = await import('/blocks/userOps.js'); const M = await import('/blocks/dataOps/facingData.js'); try { U.registerUserOp(M.facingDataDef()); } catch (_) {} });
  await page.evaluate(() => window.openWiz('user_lathe_facing'));

...
for (const z of [500, -120]) {
    test(`z=${z}: the preview PLAYS the real emit + settles at the SAME top-backoff as the editor engine`, async ({ page }) => {
        ...
        await page.evaluate(() => { const host = document.querySelector('.wiz-viz3d'); host.querySelector('.pp-run').click(); const p = window.ddcsStudio.wizardManager._activePanel; if (p && p.engine) p.engine.simSpeed = 60; });
...
test('the preview line-HIGHLIGHTS real emitted G31 lines during play (+ screenshot mid-run)', async ({ page }) => {
    ...
    await page.evaluate(() => { const host = document.querySelector('.wiz-viz3d'); host.querySelector('.pp-run').click(); const p = window.ddcsStudio.wizardManager._activePanel; if (p && p.engine) p.engine.simSpeed = 4; });
    // poll the code panel for an ACTIVE (executing) line whose text is a G31 — proof the preview highlights the REAL emit
    const found = await page.waitForFunction(() => {
        const active = document.querySelector('#wiz_user_code .g-line.active-line');
    ...
    { const _b = await page.locator('#wiz_user').boundingBox(); if (_b) await page.screenshot({ path: 'scratchpad/homing_preview_highlights_g31.png', clip: _b }); }   // t710 clip capture (rAF-starvation dodge)
});

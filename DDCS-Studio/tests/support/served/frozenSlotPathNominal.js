/**
 * tests/support/served/frozenSlotPathNominal.js — THE FROZEN **NOMINAL-FLOOR** SLOT KERNEL (t1506). TEST-ONLY.
 *
 * ── ⚠ THERE ARE NOW TWO FROZEN SLOT KERNELS, AND THEIR JOBS ARE DIFFERENT. Read this before assuming either is
 *    stale, because the obvious guess — that the older one was superseded — is exactly backwards. ────────────────
 *
 *   frozenSlotPath.js (t1496)          THE ACTUAL-DROP KERNEL → now the **IDENTITY REFERENCE**.
 *                                      It descends from the REAL previous floor (`prevZ: -prevD`). t1506 moved the
 *                                      live kernel off it onto the nominal floor; t1524 moved the whole FAMILY onto
 *                                      the actual floor instead, so the live kernel matches this freeze BYTE FOR
 *                                      BYTE again. `slot-frozen-reference-1496.spec.js` asserts that identity —
 *                                      restored, not relaxed.
 *
 *   frozenSlotPathNominal.js (THIS)    THE NOMINAL-FLOOR KERNEL → the **DIVERGENCE WITNESS**.
 *                                      It descends from `z + stepdown`, which is what the whole family cut between
 *                                      t1506 and t1524. Nothing ships this any more, and that is the point: it
 *                                      measures what t1524 lifted the family FROM, diverging from the live kernel
 *                                      and from the atom by exactly `(stepdown − lastBite)/tan(rampAngle)` on a
 *                                      partial final level — 9.54mm at the slot's shipped defaults.
 *
 * WHY A SECOND FREEZE AT ALL, recorded because the cheaper roads were considered and rejected (t1523 ruling): once
 * the family converged onto the t1496 floor, the t1496 copy could no longer show what changed — a convergence test
 * that cannot show what it converged FROM is indistinguishable from a test of nothing (t1506's own words). The
 * witness had to move, and it had to stay a SECOND IMPLEMENTATION rather than become a formula the spec supplies
 * itself, which is the standard this project keeps refusing to trade away.
 *
 * ⚠ IT IS AN EXTRACTION OF AN EXTRACTION, NOT A TRANSCRIPTION — generated from `frozenSlotPath.js` by replacing
 * exactly ONE expression (`prevZ: -prevD` → `prevZ: z + num(p.stepdown, 1.5)`, which is verbatim what t1506 shipped)
 * and renaming the export. Everything else is character-identical, and `slot-frozen-reference-1496.spec.js` ASSERTS
 * that the two freezes differ ONLY in the descent — so this file cannot quietly stop being that one change.
 *
 * ── the original t1496 header follows, still true of everything below the descent ────────────────────────────────
 *
 * Served at `/_test/frozenSlotPathNominal.js` by the mem-server and reachable from nothing else. This is a VERBATIM
 * copy of `slotPath` and everything it reaches, as they emitted at t1496 — before the slot capability arc's purpose
 * is cashed and the kernel's clearing is re-pointed through the parametric raster atom.
 *
    ...
    function slotPath(p) {
        ...
        for (const d of levels) {
            ...
            for (const o of offs) {
                ...
                if (first) {
                    const ctx = { entry, z, prevZ: z + num(p.stepdown, 1.5), rampAngle: num(p.rampAngle, 3), feed,
...
export const frozenNominalSlotPath = SLOT.slotPath;

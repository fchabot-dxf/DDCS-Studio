...
/**
 * BACKLOG #71, THIRD GESTURE (t2521) — the GUI RECT-HANDLE canvas block. `rect_handle`, nested inside a
 * `feature_canvas` block's own mouth, DECLARES a draggable 2D SIZE (W×H) handle bound to TWO params from a
 * FIXED literal anchor. `handleBindingsFromStack` expands it to two socket-less {group, role:w/h,
 * anchor:{kind:'rect', ax, ay, sx, sy, minw/maxw/minh/maxh, valueField, label}} bindings.
 *
    ...
    const template = [{
        ...
        uiChildren: [
            { type: 'feature_canvas', params: { panel: 'form2d' }, children: [
                { type: 'rect_handle', params: { field: 'boxw', fieldH: 'boxh', value: '40', valueH: '30', ax: '0', ay: '0', sx: '1', sy: '1', minw: '5', maxw: '', minh: '5', maxh: '', valueField: 'field', label: 'W×H' } },
            ] },
        ],
        children: [{ type: 'comment', params: { text: 'rect handle pilot' } }],
    }];
    U.createUserOp(U.userOpFromStack('${OPTYPE}', 'RH Pilot', template, [], 'form2d'));
`;

test('round-trip: a rect_handle nested in feature_canvas <-> two socket-less {group,role,anchor} bindings (bindingsFrom/ToBlocks), valueField preserved', async ({ page }) => {
    ...
    expect(r.bindings.length, 'one rect_handle -> two socket-less bindings (w, h)').toBe(2);
    expect(r.bindings.map((b) => b.role)).toEqual(['w', 'h']);
    expect(r.bindings.map((b) => b.param)).toEqual(['boxw', 'boxh']);
    expect(r.bindings.map((b) => b.default)).toEqual([40, 30]);
    expect(r.bindings.every((b) => b.match === undefined && b.blockIndex === undefined), 'socket-less -> no emit (sim/form-only)').toBe(true);
    expect(r.bindings[0].anchor, 'the anchor carries ax/ay/sx/sy/clamps/valueField/label').toEqual({ kind: 'rect', ax: 0, ay: 0, sx: 1, sy: 1, minw: 5, maxw: null, minh: 5, maxh: null, valueField: 'field', label: 'W×H' });
    // reverse round-trip: the two bindings re-nest into a feature_canvas carrying the SAME rect_handle
...
test('a rect-handle op: def.bindings carry the anchor; layoutSpecFromOp renders the handle at anchor+extents; the displayed value follows valueField; emit BYTE-IDENTICAL (sim/form-only, no socket)', async ({ page }) => {
    await page.goto('http://localhost:3211');
    ...
    const r = await page.evaluate(async (t) => {
        ...
        // exercise the REAL click-to-edit path (onEdit) for BOTH axes -- this is exactly the t2495 routing.
        const { CANVAS_GESTURES } = await import('/viz/canvasWidgets.js');
        const emitDefault = emitMapped(builderOf(t)(U.defaultParams(def))).text;
        const emitDragged = emitMapped(builderOf(t)({ ...U.defaultParams(def), boxw: 99, boxh: 77 })).text;
        return {
    ...
    expect(r.hasHandle, 'layoutSpecFromOp renders a draggable rect handle for the anchored binding').toBe(true);
    ...
    expect(r.emitByteIdentical, 'the rect handle is sim/form-only -> dragging never changes the emit (byte-identical)').toBe(true);
});

test.use({ viewport: { width: 1600, height: 1000 } });
test('DRIVE THE APP, THE t2509/t2517 BAR: feature_canvas + rect_handle authored via REAL palette drags, real field edits (incl. the valueField dropdown), real save, a REAL reload, then a REAL mouse drag on the rendered SVG handle writes BOTH fields', async ({ page }) => {
    ...
    // 3) rect_handle into feature_canvas's OWN mouth
    const fcMouth = await mouthPoint('feature_canvas', 'DO');
    await searchFor('rect handle');
    await dragFlyoutBlockToMouth('rect_handle', fcMouth);
    const rhConnected = await page.evaluate(() => {
        const ws = window.__blkws;
        const fc = ws.getAllBlocks(false).find((b) => b.type === 'feature_canvas');
        const inp = fc.inputList.find((i) => i.name === 'DO');
        return !!(inp.connection.targetBlock() && inp.connection.targetBlock().type === 'rect_handle');
    });
    expect(rhConnected, 'rect_handle connected into feature_canvas own DO mouth').toBe(true);

    // 4) real field edits: feature_canvas -> form2d; rename field/fieldH; set the valueField dropdown
    await setDropdownField('feature_canvas', 'PANEL', 'form2d');
    await setTextField('rect_handle', 'FIELD', 'boxw');
    await setTextField('rect_handle', 'FIELDH', 'boxh');
    ...
    expect(fieldsSet.field).toBe('boxw');
    ...
    await page.fill('.blk-dev-savedlg .blk-dev-opname', 't2521 rect handle pilot (live)');
    ...
    const savedOpType = await page.evaluate(async () => {
        const U = await import('/blocks/userOps.js');
        const d = U.listUserOps().find((x) => x.label === 't2521 rect handle pilot (live)');
    ...
    const rendered = await page.evaluate(() => {
        ...
        return {
            ...
            wVal: (f.querySelector('[data-param="boxw"]') || {}).value, hVal: (f.querySelector('[data-param="boxh"]') || {}).value,
            svgPresent: !!svg,
    ...
    { const _b = await page.locator('#wiz_user').boundingBox(); if (_b) await page.screenshot({ path: 'verification/t2521-rect-handle-live-drag.png', clip: _b }); }
    ...
    expect(before.w, 'boxw starts at the authored default').toBe('40');
    expect(before.h, 'boxh starts at the authored default').toBe('30');

...
// Editor "✥ Position" = translate the whole program on the stock (companion to ⟳ Align). translateProgram shifts
// absolute X/Y/Z, leaves arc offsets (I/J) and G91 incremental moves alone.
test.use({ viewport: { width: 1280, height: 900 } });

test('translateProgram shifts absolute moves only; Position button is wired', async ({ page }) => {
  ...
  const r = await page.evaluate(async () => {
    ...
    return {
      ...
      hasBtn: !!document.getElementById('position-translate-btn'),
  ...
  expect(r.hasBtn && r.hasFn, 'Position button + handler wired').toBe(true);
});

...
// PROOF 6 reads two source files as raw TEXT (to regex for a bearing check) — the browser original used
// `fetch('/data/....js')` against the page's own origin; there is no origin here, so it reads the same files off
// disk instead. Same bytes, the mechanically necessary node-tier substitute for a same-origin fetch.
 ...
 * Every test here is pure: page.goto/waitForFunction, one page.evaluate that imports app modules and returns data,
 * plain expect() on that data. The original boot() called `page.on('dialog', ...)` as defensive browser boilerplate
 * (no dialog is ever raised by a pure page.evaluate); dropped here since register.mjs's page.on only understands
 * 'console'.
 */
...
test('PROOF 6 — eligibility asks the envelope; no bearing check lives in the CAM layer', async ({ page }) => {
    ...
    const r = await page.evaluate(async () => {
        const camMap = fs.readFileSync(path.join(WEB, 'data', 'opCamMap.js'), 'utf8');
        const toSlot = fs.readFileSync(path.join(WEB, 'data', 'opToSlot.js'), 'utf8');

...
//
// TIER MIGRATION WORK PACKAGE 3 — split browser→node. 3 of the 4 tests are pure: import()+evaluate over
// /blocks/devMode.js + /blocks/userOps.js + /data/stackToSlot.js + /data/exposeClassifier.js, over synthetic op
// defs — no DOM. The first test also reads `getUserDef('user_surfacing_data')` (a REAL generator twin, to prove a
// generator twin gets no cam_table), which needs the live registry the real app's `seedDefaultPortedUserOps()`
// (web/app.js) populates at boot — seeded here via one `registerUserOp(surfacingDataDef())` (Pattern 2). The 4th
// test ("S4b integration — opening a pill-based universal op in editWizardDef…") stayed in the browser tier
// (tests/cam-block-native-params-s4b-drive.spec.js): it drives `window.ddcsEditWizardDef` and reads a live
// Blockly workspace (`window.__blkws`), plus a screenshot — a genuine app+DOM+Blockly dependency.

 ...
 *
 * t2689 — TIER MIGRATION BATCH 2: moved browser→node. boot() seeds both probe twins via createUserOp (batch 1's
 * registerUserOp-vs-listUserOps bug applies here too), fresh-if-missing per call.
 *
 * ONE TEST STAYED BEHIND: "THE SCENE, BOTH WAYS" opens a wizard via window.openWiz and reads window.__ddcsLastViz —
 * a real Three.js render that needs the actual app + a settle wait, not a pure import()+evaluate. Split into
 * tests/lathe-probe-1299-scene.spec.js, left in the browser tier.
 */
test.use({ viewport: { width: 1400, height: 950 } });

const boot = async (page, kind = 'lathe') => {
    const uo = await import('/blocks/userOps.js');
    const { faceProbeDataDef, FACE_PROBE_DATA_OPTYPE } = await import('/blocks/dataOps/faceProbeData.js');
    const { odProbeDataDef, OD_PROBE_DATA_OPTYPE } = await import('/blocks/dataOps/odProbeData.js');
    if (!uo.listUserOps().some((d) => d.opType === FACE_PROBE_DATA_OPTYPE)) uo.createUserOp(faceProbeDataDef());
    if (!uo.listUserOps().some((d) => d.opType === OD_PROBE_DATA_OPTYPE)) uo.createUserOp(odProbeDataDef());

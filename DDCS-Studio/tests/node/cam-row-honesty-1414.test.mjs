 ...
 *
 * t2695 — TIER MIGRATION: the sweep test reads `getUserDef(o.opType)` for every one of pocket/surfacing/corner/edge/
 * slot/drill assuming pre-seeding from web/app.js's boot-time seedDefaultPortedUserOps() (the real browser boot the
 * original `page.goto('http://localhost:3211')` reached). The node harness's goto() stub never runs that boot, so
 * this file registers the same SEED_BUILDERS factories explicitly (seeding pattern 2) before every test. The
 * original `boot()` helper's `page.on('dialog', (d) => d.accept())` and its
 * `waitForFunction(() => document.documentElement.dataset.ddcsReady === '1', …)` were real-browser-boot mechanics
 * (a confirm() the full app might raise while loading, and a DOM-ready flag the full app sets) with no node-tier
 * equivalent — register.mjs's `document` never gets a `ddcsReady` dataset flag because nothing in the node tier
 * ever runs that boot sequence; harness.mjs's `page.on` explicitly throws for any event other than 'console' by
 * design ("this spec is not a candidate for the node tier"). Neither one is read by any assertion below — the
 * explicit registerUserOp seeding below is what the dialog/ready-wait was standing in for.
 */

 ...
 *
 * TIER MIGRATION — moved browser→node. One test stayed: "the engine HONORS the M350 pneumatics — M156-161 drive the
 * OUTPUT pins + fire io_change" registers a real `window.addEventListener('io_change', ...)` and expects
 * `dispatchEvent` to actually invoke it — register.mjs's own event-bus stub is deliberately inert (`dispatchEvent =
 * () => true`, never calling listeners), so it needs a real browser and stayed in tests/atc-dialect-drive.spec.js.
 */

 ...
 * whole, no behavioural change. The two tests that seed via DATA-OP TWINS (`user_*_data` opTypes) register those
 * twins explicitly first — the node harness never runs web/app.js's `seedDefaultPortedUserOps()`, which is what
 * populates the live user-op registry in a real boot.
 */
...
test('S1 fix: data-op TWINS are CAM-able and seed correct values', async ({ page }) => {
  ...
  const r = await page.evaluate(async () => {
    ...
    // the node harness never runs web/app.js's seedDefaultPortedUserOps(), which seeds these twins in a real boot
    const seedIfMissing = async (opType, mod, fn) => {
      if (!getUserDef(opType)) { const { [fn]: build } = await import(mod); registerUserOp(build()); }
    };
    await seedIfMissing('user_surfacing_data', '/blocks/dataOps/surfacingData.js', 'surfacingDataDef');
    await seedIfMissing('user_pocket_data', '/blocks/dataOps/pocketData.js', 'pocketDataDef');
    await seedIfMissing('user_corner_data', '/blocks/dataOps/cornerData.js', 'cornerDataDef');
    await seedIfMissing('user_bore_data', '/blocks/dataOps/boreData.js', 'boreDataDef');
    await seedIfMissing('user_contour_data', '/blocks/dataOps/contourData.js', 'contourDataDef');
    const op = (opType, params) => ({ opType, params });
    const byKey = (res, k) => (res.fields || []).find((f) => f.key === k);
...
test('S1 fix hardening: edge/slot/drill/middle twins seeded + drill placement sources originX', async ({ page }) => {
  ...
  const r = await page.evaluate(async () => {
    ...
    await seedIfMissing('user_edge_data', '/blocks/dataOps/edgeData.js', 'edgeDataDef');
    await seedIfMissing('user_slot_data', '/blocks/dataOps/slotData.js', 'slotDataDef');
    await seedIfMissing('user_drill_data', '/blocks/dataOps/drillData.js', 'drillDataDef');
    await seedIfMissing('user_middle_data', '/blocks/dataOps/middleData.js', 'middleDataDef');
    const op = (opType, params) => ({ opType, params });
    const byKey = (res, k) => (res.fields || []).find((f) => f.key === k);

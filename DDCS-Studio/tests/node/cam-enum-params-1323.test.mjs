 ...
 *
 * TIER MIGRATION WORK PACKAGE 3 — split browser→node. 6 of the 7 tests are pure: import()+evaluate over
 * /blocks/userOps.js + /data/opCamMap.js + /data/probeToSlot.js + /data/stackToSlot.js + /blocks/blockEmitter.js +
 * /blocks/opBuilders.js + /engine/trace.js, asserting on plain returned data. Several call `uo.getUserDef(...)`,
 * which reads the LIVE in-memory registry the real app's `seedDefaultPortedUserOps()` (web/app.js) populates at
 * boot — the node harness's `page.goto` stub never runs that, so a `beforeEach` here seeds the two twins these
 * tests actually read (`user_surfacing_data`, `user_corner_data`) via a plain `registerUserOp(...def())` (Pattern
 * 2 — registerUserOp is idempotent across repeated calls, unlike createUserOp). The 7th test ("THE TABLE DRAWS
 * IT") stayed in the browser tier (tests/cam-enum-params-1323-drive.spec.js): it drives the real macrosApp/CAM
 * authoring modal DOM (`window.ddcsOpenCamAuthoring`, `document.querySelector('tr[data-fkey="zMode"]')`, a real
 * `<select>`'s `.options`) — a genuine DOM dependency.
 */

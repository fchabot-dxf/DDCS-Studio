 ...
 *   2. it has a form-field binding unless exempt (a `.field` on ≥1 DICT param), and every binding landed on a
 *      real DICT param (BIND_ORPHANS empty — the binding lives ON the dict, so the field map can't drift), and
...
test('protocol: every op has a dict entry, round-trips, and is canon-clean', async ({ page }) => {
  ...
  const report = await page.evaluate(async () => {
    ...
    for (const opType of Object.keys(ops.BUILDERS)) {
      ...
      if (!Object.keys(D.paramFields(opType)).length && !EXEMPT_BINDING.has(opType)) r.errors.push('no form-field binding (DICT .field)');
  ...
  expect(orphans).toEqual([]);                           // every field binding landed on a real DICT param (no stale binding)

...
// fixture). Z = the stock resting on the fixed table (you re-zero Z per part / the datum carries the table
// reference → the stored WCS-Z is ignored for placement). Changing the WCS moves the part in XY, never the envelope.
test.use({ viewport: { width: 1280, height: 900 } });

test('envelope fixed; part rides the WCS in XY; Z = stock-on-table (stored WCS-Z ignored)', async ({ page }) => {
  ...
  const snap = (g54) => page.evaluate((off) => {
    ...
    viz.setStock({ x: 50, y: 40, z: 20, show: true, datum: 'nnp', pin: 'g54' });
  ...
  // XY rides the WCS; Z = stock-on-table (20), NOT the stored WCS-Z (-10 is ignored — it's volatile / table-noise).

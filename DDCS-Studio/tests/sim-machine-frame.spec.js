...
// the PART frame (op/stock/tool) rides +workOrigin. So changing the WCS moves the part, never the envelope.
test.use({ viewport: { width: 1280, height: 900 } });

test('envelope fixed at machine coords (home at 0); part rides +workOrigin; WCS change moves the part only', async ({ page }) => {
  ...
  const snap = (wo) => page.evaluate((w) => {
    const viz = window.ddcsStudio.wizardManager._activePanel.viz;
    viz.setMachine({ x: 300, y: 300, z: 120, show: true, workOrigin: w });
  ...
  expect(a.part, 'part frame rides +workOrigin').toMatchObject({ x: 100, y: 50, z: -10 });
  ...
  expect(b.part, 'only the part moved to the new WCS').toMatchObject({ x: 200, y: 90, z: -10 });
});

 ...
 * comment on a non-ATC post (V4.1/DM500) instead of leaking Expert-only pneumatics; (2) applyCapGating's blanket cap-ON
...
test('V4.1 folds the ATC pneumatic M-codes; Expert emits them; the tool-setter probes carry the miss-check', async ({ page }) => {
    ...
    const r = await page.evaluate(async () => {
        ...
        // the applyCapGating "slipped through the early return" case: a #var line (→ the OLD early-return fired on a vars+flow
    ...
    // ── the applyCapGating fix: a cap:'atc' line on a vars+flow post (V4.1) — previously slipped the early-return — is now gated ──

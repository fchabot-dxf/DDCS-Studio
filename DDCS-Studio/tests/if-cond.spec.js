...
test('If spawns a boolean socket; rejects a number reporter, accepts Compare', async ({ page }) => {
  ...
  // 1) drag the If block onto the canvas
  await dragFromTo(page, page.locator('#blk-pal button', { hasText: 'If' }), cx, cy);
  ...
  await dragFromTo(page, page.locator('#blk-pal button', { hasText: 'Variable' }), sb.x + sb.width / 2, sb.y + sb.height / 2);
  ...
  await dragFromTo(page, page.locator('#blk-pal button', { hasText: 'Compare' }), sb.x + sb.width / 2, sb.y + sb.height / 2);

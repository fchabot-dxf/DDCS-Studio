...
test('safe-Z frame on the rotary final park: relative byte-identical, machine = G53, frame round-trips', async ({ page }) => {
  ...
  // error retract are the machine-frame safe-Z margin (a G53, NOT G53 Z#17); t899 that G53 reads the read-only scratch #42
  // (mirrors #520). So only the final park is a G0 Z#17 here (the counts below key on Z#17, unaffected by the #520/#42 guard).
  ...
  expect(r.rel_g0, 'relative: only the FINAL PARK is a G0 Z#17 lift (the reposition/error retracts are machine-frame G53)').toBe(1);
  // machine = the new path: the FINAL park swaps to G53 Z#17; the reposition/error retracts already moved to the machine-frame G53 (t824/t826)

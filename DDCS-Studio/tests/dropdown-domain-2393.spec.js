 ...
 * carrying `'drill'` (one of `LAYOUT_TYPES`' 14 declared kinds — panelTypes.js), came back `'none'`; a
 * `feature_canvas` block (t2515 — renamed from `panel`)'s `PANEL` field, deserialized from `'commscreen'` (one
 * of `PANEL_TYPES`' 5 declared panels), came back `'form3d'`. Both are REAL wizard-authoring values
 * (`def.layout`/`def.panel`), not hypotheticals — any wizard whose canvas round-trips through Blockly with
 * either set to a non-offered value silently loses it.
 *
...
test('feature_canvas.panel dropdown offers every PANEL_TYPES key (including commscreen)', async ({ page }) => {
    await page.goto('http://localhost:3211');
    await page.waitForFunction(() => document.documentElement.dataset.ddcsReady === '1');
    const { offered, declared } = await page.evaluate(async () => {
        const { PANEL_TYPES } = await import('/wizards/ops/panelTypes.js');
        const { fieldOptions } = await import('/blocks/blockly/bridge.js');
        const { featureCanvasBlock } = await import('/wizards/ops/featureCanvas.js');
        return { offered: fieldOptions(featureCanvasBlock, 'panel').map((o) => (Array.isArray(o) ? o[1] : o)), declared: Object.keys(PANEL_TYPES) };
    });
    expect(new Set(offered)).toEqual(new Set(declared));
...
test('LIVE: a feature_canvas block loaded with panel=commscreen round-trips intact, not silently coerced to form3d', async ({ page }) => {
    await page.goto('http://localhost:3211');
    await page.waitForFunction(() => document.documentElement.dataset.ddcsReady === '1' && window.showApp);
    await page.evaluate(() => window.showApp('blocks'));
    await page.waitForFunction(() => window.__blkws);
    const panel = await page.evaluate(() => {
        const ws = window.__blkws;
        const blk = window.Blockly.serialization.blocks.append({ type: 'feature_canvas', fields: { PANEL: 'commscreen' } }, ws);

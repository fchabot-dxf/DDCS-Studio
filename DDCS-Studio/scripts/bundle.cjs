...
// Auto-detect: prefer repo root, then 'web', then 'DDCS-Studio', then first sensible subdir
if (!srcDir) {
    if (fs.existsSync(path.join(repoRoot, 'index.html'))) {
        srcDir = repoRoot;
    } else if (fs.existsSync(path.join(repoRoot, 'web', 'index.html'))) {
        srcDir = path.join(repoRoot, 'web');
    } else if (fs.existsSync(path.join(repoRoot, 'DDCS-Studio', 'index.html'))) {
        srcDir = path.join(repoRoot, 'DDCS-Studio');
    } else {
        ...
        for (const e of entries) {
            ...
            if (fs.existsSync(path.join(p, 'index.html'))) {
                if (e.name === 'DDCS-Studio') { found = p; break; }
                if (!found) found = p;
            }
        }

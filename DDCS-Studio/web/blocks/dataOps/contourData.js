...
// t2375 — SECTION MISMATCH, fixed: this array's own field ORDER (not just the `section:` names) drives the
// FLAT render's own visible grouping — `formWidgets.js`'s own section-bucketing (`renderOpForm`) buckets by a
// FIXED `SECTION_RANK` whitelist (`['IDENTITY','GEOMETRY','TOOL & CUT']`); a section name outside that list
// (every name below) gets the SAME "unranked" rank, so a stable sort leaves array order untouched — the section
// BOX order is "whichever section's first member appears earliest in this array," and each field's own
// position WITHIN its box also follows array order. Reordered below (not just renamed) so both match the
// shell (index.html:564-613) exactly: SHAPE (shape/originX/originY/offZ/stockAttach/pathDatum/[hidden stock
// fields]/wcs/w/h/dia/sides) → SIDE & TOOL (side/toolDia/rpm) → DEPTH & FEED (depth/stepdown/confirmEvery/
// entry/rampAngle/feed/plunge). `confirmEvery`/`entry`/`rampAngle` have no shell field at all (twin-only,
// same orphan class drill/pocket already have) — placed in DEPTH & FEED as the closest conceptual fit, after
// the shell-matching fields in that section, not smoothing over their own absence from the shell (recorded in

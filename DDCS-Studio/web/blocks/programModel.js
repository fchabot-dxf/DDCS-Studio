...
export function serializeWithMarkers() {
    ...
    (proj.lines || []).forEach((line, i) => {
        const op = opAtLine(i);
        if (op && op.id !== lastOpId) out.push(markerLine(op.opType, op.params || {}, defVOf(op.opType), op.disabled, op.comment));
    ...
    while (i < lines.length) {
        if (isMarker(lines[i])) {
            ...
            if (rec && rec.opType === PROG_KEY) {                                // t812 — the program-level slot: restore the xform / entry siblings (opFromMarker would drop a non-builder type)
                ...
                if (opRaw && rec.comment) opRaw.comment = rec.comment;   // t2289 — same shape, the op's OWN attached note
                if (opRaw) {

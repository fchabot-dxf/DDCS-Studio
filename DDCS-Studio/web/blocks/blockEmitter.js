...
// t2633 — a full vocabulary sweep (dispatched after t2631's own fix, on the reasoning that a hand-maintained
// list found wrong once is likely wrong elsewhere too) found THREE more transparent containers this set had
// never learned: `grid_container`/`tab_group`/`tab_page` (all `emit: (params, children) => children || []`,
// confirmed at gridContainer.js:20 / tabGroup.js:10,18 — the identical shape as group_box just above). Latent
// today (no comment-opening op declares one yet), same failure mode as the corner bug if one ever does.
const TRANSPARENT_CONTAINERS = new Set(['param_group', 'guard', 'section', 'setup', 'safetraverse', 'opunit', 'split_horizontal', 'split_vertical', 'group_box', 'grid_container', 'tab_group', 'tab_page']);
...
// t2633 — the SAME vocabulary sweep found seven more `emit: () => []` metadata-only leaves this set had never
// learned: `layout` (layout.js:9 — the lathe_profile metadata node, ON the arc's own declared vocabulary list),
// `path_anchor` (pathAnchor.js:17), `cam_table` (camField.js:26), and the four `shape_rect/circle/line/marker`
// primitives (vizBlocks.js, its own SHAPE_2D_TYPES set — confirmed `emit: () => []` at each declaration) and
// the five `form_dropdown/checkbox/segmented/diagram/action_btn` controls (formControls.js, all `() => []`).
const METADATA_ONLY_LEAVES = new Set(['param_field', 'formfield', 'layoutwidget', 'length_handle', 'point_handle', 'rect_handle', 'radial_handle', 'scale_handle', 'shear_handle', 'proj_length_handle', 'probe_vector_handle', 'diag_aim_handle', 'cross_aim_handle', 'cam_field', 'sim', 'preview3d', 'feature_canvas', 'simstart', 'param_table', 'usage_text', 'code_preview', 'field_ref', 'layout', 'path_anchor', 'cam_table', 'shape_rect', 'shape_circle', 'shape_line', 'shape_marker', 'form_dropdown', 'form_checkbox', 'form_segmented', 'form_diagram', 'form_action_btn']);   // t2511 — preview3d: the 3D-only half of the sim/panel split, same metadata-only shape as sim; t2515 — 'panel' renamed 'feature_canvas'; t2517 — length_handle: BACKLOG #71 pilot, same metadata-only shape as layoutwidget; t2521 — point_handle/rect_handle/radial_handle: same shape; t2533 — scale_handle/shear_handle: same shape; t2543 — param_table: materializeParamGroup's own new SEPARATE injection target (BACKLOG #71 owner ruling) is NOT in TRANSPARENT_CONTAINERS the way the old param_group was, so without this it stops firstRealLeaf cold on every twin materialize now reaches (corner/homing etc.) — caught live by op-title-2363.test.mjs's own buried-title canary, not assumed; t2573 — diag_aim_handle: same metadata-only shape as probe_vector_handle; t2583 — cross_aim_handle: same metadata-only shape as diag_aim_handle; t2631 — usage_text/code_preview/field_ref: same class, see the header comment above; t2633 — layout/path_anchor/cam_table/shape_*/form_*: same class, see the header comment above

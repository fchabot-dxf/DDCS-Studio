...
export { BUILDERS };   // re-export the leaf registry so importers reading it off opStacks keep working through the split
...
// ── form-vs-blocks DIFF surface → opGlow.js ─────────────────────────────────────────────────────────────────
// isOpBlockEdited / editedLinesForOp / editedRangesForOp (+ stripBlockIds + the LCS override-diff helpers) moved
// to opGlow.js — the glow / chip / Merge-gate are the same forward-only diff. Re-exported here so importers
// reading them off opStacks keep working through the split.

...
const bool = (param, label, dflt) => ({ type: structCtlType(param), label, category: 'Wizard UI', kind: 'structctl', structParam: param, defaults: { value: !!dflt }, fields: ['value'], _options: null, emit: () => [] });
const enu = (param, label, dflt, options) => ({ type: structCtlType(param), label, category: 'Wizard UI', kind: 'structctl', structParam: param, defaults: { value: dflt }, fields: ['value'], _options: options, emit: () => [] });

/** One control block per struct binding — MUST match CORNER_STRUCT_BINDINGS (asserted). Bool → checkbox; enum → dropdown. */
export const STRUCT_CTL_BLOCKS = [
    bool('probeZFirst', 'Probe Z First', false),
    enu('travelApproach', 'Travel', 'auto', [['Auto', 'auto'], ['Manual', 'manual']]),
    enu('wcs', 'WCS', 'active', [['Active', 'active'], ['G54', 'G54'], ['G55', 'G55'], ['G56', 'G56'], ['G57', 'G57'], ['G58', 'G58'], ['G59', 'G59']]),
    bool('syncA', 'Dual-Gantry Sync A', false),
    enu('corner', 'Corner', 'FL', [['Front-Left', 'FL'], ['Front-Right', 'FR'], ['Back-Left', 'BL'], ['Back-Right', 'BR']]),
    enu('probeSeq', 'Probe Order', 'YX', [['Y then X', 'YX'], ['X then Y', 'XY']]),
];

 ...
 * placeable separately in the `uiChildren` tree, the same way `panel` (2D-only, wizards/ops/panel.js) already
 * stands alone. This block is the 3D-only half of that split — same fields as `sim` (this IS its replacement,
 * not a second thing beside it: a migrated op declares `preview3d` instead of `sim`, not both — see
 * `simIntentFromStack`'s own comment for why it recognises both types), paired with `panel` as the visible
 * authoring layer for the box.
 *
 * Emits NOTHING: it's metadata read at SAVE time (devMode -> def.sim, via simIntentFromStack) and at RENDER
 * time (formWidgets.js's own `traverse()`, which builds the box — combined with an ADJACENT `panel` node when
 * one follows/precedes it in the same uiChildren list, standalone otherwise — see that file's own header

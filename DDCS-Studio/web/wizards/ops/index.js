...
export const PALETTE = [
    ...
    paramBlock, regionPickBlock, coordListBlock, featureCanvasBlock, codePreviewBlock, usageTextBlock, pathAnchorBlock, layoutBlock, splitHorizontalBlock, splitVerticalBlock, gridContainerBlock, tabGroupBlock, tabPageBlock, groupBoxBlock, formDropdownBlock, formCheckboxBlock, formSegmentedBlock, formDiagramBlock, formActionBtnBlock, shapeRectBlock, shapeCircleBlock, shapeLineBlock, shapeMarkerBlock, cornerGridPickerBlock, regionPickFieldBlock, toolLibraryPickerBlock, threadPresetPickerBlock, declaredIoPickerBlock, sliderFieldBlock, stepperFieldBlock, simBlock, preview3dBlock, simStartBlock, formFieldBlock, layoutWidgetBlock, lengthHandleBlock, pointHandleBlock, userRootBlock, sectionBlock, opUnitBlock, ...STRUCT_CTL_BLOCKS, // Wizard UI (GUI param knob + region-pick + coordinate-list + feature-canvas + layout-splitters + grid/tab/card containers + dropdown/toggle/segmented controls + SVG diagrams + region/tool/thread/IO pickers + slider/stepper inputs + preview-rig + per-pass sim-start declarations + FORM value-field blocks + LAYOUT-2D widget blocks + length-handle block + titled concern-section + declared sub-unit boundary + structural-control blocks; t2507 — the LAYOUT-2D preview box (layout2dCanvasBlock) deleted, wired but never useful; t2511 — preview3dBlock: the 3D-only half of the sim/panel split, pilot on surfacing; t2515 — panelBlock renamed featureCanvasBlock, type 'panel' -> 'feature_canvas', see featureCanvas.js's own header; t2517 — lengthHandleBlock: BACKLOG #71 pilot, nests in feature_canvas's own mouth; t2521 — pointHandleBlock: BACKLOG #71's second gesture, same template)

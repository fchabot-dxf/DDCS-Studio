 ...
 *
 * t1740 — FACTORY, NOT A SINGLETON MODULE: `createUserOpView(ns)` returns an independent `{ view, setUserOpDef }`
 * whose DOM ids are ALL namespaced by `ns` (`ns=null` → the original, unprefixed ids — byte-identical to before)
 * and whose state (`_def`/`_seed`/`_readers`/`_mgr`/`_layoutSpots`/`_simStartFracs`/the throttle timer) is
 * CLOSURE-scoped per instance, not module-level. This exists so the Blocks-tab Wizard View pane can render through
 * the SAME renderer the "Open as modal" overlay uses (blocksApp.js) — with its OWN namespaced DOM scaffold and its
 * own state — without the two ever sharing a mutable variable or colliding on an element id (two elements can't
 * legally share `id="wiz_user_form"` in one document). `userOpView`/`setUserOpDef` below are the DEFAULT instance
 * (`ns=null`) — every existing caller (wizardManager.js, blocksApp.js's openLiveAsModal) is unchanged.
 */
...
// Pure (args only, no DOM/state) — shared by every instance, not part of the per-instance factory.
 ...
 *  Pure (reads only global settings, no per-instance state) — shared by every instance. */
...
/** wireAnimOverlay — pure w.r.t. instance state (container/fc/panel/starts/… are all arguments); shared. */
...
/**
 * createUserOpView(ns) — one INDEPENDENT instance: its own state, its own namespaced DOM ids (`ns=null` → the
 * original ids, unchanged). `id(base)` is the ONE place a bare id string becomes this instance's own.
 */
export function createUserOpView(ns, opts) {
    const id = (base) => (ns ? `${ns}_${base}` : base);
    const elNS = (base) => el(id(base));
    // t1740 — optional, host-owned write-back: called (param, rawValue) on every field input/change, mirroring
    // exactly what blocksApp.js's own delegated #blk-form listener already reads (e.target.closest('[data-param]'),
    // .dataset.param, .value) — the host decides what "write back" means (writeAuthoredValue for the Blocks pane);
    // this shared file stays ignorant of any Blockly workspace. Unset for the modal (createUserOpView(null), no
    // second arg) — byte-identical to before. Presence ALSO gates the sync-in-place render() path below: both
    // exist for the same reason (a continuously-live host, not an open-once modal) and are one opt-in, not two.
    const onFieldWrite = (opts && opts.onFieldWrite) || null;
    ...
    function spanAxisOf(spb) {
        ...
        if (spb.spanAxisFrom) {
            const host = elNS('wiz_user_form') && elNS('wiz_user_form').querySelector(`[data-param="${spb.spanAxisFrom}"]`);
    ...
    function writeSimStartFrac(def, i, world, stock, starts) {
        ...
        const writeSpan = (spb2, aM, worldPos) => {
            ...
            const fld = spb2.y && elNS('wiz_user_form') && elNS('wiz_user_form').querySelector(`[data-param="${spb2.y}"]`);
    ...
    function render() {
        ...
        // later field change goes through update(), not render()) — so this branch never fires for it; behavior is
        // byte-identical. A HOST that keeps this pane continuously live (re-rendering on every upstream canvas
        // change, INCLUDING the echo its own onFieldWrite call triggers) needs this or a field being typed into
        // loses focus on its own echo — the exact bug blocksApp.js's OWN flat/tree renderer already guards against
        // (formSig/syncFormValues, now shared from ui/formWidgets.js rather than duplicated here).
        if (onFieldWrite) {
            const userRootForSig = isTree ? _def.template.find((b) => b && b.type === 'user_root') : null;
            const sig = isTree
                ? formSig(binds) + '||' + JSON.stringify((userRootForSig && userRootForSig.uiChildren) || [], (k, v) => ((k === 'id' || k === 'dflt') ? undefined : v))
                : formSig(binds);
            if (host.__sig === sig && host.querySelector('[data-param]')) { syncFormValues(host, binds); return; }
            host.__sig = sig;
        }
        host.innerHTML = '';
    ...
    view = {
        ...
        update(mgr) {
            ...
            // t1305 — resolved BEFORE the tool, because a lathe probe's stylus is scaled against the bar this op shows.
            // (A stored def carries data, not functions, so the op's declared sim-stock comes from the registry.)
            let _simStock = null;
            ...
            if (!_opTool && _def && _def.latheTool === 'probe') {
                ...
                try { _opTool = { ...latheProbeTool(params.tipRadius, latheBarOf(_def, params, _simStock), _def.latheProbeAxis), _default: true }; } catch (_) { _opTool = { type: 'probe', dia: 6, _default: true }; }
    ...
    return { view, setUserOpDef };
}

// The DEFAULT instance — unnamespaced ids, byte-identical to every pre-t1740 caller (wizardManager.js's
// _userView/open(), blocksApp.js's openLiveAsModal). Every existing `import { userOpView, setUserOpDef } from
// './userOpView.js'` keeps working unchanged.
const _modalInstance = createUserOpView(null);
export const userOpView = _modalInstance.view;
export const setUserOpDef = _modalInstance.setUserOpDef;

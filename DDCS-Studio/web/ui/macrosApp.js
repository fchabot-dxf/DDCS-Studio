...
import { cornerSlot, edgeSlot } from '../data/probeToSlot.js';
    ...
    function renderCamBuilder() {
        ...
        host.innerHTML = _camPack.slots.map((slot, si) => {
            ...
            return `<div class="cam-slot" data-si="${si}" style="border:1px solid var(--border); border-radius:6px; padding:8px; margin-bottom:10px;">
                ...
                <div class="settings-row" style="margin-top:4px;"><button class="toolbar-btn settings-io" data-act="addf">＋ Add field</button><button class="toolbar-btn settings-io" data-act="refresh" title="Re-scan the macro body's #2600 mirror reads and regenerate the field list — keeps any default/range you've edited.">🔄 Refresh fields from macro</button><span style="flex:1"></span><select class="cam-op"><option value="drill">Drill</option><option value="bore">Bore</option><option value="slot">Slot</option><option value="corner">Probe corner</option><option value="edge">Probe edge</option></select><select class="cam-op-pat"><option value="circle">bolt circle</option><option value="grid">grid</option><option value="line">line</option><option value="rect">rectangle</option></select><button class="toolbar-btn settings-io" data-act="addop" title="Generate an op into this slot — fills a blank slot, or APPENDS another op (multi-op: drill + bore on the same pattern).">➕ Add op</button></div>
    ...
    if (camHost) {
        ...
        camHost.addEventListener('click', (e) => {
            ...
            if (a === 'addf') { slot.fields = slot.fields || []; const idx = camPack.nextParam(camPack.usedParams(_camPack)); if (idx == null) { alert('The #1100–1499 form-param pool is full.'); return; } slot.fields.push({ idx, label: '', units: '', def: 0, min: 0, max: 0, type: 1, var: '#' + (slot.fields.length + 1) }); saveCamPack(); renderCamBuilder(); }
                ...
                const PROBE = { corner: cornerSlot, edge: edgeSlot };
                const gen = PROBE[method] ? PROBE[method](camPack.usedParams(_camPack), off) : slotFromOp(method, pattern, camPack.usedParams(_camPack), off);
        ...
        camHost.addEventListener('change', (e) => {
            ...
            if (pat) pat.disabled = (e.target.value === 'slot' || e.target.value === 'corner' || e.target.value === 'edge');
        });
    }

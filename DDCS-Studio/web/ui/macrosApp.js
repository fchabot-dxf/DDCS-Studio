...
import { cornerSlot } from '../data/probeToSlot.js';
    ...
    if (camHost) {
        ...
        camHost.addEventListener('click', (e) => {
            ...
            if (a === 'addf') { slot.fields = slot.fields || []; const idx = camPack.nextParam(camPack.usedParams(_camPack)); if (idx == null) { alert('The #1100–1499 form-param pool is full.'); return; } slot.fields.push({ idx, label: '', units: '', def: 0, min: 0, max: 0, type: 1, var: '#' + (slot.fields.length + 1) }); saveCamPack(); renderCamBuilder(); }
                ...
                // Probe ops (corner, …) are single self-positioning sequences, not point-patterns — they ignore the pattern select.
                const gen = method === 'corner' ? cornerSlot(camPack.usedParams(_camPack), off) : slotFromOp(method, pattern, camPack.usedParams(_camPack), off);
        ...
        camHost.addEventListener('change', (e) => {
            ...
            if (pat) pat.disabled = (e.target.value === 'slot' || e.target.value === 'corner');
        });
    }

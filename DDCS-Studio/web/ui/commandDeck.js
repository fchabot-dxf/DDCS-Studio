...
function wizItemOnclick(e) {
    if (e.kind === 'user') {
        // A user op that DECLARES a canvas panel opens the CANVAS/wizard path so its FeatureCanvas + drag handles (and,
        // for form3d+2d, the 3D sim) render (openWiz → wizardManager.open → userOpView → renderLayout2D/preview3D). The
        // wizard's shared Insert still commits the op (userOpView.update recordOp → wizardManager.insert → commitActiveOp
        // — the SAME accumulate path the quick form uses), so it both RENDERS and INSERTS. Everything else stays on the
        // fast quick-insert form (no regression). Scoped to the EXPLICIT canvas panels ('form2d', 'form3d+2d'); NOT plain
        // 'form3d' — that is the DEFAULT panel, so routing it would move EVERY panel-less user op off the quick form.
        const panel = e.def && e.def.panel;

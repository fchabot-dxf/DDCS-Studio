...
(async () => {
    ...
    const p = rects.path;
    await page.screenshot({ clip: { x: p.r - 140, y: p.y - 60, width: 260, height: 200 }, path: 'scratchpad/adv-ct-tr-att.png' });
    await page.screenshot({ clip: { x: p.x - 60, y: p.b - 140, width: 260, height: 200 }, path: 'scratchpad/adv-ct-bl-att.png' });
    await browser.close();
})();

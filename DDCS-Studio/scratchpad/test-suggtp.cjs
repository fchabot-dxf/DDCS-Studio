...
(async () => {
  ...
  console.log('after setup, opts:', JSON.stringify(await opts(p)), '| status:', await status(p));
  // click a toolpath suggestion by label if present
  for (const name of ['Line','Drill','Pocket','Contour']) {
    const loc = p.locator('.blk-sug-opt').filter({hasText:name}).filter({visible:true}).first();
    if (await loc.count()) { console.log('clicking suggestion:', name); await loc.click().catch(e=>console.log(' err',e.message.split('\n')[0])); await p.waitForTimeout(1500); break; }
  }
  console.log('after toolpath sug, status:', await status(p), '| opts:', JSON.stringify(await opts(p)));
  await p.screenshot({path:path.join(OUT,'suggtp.png')});
  await b.close();

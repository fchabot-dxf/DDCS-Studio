...
(async () => {
    ...
    await page.evaluate(() => { const s = window.ddcsGetSettings(); s.stock = { show: true, x: 200, y: 150, z: 25, datum: 'ccp' }; s.preview = s.preview || {}; s.preview.autoLoop = false; });
    ...
    const p = rects.path;
    await page.screenshot({ clip: { x: p.r - 45, y: p.y - 25, width: 70, height: 60 }, path: 'scratchpad/adv-ct-tr-ccp.png' });
    await page.screenshot({ clip: { x: p.x - 25, y: p.b - 35, width: 70, height: 60 }, path: 'scratchpad/adv-ct-bl-ccp.png' });
    await browser.close();
})();

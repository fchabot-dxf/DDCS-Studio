...
## 🔨 turn 393 (cycle 127) — ROTARY PORT SCOUT (fan-out #3; NO CODE — map first). Deliverable: inherit-vs-specific · the round-bar/RIG workpiece split · the E-decomposition + port order · forks. Method: a 5-way read-only Workflow fan-out (rotary-wizards · round-bar-stock · rig-fixture · pipeline-inherit · ground-truth); the round-bar agent hit the StructuredOutput retry cap → I mapped that area directly. NO code touched.

### 1. WHAT ROTARY IS (both wizards are ALREADY block-stack builders — not hand-rolled text)
- **rotary_center (Centreline / 4th-axis Y0+Z0)** — `rotaryCenterStack(params)` (rotaryCenterWizard.js:20) accumulates an atom array + `emitMapped(...).text`, SAME spine as edge/middle/corner. COMPOSES the hardened `probeSurfaceStack` (rotaryCenterWizard.js:50-56) + `safeZParkBlock` (:138). Scalars #1-#6 (#6=radius) are a byte-for-byte clone of the edge/middle binding set.
- **rotary_clock (Clock A0 to a flat)** — `rotaryClockStack` (rotaryClockWizard.js:17): two Z-down touches a span apart → tilt `phi=ATAN[[#52-#51]]/[#6]` → A-axis datum. HAND-ROLLS its two-pass probe (`ppZdown`, :45-48, no radius comp — a flat cancels it). #6 here = SPAN (not radius).
- **NO data-op twin exists for EITHER** — no rotaryCenterData.js/rotaryClockData.js, not in app.js:145-152 seed, not in wizardLibrary opensAs. This port is **greenfield twin creation** on the edge/middle template.
- **The two are DISJOINT — neither is a subset of the other, and they COLLIDE on #6 (radius vs span)** → each needs its OWN DEFAULTS/BINDING_SPECS; no shared spec across the pair.

### 2. INHERITS (≈100% of the hardened pipeline, VERBATIM) vs ROTARY-SPECIFIC
**INHERITS with ZERO new machinery:** `userOpFromStack` + `registerUserOp` + `instantiate`(pruneGuards→deriveBindings) (userOps.js) · `def.bindingSpecs` by #var identity (deriveBindings.js:33 — rotary's #1-#6/#17 are already clean `#N=` assigns) · the superset+GUARD prune mechanism · `opensAs` in-place swap + `builtinLabelForTwin` seamless title (wizardLibrary.js:119-120) · `def.simStartsProvider` (BUILT_IN.rotary_center already exists, opSimStarts.js:86) · the readonly flag + `seedKnobExpose` (keys off bindingSpecs.match.var — auto-ticks rotary knobs) · `applyProbeSources` postInstantiate (rotary ALREADY sources retract/fastFeed/port via srcVal/srcNote) · BUILDERS+SCHEMA already register both (opBuilders.js:38, opSchema.js:120-129 — the legacy shim stays).
**ROTARY-SPECIFIC:** the 3-point circle-FIT solver (determinant #60-#63→Yc/Zc/R, rotaryCenterWizard.js:112-134 — unique in the codebase) · the operator-jog `reposition()` between fit passes (deliberately G53-free to keep per-pass markers distinct, :57-67) · known-method flank geometry (probe a solid bar from OUTSIDE at centreline height, #11/#12) · **the A-AXIS as a datum register** (clock: RM('A'), SWO('A'), `MV('A',#58)` — the FIRST A-axis binding the twin infra ever sees; rides shared dialect atoms so no bespoke motion code) · the Z0-datum fork (OD-top #50 vs centreline #56) · the round-bar stock coupling (§5).

### 3. THE ONE MACHINERY GAP + two byte-clean source-restructures ([[restructure-source-not-abstraction]] — teach the wizard, keep the data-def dumb)
- **GAP (the only real blocker):** `rotaryCenterStack`/`rotaryClockStack` take NO `opts` and have NO `{superset:true}` branch — their forks are hand-coded if/else. To port as LIVE toggles they must be refactored to emit GUARD-wrapped arms (the `middleStack({superset:true})` GUARD/fork-helper pattern) so `pruneGuards` collapses them; `superset:false` stays byte-identical (the middleWizard.js:38 invariant).
- **Fan-out #1:** `#55 = diameter/2` computed in JS at build (rotaryCenterWizard.js:86) — a value binding would inject raw Ø. Dissolve in SOURCE like corner's #17: `#dia=diameter` + `#55=[#dia/2]` → Ø becomes a clean single-socket knob.
- **Fan-out #2:** safeZ lands via build-time `Math.round(safeZ)` (:77) — a fractional safeZ diverges. Drop the round (middle's #17=safeZ unrounded is the precedent).
- **Fork KINDS to guard:** center = method(known|fit)+approach(auto|guided) TRUE structural + datum(top|center)/wcs VALUE-swap; clock = action(set|report|rotate)+reference(top|side) + safeZFrame(relative|machine, a block swap). rotary WCS is a PURE value-swap (no base-address block) — do NOT copy middle's 7-way wcsFork.

### 4. GROUND-TRUTH — **the headline finding: there is NO dumped rotary-probing macro anywhere**
Grep across the ddcs-expert skill, the firmware backup, example-macros, and appcode/snippets.nc returns ZERO rotary-probe hits — the only 4th-axis firmware content is A-axis HOMING (fndA.nc = M98P501X3). So **"byte-identical to the DDCS ground truth" resolves to "byte-identical to the current `emitMapped(rotaryCenterStack/rotaryClockStack)` output"** — the port must **golden-snapshot the existing wizard emit as the baseline**, NOT chase a dump. WHY it's still safe: every PRIMITIVE the rotary ops fold to is dump-CONFIRMED via the Expert dialect citations (G31 `..Q1`, status `IF #192x!=2`, trigger `#192x`, DRO `#88x`, stride-5 WCS `#[805+..]`, G53-with-#var, two-operand `ATAN[dy]/[dx]`, SQRT). What is Studio-ORIGINAL / not machine-validated: the rotary SHAPE — the 3-point FIT solver (flagged ADVANCED/verify-on-machine; its operator-jog is un-simulated) and the A-clock. Extrapolated (not directly dumped): the A-axis WCS column `#808 = base+3` (only X#805/Y#806 shown in SAVE_WCS_XY_AUTO.nc). Neither rotary op probes IN A (a solid bar can't be probed on its own axis) though the controller CAN (`#1923/#1928` confirmed). TRAP: the firmware's own `3D PROBE G55.nc` uses G10 L20 = BROKEN on M350 — never an emit reference. And do NOT collapse the two-operand `ATAN[..]/[..]` (the `/[#6]` is atan's 2nd operand).

### 5. THE WORKPIECE SPLIT (advisor part-2: "material → workpiece; fixture → sim-device")
- **The ROUND BAR → `outer.shape:'round'` + `outer.d` ALREADY EXISTS** — `projectWorkpiece` maps `cylinder→round`, `diameter→outer.d` (workpiece.js:93,95); `barRadius(stock)=stock.diameter ?? min(cross)/2` (probeGeometry.js:40) is the shared radius the rig + sim-starts already read. So the model is ready.
- **RETIRE `activateCylinderStock`/`restoreBoxStock` — the 2nd stock.shape MUTATOR (the exact analogue of middle's retired `syncStockShape`).** `activateCylinderStock()` (rotaryCenterView.js:15-26) MUTATES global stock to cylinder on rotary_center OPEN (squares cross-dims to one Ø, remembers `_savedBoxStock`); `restoreBoxStock()` (:31-38) reverts, and is defensively called by rotary_clock/edge/middle views on open ONLY to undo that mutation. The diameter is PULLED read-only from the forced cylinder (update:65-74). **Retirement (mirrors middle):** the twin READS `getWorkpiece().outer` (shape:'round'+d, via barRadius) for the sim backdrop + the Ø knob — it does NOT mutate stock on open; the user DECLARES the round bar (like middle's pocket). Once activate is retired, the scattered `restoreBoxStock` reverts become no-ops → dead cleanup (defer, like syncStockShape's legacy-view retirement was deferred).
- **RIG (chuck+tailstock) → DECOUPLE from setStock, STAY in the sim-device layer.** Declared cleanly (opSimContext.js:20 `ROTARY_RIG`→showRotaryRig, same opt-in bridge as the ATC magazine), but the MESH is built INSIDE `setStock` (gcodeViz3d.js:1186-1193) + disposed there (:1106) + attached to the SPINNING `_partGroup`; `setRotaryFixture(on)` (:1261-1267) just flips a flag + calls `setStock` to rebuild ("rebuild the stock so the rig appears"). **Decouple:** a new `setRotaryRig(spec|null)` mirroring `setMagazine`'s dispose/guard/build shape — **but KEEP the rig group a child of `_partGroup`** (NOT this.scene like the siblings) so it inherits spin (`_applyPartRotation` only spins _partGroup) + datum/WCS placement (pg.position). Spec derived inline from stock+getRotaryAxes+barRadius = byte-identical; later lift into opSimContext's declared geometry (the module's own stated future, :13-16). Keep the A-jog + DRO-row UI in setRotaryFixture (orthogonal to the mesh). RISK: attaching to this.scene = spin-loss + placement-drift + golden-image break.

### 6. THE E-DECOMPOSITION + PORT ORDER (mirror middle/edge; Centreline as the pilot)
```
PHASE A — CENTRELINE (the pilot: carries EVERY mechanism + the workpiece-pivot payoff)
  RC-E0  golden-snapshot the current emit (no dump) -> superset:true + GUARD arms
         (method/approach/datum/safeZFrame) + dissolve the #55=dia/2 & safeZ-round
         fan-outs in SOURCE.  superset:false == golden byte-for-byte.  GATE: byte-diff=0.
  RC-E1  rotaryCenterDataDef via userOpFromStack; BINDING_SPECS #1-#6/#17 by #var
         identity + optional method-gated #55/#11/#12; STRUCT_BINDINGS. byte-emit test
         vs the built-in across combos + FRACTIONAL dia/safeZ sweep.
  RC-E2  bespoke def.simStartsProvider = the existing BUILT_IN.rotary_center; sim block
         {rotary:true,machine:true}.  + ROUND-BAR READ: retire activateCylinderStock ->
         read outer.shape:'round'+d (barRadius) for backdrop + the O knob.
  RC-E4/5 the rotary datum glyph (round bar + flank-approach + OD-top/centreline marker) *
         opensAs:'user_rotary_center_data' + seamless title * app.js seed * RELEASE.
PHASE B — RIG DECOUPLE (sim cleanup; parallel/after A) — setRotaryRig(spec|null) under
         _partGroup, byte-identical golden images.  (advisor part-2 fixture split.)
PHASE C — CLOCK (the simpler A-axis inheritor of the hardened rotary superset)
  CL-E0/1 superset + GUARD arms (action/reference/safeZFrame); rotaryClockDataDef;
         span->#6 clean binding; the FIRST A-axis binding (prove RM/SWO/MV('A') byte-id).
  CL-E2  single-start parity (clock has no multi-pass inferStarts today).
  CL-E4/5 opensAs in-place + title + seed + RELEASE.
```

### 7. FORKS (flagged for the advisor/human — recommendations attached)
| # | Fork | Recommendation |
|---|---|---|
| F1 | **Port ORDER: Centreline-first vs Clock-first** (the two mappers SPLIT) | **Centreline-first** — it carries the workpiece-pivot payoff (round-bar read + retire activateCylinderStock, the whole reason rotary matters to the pivot) AND exercises every mechanism (guards, optional bindings, fan-out dissolution, bespoke sim provider, probeSurfaceStack), so the scaffolding is proven on the real hard case; clock then drops in as the A-axis inheritor. COUNTER (rotary-wizards agent): clock-first de-risks the NEW twin scaffolding on a ~half-size emit before hitting center's fit solver + flank geometry. HUMAN CALL. |
| F2 | **Byte baseline** (no dumped rotary macro) | **Golden-snapshot the current wizard emit** before porting — the primitives are dump-confirmed, the rotary SHAPE lives only in the wizard. (Not really a fork — the answer.) |
| F3 | **Round-bar declaration: grow the modal's round-OUTER editing now, or defer** | **Defer the modal round-outer GUI** — the twin READS outer.shape:'round'+d (already exists via the "Rotary cylinder Ø3″" template); retire the auto-mutation so the user declares the bar (like middle's pocket). Modal round-outer drag-editing = a later slice. |
| F4 | **RIG attach point** | **Keep the rig group under `_partGroup`** (inherits spin+placement, byte-identical) vs a dedicated group needing re-plumbed rotation. |
| F5 | **RIG spec source** | **Derive inline** from stock+getRotaryAxes+barRadius NOW (byte-identical) → lift into opSimContext declared geometry as a separate later step. |
| F6 | **Clock sim-starts** | **Single-start parity** for the port; a 2-pass A/B provider is a separate enhancement (would change the marker count). |
| F7 | **method known+fit + A-writes** | Port BOTH known+fit together but preserve FIT's ADVANCED banner (don't alter its solver bytes); port the A-writes (#808, rotate move) AS-IS byte-identical, keeping the existing verify-on-machine hedges — flag #808-extrapolation + rotary-vs-linear-config as known-verify items, NOT blockers. |

**RISKS (carry into the port):** the diameter binding isn't a pure scalar today (the view pulls it read-only from stock) → the twin must decide the #dia socket reads getWorkpiece().outer.d vs a plain knob (ties to F3) · rotary_clock schema/view inconsistency (opSchema maps `rcl_safe_z_frame` but the view inputIds omit it — reconcile the binding list) · the corner-gated-pilot "no ports until corner perfect" memory is STALE (corner released V10.50; edge V10.82/83; middle V10.85 — the fan-out is underway, rotary is next per the roadmap).

**GATE — NO CODE.** Pass back the map + decomposition + forks; the advisor rules order (F1) + the modal-scope (F3) + writes the full E-spec. QUEUED after rotary: Circular, then Alignment, then the HOMING WIZARD (feature/fix, not a port).

## 🔨 turn 395 (cycle 127) — COMPOSABLE-WIZARD-AUTHORING SCOUT (NO CODE, design-first). Deliverable: the map + the two-halves design (FORM value-field blocks + GUI layout-2D widget-blocks) + the pilot spec + forks. Method: a 5-way read-only Workflow fan-out (form-render · gui-derivation · blockly-roundtrip · anchor-model · prior-intent-docs; the prior-intent agent stubbed out → covered by the 2 memories + userRoot.js read directly). NO code touched.

### 0. THE UNIFYING INSIGHT (both halves are the SAME move)
The custom-op FORM and the 2D GUI canvas are ALREADY pure functions of ONE array — `def.bindings[]`:
- `renderOpForm(host, def.bindings)` (formWidgets.js:425) → the form fields (one row per binding, widget picked by `binding.widget`/`type`).
- `layoutSpecFromOp(def, params)` (panelTypes.js:88) → the 2D canvas (groups `def.bindings` by `binding.group`, picks a FeatureCanvas gesture from the `role` set + 3 magic param-names).
So "author the UI as blocks" = make `def.bindings` (+ its anchor kind) AUTHORABLE as blocks in the `user_root` **PRESENTATION mouth** (userRoot.js:5 — uiChildren, emits nothing), instead of hand-written `*_BINDING_SPECS` data / param-name sniffing. **A FORM-value-field block and a GUI-layout-widget block are BOTH "a block that contributes one binding record" — they differ only in WHICH keys they carry** (A: widget/label/section; B: group/role/anchor). That is the shared substrate.
```
   TODAY (opaque / sniffed)                    TARGET (authored as blocks)
   ┌─ *_BINDING_SPECS (hand data) ─┐           user_root
   │  deriveBindings (#var id)     │            ├─ PRESENTATION mouth ────────────┐
   └───────────────┬───────────────┘            │  [formfield  param=dist  #1  …] │ (A) value-field blocks
                   ▼                             │  [formfield  param=port  #5  …] │
              def.bindings[]  ──────────────►    │  [layoutwidget point fx fy      │ (B) layout-2D widget blocks
              ▲            ▲                      │     anchor={kind,frame}] …      │
   renderOpForm         layoutSpecFromOp         └─────────────┬───────────────────┘
   (the FORM)           (the 2D GUI, +magic-     bindingsFromStack / bindingsToBlocks
                         name sniff corner/       (NEW, mirrors simStartsFromStack/ToBlocks)
                         axis+dir/diagTravel)            └─► def.bindings[]  ─► SAME two consumers
```
A form-field block and a layout-widget block are COMPLEMENTARY, not alternatives: a canvas handle only renders when its param is ALSO a live form field (the `_writable` gate, panelTypes.js:48) — e.g. middle's diagTravel is BOTH a readonly form field AND the ② handle. "form-field block" = this param has a form control; "layout-widget block" = this param(s) also has a canvas handle.

### 1. HALF A — FORM value-field blocks
- **THE CONSUMER SEAM is complete:** `renderOpForm` + FORM_WIDGETS (11 widgets: number/slider/dropdown/segmented/toggle/text/corner-grid/region-pick/coord-list/xy-pad/rect) already draw+read ANY binding record; nothing new on the render side. A field-block must PRODUCE one binding: `{param, type, default, widget, label}` minimum + `widgetConfig` (enum options / min-max-step-units) + optional `help, units, section, when:{param,is}, readonly, match:{type,var}, key}`. This is the MIDDLE_BINDING_SPECS row shape (middleData.js:52-69) made VISUAL.
- **The value round-trip already works both ways:** form→params via `read()`→`{[param]:v}` (formWidgets.js:93); params→socket via `instantiate` at (blockIndex,key) (userOps.js:293); live form→block via `writeAuthoredValue` (devMode.js:235).
- **GREENFIELD (the actual work):** a `formfield` block TYPE (no single block today carries label/section/help/min/max/units/when — the `param` reporter, param.js, carries only param/widget/default/options) + the forward/reverse `bindingsFromStack`/`bindingsToBlocks` (neither exists) wired via a `resolveBindingsMeta(def)` in registerUserOp (mirror resolvePanelMeta, userOps.js:236). **The blocker this UNBLOCKS:** `isMaintainedAsData` (devMode.js:542) currently REFUSES the visual Save/Update for any bindingSpecs def (corner/edge/middle) because today's collectAuthoring→buildBindings FLATTENS rich bindings (drops section/help/when/match/group/role). A LOSSLESS field-block round-trip is exactly what lifts that lock → the ports become re-authorable.
- **Socket link:** carry `match:{type,var}` + `key` (the BINDING_SPEC model), NOT a param-name join — deriveBindings already resolves match-by-#var-identity over the pruned stack (deriveBindings.js:33), survives prune/index shifts, and is the ONLY shape that round-trips the maintained-as-data ports + prune-gated optionals.

### 2. HALF B — GUI layout-2D widget-blocks
- **TODAY it is AUTO-DERIVED, not authored:** `layoutSpecFromOp` picks a gesture from (a) the binding ROLE set (x+y→point, +w+h→rect, +dia→circle, +len→length, ax+ay+bx+by+width→projLength, x+y+scale→scaleX, +slant→shear) and (b) THREE magic param-NAMES — `corner`→corner-pick (panelTypes.js:298), `axis`+`dir`→edge-pick (:272), `diagTravel`+`diagPrimary`→②-aim (:230). The generic target every gesture funnels through is `CANVAS_GESTURES[type].place(d)→{x,y,kind,label}` / `.drag(d,world)→{param:value}` + `buildCanvasWidgets(decls,setFields)→{handles,onDrag}` (canvasWidgets.js:27,115).
- **The param-WRITE loop closes with ZERO new code:** FeatureCanvas pointermove → onDrag → gesture.drag → `{param:value}` → setFields → `_writeParam` dispatches a bubbling 'input' on `#wiz_user_form [data-param]` (panelTypes.js:49) → userOpView.update() re-derives. Symmetric: type a field → re-derive handle; drag handle → write field.
- **A layout-widget block should CONTRIBUTE `{group, role, param, anchor}` bindings** (reuses the ENTIRE existing derivation — _writable guard, pass-number labels, workpieceBackdrop) rather than emit raw decls (which would fork a parallel decl path needing its own anchor/writable logic). The block↔binding round-trip HALF-exists: a param-pill's `widget` is a role-encoded token ('point-x','nrect-w') decoded by `decodeCanvasWidget`→`groupCanvasBindings` (userOps.js:70-105) into the SAME {group,role} bindings.
- **POINT-PICK has NO auto-trigger (grep: no onPointPick/clickToPlace) → the clean GUI pilot:** a plain draggable x,y in stock-min-XY, drag-only (the `point` gesture already exists, needs zero FeatureCanvas changes).
- **Target set + status:** point-pick (drag, pilot) · corner-pick (discrete, EXISTS via magic-name) · edge/wall-pick (2 enums, EXISTS) · value-handle (1-D scalar, EXISTS) · feature-pick (**greenfield** — features draw as backdrop but there is NO `_hitFeature`/onFeaturePick click gesture).

### 3. THE ANCHOR CRUX (GUI only — how a widget declares WHERE it lives)
- **The canvas frame is ONE:** stock-min-XY, X-right/Y-up, WCS zero at the stock min corner, stock spans [0..w]×[0..h] (featureCanvas.js:15-18). `spec.origin` (the datum crosshair) is **DISPLAY-ONLY** — it never re-frames handles (panelTypes.js:91-96).
- **Today the anchor KIND is SNIFFED (magic param-names), never declared.** The proposal: **DECLARE `anchor:{kind:'point'|'corner'|'edge'|'value'|'feature', frame:'stock-min'|'datum'|'pass', ...kindParams}`** on the group's primary binding. deriveBindings already forwards arbitrary passthrough fields (deriveBindings.js:74-80) → `if (s.anchor) b.anchor = s.anchor` is a ONE-LINE add; layoutSpecFromOp switches on `b.anchor.kind` instead of magic names. Keep the name-sniff FALLBACK during migration so corner/edge/middle stay byte-identical until they carry the field.
- **Point-pick FRAME (a correctness fork, not cosmetic):** store PHYSICAL stock-min-XY (datum-invariant, feeds the emit as-is, `ax=0`) and DERIVE the datum-relative number for DISPLAY only — exactly the settled datum model ([[datum-model-physical-derived-offset]]: offset = physical − datumXY; datum display-only → emit byte-identical). `frame:'datum'` would change only the shown/edited number, never the stored param. Default `frame:'stock-min'` = every existing op byte-identical.
- **Discrete enum anchors (corner/edge/feature) need their OWN small block field**, NOT the numeric role-widget dropdown — extractParamBlocks forces `type:'number'` on pills (userOps.js:136-141) so an enum anchor can't ride the canvas-widget encoding (this is WHY corner/edge fall back to name-sniffing today). Template: the 9-point stock-attach `frac(code2)` discrete-anchor family (placement.js:14, featureCanvas.js:450). Feature-pick resolves by stable feature `id` (workpiece.js:64), re-deriving pos live — valid-by-construction when the feature is edited.

### 4. SHARED vs DISTINCT machinery
**SHARED (build ONCE, both halves need it):** `bindingsFromStack(children)` + `bindingsToBlocks(def.bindings)` — the forward/reverse round-trip, mirroring `simStartsFromStack`/`simStartsToBlocks` (userOps.js:173/190) · `resolveBindingsMeta(def)` in registerUserOp (mirror resolvePanelMeta) · the block lives in the user_root PRESENTATION mouth · the socket-link by match+key · rich non-field state (widgetConfig/when/anchor) rides `block.data` (the `_expose` precedent, stackBridge.js:238) so it survives reprojection.
**DISTINCT:** A carries widget/label/section/help/units + match/key (consumed by renderOpForm; NO anchor — the simple case). B carries group/role + `anchor:{kind,frame}` (consumed by layoutSpecFromOp; adds the anchor declaration + replaces the magic-name sniff; discrete kinds add an enum-block field; feature-pick adds `_hitFeature`).
**Block-registration is FREE for both:** a new authoring block = a plain op-def dropped in PALETTE (wizards/ops/index.js:74); `jsonDef` auto-compiles it (bridge.js:187). Metadata blocks (`emit:()=>[]`) get an explicit `kind` + must be added to `devMode.isAtom`'s exclusion (devMode.js:52) so their fields don't grow the knob kit.

### 5. THE TWO PILOTS (advisor lean CONFIRMED — form-field first, then point-pick; they share the round-trip core but the anchor is a distinct concern worth isolating)
```
PILOT 1 — FORM value-field block (SIMPLEST: no anchor, no canvas — proves compose-a-section-as-blocks)
  a `formfield` block in the PRESENTATION mouth carrying {param, widget, label, default, +widgetConfig, match/key}.
  BUILD: the block def + bindingsFromStack/bindingsToBlocks + resolveBindingsMeta wiring.
  VERIFY (real-symptom + assert-the-value): author 2-3 field-blocks in Blocks -> they RENDER in the live form
  (renderOpForm draws them) -> round-trip (Blocks->def->Blocks re-renders the same blocks, LOSSLESS -
  section/help/when/match preserved) -> the value still lands in the stack (instantiate) -> emit BYTE-IDENTICAL.
  PAYOFF: unblocks isMaintainedAsData (devMode.js:542) - the ports become re-authorable.
PILOT 2 — GUI point-pick widget block (adds the ANCHOR + the full spatial loop)
  a `layoutwidget` block carrying {fx, fy, anchor:{kind:'point', frame:'stock-min'}} expanding to two
  {group,role:x/y} bindings. Drag-only, absolute (NO relTo, NO click-to-place).
  VERIFY: add block -> bind fx/fy -> a draggable point renders on featureCanvas -> drag writes fx/fy
  (assert the VALUES = the dragged world coords) -> round-trip -> emit BYTE-IDENTICAL (sim/form-only).
```
THEN (later slices): declare `anchor` on corner/edge/②-aim to retire the magic-name sniff (byte-identical, keep the fallback) · feature-pick (+ `_hitFeature`) · structural-toggle blocks (DEFERRED per the dispatch).

### 6. FORKS (recommendations attached)
| # | Fork | Recommendation |
|---|---|---|
| F1 | **Pilot order / one-vs-two** | **TWO pilots, form-field FIRST** — they share the bindingsFromStack/ToBlocks core, but the form-field proves the round-trip cheaply (no anchor/canvas) + unblocks isMaintainedAsData; point-pick then adds the anchor + spatial loop incrementally. |
| F2 | **Socket link** | **Carry `match:{type,var}`+`key`** (BINDING_SPEC, resolved by #var identity) — survives prune/index shifts + round-trips the maintained-as-data ports; NOT a param-name join. |
| F3 | **Interception** | **Stored-in-template** (the committed target, "don't forget B") — the field/widget block LIVES in the PRESENTATION mouth + is read at register time; NOT author-time collectAuthoring. |
| F4 | **Form-field granularity** | **ONE generic `formfield` block** with a `widget` dropdown (reuse the param widget list, bridge.js:153) + `ddcs_dynfields` to show only the chosen widget's config; split per-widget only if a config can't fit ([[wizard-atom-granularity-ask]]). |
| F5 | **Widget block: bindings vs raw decls** | **Contribute {group,role,anchor} bindings** — reuses the whole layoutSpecFromOp derivation + the round-trip; raw decls fork a parallel path. |
| F6 | **Point block shape** | **ONE block (fx+fy) expanding to two role-bindings** — matches "a point", avoids the incomplete-group degradation (userOps.js:99). |
| F7 | **Point coords frame** | **Store PHYSICAL, derive datum-relative for DISPLAY** — the settled datum model → emit byte-identical; `frame:'datum'` changes only the shown number. |
| F8 | **Anchor kind** | **DECLARE `anchor:{kind,frame}`** (free inert data; deriveBindings forwards it) + keep the magic-name sniff fallback during migration. |
| F9 | **Click-to-place** | **Drag-only** for the pilot (the `point` gesture exists; click-to-place needs a new _hitPoint with ambiguous start semantics — rule-of-three). |
| F10 | **Feature-pick ref** | **By stable `id`** (re-derive pos live) — valid-by-construction when the feature is edited. |

### 7. RISKS + GATE
**RISKS:** (1) the round-trip must be LOSSLESS before isMaintainedAsData's lock lifts — a field-block that drops when/group/role/section/help would silently DOWNGRADE corner/middle on re-save (devMode.js:536-542). (2) `renderOpForm` does NOT consume `section` yet (zero matches in formWidgets.js) — a section-declaring block is inert until renderOpForm grows section grouping (small add). (3) **Blockly is now 13.0.0** (drop-in from 12.5.1, geras renderer; API-NOTES updated) — the Class-B render trap holds: load new blocks via serialization, augment inside Events.disable/enable + triggerQueuedRenders; needs the render-guard spec ([[blockly skill]]). (4) `socketHeld` semantics (deriveBindings.js:71, formWidgets.js:94) — a field-block that always writes a numeric default overwrites per-structural-combo sockets → breaks byte-identity for the ports (the t114 class); carry socketHeld verbatim. (5) the magic-name sniffers must COEXIST with declared anchors during migration or double-render. (6) a layout-widget whose param isn't a live [data-param] form field produces NO handle (silent no-op) — the block must ensure its params render as settable fields.
**GATE — NO CODE.** Pass back the map + the two-halves design + the pilot spec + forks; the advisor rules the pilot order (F1) + writes the full build spec. DEFERRED (dispatch): structural-toggle blocks + corner/edge/②-aim anchor-extraction + feature-pick. ROTARY build stays PAUSED (map saved t393).

### 8. AMENDMENTS FOLDED (advisor t394, 2 — mapped via a focused 2-agent fan-out)

**AMENDMENT A — the DOUBLE-LAYER block structure (outer makeOp `op` group vs inner `user_root`).** A data-op in the Blocks tab renders as TWO nested C-blocks: OUTER `⬡ Middle Data` (the makeOp `op` wrapper, opSession.js:367 buildActiveOpStack → opBuilders.js:93 makeOp) nesting INNER `Define Custom Wizard` (the `user_root` def block, userRoot.js:12 — Presentation[panel/sim/param_group] + Execution[G-code]).
- **Each layer's ROLE:** OUTER `op` = the "op #N in the PROGRAM" unit — the ONLY carrier of the op `id`/`opType`/`requires`/`params` snapshot. INNER `user_root` = the editable wizard DEFINITION.
- **The outer is REDUNDANT-for-editing but LOAD-BEARING-for-model.** For a data-op the outer `op` block's GCODE mouth just holds `user_root` (which has its OWN mouths) and its SIM mouth is DEAD (empty simChildren, stackBridge.js:210) — pure envelope chrome when editing ONE wizard. But it is what the whole program layer keys on: **DEPENDS-ON** = the op-sequence view + editor ✎-chip (programModel.js findOpInStack `type==='op'`+id) · edit-in-place replaceOp/delete/duplicate/merge/setGroupChildParams (all findIndex on `type==='op' && id`) · reconcile/glow/Merge-Replace · emit-gating + the `( @DDCS )` marker round-trip · program accumulation (appendIntoProgram) · the Blockly serialization round-trip · even the FORM path (devMode.authoringBody finds `type==='op'` first, reads its children as the authoring body → a naive removal LOSES opType identity → save-as could DUPLICATE instead of UPDATE).
- **PROPOSAL (assessed, not guessed):** KEEP BOTH IN THE MODEL (a model-flatten breaks all of the above). For the CLEANEST view, **HIDE/DEMOTE the outer in the SINGLE-OP EDIT case only** — when the program is exactly [progstart?, op, progend?] for ONE `user_*` data-op, render `user_root` as the TOP-LEVEL editing surface (skip drawing the op C-block), carry the op record out-of-band, RE-WRAP via makeOp on reproject/commit. The multi-op PROGRAM view keeps the outer visible (it's the sequence unit + ✎-chip anchor). LOWER-RISK variant: keep the op block but drop its dead SIM mouth for user_ ops + render it as a compact titled band. ⚠ NOT Blockly `collapsed` (that hides user_root too — the opposite). This is CORE to composable-authoring: the FORM/GUI blocks (§1-§5) live INSIDE `user_root`'s Presentation mouth, so the editing surface should present `user_root` cleanly, not buried under redundant op chrome. NOTE: the two ROTARY wizards are greenfield (no data-op twin → no user_root structure yet; porting gives it — parked with the t393 map).

**AMENDMENT B — PREVIEW-PARITY BUG (Blocks-tab preview ≠ wizard preview WCS/anchor; flag to fix as part of the Blocks work).** Human screenshots: correct WIZARD preview = G54 Y work=−81, boss CENTRED; WRONG BLOCKS-TAB preview = G54 Y work=0, boss OFFSET + path on the right. Both hosts mount the SAME createPreviewPanel — only the INPUTS differ, and ONE derived flag decides the frame: `curAnchor = !forceMachine && !parsed.stats.absolute` (createPreviewPanel.js:461) → `v._anchorToStart` (gcodeViz3d.js:84).
- **DOMINANT cause — the fed G-code differs → flips stats.absolute.** WIZARD feeds ONE op's emitted macro (userOpView.js:146 emitMapped(builderOf(op))) — probe ops go incremental G91 → `stats.absolute` stays FALSE → curAnchor=TRUE → LOCAL start-anchored frame (route offset = the per-pass sim-start = boss centre; stock top-at-0; NO envelope; _probeXYOff active). BLOCKS feeds the WHOLE projected program (blocksApp.js:146 getProjection().text) whose `progstart` emits G90 + an absolute rapid (program.js:7) → `stats.absolute` LATCHES TRUE (GcodeExecutionEngine.js:944/995) regardless of later G91 → curAnchor=FALSE → ABSOLUTE machine-datum frame (path at its own coords; datum-aware stock via the :447 re-trace; envelope shown; _probeXYOff returns {0,0}, gcodeViz3d.js:1090). This one flip explains boss-offset + path-on-the-right.
- **COMPOUNDING:** blocks feeds NO getStartHints/getPinnedStarts → computePassStarts collapses to a SINGLE pass (multi-pass ops lose their per-pass starts) · getStart comes from the stale GLOBAL getLastOp (blocksApp.js:147) → for a `user_*` op it is null → getStartPos falls to viz.starts[0]={0,0,0} → DRO Work=0 (vs the wizard's sim-start value). The G54 LABEL is shared (both read settings.machine.wcs.table) — only the Work value differs.
- **WHY IT MATTERS TO THIS SCOUT:** composable-authoring REQUIRES the Blocks-tab preview to be ONE render / ONE anchor-source with the wizard preview — if the Blocks preview lies about the path, composing GUI/form blocks there is building on a false picture. So this is a **prerequisite bug to fix as part of the Blocks work** (before/with the layout-widget pilot).
- **FIX = a DESIGN call, NOT a flag flip (reproduce-live FIRST — the advisor got burned twice code-guessing anchors):** the Blocks tab legitimately renders the WHOLE program in the absolute frame; parity likely means previewing the single ACTIVE op (like the wizard) and/or feeding getStartHints/getPinnedStarts in the Blocks path — do NOT force-flip curAnchor / setForceMachine. REPRODUCE-LIVE: with the same op open in both, read on `window.__blkPanel` vs the wizard's `host.__panel` — (a) `panel.getAnchor()` (=curAnchor; expect TRUE wizard / FALSE blocks — proves the latch), (b) the fed code (confirm only blocks has progstart/G90), (c) `panel.getStartPos()`/`getPassStarts()` (wizard=the sim-start array; blocks=single {0,0,0}), (d) compare wcsForViz to RULE OUT a WCS-table diff. FIRST pin the op identity (user_* vs built-in): a user_* op ALSO loses getStart→{0,0,0}; a built-in shares the pass-0 start so its divergence is purely the absolute-frame flip.

## 🔨 turn 397 (cycle 127) — PILOT 1: the FORM value-field BLOCK (composable-authoring; forks F1-F4 ruled). BUILT + verified: a `formfield` block authors a wizard form field as a block, round-trips LOSSLESSLY, and LIFTS the isMaintainedAsData lock. Full suite 674 pass.

**WHAT (the composable substrate — a `formfield` block IS a deriveBindings SPEC row made visual):**
- **`formfield` block** (web/wizards/ops/formField.js) — a metadata block (`kind:'formfield'`, `emit:()=>[]`) in the `user_root` PRESENTATION mouth, mirroring `simStartBlock`. Carries the full deriveBindings spec vocabulary: `param · widget · label · dflt (empty = socket-held) · matchvar(#N) + key (the F2 socket link) · type · section · help · optional · readonly(+hint) · when(param/is) · widgetConfig (dropdown options, or number min/max/step/units)`. `dynamic:'widget'` (ddcs_dynfields) shows only the chosen widget's config (F4). Registered in PALETTE (index.js) → jsonDef auto-compiles; `optionsFor` gained the widget + type dropdowns (bridge.js); `devMode.isAtom` excludes the `formfield` kind (authoring metadata, not a knob).
- **The SHARED round-trip** (userOps.js, mirroring simStartsFromStack/ToBlocks exactly): `bindingsFromStack(children)` (formfield blocks → deriveBindings spec rows) + `bindingsToBlocks(specs)` (reverse — renders a hand-written spec set AS blocks so a port is authorable). Socket-link by macro-var IDENTITY `match:{type:'assign',var}` (F2), NOT a param-name join → deriveBindings re-finds the flat index (prune-safe).
- **`resolveBindingsMeta(def)` wired in registerUserOp** (mirror resolvePanelMeta): if the template authors its bindings as formfield blocks, DERIVE `def.bindingSpecs` (the emit re-derivation) + `def.bindings` (the form + schema) from them. **ADDITIVE — returns null when there are no formfield blocks, so a hand-written-spec def (today's corner/edge/middle, whose templates carry none) is UNTOUCHED → byte-identical.** The LIVE Blocks form (devMode.deriveAuthoredDef) also reads them (`formfieldBindings`, deduped vs the inline/param knobs) so a field shows in the form AS YOU AUTHOR IT.
- **isMaintainedAsData LOCK LIFTS** (devMode.js): returns false once EVERY bindingSpec is authored as a formfield block (the visual save round-trips them losslessly); a hand-written-spec port (no blocks) STAYS locked (byte-identical, unchanged).

**DECISION FLAGGED (native fields vs block.data):** the scout design said rich state "rides block.data (the _expose precedent)". I carried it as **native Blockly fields** instead — the `simStartBlock` precedent (whenparam/whenis/axis/… are all native fields), because every rich value FITS a native string field (widgetConfig.options → a "Label=val, …" string like param.js; `when` → whenparam/whenis; section/help → text). Native fields round-trip via the standard leaf-generic path (no block.data serialization, no restoreExpose-style rehydration) and are PROVEN lossless by the round-trip test. block.data stays reserved for state a field can't hold (regionpick's structured spec, the _expose map) — none here. Simpler + matches the shipped metadata-block pattern; flagging in case the advisor prefers the block.data seam.

**VERIFY (real-symptom + assert-the-value; tests/formfield-block.spec.js, 4/4):**
1. **LOSSLESS round-trip** — `bindingsFromStack(bindingsToBlocks(MIDDLE_BINDING_SPECS))` deep-equals the original 12 hand-written specs (section/help/when/match/readonly/optional/default ALL preserved) — the port's real specs round-trip byte-lossless without touching middle's def.
2. **Derivation + value lands** — a formfield-authored pilot op (3 fields bound to #1/#2/#3): registerUserOp derives def.bindingSpecs (3) + def.bindings (form, with resolved blockIndex by #var identity); building with `dist:77` lands `#1=77` in the emit (the socket link works); default build lands `#1=30`.
3. **Lock** — isMaintainedAsData is FALSE for the formfield-authored def (lossless), TRUE for middle (hand-written specs, no blocks → stays locked/byte-identical).
4. **DRIVE THE APP** — the pilot op's wizard opens with its 2 authored fields rendered in the live form (renderOpForm: "Max Probe Dist"=30, "Retract"=2) + CODE PREVIEW shows #1=30/#2=2 (screenshot scratchpad/formfield_pilot_form.png, opened). **Class-B render guard (blockly skill, Blockly 13.0.0):** the formfield blocks actually DRAW in the Blocks workspace (getHeightWidth height > 0, not phantom models).

**FULL SUITE: 674 passed, 2 skipped, 0 failed** (4 new + 670). Shared files touched (userOps.js/devMode.js/bridge.js/index.js) → NO regression: corner/edge/middle emit BYTE-IDENTICAL (their emit specs green — resolveBindingsMeta is a no-op for them; isMaintainedAsData unchanged for hand-written ports). Files: formField.js (new) · index.js · bridge.js · devMode.js · userOps.js · formfield-block.spec.js (new).

**SCOPE / DEFERRED (v1):** value-field blocks only (STRUCTURAL-toggle blocks deferred per dispatch) · assign-var sockets (match:{type:'assign',var} — non-assign matches later) · UNGUARDED authored ops (a guarded formfield-authored op = a later slice; resolveBindingsMeta derives over the unpruned template) · CONVERTING the real ports (corner/middle) to formfield-authored templates is a follow-on (their specs round-trip losslessly per test 1, so it's a safe swap when wanted — kept byte-identical here by NOT touching them). **NEXT: PILOT 2 (GUI point-pick + the anchor).** PASS BACK.

## 🔨 turn 399 (cycle 127) — PILOT 2: the GUI POINT-PICK widget block (composable-authoring; the ANCHOR + the spatial drag loop). BUILT + verified: a `layoutwidget` block declares a draggable point bound to fx/fy, drag writes the exact world coords, emit byte-identical (sim/form-only), corner/edge/middle unregressed. Full suite 677 pass.

**WHAT (the GUI twin of Pilot 1's formfield — a block that declares a canvas HANDLE):**
- **`layoutwidget` block** (web/wizards/ops/layoutWidget.js) — a metadata block (`kind:'layoutwidget'`, `emit:()=>[]`) in the user_root PRESENTATION mouth, mirroring the formfield block. v1 = POINT-PICK: fields `fx · fy · anchor(point) · frame(stock-min|datum) · xval · yval · label`. Registered in PALETTE (jsonDef auto-compiles); `optionsFor` gained the anchor + frame dropdowns; `devMode.isAtom` excludes the `layoutwidget` kind.
- **The round-trip** (userOps.js): `layoutBindingsFromStack(children)` EXPANDS each layoutwidget → TWO SOCKET-LESS bindings `{param:fx, group:'lwN', role:'x', anchor:{kind:'point',frame:'stock-min'}, default:xval}` + `{param:fy, role:'y', …}` — form + canvas only (NO match/blockIndex → NO emit, so sim/form-only, byte-identical). `layoutBindingsToBlocks(bindings)` re-pairs each group's x/y back into the layoutwidget (reverse). Wired into `resolveBindingsMeta` (now derives BOTH formfield value specs + layoutwidget layout bindings; still ADDITIVE → null when neither → ports byte-identical). The LIVE Blocks form (`authoredExtraBindings` = formfield + layoutwidget) shows the fields/handle as you author.
- **THE ANCHOR** (the crux): the bindings carry a DECLARED `anchor:{kind,frame}` (deriveBindings forwards it — a one-line passthrough add). `layoutSpecFromOp` SWITCHES on `anchor.kind` (a new explicit branch: `kind:'point'` → the `pos()` point decl, ax=0 = absolute PHYSICAL, stock-min-XY) — the role ladder + the corner/edge/② magic-name sniffs stay as the FALLBACK (untouched → corner/edge/middle declare no anchor → byte-identical). The EXISTING x+y→`point` derivation + `CANVAS_GESTURES.point` render the draggable handle; the drag writes fx/fy the world coords with ZERO gesture changes.
- **Point coords PHYSICAL** (stock-min-XY, ax=0) — the datum model [[datum-model-physical-derived-offset]]. frame='datum' (datum-relative DISPLAY only) is declared but DEFERRED (stock-min is the default → emit byte-identical). Drag-only, absolute (NO relTo/incremental, NO click-to-place — the clean pilot).

**VERIFY (real-symptom + assert-the-value; tests/pointpick-block.spec.js, 4/4):**
1. **Round-trip** — a layoutwidget block ⇄ two socket-less {group,role:x/y,anchor:{kind:point,frame:stock-min},default} bindings (assert NO match/blockIndex = no emit); the reverse re-pairs into the SAME block.
2. **The POINT gesture (assert-the-value)** — `CANVAS_GESTURES.point.drag(fx=px/fy=py, ax=0, world{30,45})` = `{px:30, py:45}` (EXACTLY the dragged world coords, physical); two-way: `place({x:70,y:25})` → the point moves to {70,25}.
3. **The op end-to-end** — a point-pick op (fx=px/fy=py, default 40/60): def.bindings carry anchor.kind='point'; layoutSpecFromOp (form open) renders a draggable point handle AT (40,60); the REAL drag path `spec.onDrag(id, {30,45})` writes `px='30'`, `py='45'` to the form (assert the values); **emit BYTE-IDENTICAL** — `emit(default) === emit({px:99, py:88})` (px/py have no socket → dragging never changes the emit).
4. **DRIVE THE APP** — the wizard opens with px=40/py=60 fields + a CYAN draggable "pos" point on the 2D FeatureCanvas at (40,60) (screenshot scratchpad/pointpick_pilot.png, opened) + CODE PREVIEW `( point pick pilot )` (no emit). **Class-B render guard (blockly skill, Blockly 13.0.0):** the layoutwidget block DRAWS in the Blocks workspace (getHeightWidth height > 0).

**FULL SUITE: 677 passed, 2 skipped, 1 flaky (the pre-existing middle-animator stroke-dashoffset timing flake — untouched file, passed on retry), 0 real failures.** **panelTypes.js is shared by EVERY layout op → the critical check: corner/edge/middle UNREGRESSED** (the anchor switch is additive, only fires for a declared anchor.kind; corner/edge/② declare none → the role/sniff ladder runs byte-identical; corner-start-datum-drag + all layout specs green). Files: layoutWidget.js (new) · index.js · bridge.js · devMode.js · userOps.js · deriveBindings.js · panelTypes.js · pointpick-block.spec.js (new).

**SCOPE / DEFERRED (v1):** point-pick only (corner/edge/②-aim anchor-extraction to retire the magic-name sniff · feature-pick · value-handle — all later) · socket-less fx/fy (a point that ALSO emits = a formfield value-binding + a layoutwidget handle on the SAME param, the complementary merge — later) · frame='datum' datum-relative display (declared, deferred; stock-min is byte-identical). **NEXT (per dispatch): declare anchor on corner/edge/②-aim to retire the sniff (byte-identical) · feature-pick · structural-toggle blocks.** PASS BACK.

## 🔨 turn 402 (cycle 127) — ROTARY CENTRELINE E0 (superset gate + golden baseline) BUILT + FULLY VERIFIED, then STOOD DOWN mid-turn (amendment: mill+ATC scout reprioritized BEFORE the probe fan-out). Committing the COMPLETE, GREEN, self-contained E0 increment (clean building tree per the amendment's "commit a clean WIP if a sub-part is genuinely complete and builds"). Rotary PAUSED pending the human's post-scout re-order.

**BUILT (E0 — mirrors the middle E0 superset pattern):**
- **`rotaryCenterStack(params, opts={})` + `opts.superset`** (rotaryCenterWizard.js): the ONLY block-shape forks are **method(known|fit)** + the **approach(auto|guided)** confirm gate NESTED in known → wrapped in GUARD arms via `methodFork`/`approachGuided` (the middleStack helpers). datum / wcs / safeZFrame are VALUE swaps (same block shape, different value read from params on BOTH sides) → NOT guarded (the scout's ruling: rotary wcs is a pure value-swap, NOT middle's 7-way wcsFork). `superset:false` is the concrete path (byte-identical to the default builder).
- **The 2 byte-clean restructures the scout flagged:** (1) **dissolve `#55=dia/2`** → `#57=diameter` (a single socket, E1-bindable) + `#55=[#57/2]` (R); value-equivalent (R = 38.1 at default). (2) **drop the `safeZ Math.round`** → `#17=safeZ` raw (byte-identical at the integer default 15; a value binding must land the raw value).
- The FIT ADVANCED banner STAYS (Studio-original, machine-unvalidated — the fan-out scout t393 confirmed no dumped rotary macro; the wizard's own emit IS the ground truth).

**VERIFY (the E0 gate; tests/rotary-center-superset.spec.js, 2/2):**
1. **THE GATE — byte-diff = ZERO** — `prune(rotaryCenterStack(S, {superset:true}), S)` === concrete `rotaryCenterStack(S, {superset:false})` BYTE-IDENTICAL across the FULL structural sweep (method × approach × datum × wcs × safeZFrame = 2×2×2×7×2 = **112 combos**, 0 diffs). The superset carries guards (≥3), prune leaves ZERO guards. INDEPENDENT TRUTH: the concrete builder is a SEPARATE code path.
2. **Restructures + golden** — #57=76.2 (dia socket), #55=[#57/2] (R), #17=15 (safeZ raw, no round), the FIT ADVANCED banner + solver (SQRT + determinant) intact, superset:false === the default concrete.

**FULL SUITE: 680 passed, 2 skipped, 0 failed** (2 new rotary E0 + 678). The emit CHANGE (the #57 dissolve) did NOT break any existing rotary sim test (rotary-center-sim / rotary-fit-sim / rotary-bar-declared / rotary-collision assert probe POSITIONS/behavior via the sim, not the #55 expression; no rotary_center emit golden exists — the scout's "no dumped macro" point). **EMIT DELTA vs pre-E0** (for transparency — the intended restructure): +`#57=76.2 ( Known diameter )`, `#55=76.2/2`→`#55=[#57/2]` (known arm); `#17` Math.round→raw (byte-identical at integer safeZ). Value-equivalent (same probe behavior).

**STAND-DOWN (amendment t401→this turn):** the human reprioritized — the **mill+ATC scout must come BEFORE the probe fan-out, even Rotary** (the advisor runs that scout itself, read-only). E0 was already complete + green when the halt arrived, so per the amendment ("commit a clean WIP ONLY if a sub-part is genuinely complete and builds") I committed the COMPLETE E0 increment rather than revert verified work — the tree is CLEAN + BUILDING (full suite green), NOT a half-applied superset. **Rotary is PAUSED**: E1 (data-op + bindings) is NOT started; Rotary resumes ONLY after the human reviews the mill+ATC scout map + sets the order. If the human decides to drop/redesign Rotary, E0 is a clean, self-contained, revertible commit (additive: superset:false byte-identical; the restructures are value-equivalent). Files: rotaryCenterWizard.js · rotary-center-superset.spec.js (new). **NEXT: the advisor's mill+ATC scout, then the human's re-order.** PASS BACK.

## 🔨 turn 405 (cycle 127) — DRILL FINISH-TO-IN-PLACE (the QUICK-WINS PILOT). Drill goes IN-PLACE via opensAs (one-line, byte-identical); the pilot SURFACED 2 gaps (form-clutter, Bore-needs-its-own-twin) + a clean BATCH recommendation. Full suite 683 pass.

**WHAT (the corner/edge/middle E4/E5 opensAs mechanism, one declaration):**
- **`opensAs: 'user_drill_data'` on the built-in Drill entry** (wizardLibrary.js:59). ONE declaration drives all three (verified — the mechanism is one-source): (a) the click re-points → `openWiz('user_drill_data')` (commandDeck.js:84, opensAs precedence over variant); (b) the twin's own Mill-Data-Wiz entry AUTO-HIDES (OPENS_AS_TARGETS → userEntries filter); (c) the seamless title reads plain "Drill" (builtinLabelForTwin). The legacy `drillStack` stays the byte-identical shim. STATIC shape kept (method/clearance baked — no toggles added).

**VERIFY (the FULL in-place experience; tests/drill-in-place.spec.js, 3/3 + screenshot):**
1. **Wiring** — Drill opensAs the twin, title "Drill", the separate Mill-Data-Wiz entry RETIRED; Bore stays built-in (no opensAs).
2. **Emit BYTE-IDENTICAL** — user_drill_data === drillStack across a 5-combo sweep (default/circle/line/off-origin/wcs+skip) — the drill golden, ZERO diff (the twin is unchanged by the wire).
3. **DRIVE THE APP** — Drill opens IN-PLACE with a NON-EMPTY form (29 knobs render: wcs/placement/pattern/geometry/cut) + the 3D sim renders the drill toolpath ("Drill (data) · 31 lines", tool + stock + path). Screenshot scratchpad/drill_inplace.png (opened). Title reads "Drill".
- Updated 2 tests that pinned the PRE-in-place routing (the same updates edge/middle needed when they went in-place): edge-seamless-title (builtinLabelForTwin('user_drill_data') now 'Drill', was null) + wizard-bar (Drill onclick now openWiz('user_drill_data'), was openWiz('drill','drill')).

**FULL SUITE: 683 passed, 2 skipped, 0 failed.**

### PILOT FINDINGS (the deliverable — decides batch-vs-one-per-turn + surfaces the emit-only-twin gaps):
- **THE WIRE-UP IS CLEAN + QUICK** — a one-line opensAs declaration, byte-identical, form non-empty, sim renders. **No one-source BLOCKER.**
- **GAP 1 (form clutter — functional, NOT broken):** the in-place form is a FLAT 29-field list (grid cols/rows/dx/dy + circle count/dia/startAngle + line spacing/angle + rect w/h/nx/ny ALL shown at once — the emit-only twin's auto-generated form has no pattern-driven when-gating / sections, unlike the built-in drill form). The CLEAN fix is NON-TRIVIAL, NOT a quick one-liner: (a) `when`-gating is awkward — `count` is SHARED by circle+line, so a single-value `when:{param,is}` can't express "circle OR line"; (b) renderOpForm does NOT consume `section` (the form-render scout flagged this). So it needs a form-render enhancement (a multi-value/OR `when`, or section support), NOT a per-op declaration. **DRILL IS THE WORST CASE** (the only op with a 4-way mutually-exclusive pattern explosion) — Slot/Surfacing/Text/atcWarmup are single-shape → their flat forms are fine.
- **GAP 2 (Bore needs its own twin):** user_drill_data is PECK-ONLY — `method:'helical'` swaps the block TYPE (`drill`→`bore`, drillWizard.js:32, frontier #1) with DIFFERENT keys (holeDia/toolDia/pitch/ramp vs peck). So Bore CANNOT reuse the peck twin (it would emit a peck drill). Bore stays built-in; Bore-in-place = a small SEPARATE follow-on (a `user_bore_data` helical twin, seeded from drillStack method:'helical' + bore-key bindings). Deliberately did NOT wire Bore→user_drill_data (would ship a wrong emit).
...
## 🔨 turn 409 (cycle 127) — ATC TOOL LENGTH: light NEW port (the pilot for the light-ATC-port recipe; Tool Check inherits it). A NEW twin created + wired in-place; the recipe (bindingSpecs + header-recompose + source-chips) held CLEANLY. Full suite 690 pass.

**WHAT (a NEW twin — unlike the 5 in-place finishes, atcLengthStack had NO twin):**
- **`atcLengthDataDef`** (new blocks/dataOps/atcLengthData.js) via userOpFromStack (mirror atcWarmupData — the closest static-shape ATC precedent). Static shape (no guards). **7 scalar BINDING_SPECS by #var identity** (deriveBindings, like corner/middle): #1 maxDist · #2 retract · #3 f_fast · #4 f_slow · #5 port · #6 blockHeight · #19 safeZ. `level` baked (a machine constant — the corner/edge/middle convention). Seeded in app.js.
- **THE TWO SOURCE-TOUCHES (the corner precedent — so the twin is byte-identical to the built-in on ALL scalars + BOTH profiles, not just the default seed):**
  - **(a) HEADER RECOMPOSE:** SOURCE-TOUCH to atcLengthWizard.js — extracted the 2 interpolated SUMMARY comments (blockHeight/safeZ/fFast/fSlow/maxDist) into ONE shared `atcLengthHeaderComments(params)` helper (the built-in uses it); the twin's `applyHeaderComments` postInstantiate RECOMPOSES the frozen default header from the resolved params (bindings touch #N VALUES, not composed comment text) → the header matches for ALL scalars, not just defaults.
  - **(b) SOURCE-CHIPS:** `applyProbeSources` postInstantiate — on Expert (window.ddcsResolveProbeSources present), rewrite #5 (setterPort) + #6 (blockHeight) to the controller register via the SAME srcVal/srcNote the built-in uses. Studio / no native register → resolve returns {} → byte-identical (unchanged). postInstantiate = applyHeaderComments(applyProbeSources(stack), resolved).
- **IN-PLACE:** `opensAs: 'user_atc_length_data'` on the built-in ATC Tool Length entry (wizardLibrary.js) — re-points the click + hides the twin's atc_datawiz entry + seamless "Tool Length" title. Panel `form3d` + sim `{forceMachine:true}` — the built-in atcLengthView is twoPane with a 3D machine preview (preview3D + previewMachine).

**VERIFY (tests/atc-length-in-place.spec.js, 3/3 + screenshot):**
1. **emit BYTE-IDENTICAL to atcLengthStack across a scalar sweep — BOTH profiles (the middle E1-fix precedent):** STUDIO (resolver → {}, source-chips literal) = 0 diffs; EXPERT (resolver STUBBED ON → #5/#6 resolve to registers #1078/#1170) = 0 diffs (the twin's applyProbeSources reproduces the built-in's srcVal/srcNote exactly). The sweep varies blockHeight/safeZ/maxDist/retract/feeds/port (incl. fractional blockHeight 25.4) → the header-recompose + bindings hold.
2. **IN-PLACE wiring:** opensAs the twin, title "Tool Length", the separate atc_datawiz entry RETIRED.
3. **DRIVE THE APP:** opens in-place with a NON-EMPTY 7-knob form (maxDist/retract/f_fast/f_slow/port/blockHeight/safeZ) + the 3D machine-frame sim renders (screenshot scratchpad/atc_length_inplace.png, opened — the green machine envelope + the tool-length macro in CODE PREVIEW).
- Updated the pinned wizard-bar ATC-group routing (atc_length → user_atc_length_data).
...
## 🔨 turn 413 (cycle 127) — ROTARY CENTRELINE E1: the data-op twin + bindings by #var identity (resuming the probe fan-out; quick wins done + V10.87 released). Byte-diff ZERO across the 112 structural + scalar sweep on BOTH profiles. E0 (c2bfe6e) UNCHANGED. Full suite 695 pass.

**WHAT (mirror middle E1 / cornerData):**
- **`rotaryCenterDataDef`** (new blocks/dataOps/rotaryCenterData.js) via userOpFromStack wrapping `rotaryCenterStack(params, {superset:true})` (E0) in user_root — byte-transparent wrap. panel form3d+2d + sim `{rotary:true, machine:true}` (rotary declares the ROTARY_RIG). NOT seeded / NOT opensAs yet (E2/E4).
- **8 VALUE bindings by #var IDENTITY** (deriveBindings over a CANONICAL method='known' pruned stack): #1 dist · #2 retract · #3 f_fast · #4 f_slow · #5 port · #6 radius · #17 safeZ + **#57 diameter (the E0 dissolve), OPTIONAL** (known-only — deriveBindings optional-skips it when the fit arm is pruned; the middle optional-binding precedent). **#11/#12 flank-geometry are NOT bound** — they are CONSTANT expressions (`[#55+#2+#6]` / `[#17+#55]`) referencing the other #vars → they recompute for free.
- **2 STRUCT bindings drive the guards:** method(known|fit) + approach(auto|guided) — the E0 block-shape forks.
- **4 source-chips** (#2/#3/#5) via `applyProbeSources` postInstantiate (corner/atc precedent) — Expert registers.

**THE KEY E1 FINDING — the frozen-template value-swaps (E0's per-combo build HID this):** unlike E0 (which built the superset per-combo), E1 freezes the superset with DEFAULTS + prunes → the **value-swaps don't respond via prune alone**. Per the port plan (datum/wcs stay value-swaps, NOT structural) they are RECOMPOSED from the resolved params in postInstantiate (the cornerData applyHeaderComments precedent, EXTENDED to more sockets — rotary's built-in interpolates more than middle's static header):
  - **The interpolated comments** (a source-touch to rotaryCenterWizard.js → a shared `rotaryCenterHeaderComments` helper): top1 (method/diameter/datum/wcs), top3 (scalars), the **KNOWN-arm header** (interpolates approach — "auto-centring"|"operator-guided" — the diff that surfaced: it's inside the known arm + baked at the default approach), and the write-origin (datum). Recompose by matching the frozen DEFAULT text → the resolved.
  - **`applyDatumWcs`**: the WCS arg on every work-offset write (wcs → #578 / Gnn−53) + the Z offset value (datum → OD top #50 / centreline #56).
  - **`applySafeZFrame`**: the final PARK block (safeZFrame relative→`move` / machine→`machinemove`), swapped in place via the 'Final retract' comment anchor.
  So datum/wcs/safeZFrame are FORM controls (ROTARY_CENTER_VALUESWAP_BINDINGS — enum, no guard, no #var socket) that drive the recompose. **E0 (the banked superset) is UNCHANGED** — no new guards.

**VERIFY (tests/rotary-center-data-emit.spec.js, byte-diff ZERO):** the twin emit == rotaryCenterStack across (a) the FULL 112-combo structural sweep (method×approach×datum×wcs×safeZFrame) at default scalars, (b) a scalar sweep (incl. fractional diameter 50.8/25.4, both method arms), (c) WIRING (dist=99→#1, diameter=12.7→#57) — on STUDIO (0 diffs) AND EXPERT (source-resolver stubbed, #2/#3/#5→registers, 0 diffs). The FIT ADVANCED banner stays. INDEPENDENT TRUTH: the built-in is a separate code path.

**FULL SUITE: 695 passed, 2 skipped, 0 failed.** rotaryCenterWizard.js touched (the header helper + knownHeader extraction) → the E0 gate + the built-in rotary tests stay byte-identical (the helper reproduces the exact text). Files: rotaryCenterWizard.js (header helper) · rotaryCenterData.js (new twin) · rotary-center-data-emit.spec.js (new). **NEXT (per dispatch): E2 (sim-starts provider + the round-bar workpiece read, retire activateCylinderStock).** Rotary Clock is the disjoint sibling (later). PASS BACK the byte-diff ZERO.

## 🔨 turn 415 (cycle 127) — ROTARY CENTRELINE E2: the per-pass sim-starts provider (sim-only, emit byte-identical). E1 banked (6764005). Full suite 697 pass.

**WHAT (mirror middle E2 — reuse the built-in logic via the sim registry):**
- **`def.simStartsProvider = (params, stock) => opSimStarts('rotary_center', params, stock)`** (rotaryCenterData.js) — reuses `BUILT_IN.rotary_center` (opSimStarts.js:86) VERBATIM via the public dispatch. registerUserOp wires it through the sim registry (`setUserSimStarts`), NOT the builder → **sim-only, emit BYTE-IDENTICAL**. KNOWN method = 1 pass (over the top, probe down Z); FIT method = 3 passes (top + the ±Y flanks, spread to distinct points so the 3-point circle solve isn't degenerate).
- The sim block already declares `{rotary:true, machine:true}` (E1) → the rotary machine-frame preview. (The rig decouple + the round-bar read are LATER slices, E3/E4.)

**VERIFY (assert-the-value vs BUILT_IN; tests/rotary-center-sim-starts.spec.js, 2/2 + screenshot):**
1. **POSITIONS + COUNT == BUILT_IN.rotary_center** — `opSimStarts('user_rotary_center_data', c, stock)` deep-equals `opSimStarts('rotary_center', c, stock)` (x/y/z per pass) across method × approach × scalar combos (0 pos fails). KNOWN = 1 pass, FIT = 3 passes (the built-in probe passes). **emit BYTE-IDENTICAL** (twin == rotaryCenterStack for known + fit — the provider is via the registry, not the builder).
2. **REAL-SYMPTOM** — the twin opens with the FIT method → the sim declares 3 per-pass starts + the rotary machine-frame preview renders (the green bar + tool + probe + the fit macro; screenshot scratchpad/rotary_center_simstarts.png, opened).

**FULL SUITE: 697 passed, 2 skipped, 0 failed.** The rotary-center-data-emit (E1) + rotary-center-superset (E0) specs stay green (the provider doesn't touch the builder → emit unchanged). Files: rotaryCenterData.js (the provider line + import) · rotary-center-sim-starts.spec.js (new). **MINOR (noted, not E2):** the twin form shows the `approach` control even for the FIT method (approach is known-only) — the drill-form-clutter pattern (a when-gate on approach=known would clean it; polish, not emit-affecting). **NEXT (per dispatch): E3 (the round-bar workpiece read → outer.shape:'round'+d, retire activateCylinderStock — the 2nd stock.shape mutator).** NOT seeded/opensAs yet (E4). PASS BACK.

## 🔨 turn 417 (cycle 127) — ROTARY CENTRELINE E3: the round-bar sim read (retire activateCylinderStock for the TWIN path; SIM-ONLY, emit byte-identical). The workpiece-pivot payoff for rotary. F3 pre-ruled (#57 is the user bar-Ø control). Full suite 700 pass.

**THE FINDING (E3 needed a NEW mechanism):** the built-in rotaryCenterView.onOpen calls `activateCylinderStock()` which MUTATES the global settings.stock → a cylinder (the hack) so the sim renders a bar. The generic userOpView does NOT — so the twin's sim read the global stock (a box). Rendering a round bar WITHOUT mutating the global (F3: derive from #57 at sim-time) needed a **per-op SIM-STOCK override** — the preview always read the global `stockForViz()`, no per-op hook. This is the clean declare-not-mutate mechanism (mirrors the sim-starts registry); it does NOT require the deferred modal round-outer UI.

**WHAT (a per-op sim-stock, 6 files):**
- **`def.simStock = (params, stock) => a round bar`** (rotaryCenterData.js): DERIVES `{shape:'cylinder', diameter:#57, cross=#57, axial=stock.x, datum:'nnp', show:true}` from #57 (the bar Ø — the USER's clean control, [[dont-declare-away-user-responsibility]]) + the rotary axis (rotaryAxisOf). Mirrors activateCylinderStock's cross-square, but the Ø comes from #57, NOT the max cross dim. NO applySettings → NO global mutation.
- **The registry** (opSimStarts.js): `setUserSimStock`/`getUserSimStock` (a LIVE fn, re-attached from the seed like simStartsProvider — dropped on JSON persistence). registerUserOp calls `setUserSimStock(def.opType, def.simStock)`; deleteUserOp clears it.
- **userOpView**: computes `_simStock = getUserSimStock(_def.opType)?.(params, globalStock)` ONCE and feeds it to BOTH the sim-starts (`opSimStarts(..., _simStock)`) AND the preview (`preview3D(..., _simStock)`), so the bar + the flank starts agree.
- **wizardManager.preview3D** gains a `simStock` param → `host.__simStock` + `getStock:()=>host.__simStock` in the panel config.
- **createPreviewPanel**: a `previewStock()` helper — `opts.getStock() || stockForViz()` — replacing the 5 op-preview stockForViz() calls (viz/2D/engine/trace/re-trace). ADDITIVE: every other op omits getStock → previewStock() === stockForViz() (byte-identical behaviour).

**VERIFY (tests/rotary-center-roundbar.spec.js, 3/3 + screenshot):**
1. **unit** — def.simStock derives a cylinder (Ø=50 from #57, cross y/z=50, axial x=200 kept, barRadius=25); the GLOBAL stock stays a BOX (200×100×100 — NO mutation, the hack is gone for the twin); **emit BYTE-IDENTICAL** (twin == rotaryCenterStack for known + fit — the round-bar read is sim-only).
2. **restoreBoxStock still works** — the built-in shim's activateCylinderStock still forces a cylinder (deferred dead-code cleanup); restoreBoxStock reverts (the defensive edge/middle/clock revert intact).
3. **REAL-SYMPTOM** — the twin opens on a BOX global stock but the 3D sim RENDERS a round bar (Ø=76.2 from #57), global unmutated (screenshot scratchpad/rotary_center_roundbar.png, opened — the green cylinder, not the box).

**FULL SUITE: 700 passed, 2 skipped, 0 failed.** 5 shared core files touched (createPreviewPanel / wizardManager / userOpView / opSimStarts / userOps) → NO regression (the getStock override is additive; the rotary-center-data-emit E1 + superset E0 + sim-starts E2 all green). Files: rotaryCenterData.js · userOps.js · createPreviewPanel.js · opSimStarts.js · wizardManager.js · userOpView.js · rotary-center-roundbar.spec.js (new). **NEXT (per dispatch): E4 (rig decouple — setRotaryRig under _partGroup, decouple from setStock), E5 (in-place + release).** The built-in activateCylinderStock is KEPT as the legacy shim (deferred dead-code cleanup). PASS BACK.

## 🔨 turn 419 (cycle 127) — ROTARY CENTRELINE E4: the rig decouple (sim-device layer; sim-only, emit byte-identical). ONE file of substance (gcodeViz3d.js) + 1 new spec. Full suite 702 pass / 2 skip / 0 fail.

**THE ENTANGLEMENT (before):** the 4th-axis RIG (chuck + tailstock mesh) was built INLINE inside `setStock` (the `if (this._showRotaryFixture) { … _buildRotaryFixture(pg, …) }` block) + disposed at setStock's start, and `setRotaryFixture(on)` rebuilt it by calling `if (this._stock) this.setStock(this._stock)` — a FULL stock rebuild just to toggle a fixture mesh. The rig's lifecycle was tangled with the stock's.

**THE DECOUPLE (mirrors setMagazine's dispose/guard/build shape — BUT a CHILD of _partGroup, NOT this.scene):** three new members on GcodeViz3D + two edits to the old sites, all in `web/viz/gcodeViz3d.js`:
- **`_rotaryRigSpec()`** — derives `{axis, r, L, jaws, cu, cv}` from `this._stock` + `getRotaryAxes()`. A BYTE-IDENTICAL extract of the old inline logic: `r = (cylinder ? min(cross) : max(cross))/2`, `L = fd[axis]/2`, `jaws = cylinder?3:4`. Returns `null` when there's no valid stock.
- **`setRotaryRig(spec)`** — the decoupled sim-device: dispose the old `_rotaryFixture` (remove from `_partGroup` + traverse-dispose geo/mat), then `if (spec) _buildRotaryFixture(this._partGroup, spec.axis, spec.r, spec.L, {jaws,cu,cv})`. `_buildRotaryFixture` already does `pg.add(grp); this._rotaryFixture = grp` — so passing `_partGroup` (not `this.scene`) keeps the rig a CHILD of the part group. THAT is the whole point: `_partGroup` is what `_applyPartRotation` spins + what `pg.position` datum/WCS-places, so the rig inherits BOTH for free. (The scout RISK was exactly this: attach to `this.scene` like the magazine sibling → spin-loss + placement-drift. Attached to `_partGroup` instead.)
- **setStock** — removed the inline rig build; at its END (AFTER `if (this._jogA) this._applyPartRotation(...)`) it now re-derives: `if (this._showRotaryFixture) this.setRotaryRig(this._rotaryRigSpec())`. Guarded on `_showRotaryFixture` so the stock modal (which sets it false) keeps setStock's no-render / no-rig contract intact.
- **setRotaryFixture(on)** — now `this.setRotaryRig(on ? this._rotaryRigSpec() : null)` instead of a full `setStock` rebuild. KEEPS `_showRotaryJog(on)` (the A± jog row) + the flag flip — the jog/DRO UI is orthogonal to the mesh, untouched.

**WHY it converges under both call orders:** the rig now re-derives in TWO places (setStock-end + setRotaryFixture), so whichever fires last with a valid stock builds it — `setRotaryFixture(true)` before the first setStock → `_stock` null → no rig YET, but the flag is set, so the following setStock builds it; setStock before the toggle → the toggle builds it. (The old single-inline-site couldn't express that.)

**VERIFY (tests/rotary-center-rig-decouple.spec.js, 2/2 + screenshot) — REAL SYMPTOM on the live rotary_center preview:**
1. the rig STILL renders (`_rotaryFixture` built) — screenshot scratchpad/rotary_rig_e4.png shows the green bar with the dark CHUCK+3-jaws on −X and the TAILSTOCK body+live-centre cone on +X, probe descending on top.
2. it is a CHILD of `_partGroup` (`rig.parent === viz._partGroup`) and NOT in `scene.children` (the anti-scout-risk assertion).
3. it SPINS with the part — `_applyPartRotation(37,0)` + `updateMatrixWorld` → the rig's `matrixWorld.elements` CHANGE (asserts the world transform actually follows the part, not just the structural parent link).
4. toggling the rig OFF disposes it AND the `stockMesh` object identity is UNCHANGED (the decouple: no stock rebuild); toggling back ON re-builds under `_partGroup`, stockMesh STILL unchanged.
5. a `setStock` with new dims RE-DERIVES a fresh rig (new object, still child of `_partGroup`).
6. emit BYTE-IDENTICAL — twin == rotaryCenterStack for known + fit (the rig is a sim-device; no emit path touched).

**FULL SUITE: 702 passed, 2 skipped, 0 failed** (grew 700→702 with the 2 new E4 tests). NO regression in the rig/rotary/stock-modal/magazine paths — blocks-rotary-rig (the `_showRotaryFixture` flag semantics unchanged), rotary-a-jog (the A± jog + `_applyPartRotation` untouched), stock-modal-3d-refresh (its `_showRotaryFixture`=false path never builds a rig, contract intact), atc-magazine-frame (setMagazine untouched) all green. Files: gcodeViz3d.js · rotary-center-rig-decouple.spec.js (new). Throwaway screenshot spec run from tests/ then deleted (not committed). **NEXT (per dispatch): E5 (opensAs in-place + builtinLabelForTwin title + seed + RELEASE — the Rotary Centreline milestone).** PASS BACK.

## 🔨 turn 421 (cycle 127) — ROTARY CENTRELINE E5: the IN-PLACE milestone (opensAs + seamless title + seed) + the RIG-in-twin gap FIXED. Full suite 704 pass / 2 skip / 0 fail (1 pre-existing flake). RELEASE is the ADVISOR's (dispatch: "then I RELEASE V10.88").

**THE ONE-SOURCE WIRE (2 tiny files):** `opensAs: 'user_rotary_center_data'` on the built-in Centreline entry (wizardLibrary.js:52) — ONE declaration drives ALL THREE: the click re-point (commandDeck.js:84 → openWiz), the twin's own-entry HIDE (OPENS_AS_TARGETS auto-includes it), and the seamless plain "Centreline" title (builtinLabelForTwin auto-maps). + the app.js seed (import rotaryCenterDataDef + add to seedDefaultPortedUserOps). NO WIZ_SPECIAL_OPENER for rotary_center to retire (only alignment has one); WIZ_SUBLABEL{rotary_center:'Rotary'} stays (the built-in menu entry's sublabel, orthogonal). The built-in rotaryCenterWizard/rotaryCenterView stay as the legacy shim.

**THE E5 GAP I FOUND + FIXED (the milestone's real work) — the RIG did NOT render in the TWIN.** Driving the in-place experience surfaced it: the 3D sim showed the round bar (E3) but NO chuck/tailstock rig. ROOT: `previewRotaryFixture(...,true)` was called ONLY by the BUILT-IN views (rotaryCenterView.js:114, rotaryClockView.js:61) — the generic `userOpView` never called it. The built-in path (E4's rig screenshot) opened rotaryCenterView so the rig showed; the TWIN path (openWiz → userOpView) never enabled it. The machine FRAME still showed (the rotary macro's G53 anchors it, independent of the rig), which masked the gap.
- **THE FIX (declare-driven, generic, 1 file):** the twin's stack already DECLARES `sim{rotary:true}` → `setUserSimIntent` → `opSimContext('user_rotary_center_data').showRotaryRig === true` (the ONE declared source the Blocks tab already reads, blocksApp.js:332). userOpView now READS it after preview3D and calls `mgr.previewRotaryFixture(cont, opSimContext(opType).showRotaryRig)` — the GENERIC mirror of the built-in view + the Blocks tab. Container = 'userViz3dContainer' (form3d+2d) / 'userVizContainer' (form3d). Non-rotary ops → showRotaryRig false → previewRotaryFixture(false) → setRotaryFixture early-outs (already off) → HARMLESS no-op. This is declare-never-infer + one-source: EVERY future rotary twin (rotary_clock NEXT) inherits the rig for free — no per-view hard-call. [[custom-op-sim-intent-infer-vs-declare]] [[declared-seam-before-declaring-gui]]

**THE IN-PLACE EXPERIENCE REPORT (the advisor asked, esp. the 2D state) — screenshot scratchpad/rotary_center_e5_inplace.png (opened):**
1. **click re-points** — the built-in Centreline slot opens user_rotary_center_data in-place; title reads plain **"Centreline"** (seamless); the twin's own data-wiz entry is GONE.
2. **FORM non-empty** — 13 knobs render: dist, retract, f_fast, f_slow, port, radius, safeZ, **diameter (#57)**, method, approach, datum, wcs, safeZFrame.
3. **3D SIM** — the ROUND BAR (cylinder Ø76.2 from #57, E3) + the 4th-axis RIG (chuck+3-jaws on −X, tailstock body+live-centre cone on +X, E4/E5) + the multi-pass DRO ("6 probes · 14 rapids", E2). Rig is a child of _partGroup (spins + datum-placed).
4. **2D VIEW STATE = COHERENT (not broken/empty).** The form3d+2d panel's 2D toggle works; the layout canvas draws the STOCK FOOTPRINT (rectangle) + a "Start" marker at centre (the generic top-down layoutSpecFromOp derive). It is NOT a rotary-SPECIFIC view — no bar cross-section / datum glyph — but it is COHERENT, not a broken/empty canvas, so it PASSES the dispatch's "COHERENT or ABSENT" bar. **FLAG (as pre-ruled):** the proper rotary datum-glyph 2D (bar cross-section) is the DEFERRED follow-on; NO panel change made (coherent-is-acceptable; if the advisor prefers ABSENT, a one-line panel form3d+2d→form3d does it, display-only, emit byte-identical).
5. **emit BYTE-IDENTICAL** — twin == rotaryCenterStack across a 6-row method/approach/datum/wcs/safeZFrame/scalar sweep (the menu/seed wire never touches the stack).
6. **FIT advanced label** — method=fit surfaces the "advanced" labeling (the "3-point fit (advanced)" option + help "verify on machine").

**FULL SUITE: 704 passed, 2 skipped, 0 failed** (grew 702→704... wait +3 new = 705; the E4 spec's 2 + E5's 3, minus none — 707 total collected, 704 pass + 2 skip + 1 flaky-passed-on-retry). The flake = middle-animator stroke-dashoffset (a PRE-EXISTING CSS-animation timing flake, unrelated to userOpView — passed on retry). Shared userOpView.js touched → NO regression across ALL user-op previews (drill/slot/surfacing/text/atc/corner/edge/middle in-place all green; the non-rotary previewRotaryFixture(false) is a no-op). Files: wizardLibrary.js · app.js · web/wizards/views/userOpView.js · web/viz/opSimContext.js-imported (no change) · tests/rotary-center-in-place.spec.js (new). **THE ROTARY CENTRELINE MILESTONE IS IN-PLACE + VERIFIED — ready for the advisor's V10.88 release.** NEXT (queued): Rotary CLOCK (the disjoint sibling — it now inherits the rig-in-twin wiring for free). PASS BACK.

## 🔨 turn 423 (cycle 127) — ROTARY CLOCK E0: the superset gate + golden baseline (phase C, the A-axis inheritor; Centreline COMPLETE + V10.88 released). Byte-diff ZERO across the 84-combo structural sweep. 1 byte-clean restructure. 2 files.

**THE CLOCK IS A DISTINCT WIZARD (not a Centreline variant):** rotaryClockStack (rotaryClockWizard.js:17) datums the rotary axis off a FLAT — two Z-down touches a SPAN apart (ppZdown, its OWN hand-rolled two-pass probe) → tilt phi = atan2(dZ, span) → datum A. #6 = SPAN (not radius); NO radius comp (a flat cancels it). NO dumped M350 macro → golden-snapshot its OWN emit (the wizard IS the ground truth, like Centreline).

**THE E0 STRUCTURE (mirror rotaryCenterStack E0):** `rotaryClockStack(params, opts={})` + `const superset = !!opts.superset`. The KEY realisation (from the Centreline E0 test): the gate passes the SAME full params to BOTH sides — `rotaryClockStack(c,{superset:true})` pruned by c vs `rotaryClockStack(c,{superset:false})` — so any VALUE-swap that READS from params is byte-identical on both sides WITHOUT a guard; only a BLOCK-SHAPE fork (the superset must carry ALL arms regardless of c) needs guarding.
- **The Clock's ONE block-shape fork = action(set|report|rotate)** → GUARD-wrapped via a new `actionFork(setKids, reportKids, rotateKids)` (superset → 3 GUARD({param:'action',is}) arms; concrete → the selected arm). Converted the 5 wrappers the arms use (C/A/MV/RM/SWO) to return-factories (mkC/mkA/mkMV/mkRM/mkSWO) + kept the push wrappers reusing them (mirror rotaryCenterStack's mk-split). The A-axis writes (RM/SWO/MV of A) + the refTerm value-swap STAY in the concrete arms — E1 binds the A-axis.
- **reference(top|side) + safeZFrame(relative|machine) + wcs = VALUE swaps → NOT guarded.** reference → refTerm (``|`-90`) + refLabel interpolated INTO the action arms; safeZFrame → safeZParkBlock (the same single call, block-swaps internally); wcs → wcsArg. All read from params on both gate sides → byte-identical. (This is the Centreline safeZFrame precedent the dispatch offered — deferred to E1 recompose, NOT a block-swap GUARD, because the gate confirms it needs none.)

**THE 1 BYTE-CLEAN RESTRUCTURE:** dropped the build-time `Math.round(safeZ)` on #17 (line 58) → `A('#17', safeZ, 'Safe Z')`. Byte-identical at the integer default 10 (Math.round(10)=10); a fractional safeZ now lands RAW (#17=12.5, not 13) so the E1 value-binding on #17 emits the raw value. (The SAME restructure #2 the Centreline made, for the same E1 reason.) NO diameter/radius dissolve needed — the Clock has none (#6=span is already a single socket).

**VERIFY (tests/rotary-clock-superset.spec.js, 2/2):**
1. **E0 GATE** — prune(superset, c) === concrete rotaryClockStack(c) BYTE-IDENTICAL across ALL 84 combos (action 3 × reference 2 × safeZFrame 2 × wcs 7). diffCount=0, leftoverGuards=0, the superset carries the 3 action guards. INDEPENDENT TRUTH: the concrete builder is a separate code path.
2. **GOLDEN + restructure** — #17=10 raw at default (byte-identical), #17=12.5 raw at 12.5 (the Math.round drop proven); #6=20 span socket; SET@top emits SWO A [#54-#53] (no side term/rotate), SET@side value-swaps refTerm → [#54-#53-90], REPORT emits measure-only + NO SWO of A, ROTATE emits #58=[0-#53] + the SPINS-THE-PART warning; superset:false === default (self-consistent).

**FULL SUITE: 706 passed, 2 skipped, 1 flake** (project-drawer-smoke "Cloud tab load without import errors" — a transient console-error flake under full-suite load; PASSES in isolation; unrelated to the Clock-emit-only change, which also passes in isolation + adds no imports). NO regression: every existing rotary-clock test stayed green (the Math.round drop is byte-identical at the default). RotaryClockWizard.generate still calls rotaryClockStack(params) → superset:false → unchanged. Files: web/wizards/rotaryClockWizard.js · tests/rotary-clock-superset.spec.js (new). **BYTE-DIFF = ZERO — the E0 gate is PASSED.** NEXT (per dispatch): CL-E1 (the twin + bindings + the FIRST A-axis binding). PASS BACK.

## 🔨 turn 425 (cycle 127) — ROTARY CLOCK E1: the data-op twin + bindings + THE A-AXIS EMIT (the first A-axis atoms any twin makes). Byte-diff ZERO across 84 struct + a scalar sweep on BOTH profiles. Full suite 708 pass. 3 files.

**THE TWIN (mirror rotaryCenterData):** rotaryClockDataDef via userOpFromStack wrapping rotaryClockStack(params, {superset:true}) in user_root (panel form3d+2d, sim {rotary:true, machine:true} → the rig). NOT seeded/opensAs yet (E4).
- **7 VALUE bindings by #var identity:** #1 dist / #2 retract / #3 f_fast / #4 f_slow / #5 port / **#6 SPAN** / #17 safeZ. The Clock's #6 is the Y SPAN between the two flat touches (NOT a radius — distinct from the Centreline #6 stylus radius; a flat has no comp). No optional/when-gated binding (all 7 are LINEAR, present under any action) → the canonical prune is just the DEFAULTS.
- **#58 is COMPUTED, NOT a knob:** the A rotation to the reference is `[0-#53±refTerm]` off the measured tilt #53 — left a concrete expr (like the Centreline computed #11/#12), value-swapped by the recompose, never bound.
- **1 STRUCT binding — action(set|report|rotate)** — drives the E0 guard.
- **VALUE-SWAPS via postInstantiate recompose** (E0 superset UNCHANGED): applyHeaderComments (top1/top3 + the set/rotate arm comments + the action-dependent final MESSAGE — a `message` block, so the recompose scans comment AND message), applyReferenceWcs (the wcsArg on every A SWO + the refTerm on the SET-arm A-datum formula [#54-#53±90] + the ROTATE-arm #58 [0-#53±90]), applySafeZFrame (the park move→machinemove, reused verbatim), applyProbeSources (#2/#3/#5 source-chips). Extracted rotaryClockHeaderComments to rotaryClockWizard.js (the shared text format the stack + the recompose BOTH read → can't drift), like rotaryCenterHeaderComments.

**THE A-AXIS EMIT (the dispatch's headline ask) — CONFIRMED A NO-OP, zero special handling:** the Clock's A-datum atoms emit entirely through the ACTIVE DIALECT:
- `RM('A', #54/#59)` → `#54=#883` / `#59=#883` — the A machine DRO register #883 (dialect readmachine).
- `MV('A', #58)` → `G0 A#58` — the ONLY A-LETTER G-code word (dialect move).
- `SWO('A', …)` → `#[805+[#578-1]*5+3]=#59` — the A work-offset write via the WCS stride, +3 = the A column (dialect setworkoffset).
These are CONCRETE atoms (readmachine/setworkoffset/move) kept by prune, and the value bindings only match `assign` #vars → they pass through BYTE-IDENTICAL. The twin machinery DROPS NOTHING. The only A-axis TEXT the recompose touches is the value-swaps (the SWO/#58 refTerm + the wcsArg) — all byte-clean. **NO A-dialect gap: the first A-axis emit by a twin works exactly as predicted (a no-op).**

**VERIFY (tests/rotary-clock-data-emit.spec.js, 1/1):** byte-diff ZERO across (a) the full 84-combo structural sweep (action 3 × reference 2 × safeZFrame 2 × wcs 7) at default scalars, (b) a 4-row scalar sweep (incl. a fractional span on the rotate arm), (c) WIRING (dist=99→#1, span=99→#6), on BOTH the STUDIO profile (resolver→{}) AND the EXPERT profile (source-chips #2/#3/#5 → registers, matching the built-in with sources). Plus the A-axis confirm: the rotate emit is byte-identical to the built-in + carries #58=[0-#53-90] + G0 A#58 + #59=#883 + the WCS-stride+3 write; REPORT emits no A work-offset (the guard pruned the SWO). INDEPENDENT TRUTH: the built-in rotaryClockStack is a separate code path.

**E0 GATE STILL GREEN after the rotaryClockHeaderComments refactor** (the _hdr texts reproduce the inline strings byte-for-byte; the superset==concrete sweep re-passed). **FULL SUITE: 708 passed, 2 skipped, 0 failed** (no flake this run). Files: web/wizards/rotaryClockWizard.js (added the header-comments helper + wired _hdr) · web/blocks/dataOps/rotaryClockData.js (NEW) · tests/rotary-clock-data-emit.spec.js (NEW). **BYTE-DIFF = ZERO, both profiles, A-axis a clean no-op.** NEXT (per dispatch): CL-E2 (sim-starts single-start parity). PASS BACK.

## 🔨 turn 427 (cycle 127) — ROTARY CLOCK E2: the clock sim (single sim-start + inherited box/rig; sim-only, emit byte-identical). E1 banked (f996395). Full suite 710 pass. 4 files.

**THE KEY FINDING — the clock is a FLAT ON A BOX, NOT a round bar.** The built-in rotaryClockView.onOpen calls `restoreBoxStock()` (revert a prior rotary round bar) — the OPPOSITE of the Centreline's activateCylinderStock. The clock datums the A axis off a FLAT, so the workpiece is a rectangular BOX on the rotary axis. So — UNLIKE the Centreline — the Clock needs **NO def.simStock**: the global box stock is already correct, and the rig renders a **4-JAW chuck** (box) instead of the 3-jaw (bar). (_rotaryRigSpec already derives jaws from stock.shape → the E4 rig handles both.)

**THE SINGLE SIM-START (no BUILT_IN.rotary_clock existed → added it):** the clock does 2 Z-down touches a SPAN apart, but that is ONE start + a +Y STEP (not a repositioned pass), so it is single-start (the built-in used `wizard.inferStart`, a single {x,y,z}). ADDED `BUILT_IN.rotary_clock(params, stock)` to opSimStarts.js returning `[{x:sx/2, y:sy/2−span/2, z:min(5,sz/2)}]` (above the flat near the top, at POINT A = −Y half the span). ONE-SOURCE: refactored the built-in `RotaryClockWizard.inferStart` to delegate to `opSimStarts('rotary_clock', …)[0]` (+ added `inferStarts`) — byte-identical values, so the built-in + the twin now read the SAME formula (mirrors the Centreline inferStarts delegate). `def.simStartsProvider = (params, stock) => opSimStarts('rotary_clock', params, stock)` on the twin (sim-only, via the registry → setUserSimStarts, NOT the builder).

**THE RIG INHERITS FREE:** the twin's declared `sim{rotary:true}` (E1 stack) + the generic userOpView rig wiring (Centreline E5) → the 4th-axis rig shows on the clock with ZERO new code. Confirmed rendering (4-jaw on the box).

**VERIFY (tests/rotary-clock-sim-starts.spec.js, 2/2 — assert-the-value vs an INDEPENDENT-TRUTH formula, not just twin==built-in):**
1. **unit** — opSimStarts('user_rotary_clock_data', c, stock) COUNT = 1 + POSITION == the hand-derived `{sx/2, sy/2−span/2, min(5,sz/2)}` across span/stock/action combos (incl. a small-z clamp + no-stock defaults), AND == the built-in RotaryClockWizard.inferStart (single-start parity by construction, asserted); emit BYTE-IDENTICAL (twin builder == rotaryClockStack for set + rotate/side).
2. **REAL-SYMPTOM** — the twin opens on a BOX stock → the 3D renders the BOX + the 4th-axis RIG (child of _partGroup, spins/datum-placed) + the single Start marker; the 2D layout shows the stock footprint + Start (screenshot scratchpad/rotary_clock_e2_sim.png — the green box in a 4-jaw chuck + tailstock, probe on point A, NOT a bar).

**FULL SUITE: 710 passed, 2 skipped, 0 failed.** NO regression from touching shared opSimStarts.js (added a new BUILT_IN key) or the built-in inferStart (refactored to delegate — byte-identical values, the Blocks-tab + rotaryClockView callers still get a single {x,y,z}). rotary-clock-data-emit still ZERO (sim-only). Files: web/viz/opSimStarts.js (+ BUILT_IN.rotary_clock) · web/wizards/rotaryClockWizard.js (inferStart delegates + import) · web/blocks/dataOps/rotaryClockData.js (+ def.simStartsProvider, no simStock) · tests/rotary-clock-sim-starts.spec.js (NEW). **NEXT (per dispatch): CL-E3 (opensAs in-place + title + seed + RELEASE — COMPLETES Rotary, then Alignment).** PASS BACK.

## 🔨 turn 429 (cycle 127) — ROTARY CLOCK E3: the IN-PLACE milestone (opensAs + seamless title + seed) — COMPLETES the Rotary port (both wizards). Full suite 713 pass. RELEASE is the ADVISOR's (dispatch: "then I RELEASE V10.89").

**THE ONE-SOURCE WIRE (2 tiny files, the corner/edge/middle/centreline mechanism):** `opensAs: 'user_rotary_clock_data'` on the built-in Clock A0 entry (wizardLibrary.js:53) — ONE declaration drives ALL THREE: the click re-point (commandDeck.js:84 → openWiz), the twin's own-entry HIDE (OPENS_AS_TARGETS auto-includes it), and the seamless plain "Clock A0" title (builtinLabelForTwin auto-maps). + the app.js seed (import rotaryClockDataDef + add to seedDefaultPortedUserOps). NO WIZ_SPECIAL_OPENER for rotary_clock (only alignment has one); no WIZ_SUBLABEL. The built-in rotaryClockWizard/rotaryClockView stay the legacy shim (blocks-rotary-rig, which opens rotary_clock via wizardManager.open, still green).

**THE IN-PLACE EXPERIENCE REPORT — screenshot scratchpad/rotary_clock_e3_inplace.png (opened):**
1. **click re-points** — the built-in Clock A0 slot opens user_rotary_clock_data in-place; title reads plain **"Clock A0"** (seamless, no "(data)"); the twin's own data-wiz entry is GONE.
2. **FORM non-empty** — 11 knobs: dist, retract, f_fast, f_slow, port, **span (#6)**, safeZ, **action**, **reference**, wcs, safeZFrame.
3. **3D SIM** — the BOX (a flat, not a bar) + the 4th-axis RIG (4-JAW chuck on −X, tailstock body+cone on +X) + the single Start marker at point A. DRO "4 probes · 7 rapids". The rig is a child of _partGroup (spins + datum-placed).
4. **2D VIEW = COHERENT** — the form3d+2d 2D toggle works; the layout canvas draws the stock footprint + a "Start" marker (generic top-down, same as the Centreline — NOT broken/empty; the rotary datum-glyph 2D stays the deferred follow-on).
...
## 🔨 turn 433 (cycle 127) — ALIGNMENT E0: the superset gate + golden + the F2 restructure (the LAST probe fan-out port). Forks RULED F1=GUARD/F2=YES/F3=display-only. Byte-diff ZERO across the 8-combo structural sweep. 2 files.

**F2 RESTRUCTURE (first):** #20 from `[0-${safeZ}]` (baked literal `[0-10]`) → `[0-#19]` (references #19=safeZ). Matches the established #9=[0-#2] pair pattern + enables the E1 safeZ→#19 binding (the ref TRACKS #19, never re-bakes the value). The emitted VALUE is unchanged ([0-#19] resolves to [0-safeZ]=-10) but the emit is NOT byte-identical to the pre-restructure literal — a one-line literal→#ref change. Since ground-truth = our OWN golden (no dumped angle macro), this is the golden.

**F1 SUPERSET (checkAxis × probeDir = 4 arms, GUARDED):** refactored alignmentStack → `alignmentStack(params, opts)` with `buildBody(checkAxis, probeAxis, plus)` (the whole probe body parameterized by the fence axis + probe dir). superset:true wraps 4 arms — nested GUARD({checkAxis:X/Y}, [GUARD({probeDir:pos/neg}, buildBody(...))]); pruneGuards collapses to one. WHY GUARD BOTH (my call from the code per the dispatch's latitude): checkAxis threads the axis LETTER through ~16 atoms (RM/PR/CK/RD/MV) AND probeDir's dirLabel is ENTANGLED in the same header comment (`Probe: ${probeAxis} ${dirLabel}`) + its probeVar/retractVar (#7/#8, #9/#10) thread ~8 twoPass atoms → recomposing either after the other's prune is fragile. Guarding both → each arm fully correct → the E1 twin recomposes ONLY the axis/dir-INDEPENDENT value-swaps (safeZFrame + the scalar/tolerance header comment). Valid-by-construction (4× build-time superset, prune→1). safeZFrame stays a VALUE-swap (read from params on both gate sides, no guard — the clock precedent).

**VERIFY (tests/alignment-superset.spec.js, 2/2):**
1. **E0 GATE** — prune(superset, c) === concrete alignmentStack(c) BYTE-IDENTICAL across ALL 8 combos (checkAxis 2 × probeDir 2 × safeZFrame 2). diffCount=0, leftoverGuards=0 (the nested guards fully collapse), the superset carries ≥4 guards (2 outer checkAxis + 4 inner probeDir = 6). INDEPENDENT TRUTH: the concrete builder is a separate path.
2. **GOLDEN + F2** — #20=[0-#19] (the ref, NOT [0-10]); the old baked literal is GONE; #19=10 (the binding target); at safeZ=25 → #19=25 and #20 STAYS [0-#19] (the ref tracks, never re-bakes); the misalignment-angle compute (#1512=atan) is intact; superset:false === default (self-consistent).

**ALIGNMENT NOTES (no wcs fork — a measurement, no SWO; no dumped angle macro — golden own emit).** **FULL SUITE: 715 passed, 2 skipped, 0 failed.** NO regression from the alignmentStack refactor: the concrete emit is byte-identical to the prior EXCEPT #20 ([0-10]→[0-#19], F2); no existing test asserted the old #20 (the #20= greps were the MIDDLE golden, a different op). Files: web/wizards/alignmentWizard.js (refactored: buildBody + superset) · tests/alignment-superset.spec.js (new). **BYTE-DIFF = ZERO — the E0 gate is PASSED; F2 landed (#20=[0-#19], value-equivalent).** NEXT (per dispatch): E1 (twin + bindings; F3 tolerance display-only comment recompose), E2 (sim-starts = the existing BUILT_IN.alignment), E3 (in-place + RELEASE — completes the probe fan-out). PASS BACK.

## 🔨 turn 435 (cycle 127) — ALIGNMENT E1+E2 (bundled): the data-op twin + bindings + the (existing) sim-starts. Byte-diff ZERO across 8 struct + a scalar sweep on BOTH profiles; sim-starts == BUILT_IN.alignment. Full suite 718 pass. 3 files.

**THE TWIN (mirror rotaryClockData, the simplest port):** alignmentDataDef via userOpFromStack wrapping alignmentStack(params,{superset:true}) in user_root (panel form3d+2d, sim {rotary:false, machine:true} → the DEFAULT BOX, NO rig — the edge/middle pattern). NOT seeded/opensAs yet (E3).
- **6 VALUE bindings by #var identity:** #1 dist / #2 retract / #3 f_fast / #4 f_slow / #5 port / **#19 safeZ** (bindable via the E0 F2 restructure — #20=[0-#19] tracks it, so the ±safeZ pair is ONE source). NO #6 (alignment has no radius/span). No optional/when-gated binding → the canonical prune is just the DEFAULTS.
- **2 STRUCT bindings — checkAxis(X|Y) + probeDir(pos|neg)** — drive the E0 4-arm guards (segmented widgets).
- **VALUE-SWAPS via postInstantiate recompose:** applyHeaderComments (ONLY the scalar/tolerance header line — the checkAxis/probeDir comments are GUARD-handled, the pruned arm is already correct, so only the axis/dir-INDEPENDENT line 3 needs recompose) + applySafeZFrame (the park move→machinemove; alignment has NO 'Final retract' anchor so it targets the block AFTER the #54 ATAN assign — disambiguates from the earlier MV Z#19 lift). **F3: tolerance is DISPLAY-ONLY** — a number field that recomposes ONLY the header comment (no #var; the macro measures the angle, never enforces a tolerance). Plus applyProbeSources (#2/#3/#5 source-chips). Extracted alignmentHeaderComments (the shared scalar-comment source).
- **E2 (a one-line wire):** def.simStartsProvider = opSimStarts('alignment') — the EXISTING BUILT_IN.alignment (2 starts A/B, checkAxis-aware), sim-only. NO def.simStock (the default box is correct).

**VERIFY (tests/alignment-data-emit.spec.js, 3/3 — assert-the-value):**
1. **byte-diff ZERO** across (a) the 8-combo structural sweep (checkAxis 2 × probeDir 2 × safeZFrame 2) at default scalars, (b) a scalar sweep incl **safeZ** (→#19, #20 tracks) AND **tolerance** (the F3 header recompose), on BOTH the STUDIO profile (resolver→{}) AND the EXPERT profile (source-chips → registers); WIRING (dist=99→#1, safeZ=99→#19 with #20 STAYING [0-#19]). INDEPENDENT TRUTH: alignmentStack is a separate path.
2. **sim-starts** — the twin's opSimStarts('user_alignment_data', c, stock) COUNT=2 + POSITIONS == BUILT_IN.alignment across checkAxis combos; emit byte-identical.
3. **REAL-SYMPTOM** — the twin opens on a BOX → the 3D renders the BOX + the 2 fence starts (A/B) + NO rig (viz._rotaryFixture falsy); the form is non-empty with the checkAxis/probeDir/safeZ/tolerance knobs (screenshot scratchpad/alignment_e2_sim.png — the box with the probe on the +Y fence edge, 2 orange markers, the code preview showing #20=[0-#19]).

**FULL SUITE: 718 passed, 2 skipped, 0 failed.** NO regression from the alignmentWizard.js _hdr refactor (E0 re-passed byte-identical) or the new alignmentData.js. Files: web/wizards/alignmentWizard.js (+ alignmentHeaderComments, use _hdr) · web/blocks/dataOps/alignmentData.js (NEW) · tests/alignment-data-emit.spec.js (NEW). **BYTE-DIFF = ZERO both profiles; sim-starts parity; box + 2 starts render.** NEXT (per dispatch): E3 (opensAs in-place + title + seed + RELEASE — COMPLETES THE PROBE FAN-OUT). PASS BACK.

## 🔨 turn 437 (cycle 127) — ALIGNMENT E3: the IN-PLACE milestone (opensAs + seamless title + seed) — COMPLETES THE PROBE FAN-OUT (all 6 probe wizards in-place). Full suite 721 pass. RELEASE is the ADVISOR's (dispatch: "then I RELEASE V10.90").

**THE ONE-SOURCE WIRE (the corner/edge/middle/rotary mechanism):** `opensAs: 'user_alignment_data'` on the built-in Align entry (wizardLibrary.js) — ONE declaration drives the click re-point (commandDeck.js:84 → openWiz), the twin's own-entry HIDE (OPENS_AS_TARGETS), and the seamless plain "Align" title (builtinLabelForTwin). + the app.js seed. **RETIRED the WIZ_SPECIAL_OPENER** — alignment was the LAST entry in it (`{ alignment: 'openAlignmentWiz' }`); now `WIZ_SPECIAL_OPENER = {}` (all the special 3D-animated openers are gone from the menu — the opensAs generic path replaced them). The openAlignmentWiz global SURVIVES (index.html/globalFunctions) as the legacy shim, just no longer menu-routed. Updated tests/wizard-bar.spec.js (the pinned Align routing: openAlignmentWiz → openWiz('user_alignment_data')).

**(7) THE G68 REAL-CORRECTION IS UNAFFECTED — CONFIRMED.** The ⟳ Align rotate flow (web/data/rotateProgram.js + window.ddcsAlignRotate, the program-rotation transform) is COMPLETELY SEPARATE from openAlignmentWiz (the measurement wizard). opensAs re-points only the MEASUREMENT wizard's menu slot; rotateProgram/ddcsAlignRotate are a different code path (the editor ⟳ button), untouched. The test asserts openAlignmentWiz still resolves (the shim survives) — the rotate button never used it.

**THE IN-PLACE EXPERIENCE REPORT — screenshot scratchpad/alignment_e3_inplace.png (opened):**
1. **click re-points** — the built-in Align slot opens user_alignment_data in-place; title reads plain **"Align"** (seamless, no "(data)"); the twin's own entry is GONE; openAlignmentWiz un-routed but still defined.
2. **FORM non-empty** — 10 knobs: dist, retract, f_fast, f_slow, port, safeZ, **checkAxis (X/Y segmented)**, **probeDir (+/− segmented)**, safeZFrame, **tolerance**.
3. **3D SIM** — the BOX (a fence, not a bar) + the 2 fence Start markers (A/B along the +Y edge) + NO rig (alignment is not rotary). DRO "4 probes · 7 rapids".
4. **2D VIEW = COHERENT** — the layout canvas draws the stock footprint + Start (generic top-down, not broken/empty).
...
## 🔨 turn 879 — TWO-SIDED JOBS, BUILD PHASE 1 (backlog item 11, Option A per-section flip). Advisor ruled A + phased; this turn built phases 1's five pieces (emit + byte-identity + prog-marker round-trip + setup sheet + estimate split). Phase 2 (preflight-per-setup) + phase 3 (sim stock-flip + carve-carry) are QUEUED, not built.

### THE MODEL (the mirror is baked into coordinates — the xform-rotate precedent)
- **`mirrorProgram(gcode, axis, sx, sy, sz)`** in data/rotateProgram.js — modeled on rotateProgram/translateProgram (parse lines, track G90/G91, skip G53). Flip-about-X reflects Y (`Y'=sy-Y`); flip-about-Y reflects X (`X'=sx-X`) — about the STOCK SPAN so the part stays in its footprint (re-registered to the same corner). Arc mirror reverses handedness (G2<->G3) + flips the reflected centre offset (J for X-flip, I for Y-flip).
- **THE Z-INVERT DECISION (non-obvious):** a whole-program Z-flip `Z'=-(Z+sz)` is correct for CUT geometry but WRONG for clearance/rapids (a rapid at Z+5 above the top would map below the stock — a dangerous plunge). So I invert Z only for `Z<=0` (cut geometry); clearance `Z>0` stays above. This is the honest phase-1 approximation; the authoring-model + clearance edge is a refinement worth its own pass. The acceptance's hand-computed asserts are on the XY mirror (exact).
- **`setup` + `flip` blocks** (ops/transform.js, registered in PALETTE Transforms): `setup{title,index}` is a transparent section-derivative container (added to the emit's transparent list next to section/param_group/guard, so wrapping ops is byte-transparent); `flip{axis,setup}` is a flat CHILDLESS sibling modeled EXACTLY on xform (kind:'flip' -> emits [] like xform/entry). The flip carries a SETUP INDEX rather than being nested in the section — this makes the prog-marker round-trip clean (the marker slot carries top-level siblings) AND keeps one consistent shape. `flipForSetup(stack, index)` finds it.

### WHY the flip is a top-level sibling (not nested in the setup) — the round-trip forced it
- The prog marker (programModel.progMarkerLine) carries CHILDLESS PROGRAM-LEVEL siblings (xform + entry). A transparent setup CONTAINER cannot round-trip through the .nc builder-marker system (its children are arbitrary user ops, not builder-derived). So I kept the setup containers on the .mjson/Blocks round-trip (registered blocks serialize wholesale) and put the FLIP on the prog marker exactly like xform (`flipAxis`/`flipSetup` added to progMarkerLine + progBlocksFromMarker + makeFlip in programFraming). Verified: export carries the marker, reimport recovers the flip sibling.

### THE EMIT SEAM (scoped, not whole-program)
- `applySetupFlips(T, opRanges, blocks, settings)` in blockEmitter.emitMapped — for each top-level `setup` block, find its flip (by index), mirror ONLY that setup's own `opRanges` line range (data/mirrorProgram). Placed RIGHT AFTER the emit (line 303), BEFORE applyToolChanges/applyEntryWaypoint — because those two INSERT lines into T (splice), which would stale opRanges; mirrorProgram is a 1:1 line map (length-preserving) so the ranges stay valid for the later passes. Unlike applyProgramTransform (whole-program), this is scoped per setup so setup 1 stays put.
- **Byte-identity:** no setup blocks -> applySetupFlips is a no-op; a setup with no flip -> transparent; verified wrapped==flat + two-setup-no-flip==unmirrored. (The full gate's 1291 passes cover the goldens.)

### SHEET + ESTIMATE + HONEST NOTE
- setupSheet.js: `setupGroups(raw)` collects ops per setup container (recursive `collectOps`); no setups -> null -> the classic single Operations section (byte-identical path). Two setups -> a `.ss-setup` section per setup with a page break, the flip instruction (`flipInstruction` — plain wording pending the user's taste), and a per-setup run-time subtotal. `setupTimeSplit()` exported = the estimate split (per-setup op-time sum). Screenshot verified: FRONT/SETUP 1 (5s) + BACK/SETUP 2 (7s, flip instruction + honest note).
- **The honest carve note** (advisor's interim stand-in for phase 3): the side-2 toolpaths render mirrored FOR FREE (the emit bakes the mirror; the sim traces the emitted coords 1:1 — the free correctness), but the SIM does not yet re-show the prior-setup carved stock flipped. Surfaced in TWO places: the sheet's setup-2 section, and the preview panel's `.pp-carve-note` (createPreviewPanel — when compOps() finds a flip; takes precedence over the carve-tip note; non-two-sided programs untouched).

### VERIFY
- tests/two-sided-879.spec.js — 5/5 pass: mirror-about-X (hand-computed: stock 100x80x20, Y 10->70, Z -5->-15, X kept 10), mirror-about-Y (X 10->90), byte-identity (wrapped==flat + two-setup-no-flip==unmirrored), prog-marker round-trip (marker carries flipAxis/flipSetup, reimport recovers the flip), setup-sheet two pages + flip instruction + honest note + estimate split ([1,2]) + screenshot.
- **FULL GATE: 1291 passed / 1 failed / 4 skipped (12.5m).** The 1 failure = blocks-mobile-drawers.spec.js:8 — a 30s PAGE-SETUP TIMEOUT (not an assertion), on a known load-fragile mobile UI test whose golden was already modified before this turn. Under the heavy full-suite load (dozens of concurrent worker-process crashes, code=3221225794) it exceeded the timeout. RE-RAN IT IN ISOLATION -> 1 passed (14.6s). Confirmed infra flake, NOT a regression (check-console boots ZERO-error passed; all emit/block/palette/sheet tests passed; my changes don't touch mobile layout or boot).
- **CWD-DRIFT TRAP (field note):** after the gate, my handoff.py calls `cd git-root` drifted the shell cwd; subsequent single-file `npx playwright test` ran from the git root -> "two versions of @playwright/test" / test.use() collection error (looked like a corrupted Playwright, wasn't). Fix: always `cd DDCS-Studio` for the test runner (mirror of [[handoff-cwd-split-brain]] for the runner, not just handoff.py).

### QUEUED (fast-follows, not built)
- Phase 2: preflight per-setup frames (envelopeCheck one-trace -> per-setup). Phase 3: the sim stock-flip + carve carry-over across the flip (the honest note comes down then). Taste-parked for the user: the default flip axis + registration-edge convention; re-zero vs probe wording. Also: the Z-invert clearance-vs-cut refinement (authoring-model).

## 🔨 turn 881 — TWO-SIDED FAST-FOLLOWS, phases 2 + 3 in one turn (finish the feature). Phase 2 preflight-per-setup + phase 3 sim stock-flip + carve-carry; the honest t879 note comes DOWN. Grounded both seams with parallel Explore agents (envelope + sim) before building.

### PHASE 2 — PREFLIGHT PER SETUP (nearly free: a REPORTING split, not a re-trace)
- GROUNDED (Explore): checkEnvelope(program, settings) is ONE flat whole-program trace → {status, violations:[{line(1-based), axis, overshoot}], uncheckedProbes, reason}. Because it consumes the EMITTED text (which already has the baked side-2 mirror from t879's applySetupFlips), the single trace ALREADY sees the mirrored side-2 coords correctly. So per-setup is purely a REPORTING boundary — no re-trace, no per-setup WCS frame (that's a phase-2-frame extension the advisor's reminder parks).
- BUILT (setupSheet.js): `violationsBySetup(verdict, groups)` buckets each violation onto its owning setup via the PROJECTION MAP — `proj.map[v.line-1][0]` is the top-level ancestor block id; a `setup` is top-level so its id is [0]. `setupPreflightHTML(list)` names line·axis·overshoot on that setup's page (or a green line). Added the setup block `id` to setupGroups for the match. Rendered per `.ss-setup` section. Exported both for the unit test.
- Verified: a PLANTED side-2 violation (line in setup-2's range) buckets to setup 2 + is named "line 4 · Y+ · 3.2 mm over" on the side-2 page; a side-1 violation to setup 1; a clean setup → green line.

### PHASE 3 — SIM STOCK-FLIP + CARVE-CARRY (the honest note retired)
- GROUNDED (Explore): the carve height-field `HeightmapCarve` has h/hc Float32Arrays + idx(i,j) + spans X/Y/Z. The visual flip lever ALREADY exists — `_applyPartRotation` rotates `_partGroup`, and stock+carve are its children while the TOOLPATH/tool are children of partFrame (NOT flipped) → flipping _partGroup flips ONLY the stock+carve, leaving the already-coordinate-mirrored side-2 toolpaths put (exactly the seam we want). The boundary is invisible in the plain emitted G-code (setup emits transparently) but the PROJECTION MAP carries the setup id in every emitted line's ancestry → ddcsLinesForOp(setupId) min = the boundary line.
- **THE 2.5D CARRY LIMIT (the honest core):** the field is a TOP-ONLY 2.5D map. Under a physical flip it can only carry a THROUGH cut (h reaches -Z stays open from the new top); a BLIND top pocket becomes an underside feature a top-down field can't show → reads intact (0) from the new top. This is WHY the acceptance is a THROUGH-HOLE case. `mirrorField(axis, thickness)` (stockRemoval.js): reflect indices (j'=ny-1-j for X-flip / i'=nx-1-i for Y-flip) + the through-hole-preserving map; re-seeds the baseline so side-2 carves on the carried holes.
- **NON-OBVIOUS: `-(Z+h)` is BACKWARDS for the field.** The advisor's "mirror + Z-invert, same transform as the emit" is the AXIS+THICKNESS one-source, NOT literal arithmetic: mirrorProgram's `Z'=-(Z+z)` is a POINT-Z map; applied to a remaining-HEIGHT field it inverts full<->through (wrong). The field's correct carry is the through-hole-preserving map above. The XY reflection DOES align side-1 and side-2: the field reflects side-1's hole from y to sy-y, which is exactly where the emit put side-2's mirrored cut → they overlap correctly (NO double-mirror).
- BUILT (gcodeViz3d.js): `setPartFlip(axis)` sets `_partFlip` + is threaded INTO `_applyPartRotation` (adds π to the axis) so it survives the play loop's per-frame re-apply + a setStock rebuild; `carveMirrorField(axis, thickness)` calls the field mirror + re-meshes. (createPreviewPanel.js): `computeFlipBoundaries()` reads compOps()+ddcsLinesForOp → [{line, axis}]; onLineChange fires the flip+carry once per boundary per run; a fresh run clears the flip + recomputes. The t879 honest note is REMOVED (both the preview `.pp-carve-note` override AND the sheet flipInstruction "coming next" line).
- Verified: mirrorField unit (through-hole carries to the reflected cell as -Z; blind pocket → 0, the honest limit); setPartFlip('X') → _partGroup.rotation.x ≈ π + the carried stock renders (screenshot VIEWED); the note is gone (sheet + panel).

### MIGRATION (not a regression)
- The t879 sheet test asserted the "not yet re-shown flipped" caveat is SHOWN; phase 3 intentionally retires it. Updated that test (dropped the stale caveat assertion) — t881 owns the retirement assertion now. [[confirm-premise-before-regression-blocker]] / [[assert-the-value-not-the-change]].

### VERIFY
- tests/two-sided-881.spec.js 4/4 + tests/two-sided-879.spec.js 5/5 (updated). App boots ZERO console errors.
- **FULL GATE: 1296 passed / 0 failed / 4 skipped (12.5m), GATE_EXIT=0.** Fully green (the t879 blocks-mobile-drawers infra flake did NOT recur — load was fine this run). Ran from DDCS-Studio (cwd discipline held).
- Single-setup untouched: _applyPartRotation adds the flip ONLY when _partFlip set (null default); onLineChange fires ONLY when _flipBoundaries non-empty; mirrorField/setPartFlip never called without a flip declaration.

### STILL QUEUED / PARKED
- The 2.5D blind-feature carry (a two-sided FIELD tracking both top+bottom removal) — beyond the through-hole acceptance; a bigger change. The per-setup WCS FRAME in preflight (today one active offset for the whole trace). The authoring-model question (part-frame through-thickness Z vs per-setup depth) — PARKED for the user per the standing reminder; did NOT refactor the model.

## 🔨 turn 883 — ATC LEFTOVERS (backlog item 12): GROUND-then-GATE. The dispatch framed 3 "deferred, unbuilt" pieces; grounding (2 parallel Explore agents over the ATC sim + magazine model) found 2 of the 3 are ALREADY BUILT + test-locked. Reporting the honest state + the ONE real gap + the DECLARE fork, per the dispatch's own "ground honestly, name the gap first, gate back rather than build" clause. NO code this turn (a gate).

### THE HONEST STATE (premise correction — the 3-piece "unbuilt" framing doesn't match the code)
- **PIECE 1 — machine-frame ATC sim: ALREADY BUILT + tested (NOT a gap).** gcodeViz3d.setMagazine (1865-1899) / highlightStation (1909-1940) / setStationDevices (1974-2002) / showRetiredTool (1948-1965) all add to `this.scene` = the FIXED MACHINE FRAME at RAW declared machine coords, sourced from magazinePockets(settings.atc) (atcViews.js:62-84). setMachine (1820-1856) draws the envelope with machine-home at scene 0. Wired via applyPreviewIntent (atcViews.js:408) + setAtcSwap (createPreviewPanel.js:1037) + io_change M-codes. LOCKED by ~6 sim specs (atc-magazine-frame [pocket at machine(100,100,-50) renders there, WCS-independent], atc-station-highlight, atc-station-devices, atc-collet, atc-pocket-swap, atc-disk-swap). ACCEPTANCE #1 (magazine at its declared spot) ALREADY PASSES. Residuals (NOT the named feature, would churn the 6 specs): the ANIMATED moving tool rides the PART frame + aligns to machine coords via a WCS-cancel trick (works, documented in atc-magazine-frame.spec:6-11); undeclared pocket XYZ coerce to 0 -> pile at the origin (data hygiene).
- **PIECE 3 — 3D magazine glyphs: LARGELY BUILT.** Each OCCUPIED pocket already draws the real tool as a revolved silhouette (toolHalfProfile honors type/dia/length/angle, toolProfile.js:21-48) + a wireframe slot box + a number sprite. Tool-table tie-in DONE (magazinePockets.toolOf -> byNum map from atc.tools; getTool(num)). GAPS: the disk RING/PLATE is NOT a 3D mesh (only the pockets are ring-laid; the 2D atcSetupCanvas draws the ring guide, the 3D view has no carousel plate); EMPTY slots aren't drawn.
- **PIECE 2 — disk indexing: THE REAL DECLARE GAP.** The disk declares WHERE the ring sits (atc.pickup {x,y,z} + diskDia + diskAxis) + WHO is in each pocket (magazine[].tool), but the INDEXING SEMANTIC IS INFERRED FROM ARRAY ORDER, never declared: magazinePockets disk branch = `ang = ang0 + (i/n)*2pi + theta`, n = magazine.length (computed), i = ARRAY POSITION, ang0 hard-wires pocket-0-at-pickup (offset == 0). diskTheta(toolN) = -(i/n)*2pi mirrors it. So: no declared slot count, no declared per-slot index/angle, NO declared angular offset. (Also: diskAxis is the "disk toward" pickup->centre DIRECTION, NOT the rotation axis — rotation is implicitly machine Z.)

### THE DECLARE FORK (piece 2 — the advisor rules; north-star "extend the declaration, never infer")
- **A (minimal, my lean): declare `atc.diskOffsetDeg`** — ONE whole-carousel angular offset folded into ang0. index = array position, n = magazine.length (status quo). Slot angle = i*360/n + offset. LITERAL to the advisor's formula ("index x 360/n + the declared offset"); the smallest DECLARE that closes the named gap.
- **B (+ declared slot count `atc.diskSlots`):** space by PHYSICAL slots (>= loaded), so empty slots get real angles. Needed ONLY if a carousel has empty physical slots — rule-of-three, add when a real config forces it.
- **C (+ per-slot declared index/angle):** each magazine row names its carousel position — fully explicit, survives reorder without array-order coupling. Over-declaration for v1 (array order + offset suffices); the long-tail.

### RECOMMENDATION (focused real scope for the re-dispatch)
1. PIECE 1: VERIFY by screenshot (no build — it exists); optionally flag the animated-tool-part-frame seam + undeclared-XYZ-origin-pile as separate small items.
...
## 🔨 turn 887 — THEME MOTION PERSONALITIES (backlog item 15): finish the per-theme drawer-motion token sets (design ratified t741/t752). GROUNDED first (1 Explore agent over the token system + the one engine), then built the 5 complete distinct personalities + closed every silent no-op. View-only; no emit path.

### GROUNDING — the state + the gaps (the crux = silent no-ops)
- The tokens live as CSS `--drawer-*` blocks per `[data-theme]` (styles.css); the ONE engine (paneAccordion.js) reads them via getComputedStyle(body) + sets data-reveal/-dir + --drawer-{dur,ease}-eff. All 5 themes already had a COMPLETE 6-token block — but several DECLARED VALUES were SILENT NO-OPS: (a) `slide` had CSS only for dir=left/right → studio+normal+:root all declared slide+`up` = NO reveal; (b) `dir: up/down` had NO CSS rule anywhere (all 5 declared `up`) → direction dead; (c) the 8-way diagonals never referenced; (d) easing was a raw CSS timing-function pass-through — the ratified KEYWORD vocab (spring/ease-out/linear/overshoot) was NOT implemented (a keyword would reach CSS as an invalid timing-function → no-op); (e) `fade` implemented but used by no theme (orphan). Caps already shipped: DUR_CAP=350 (paneAccordion:23,35) + reduced-motion=>instant (24/72/77-84 + CSS backstop). Consumers: pane accordions (wizardManager:271) + form-section folds (formWidgets:837-838) route through the engine; the DRAWERS (projectModal/gcodePreviewTab/jogPendant/blocksApp) are BESPOKE, NOT engine consumers.

### BUILD — close every no-op, then 5 tasteful distinct personalities
- **dir consumed by EVERY reveal:** direction now drives the fold `transform-origin` (up=top/down=bottom/left=left/right=right), so roll/unfold pivot from it; added slide up/down (translateY) + fade DRIFT per dir + made wipe clip from the dir edge (4-way). So every theme's declared dir has a real effect. TRIMMED the 8-way vocab to 4-way HONESTLY (diagonals never implemented + read poorly on a vertical accordion — a trimmed vocab beats a no-op token).
- **easing KEYWORD map** in the engine (paneAccordion.js): `mapEase` = {linear, ease-out, overshoot, spring} → curves; a raw bezier still passes through. So a theme declares `--drawer-ease: overshoot` (semantic) + it resolves to a real curve (never an invalid timing-function). Applied at --drawer-ease-eff.
- **fade de-orphaned:** normal uses fade. Each of the 5 themes now uses a DISTINCT reveal (slide/fade/wipe/unfold/roll — the whole vocabulary, one per theme).
- **The 5 personalities (aesthetic judgment within the vocab):** studio = CALM ease-out slide up, near-square (2); normal = NEUTRAL linear fade drifting down (5); futuristic = QUICK overshoot wipe from the right, square (0); organic = SOFT spring unfold from the top hinge, big morph (14→22); steampunk = FIRM ease-out roll down, radius tightens (8→6). No two blocks identical.

### VERIFY
- tests/theme-motion-887.spec.js 4/4: every block complete (6 tokens) + distinct (5 unique signatures + 5 distinct reveals); the NO-OP SWEEP (runtime — for each theme the declared reveal produces a motion [transform/opacity/clip], the dir CHANGES the motion [consumed], the easing maps to a real curve); duration capped to 350 (asserted with a 9999ms declaration); reduced-motion => 0ms instant (asserted); screenshots per theme (VIEWED — organic soft/rounded/unfold vs futuristic sharp/square/wipe are clearly distinct personalities).
- The real consumers hold: collapsible-panes-752 (13) + form-section-collapse-820 pass, incl. 752's own distinctness + cap/reduced-motion + emit-byte-identical-when-collapsed. App boots ZERO console errors.
- **FULL GATE: 1304 passed / 0 failed / 4 skipped (12.4m), GATE_EXIT=0.** Fully green. Ran from DDCS-Studio.
...
## 🔨 turn 891 — THE TRANSAXIAL TRAVERSE (user live finds): GROUND-then-GATE. Grounded both wizard emit paths (2 Explore agents, corner + middle). 2 of the 3 dispatched premises are CONTRADICTED by the code + goldens — gating rather than "fix" a crash-class emit gap the corpus PROVES doesn't exist (a change there would risk breaking proven-safe emit). NO code this turn (a gate). Amendment (wall-2 draggable marker) folded into the finding-2 plan.

### FINDING 1 — CORNER DIAGONAL "DOES NOT LIFT": the EMIT is PROVEN SAFE (premise contradicted)
- Both travel shapes traverse at the SAME machine-frame G53 safe margin, set UPSTREAM of the travel-shape fork: the per-wall `safeRetractNode({restore:'inc'})` at cornerWizard.js:215 (emits `G53 Z#520`), shared by dogleg AND diagonal. The wall1->wall2 reposition (`repoArmR`, cornerWizard.js:318-324) forks ONLY on `firstAxis` (dogleg=sA / diagonal=undefined) → `safeTraverseStack` mode:'seq' (probeSurface.js:150-157): dogleg = two axis-aligned rapids, diagonal = one combined XY rapid; NEITHER passes `lift`; `doLift()` is a no-op for both; both drop the SAME `#18`/`[0-#19]` AFTER the XY. So the diagonal adds ZERO Z vs the dogleg. The t826 sweep did NOT miss it (it changed the shared upstream retract, line 215).
- PROVEN by goldens: **corner-travel-shape.spec.js** `onlyDiffIsTheMerge` (diagonal === dogleg with `G0 X#23\nG0 Y#24` merged to `G0 X#23 Y#24` — nothing else, no Z), `zCountEqual`, `dropValueOk`; **corner-z-trust.spec.js** asserts `^G53 Z#520$` present + the retired incremental lifts absent.
- THE LIKELY SOURCE OF THE USER REPORT: (a) a STALE, SELF-CONTRADICTING COMMENT at cornerWizard.js:302-305 ("this traverse runs at SCAN DEPTH … a diagonal could clip the corner") — pre-t824/t826 behavior, contradicted 13 lines below at 315-317 ("ZERO added Z"); echoed at corner-travel-approach.spec.js:57. (b) the 2D top-down sim shows the diagonal LINE crossing the corner in XY while it is LIFTED in Z — visually alarming, actually safe.
- **NOT a crash-class EMIT gap.** The open question is the SIM: does the 2D/3D sim RENDER the traverse at the lifted Z (then it's a pure visual-misread + a stale comment), or does the sim render it at scan depth (then it's a SIM-RENDER bug worth fixing — the sim would be lying about a safety move)? I did NOT reproduce the sim this turn (gate). RECOMMEND: reproduce the corner diagonal sim FIRST; then fix the stale comment + (if the sim under-renders the lift) the sim's traverse-Z rendering. This is a CLARITY/sim fix, not a G-code safety fix.

### FINDING 2 — MIDDLE TRAVERSE DOES NOT CONNECT: a REAL geometry bug (confirmed)
- The trans-axis traverse (`transTraverseR`, middleWizard.js:148-154 → safeTraverseStack mode:'center', probeSurface.js:122-138) lands the SECONDARY axis at `centre ± #21` (`diagTravel`, default **50**, CENTRE-relative) for BOTH dogleg + diagonal. But the WALL-2 probe START (`opSimStarts.js` BUILT_IN.middle `outside()`, lines 51-63) fires from `edge ∓ outset` where `outset = clamp(dist*0.6, 6..15)` (default **15**, EDGE-relative). Two INDEPENDENT formulas → a visible XY gap in 2D+3D (e.g. 100×80 boss X-primary: traverse ends ~Y=40±50, probe starts ~Y=0∓15). NO golden asserts traverse-end XY == wall-2 probe-start XY (the untested, broken continuity). The engine re-anchors the wall-2 pass to `outside()` independent of the traverse end (GcodeExecutionEngine.js:704-709; middle carries no `anchorsAtPrev`).
- AMENDMENT (folded in): make the wall-2 probe START a DRAGGABLE 2D-layout MARKER that DECLARES into the existing #21/#22 params (`diagTravel` `m_diag_travel` / `diagPrimary` `m_diag_primary`; the "Wall 2 dX/dY"). The auto default = the computed landing; a drag writes the params (the `tieDiagTravel` pattern, middleView.js:20-36 + the `diagAim` gesture, canvasWidgets.js:88-91). THE FIX SHAPE: make `opSimStarts.middle`'s wall-2 start READ the declared #21/#22 (the marker) instead of the independent `outside()` formula, so traverse-end == wall-2 probe-start BY CONSTRUCTION, wherever the user drags ②. Handles-independence holds (the drag pins only its own pass — createPreviewPanel.js:192/378). Round-trips via the params + the marker slot.
- This IS the real work. But its fix couples the emit landing, the sim start marker, AND the new draggable declaration — and the "which end moves to meet the other" is resolved by the amendment (the DECLARED marker is the source; both the traverse land + the probe start follow it).

### FINDING 3 — "CONSOLIDATE TO ONE SHARED TRAVERSE BUILDER": ALREADY DONE
- `safeTraverseStack` (wizards/ops/probeSurface.js:94) IS the one shared builder, imported + consumed by BOTH cornerWizard.js:19 (mode:'seq' + the manual/machineLift paths) AND middleWizard.js:24 (mode:'center'/manual). Grep finds NO second traverse builder. The only asymmetry: corner gets its G53 lift OUT-OF-BAND (the per-wall retract) while middle folds it IN-BAND (`machineLift:true`, probeSurface.js:100) — both via the shared builder. Nothing to consolidate.

### RECOMMENDATION (re-scope for the advisor's ruling)
1. CORNER (finding 1): NOT a crash-class fix. First REPRODUCE the sim (both shapes) to characterize the user's screenshot; then fix the stale comment (cornerWizard.js:302-305 + corner-travel-approach.spec.js:57) and, only if the sim under-renders the lift, the sim's traverse-Z. The EMIT stays (goldens prove it safe — do NOT touch it).
2. MIDDLE (finding 2 + amendment): BUILD the real fix — the wall-2 start becomes the DECLARED draggable marker into #21/#22, and `opSimStarts.middle` reads it so traverse-end == wall-2 probe-start by construction (numeric assert both shapes); auto default = byte-identical for untouched programs; round-trips; handles-independent.
3. CONSOLIDATION (finding 3): NONE needed — `safeTraverseStack` is already the one shared builder.
Awaiting the ruling: confirm the re-scope (corner = clarity/sim not crash-fix; middle = the marker fix; no consolidation), so I don't "fix" proven-safe emit. Full detail + line refs above.

### AMENDMENTS 2 + 3 (folded in) — the scope becomes a MAJOR architectural build; gating with the plan
- **AMD-2 DECLARE-INTENT INVERSION (the root cause, the user's model):** today the params declare the TRAVEL END (#21 diagTravel / #22 diagPrimary; corner #23/#24) and the probe start falls out → the disconnect. FLIP it (the [[datum-model-physical-derived-offset]] rule — declare the PHYSICAL point, derive the plumbing): the WALL-2 PROBE START is the DECLARATION (the draggable marker + fields express THAT point); the TRAVEL is DERIVED (wall-1 end → the declared probe start; dogleg|diagonal = just the SHAPE of the derived path). User: "right now I'm choosing the end of the travel — the travel should be calculated based on the probe start." Corner's reposition travel (#23/#24) gets the SAME semantic check — it has the same inversion (the crossX/crossY declare the travel target, not the probe start); fix under the same model if so.
  - MIGRATION (grounded + the honest choice NAMED): auto stays auto → byte-identical for untouched programs. A saved program with a TYPED travel-end (#21/#22 or #23/#24 literal) declared the travel END under the old model; under the new model the PROBE START is declared. The lossless honest map: probe-start := wall-1-end + the old travel vector (i.e. reinterpret the old typed travel-end as the probe start it implied) — geometry-preserving, no flag needed. (If a value can't map unambiguously, flag it rather than silently reinterpret.) RECOMMEND: reinterpret-to-probe-start for typed values; auto = byte-identical.
- **AMD-3 THE TRAVERSE BECOMES A REAL ATOM (user-ratified granularity):** `safetraverse { to: <declared target>, shape: dogleg|diagonal }` — a BLOCK that COMPOSES the existing `saferetract` atom (lift = saferetract untouched → the shaped travel → descend onto the target). ONE block per reposition in BOTH wizard stacks, visible in Blocks as one brick, round-trips (params + Blocks + gcodeToStack), available to custom-wizard composers. Today the traverse is a BUILDER FUNCTION (safeTraverseStack) that inlines the moves; the atom-ification makes it a first-class composable brick. saferetract emit stays byte-identical (composition, not duplication). Grep-acceptance: no inline traverse move-assembly left in either wizard builder.

### WHY GATE (not build this turn) — honest
- The task GREW from "fix a lift + connect a traverse" into a MAJOR architecture change: a NEW round-tripping `safetraverse` ATOM (composing saferetract, PALETTE + Blockly + gcodeToStack round-trip) + a MODEL INVERSION (declare the probe start, derive the travel) across BOTH probe wizards + a MIGRATION + a draggable marker + regenerated goldens + both-wizards-both-shapes screenshots. This is multi-turn work on a SAFETY-ADJACENT path (probe traverses a real machine). 2 of the 3 original premises were contradicted by the corpus (corner emit proven safe; the builder already shared) — so the framing needed correcting regardless.
- A rushed half-done atom refactor / model inversion would leave the probe wizards broken — worse than none. Gating with the precise grounding + the amendment-incorporated design lets this be BUILT deliberately (phased) rather than forced.

### PHASED BUILD PLAN (for the re-dispatch)
- P1 CORNER CLARITY: reproduce the corner sim (both shapes, VIEWED); fix the stale comment (cornerWizard.js:302-305 + corner-travel-approach.spec.js:57); fix the sim traverse-Z rendering ONLY if the sim under-renders the lift. Do NOT touch the proven-safe emit.
- P2 THE `safetraverse` ATOM: author the atom (composes saferetract + shaped derived travel + descend); register in PALETTE + opSchema; Blockly + gcodeToStack round-trip; port safeTraverseStack's emit into it byte-identically (saferetract unchanged).
- P3 THE INVERSION: flip both wizards to DECLARE the probe start (marker + fields) and DERIVE the travel to it; the draggable wall-2 marker writes the declared point; opSimStarts.middle + the emit both read it → traverse-end == probe-start by construction; migration reinterprets old typed travel-ends.
- P4 VERIFY: numeric assert traverse-end == wall-2 probe-start (both shapes, both wizards); handles-independence; auto = byte-identical; round-trip (params+Blocks+gcodeToStack); goldens regenerated where the fixes land (each justified); screenshots both wizards both shapes; FULL gate.
Awaiting the ruling on the plan / phasing before building (so the safety-adjacent atom + inversion are done deliberately, not rushed).

## 🔨 turn 893 — TURN A build: P1 corner clarity + the 2D distinct-travel + chip rider DONE + verified; P2 (the atom) DEFERRED; the mid-turn amendment AVALANCHE (6 riders incl. a CRASH-ADJACENT one) GROUNDED + flagged. View-only + comments this turn; no emit path touched → emit goldens byte-identical.

### DONE + VERIFIED THIS TURN
- **P1a — stale comments GONE:** cornerWizard.js:302-305 rewritten (the reposition runs at the SAFE-TRAVERSE height; both dogleg + diagonal cross LIFTED; firstAxis is just the dog-leg ROUTING ORDER, not a clip-avoidance); the echo at corner-travel-approach.spec.js:57 updated.
- **P1b — the 2D LAYOUT DRAWS SAFE-HEIGHT TRAVEL DISTINCTLY (the root fix):** a new `lifted` PATH_TYPE in the ONE palette (pathStyle.js — dimmed grey-blue #8a97ad, dashed [6,5], widthScale .6, dim .7). toolpath2d.typeOf classifies a HORIZONTAL rapid (z1≈z2, XY-moving — dog-leg leg OR diagonal, frame-independent, no fragile z-threshold) as `lifted` → dimmed/dashed; a Z-changing rapid (plunge/retract) stays `rapid`; feeds stay solid. The manual-jog arc preserved (transV reads the RAW rapid). So a top-down view can NEVER read the lifted corner-diagonal cross-over (crosses the corner in XY, safe in Z) as a cutting move.
- **P1c — the SIM REPRO VERDICT:** reproduced the middle wizard sim (VIEWED — 2D + 3D + honest DRO Mach DASHES). The corner EMIT was ALWAYS SAFE (goldens corner-travel-shape onlyDiffIsTheMerge + corner-z-trust G53 Z#520; the advisor verified 3/3). The report = a TOP-VIEW MISREAD (diagonal crosses in XY, lifted in Z) + the stale comment. The 3D traces the SAME lifted emit (shows the Z lift) → NO 3D under-render → NO sim-code change needed. Fix = the 2D distinct-travel (done) + the comment (done).
- **RIDER 1 — the op-edit chip never overlaps the pre-flight badge:** editorOpHover.placeChip — the chip is LINE-anchored (a literal flex row can't hold it) → a declared collision rule: flow the chip RIGHT of the badge (a row, gap 6) or STACK below at a narrow editor. Bounding boxes never intersect (asserted, 1300 + 850). [The THIRD rider re-contextualises this — see below — but placeChip STILL applies to the amber chip + op-edit chip.]
- **MIGRATION (not regressions):** the full gate found EXACTLY 2 failures — corner-travel-shape:110 + marker-colour-by-source:106 — both asserting the SUPERSEDED t328 "traverse = rapid-YELLOW". t893 makes the lifted traverse DISTINCT (the advisor's ruling). Updated both to assert the new lifted style (the segment TYPE stays 'rapid'; the reclassification to `lifted` is render-time in typeOf; NEVER source-cyan; the t328 motion-type intent preserved). Used segColor (robust logic) where pixel-sampling a thin dashed dimmed line was fragile. tests/traverse-clarity-893.spec 2/2 + the 2 updated + siblings 10/10.

### P2 (the safetraverse atom) — DEFERRED with the plan (safety-adjacent, byte-identical refactor; not rushed at depleted context). Plan unchanged from t891: a container block whose builder IS safeTraverseStack → byte-identical BY CONSTRUCTION; both wizards emit one block; round-trip via the parent op marker; Blockly auto.

### THE MID-TURN AMENDMENT AVALANCHE (6 riders consumed; recovered from .handoff/amendments.tsv after a head-truncated display) — GROUNDED + FLAGGED, not built (safety-critical + far beyond one turn at this context)
- **RIDER 4 — CRASH-ADJACENT, the advisor's "promote to the TOP" (HIGHEST PRIORITY next):** the LIFT/DROP PAIRING broke post-t826. The per-wall lift is now ABSOLUTE (G53 Z#520 to the machine margin) but the reposition DROP is still RELATIVE (corner: `z ? '#18' : '[0-#19]'`; middle: `[0-#17]`/`[0-#18]`). Pre-t826 BOTH were relative → up-then-down NET ZERO back to the probe depth. Now: the tool lifts to the ABSOLUTE #520, then drops RELATIVE by the old amount → it lands at #520+drop = an ARBITRARY Z, NOT the probe depth → the next probe starts from the wrong Z (crash if too low / miss if too high). The Z-TRUST comment (cornerWizard ~311 "round-trip nets zero") is now FALSE. SYMPTOM (user): the middle Safe-Z field "changes nothing in the sim" — because the drop no longer tracks the safe-Z reference the lift uses. NB corner-z-trust PASSES (it asserts the emit AS-DESIGNED) — so this is a latent DESIGN correctness bug the emit-golden doesn't catch (assert the RESULTING Z, not the literal). NEEDS: careful grounding of the lift Z (absolute #520) vs the drop reference across corner + middle, then re-pair (the drop must return to the probe depth given the absolute lift — likely an absolute drop to the probe-Z, or the atom owns the pairing). DO THIS FIRST next turn.
- **RIDER 3 (supersedes RIDER 2) — pre-flight UX overhaul:** RETIRE the red summary chip entirely; each violating line shows its diagnostic AT THE LINE END inline ('G0 Z8   <warn> Z+ 3.0 mm over'), red-tinted, one per bad line, from the SAME envelopeCheck output (one source). STATES: RED = no chip, per-line annotations only; AMBER can't-verify = a SMALL chip (no lines to annotate); GREEN = NOTHING (the green chip retires too). The send-path confirm reads the CHECK not the chip (unchanged). The chip-row rider (placeChip) then applies to the amber chip + op-edit chip. Substantial new UX — not built.
- **RIDER 5 + NUANCE — #520 register ownership (user-RULED):** #520 is NEVER written by programs — rewrite the inline guard to a SCRATCH-VAR read-with-fallback (`#30=#520 / IF #30<0 GOTO / #30=<baked margin> / N / G53 Z#30`) so the persistent register is READ-ONLY to emitted programs (same safety, same line count). Sysstart seeding out. PLUS the sanctioned write door: the Settings safe-Z margin field gains APPLY NOW (a one-line run-once `#520=<margin> / M30` via submitJob, gateway-live only, confirmed, in job history). Co-located with RIDER 4's emit sites. Safety-critical (a register write) — not built.

### GATE
- **FULL GATE re-run (after the 2 migration fixes): 1306 passed / 0 failed / 4 skipped (12.6m), GATE_EXIT=0.** Fully green. My changes are view-only (pathStyle/toolpath2d/editorOpHover) + comments — no emit path → the corner/middle emit goldens are byte-identical (every corner/middle emit spec passed).
- HONEST LANDING: I delivered a coherent P1 + chip-rider (verified). The 6 riders (a CRASH-ADJACENT bug + a register-ownership change + a pre-flight UX overhaul) + P2 are safety-critical, multi-turn scope that arrived mid-turn — grounded + flagged for deliberate sequencing (RIDER 4 first), NOT rushed at this session's depleted context. A rushed crash-adjacent/register fix is worse than a well-grounded flag.

## 🔨 turn 895 — RIDER 4: THE CRASH-ADJACENT LIFT/DROP PAIRING FIX — EMIT FIX BUILT + VALUE-VERIFIED, then REVERTED + GATED on a discovered SIM blocker (the sim's undeclared-placement G53 approximation). The emit fix is CORRECT for the machine + declared-placement sim; but it exposed a real engine gap in the UNDECLARED-placement preview that I cannot safely close at this session's depleted context (a wrong safety-SIM is dangerous). Gating with the verified design + the blocker + the complete plan, per the safety discipline.

### THE FIX (grounded via an Explore agent — the exact scope, register, and hook points)
- The bug lives ONLY where an ABSOLUTE G53 lift precedes a RELATIVE drop: CORNER (probeWallR:215 safeRetractNode ↔ repoTraverse drop `z?#18:[0-#19]`) + MIDDLE (repositionR machineLift:true ↔ `[0-#17]`). NOT middle's transTraverse/traverseOver (relative-symmetric lift+drop, nets zero — left UNCHANGED).
- MACHINE-Z REGISTER = **#882** (the Expert machine-DRO block #880+axis; Z=#882; dialect.readMachine). Free scratch = **#95** (unused in both wizards). The engine PROXIES #882 as the live machine Z (GcodeExecutionEngine DRO_BASES=[880,…]) — so the SIM models the round-trip too (that was the user's symptom surface).
- NEW node `saveMachineZNode(varRef)` (safeZframe.js) → a `readmachine` block emitting `#95=#882`. The RETURN reuses `safeZParkBlock('machine', '#95', restore)` → the wrapped `G53 Z#95` (this is the t826 "inter-move machine lift needs a drop-back" follow-up that file's own scope note deferred).
- safeTraverseStack (probeSurface.js): a `returnZ` param. `doLift()` — when machineLift+returnZ, emit the SAVE before the safeRetractNode. A new `emitDrop(restore)` — when returnZ, emit the `G53 Z#95` return instead of the relative drop; else the relative drop (byte-identical for the unaffected callers). `restore` is PER-ARM (safety: a wrong G90/G91 could make a G53 move incrementally): 'inc' for `seq` (no trailing distmode → the return restores G91), 'abs' for manual/center/in-axis (the arm's own distmode restores G91 — no double).
- CORNER (cornerWizard.js): the SAVE (`#95=#882`) inserted in probeWallR BEFORE the per-wall G53 lift (line 215); repoTraverse passes `returnZ:'#95'`; the repoArmR drop args became PRESENCE GATES (true / z?true:undefined) — the value is always the saved-Z return now. The now-FALSE z-trust "nets zero" comment (~311) rewritten. wall2's save is unused (no traverse follows) — harmless scratch.
- MIDDLE (middleWizard.js): repositionR gains `returnZ:'#95'` (its machineLift path saves via doLift). transTraverse/traverseOver untouched.

### VERIFICATION — the RESULTING Z (assert-the-value, the whole point)
- tests/lift-drop-pairing-895.spec.js 3/3: (1) the sim resolves #882 — `G53 Z-8 / #95=#882 / G53 Z-2 / G53 Z#95` ends at Z=-8 (round-trip nets zero); (2) CORNER both probeZ arms — EVERY wall probe (filtered: constant-Z XY-moving G31s) fires from ONE Z (wall-2 returned to wall-1's probe depth); (3) MIDDLE boss — every wall probes from one Z. This asserts the RESULTING Z (SIMULATE the macro) — corner-z-trust PASSED on the buggy code because it asserted the emit AS-DESIGNED; this catches what that couldn't.
- GOLDENS REGENERATED (each diff = THIS fix, justified): corner-z-trust (drop [0-#19]/#18 → SAVE #95=#882 + RETURN G53 Z#95, both probeZ arms), corner-travel-shape (dropValueOk → G53 Z#95), middle-reposition-refactor (the frozen 6-row __GOLDEN__ regenerated: 0 old [0-#17] left, 12 save/return pairs 1:1, the AUTO row unchanged). corner-travel-approach / corner-data-emit / middle-trans-traverse / corner-pass-align stay green (relative/XY paths unaffected). Verified the regen has ZERO `[0-#17]` + 12 `#95=#882`↔`G53 Z#95` pairs before splicing.

### THE DISCOVERED SIM BLOCKER (why this GATED, not shipped) — a real engine gap the fix exposes
- The full gate on the built fix = 1307 passed / **2 failed**. One (probe-surface-block:77) is a byte-identity golden that legitimately changes (tractable). The OTHER (corner-draw-anchor:90) is the BLOCKER: under UNDECLARED placement (no WCS row), the sim's `g53ApproxZ` BLANKET-forces EVERY `G53 Z` move to the margin approximation (GcodeExecutionEngine.js:1082 — `field==='z' && _g53ApproxZ!=null ? _g53ApproxZ : …`). So the fix's `G53 Z#95` RETURN renders at the MARGIN, not the saved probe depth → the tool stays high → the wall-2 probe MISSES in the preview (#1925 X-contact = 952 not -2). i.e. the emit fix, WITHOUT a matching sim fix, makes the UNDECLARED-placement preview WORSE (the user's symptom surface IS the sim).
- WHY it's not a 2-line fix: the engine must distinguish a MARGIN lift (`G53 Z#520` → approximate) from a SAVED return (`G53 Z#95` → restore the pre-lift part Z). It has only the RESOLVED VALUE at line 1082, not the var. #882 (DRO) resolves to `O.z + pos.z` (_updateDro:588); the correct return maps `value − O.z` = the probe depth. Doing that reliably needs VAR-PROVENANCE tracking (mark vars assigned from a DRO register #88x as "saved machine Z"; a `G53 Z<that var>` is a return → map back), OR an emit-side signal the sim reads. That is a real engine change — safety-adjacent (a wrong sim mis-shows a safety move), NOT to be rushed at depleted context.
- FOR DECLARED placement (a real WCS — the user's actual probing case) the sim maps G53 EXACTLY (`_g53ApproxZ` null) → the fix renders correctly. So the blocker is specifically the undeclared-placement PREVIEW.

### GATE OUTCOME — REVERTED to clean (green), NOT shipped
- REVERTED all t895 code (safeZframe saveMachineZNode, probeSurface returnZ/emitDrop, cornerWizard save+returnZ+comment, middleWizard returnZ) + the golden updates (corner-z-trust, corner-travel-shape, middle-reposition-refactor) + the new value-assert test → back to the t893-committed GREEN state (1306/0). The ONLY committed change this turn is this WORK-LOG doc. I will NOT ship a green gate that requires a broken undeclared-placement safety-sim, nor rush the engine change.
- THE VERIFIED DESIGN (ready to re-apply, DON'T re-derive): SAVE `#95=#882` (new `saveMachineZNode` → a `readmachine` block) BEFORE the G53 lift; RETURN via `safeZParkBlock('machine','#95',restore)` = wrapped `G53 Z#95`. Wire in safeTraverseStack (a `returnZ` param → save in doLift for machineLift + a per-arm `emitDrop(restore)`: 'inc' for seq, 'abs' for manual/center/in-axis). CORNER: save in probeWallR:215 before the G53, repoTraverse `returnZ:'#95'`, repoArmR drops → presence gates. MIDDLE: repositionR `returnZ:'#95'`. Register #882 (Expert DRO #880+Z), free scratch #95. Scoped to the two ABSOLUTE-lift sites ONLY (NOT transTraverse/traverseOver — relative-symmetric). The EMIT was byte-verified (12 save/return pairs, 0 old [0-#17]) and the RESULTING-Z asserted GREEN (a value-assert spec: every wall probes from ONE Z, both wizards, both probeZ arms; the sim resolves #882 for the round-trip WITHOUT the g53Approx). corner-z-trust/travel-shape/middle-reposition-refactor goldens regenerate as shown.
- THE PLAN for the re-dispatch (do BOTH together): (P-a) the emit fix above; (P-b) the SIM engine fix — track DRO-origin vars (a `G53 Z<saved-machine-Z-var>` under g53Approx restores the pre-lift part Z via `value − O.z`, not the blanket margin); then re-assert corner-draw-anchor (z1927/x1925 now = the probe depth, NOT the buggy -10 margin drop the OLD test codified) + regenerate probe-surface-block; full gate. This CORRECTS the base Turn B's clearance modes build on.
- STILL QUEUED (per the ruling, unchanged): RIDER 5 (#520 read-only scratch-var), P2 (safetraverse atom) + RIDER 3 (inline diagnostics).


## 🔨 turn 897 — RIDER 4 RE-DISPATCH: the CRASH-ADJACENT lift/drop pairing, EMIT + SIM together (P-a + P-b), SHIPPED. The verified t895 emit fix re-applied AND the advisor's DECLARE-not-infer sim fix built, so the honest return renders correctly under undeclared placement (the user's symptom surface). Full gate GREEN.

### THE FIX (both halves, per the advisor's ruling)
- **P-a EMIT (re-applied the value-verified t895 design):** an ABSOLUTE machine-frame G53 lift followed by a RELATIVE drop landed the tool at an ARBITRARY Z -> the next wall probed from the wrong height (crash if low, miss if high; the user's "Safe-Z changes nothing" symptom). Now the lift/drop is PAIRED HONESTLY: `saveMachineZNode('#95')` records the pre-lift machine Z (`#95=#882`, Expert DRO Z) BEFORE the G53 lift, and the paired `safeZParkBlock('machine','#95',restore,{ret:true})` returns to it (`G53 Z#95`), so every wall probes from the SAME Z BY CONSTRUCTION. Scoped to the two ABSOLUTE-lift sites: corner probeWallR (save) <-> repoTraverse (return, seq restore 'inc'), middle repositionR (machineLift, manual restore 'abs'). Relative-symmetric traverses (middle transTraverse/traverseOver) UNTOUCHED. safeTraverseStack gained a `returnZ` param (doLift saves for machineLift; a per-arm `emitDrop(restore)` emits the return in place of the relative drop — 'inc' for seq, 'abs' for manual/center/in-axis). Corner repoArmR drops became PRESENCE GATES (auto=true; manual=z?true:undefined — no auto-adjust after a human jog).
- **P-b SIM (DECLARE, not infer — the advisor's ruling):** a NEW `data/simMarkers.js` declares TWO marker tokens (`@saveProbeZ`, `@returnProbeZ`, non-overlapping substrings). The emit appends them as WHITESPACE-separated trailing G-code comments via an optional `mark` param on readMachineBlock + machineMoveBlock (unset -> byte-identical for every other caller). The sim reads them OFF THE RAW LINE (the SAME established pattern as the `REPOSITION:` pass marker, GcodeExecutionEngine:698): on `@saveProbeZ` it PUSHES the current scene-Z onto a LIFO `_savedReturnZ` stack (reset in resetState); on a `g53 && @returnProbeZ` move it POPS and sets `target.z` EXACTLY, BYPASSING the undeclared-placement g53 approximation (an exact round-trip needs no machine->part map, so it is correct under BOTH declared AND undeclared placement). Every OTHER G53 keeps the existing g53Approx/exact path unchanged. The flag also rides the `_move` event object (`machineReturn`, parity with g53/probe). This is DECLARE-not-infer: NOT register-provenance sniffing (no "mark vars from #88x") — the emit self-describes the pairing, the sim honours the declaration. Round-trip-SAFE: parseLine strips the trailing comment to a side-channel, so the save/return lines still round-trip to readmachine/machinemove (the marker is dropped on parse -> a round-tripped program loses only the sim-exact preview, never any machine motion; the emitted G-code is byte-identical with/without the comment).

### VERIFICATION — the RESULTING Z (assert-the-value, the whole point)
- **tests/lift-drop-pairing-897.spec.js 5/5** (SIMULATES the macro, asserts the resulting Z — what corner-z-trust's emit-literal golden could NOT, as it passed on the buggy code):
  - (1) PRIMITIVE: a MARKED save/return round-trip nets zero (restores the saved -8 despite an intervening lift); the identical BARE (unmarked) program falls to the g53Approx margin (5) — proving the marker DRIVES the exact restore, and the sim is honest without it.
  - (2) CORNER both probeZ arms (undeclared): every lateral wall probe fires from ONE Z (probeZ-off all 0; probeZ-on all -5) AND wall-2 HITS the face (#1925 = -2, NOT the g53Approx miss 952).
  - (3) DECLARED placement stays exact: the marked return restores the saved Z; a non-return G53 keeps the exact machine->part map (-2 - (-50) = 48), unchanged.
  - (4) MIDDLE boss (manual in-axis): every wall probe fires from ONE Z (the reposition returns to the probe depth).
- **corner-draw-anchor:90 RE-ASSERTED (migration noted):** #1927 -10 -> 0 (the OLD -10 CODIFIED THE BUG = absolute margin lift + relative -#19 drop; now the honest return to the saved probe depth). #1925 = -2 unchanged (still hits, now from the CORRECT depth). The other 7 cases untouched.
- **EMIT GOLDENS regenerated + DIFF-VERIFIED (each diff = THIS fix, nothing else drifted):**
  - corner-z-trust: the OLD relative drops ([0-#19] off / #18 on) RETIRED; the SAVE + RETURN present both probeZ arms; probeZ-on's `G0 Z#18` count 2->1 (the wall-1 PLUNGE stays, the reposition drop gone).
  - corner-travel-shape: the diagonal keeps the honest return (identical to dogleg) + onlyDiffIsTheMerge still holds (diagonal IS dogleg with the XY L merged; the return unchanged between shapes).
  - middle-reposition-refactor (frozen 6-row __GOLDEN__): regenerated; DIFF = each manual reposition's `G0 Z[0-#17]` -> save + `G90`/`G53 Z#95`/`G91`; ROW 4 (the AUTO traverseOver/transTraverse controls) BYTE-IDENTICAL (proves the fix is scoped to the absolute-lift sites).
  - probe-surface-block CORNER golden (stripAnnotations): regenerated; DIFF = 2 saves per corner (wall-2's is a harmless unpaired scratch) + 1 return replacing the drop; spot-checked the structure (save before the per-wall G53 lift, return replacing the drop, wall-2 probe follows).
- ADJACENT green (unaffected, verified): corner-travel-approach, corner-data-emit, middle-trans-traverse, corner-pass-align, marker-colour-by-source, traverse-clarity-893 (17/17). twin==built-in parity holds (both change together via cornerStack).

### DECLARE-OR-HAND-ROLL (the gate)
- DECLARED: the marker tokens (data/simMarkers.js, one source, inert data both the emit and the sim read); the `mark`/`returnZ`/`ret` params (opt-in, byte-neutral); saveMachineZNode (a named reusable node). The sim reads a self-describing comment, never infers from register provenance. No new engine machinery beyond the stack + the two hooks.

### HONEST NOTES
- The marker comment is DROPPED on a gcodeToStack round-trip (the parser strips trailing comments). Consequence: a round-tripped program re-sims WITHOUT the exact-return (falls back to g53Approx) — the SAME as today for any round-tripped program (no regression); the machine G-code is byte-identical with/without the comment, so real motion is unaffected. Carrying the marker through the block params (round-trip-durable) is a follow-up only if a round-trip preview needs it (rule-of-three: no test requires it).
- "Declared placement still exact" asserted via the PRIMITIVE (the marked return restores exactly + a non-return G53 keeps the exact map), NOT a full corner trace under declared placement — a realistic corner-declared sim needs the full "Sits at WCS" geometry setup (orthogonal to this lift/drop pairing; my arbitrary wcsOffset made wall-2 miss on BOTH the baseline and the fix, so it is not a regression).
...
## 🔨 turn 901 — P2 THE SAFETRAVERSE ATOM: the byte-identical CORE landed (the declared atom + the wizards use it as ONE brick, byte-identical + twin-parity verified). GATING the two deeper aspects (traverseOverR non-subsumable + the full standalone/round-trip param-surface). RIDER 3 split to next per the advisor's offer. Full gate 1313 passed / 1 load-flake (isolation-verified green).

### GROUNDING (agent + reads + a SPIKE) — the decisive facts
- **Byte-identity is FREE.** emitMapped (the live projection + the goldens) is MARKER-FREE; `( @DDCS )` markers exist ONLY in the export-only serializeWithMarkers (programModel.js:93-96). So a safetraverse op-block keeps emitMapped byte-identical (goldens untouched) and round-trips via the export marker — no byte cost. (Explore-agent grounded, confirmed by the 0-marker corner/middle goldens.)
- **The required form: a TRANSPARENT container whose children ARE the real safeTraverseStack block records** — NOT a leaf whose emit re-runs the builder. Because emitMapped runs whole-program passes AFTER the fold: `uniquifySafeRetractLabels` (blockEmitter:286) recurses `b.children`/`b.uiChildren` and rewrites every `saferetract` label unique across the program. A saferetract hidden inside a leaf's emit would be invisible to that walk -> duplicate N91 -> NOT byte-identical. Real children (transparent emit) keep it visible -> byte-identical.
- **makeOp is program-level, NOT wizard-internal:** makeOp lives in opBuilders which IMPORTS the wizards -> a wizard calling makeOp is a CYCLE. So the in-wizard grouping is a plain `newBlock('safetraverse')` transparent container (cycle-free; cornerWizard/middleWizard already import newBlock), added to blockEmitter's transparent-type list (op/section/setup/safetraverse).
- **SPIKE (the deciding fact):** wrapped ONE corner site, ran the byte-identity + twin-parity goldens -> 23/23 GREEN. My earlier fork/prune/CORNER_BINDINGS concern was UNFOUNDED — the transparent container is invisible to byte-identity AND to the twin's superset/prune/bindings machinery (corner-data-emit green). So P2's core IS buildable byte-identically this turn.

### WHAT LANDED (byte-identical, verified)
- **The declared atom:** `safeTraverseBlock` (saferetract.js) — type 'safetraverse', a transparent BUNDLE composing lift (saferetract) + shaped travel + descend/return (reuses the t897 save/return-Z). Registered in PALETTE + BLOCKS + Blockly (auto from PALETTE). blockEmitter treats it transparently (emits its composed children in order).
- **The 4 wizard sites now use ONE safetraverse brick each (byte-identical):** corner zWall1 + repoTraverse, middle repositionR + transTraverseR. Each: `newBlock('safetraverse')` with children = safeTraverseStack(the same params). Emit recurses the children -> IDENTICAL lines; the nested saferetract's label is still uniquified (the walk recurses).
- **VERIFIED:** the full byte-identity + twin-parity cluster GREEN (38): corner-z-trust (byte), corner-draw-anchor (twin byte-parity #6), corner-data-emit (twin emit), corner-travel-shape, probe-surface-block, middle-reposition-refactor, middle-trans-traverse, lift-drop-pairing-897 (t897 holds), safe-z-retract-822 (t899 holds). Block machinery GREEN (23): blockly-port, guard-prune, blocks-render, blocks-roundtrip-toast, federated-registry, form-kernel-720, protocol-validator, gcode-to-stack, formfield-block, learner-library. The goldens are UNTOUCHED (no regen — the emit is byte-identical, as the advisor required).

### GATED (the two deeper aspects — per the advisor's "gate back rather than force a golden churn")
1. **traverseOverR CAN'T be byte-identically subsumed (a specific SHAPE gate-back).** middle:149 `traverseOverR` = hand-rolled `[mkMV(Z,#18), mkMV(ax,±cv), mkMV(Z,[0-#18])]` — 3 bare moves, NO distmode, NO comment. safeTraverseStack's `in-axis` arm adds a trailing `distmode inc` + an optional comment + a `G0 Z` lift (vs mkMV) -> routing it through the atom would ADD lines (a golden churn). Per the advisor's rule, it STAYS hand-rolled (the one remaining inline traverse). Subsuming it needs a new byte-exact in-axis mode in safeTraverseStack (a small follow-up) — flagged, not forced.
2. **The FULL standalone + gcodeToStack round-trip is a deeper phase (design-open).** The atom is byte-identical + palette/Blockly-visible, but it's currently WIZARD-COMPOSED (children set by the wizard; b.params carries display {to,shape}, not the full surface). For a STANDALONE palette-drop + round-trip, the block must be VALID-BY-CONSTRUCTION (b.params = safeTraverseStack's FULL param surface — mode/approach/lift/drop/returnZ/crossX-Y/firstAxis/diagPrimary-Travel/wall2Var/radiusVar/dir1-2Plus/axis/second/dogleg/comment; ~18 params) + a params->children instantiate hook + a SCHEMA entry (so serializeWithMarkers round-trips it). PLUS the NESTED-marker question (a safetraverse inside the corner op — does serialize/import handle a nested op marker). This is a real phase with a param-surface design decision; recommend it as P2.5 (carry the full params + SCHEMA + instantiate + a round-trip test). A palette-dropped safetraverse is currently inert (emit []) until that phase — noted (the Blocks/Blockly surface is advanced; the atom's primary use today is wizard-composition).

### RECOMMENDATION (for the advisor)
- ACCEPT the byte-identical core (the atom + the 4 wraps) as "P2 the atom, byte-identical" (the advisor's own framing).
- RULE on: (a) traverseOverR — add a byte-exact in-axis mode to subsume it, or leave it hand-rolled (it IS a traverse but a trivial one); (b) the round-trip phase — confirm the full-param-surface + SCHEMA + instantiate approach (P2.5) before I build it (a param-surface design + the nested-marker handling warrant a ruling, per restructure-source-not-abstraction).
- Then RIDER 3 (inline pre-flight diagnostics) — the separate UX turn.
...
## 🔨 turn 905 — P2.5 THE SAFETRAVERSE STANDALONE ROUND-TRIP: GROUND-then-GATE. The nested case (safetraverse INSIDE an op) ALREADY round-trips — VERIFIED + locked in with a test. The STANDALONE palette-drop is a genuine t812-class STRUCTURAL hole (marker codec + builder + SCHEMA + a leaf-instantiate hook) — GATED per the advisor's explicit "gate if structural" offer, with the finding + 3 options + a recommendation. The atom STAYS hidden (documented why). No source change; the only change is the verification test. Full gate: 1318 passed / 1 load-flake (isolation 6/6).

### THE GROUNDING (Explore agent — the deciding structural facts)
- **serializeWithMarkers (programModel.js:96) marks ONLY blocks whose `type==='op'`** (via findOpInStack:37, OUTERMOST wins). A `safetraverse` is `type:'safetraverse'`, `kind:'leaf'` → it NEVER gets a marker of its own.
- **NESTED safetraverse (inside a corner op) round-trips as a BYPRODUCT of the OWNING op's marker:** the corner marker declares the corner params; on reimport opFromMarker → builderOf('corner') (cornerStack) REGENERATES the whole corner body, including the nested safetraverse child. findOpInStack returns the OUTER corner first (before descending to the nested safetraverse) → no double-marking. So the nested case works WITH NO CHANGE.
- **STANDALONE (top-level palette-dropped) safetraverse is a t812-class hole PLUS 2 deeper gaps:** (a) no marker (it's not type==='op'); (b) no builderOf('safetraverse') for opFromMarker to reconstruct from; (c) no SCHEMA entry; and — the deeper one — (d) a FRESH palette drop is CHILDLESS (its children are COMPUTED by safeTraverseStack, not a static template), and there is NO leaf-block instantiate hook that populates a block's computed children on drop/edit. An emit-time rebuild would put the nested `saferetract` AFTER uniquifySafeRetractLabels (which runs pre-emit on the stored children) → duplicate N91 labels for 2+ standalone safetraverses = an ambiguous guard GOTO (a SAFETY bug). So the standalone needs a pre-emit children-population mechanism that doesn't exist for leaf blocks.
- (The t812 comment is explicit: "Only TOP-LEVEL siblings ride [the prog] slot — a per-op nested entry stays on its own op marker." A standalone safetraverse is exactly the top-level non-op compound that has no slot.)

### DELIVERED + VERIFIED (the nested case = the real composability that works today)
- **tests/safetraverse-nested-roundtrip-905.spec.js (NEW, passes):** a corner (data-twin) op with a nested safetraverse survives .nc export (serializeWithMarkers) → reimport (importMarkedNc) BYTE-IDENTICALLY (emitMapped(orig)===emitMapped(back)), the .nc carries the corner op marker, and the safetraverse child RETURNS in the reimported stack (the corner builder regenerated it). This locks in the advisor's "a corner-with-safetraverse survives .nc export/reimport" acceptance for the NESTED case — the composability that actually works today: a wizard/op that COMPOSES safetraverse round-trips via its own op marker + builder.

### GATED (the standalone-palette-drop — a real structural feature, per "gate rather than force")
- The atom STAYS HIDDEN (the t903 hidden flag holds). Un-hide is gated on "a dropped standalone emits valid AND round-trips" — both blocked by the structural gaps above. Documented why (below) so the flag isn't a silent limitation.
- **OPTIONS for a dedicated turn:**
  - **(A) Extend the marker path** to treat a TOP-LEVEL safetraverse as a markable compound: findOpInStack matches `(type==='op' || type==='safetraverse')` (OUTERMOST-wins still gives corner the nested one, safetraverse the standalone one), markerLine uses `opType||type`, register builderOf('safetraverse')=safeTraverseStack, add a SCHEMA entry. STILL needs (d) the drop-instantiate for computed children (a NEW leaf-instantiate hook, pre-uniquify, to avoid the multi-standalone N91 collision). Round-trip-codec-adjacent (findOpInStack runs per line for EVERY op) + makes safetraverse op-like for hover/edit/glow — a real behavior surface.
  - **(B) A flat/prog-style bespoke marker** for a standalone safetraverse (like the t812 prog slot) — avoids touching findOpInStack but is a third bespoke path (the t812 comment resisted "no third bespoke path").
  - **(C) Promote a palette-dropped safetraverse to a real OP** (BUILDERS+SCHEMA+OP_LABELS+route the palette/Blockly drop through makeOp) — uses the EXISTING marker machinery, but DIVERGES from the nested-block form (nested = a block riding corner; standalone = an op) + needs the palette-drop→makeOp routing.
- **RECOMMENDATION:** the composability payoff that MATTERS — composing safetraverse INSIDE a wizard/custom op — ALREADY round-trips (via the owning op's marker+builder; verified). The STANDALONE-palette-drop is a distinct, lower-value affordance whose enablement needs a marker-codec change + a NEW leaf-instantiate hook (both round-trip/SAFETY-adjacent: the marker path resolves EVERY op, and the drop-instantiate guards the N91 label collision). Recommend (A) as a DEDICATED turn (build the full param surface + defaults + SCHEMA + builder + the marker-extend + the pre-emit drop-instantiate together, with the multi-standalone label-safety asserted), NOT forced onto this session's tail. Keep the atom hidden until then. Do NOT ship (1)(2)(3) (builder/SCHEMA/defaults) speculatively without the marker+instantiate — that's foundation with no working consumer (YAGNI), and the eventual approach (A/B/C) shapes them.

### WHY GATE (honest)
- The standalone needs a change to the marker CODEC (findOpInStack, which resolves the owning op of every projected line) + a NEW instantiate mechanism for a leaf-compound's COMPUTED children (the label-uniquify safety). Both are round-trip/safety-adjacent. Rushing them at this session's depth risks a subtle round-trip break affecting EVERY op's export/reimport. The nested composability (the real payoff) is delivered + verified; the standalone is a deliberate, well-scoped follow-up.

### GATE
- FULL gate: **1318 passed / 1 failed / 4 skipped (12.3m), GATE_EXIT=0.** The 1 failure = blocks-live-form:89 (a Blocks-authoring UI test) which passes **6/6 in isolation** — a load-sensitive flake under the 4-worker contention (same file flaked at t901). DEFINITIVELY not mine: this turn made ZERO source changes (only added a test), so it cannot affect blocks-live-form. My new nested round-trip test passed. Files: tests/safetraverse-nested-roundtrip-905.spec.js (new; the ONLY change — no source touched). The safetraverse atom stays hidden (t903 flag) with this documented why. Queued after the advisor's ruling: the standalone-round-trip dedicated turn (option A recommended) — then Turn B opens.


## 🔨 turn 907 — TURN B1 THE PROBE-START INVERSION (finding 2): GROUND-then-GATE. Grounded the disconnect COMPLETELY (2 Explore agents + the emit structure + an empirical trace dump); the fix is BOUNDED + SIM-ONLY (emit byte-identical) BUT safety-adjacent (operator-trusted sim geometry) with real param-combinatorics + sign/pose precision + a full acceptance surface (drag/round-trip/handles-independence/screenshots). GATED with the precise de-risked design for a FRESH-CONTEXT build (this is turn 7 of a very deep session; the standing discipline is value-verify not force). No code this turn.

### THE DISCONNECT — grounded + EMPIRICALLY CONFIRMED
- **The EMIT's trans-axis traverse (transTraverseR, middleWizard.js:228, transAxis AUTO) lands at the DECLARED #21/#22 point:** primary axis = #22 (diagPrimary, default '#53' = the measured centre), secondary axis = centre_secondary ± #21 (diagTravel, default 50). CENTRE-relative. (probeSurface.js:143-147 'center' arm.)
- **The SIM's wall-2 (secondary) start recomputes INDEPENDENTLY:** opSimStarts.middle (opSimStarts.js:51-54) `outside()` = edge ∓ outset, outset = clamp(dist*0.6, 6..15) (default 12-15). EDGE-relative. DUPLICATED in the data-twin provider (middleData.js:136-141, anchor:'edge' out:'@outset').
- **EMPIRICAL DUMP (boss, twoAxis, transAxis auto, 100x80 stock, dist 20):** the sim anchors the secondary pass at **(x=50, y=92)** = edge(80)+outset(12); the declared traverse landing is (x=#22=centre 50, y=centre_40 ± #21_50) = **(50, 90) or (50, -10)**. TWO different points = the user's visible XY gap. (WORK-LOG:1412 noted a ~5mm residual even after the #21-magnitude fix — the ORIGINS never reconcile.)

### THE FIX (the inversion — declare the probe start, derive the travel; mirror corner's ONE-source)
- Make the SIM's secondary wall-1 start DERIVE from the declared #21/#22 (= the traverse landing) instead of edge±outset. Then traverse-end == wall-2-probe-start BY CONSTRUCTION — the exact invariant CORNER already satisfies (cornerReposOffsets is ONE geometry source, chained: wall2 = wall1 + off; verified — corner needs NO fix, it is the TEMPLATE).
- SCOPE: gate the change to the ONE case that runs the #21/#22 traverse — boss && twoAxis && transAxis==='auto'. All other combos (transAxis manual = a jog, single-axis, pocket, inAxisManual far-wall) keep their current start (they don't use #21/#22).
- ONE-SOURCE HELPER (the #21/#22 analog of cornerReposOffsets): a `middleReposLanding(params, stock)` that returns the secondary landing point from #21/#22; read by BOTH opSimStarts.middle (BUILT_IN) AND middleData's provider (a new 'landing' anchor or @diagTravel/@diagPrimary tokens on the secondary rows). resolveDiagPrimary: a number -> itself; '#53'/#var -> the primary-axis centre.
- THE INVERSION realized: the ② marker (tieDiagTravel, middleView.js:20-36) ALREADY drags-to-write #21/#22 (the marker EXISTS). After the sim fix, the ② marker sits at the connection point; dragging it writes #21/#22 -> the emit landing (already reads them) AND the sim wall-2 start (now reads them) both follow -> connect. Declaring the probe start (#21/#22), the travel derives.

### WHY GATE (the precise risk — not a punt)
- **The exact landing SIGN + POSE need empirical nailing before I set operator-trusted geometry.** The emit moves are INCREMENTAL (G91) from a pose-dependent position; the declared landing is centre ± #21 but the SIGN (centre+50=90 vs centre-50=-10, from dir2 + the probe direction) + which combos apply must be traced-and-asserted, not hand-derived. A wrong sim start MISLEADS the operator about where the probe fires (sim-only, but the sim is the trust surface).
- The FULL acceptance is a real surface even with a bounded core: drag->params+both-follow, byte-identical goldens (middle-reposition-refactor + middle-trans-traverse), handles-independence (the ② drag writes ONLY #21/#22 — tieDiagTravel already scoped; but it ALSO calls onStartDrag pinning the pass — decide whether to drop that for a pure one-source), the marker round-trip (#21/#22 are opSchema params -> ride the op marker; middleWizard:193-194 reparse — round-trips today), and screenshots (spatial GUI, VIEWED).
- After 7 deep turns (t897/t899/t901/t903/t905 + this grounding), rushing the sign/pose + the full verification risks a subtle misleading-sim error. The grounding here FULLY de-risks the fresh-context build.

### THE FRESH-CONTEXT BUILD PLAN (de-risked, ready)
1. `middleReposLanding(params, stock)` helper (opSimStarts.js, exported) = the secondary landing from #21/#22 (resolveDiagPrimary + centre ± #21). NAIL THE SIGN by tracing the actual traverse MOVE end (not just the probe start) and asserting sim-start == traced-landing.
2. opSimStarts.middle: sec[0] (the secondary wall-1) = (boss && twoAxis && transAxis auto) ? middleReposLanding : outside(second, dir2Plus). sec[1] (inAxisManual far wall) unchanged.
3. middleData's provider: the secondary wall-1 row -> the landing (a 'landing' anchor in rowToStart calling the same helper), gated on transAxis auto.
4. VERIFY: numeric — trace the emit, the secondary G31 start == the sim wall-2 start, BOTH shapes (dogleg + diagonal); byte-identity — middle-reposition-refactor + middle-trans-traverse goldens UNTOUCHED (emit not changed); drag — dragging ② writes #21/#22 and both the emit landing + the sim start follow; handles-independence — the drag moves no other pass; round-trip — #21/#22 survive params+Blocks+marker; corner — assert its wall-2 sim start still == its traverse dest (unchanged, the template). Screenshots (the sim showing the connection, VIEWED). FULL gate.
5. MIGRATION (trivial, name it): auto=auto=byte-identical (the EMIT is untouched, so every existing program's .nc is unchanged). A typed old #21/#22 IS already the declared point (#21/#22 have always been the traverse target); the sim just starts reading them — NO value remap, NO flag. (The corner has no inversion, so no corner migration.)

### CORNER (grounded — NO B1b needed)
- Corner's wall-2 sim start is DERIVED from the SAME #23/#24 reposition offsets (cornerReposOffsets, ONE geometry source; cornerData.js:145-146 chains wall2 = wall1 + off.wall2). Its traverse-destination and wall-2 sim start are the SAME point by construction TODAY. Corner is already what middle must become — NO corner fix, NO B1b.

### GATE
- NO gate run (no source change this turn — only this WORK-LOG doc). The tree is green at the t905 baseline (1318). Recommend the advisor greenlight the fresh-context build (the plan above) OR adjust scope; then B2 (clearance modes) + B3 (tall-stock assists) follow.

## 🔨 turn 909 — TURN B2a THE CLEARANCE-MODE UNIFICATION (mechanism + unification, Max default): BUILT. The user's "not the same in both" + "i thought we added a safe-z height mechanism": three traverse clearances had DIVERGED (corner→#520 machine margin, middle trans-axis→+#18 relative hop, park→safeZframe) and byte-identity FROZE the divergence. B2a declares ONE clearance MODE both wizards read + unifies middle's trans-axis traverse to the SAME #520 machine margin as corner. This DELIBERATELY changes the emit (row-4 golden regenerated + diff-verified). GROUNDED FIRST (the exact corner+middle mechanisms + the per-post safeRetract + the sim anchoring), found NO safety concern in the unification (it's the corner machinery, safe by construction), so per the advisor's split I BUILT B2a; Hop+Plane (incl. the one net-new hazard, the Plane floor) are the de-risked B2b below.

### THE DECISION (build, not gate — reconciled with the advisor's dispatch)
- The advisor's dispatch = BUILD B2, split if big (B2a mechanism+unification Max-default FIRST), gate-back ONLY if "unifying corner+middle's traverse surfaces a safety concern." Grounding showed the unification is SAFE BY CONSTRUCTION (it reuses the t826/t897 machineLift+returnZ pairing corner's repoTraverse + middle's MANUAL reposition already use + prove): higher lift (the #520 margin clears the boss by MORE than +15) + the honest save/return (never lift-to-margin + a relative drop = an arbitrary Z, the t897 bug). So the gate-back trigger does NOT fire for B2a — it fires for PLANE mode (B2b, the net-new hazard). Built B2a.

### GROUNDING (the exact mechanisms — so the change is surgical)
- **Corner is ALREADY "max":** each wall probe lifts OUT-OF-BAND (probeWallR: saveMachineZNode('#95') + safeRetractNode → G53 #520 margin, cornerWizard.js:219), then repoTraverse does the seq XY at margin height + returnZ:'#95' (G53 Z#95 return). Middle's MANUAL reposition (repositionR) ALSO already = max (machineLift:true+returnZ:'#95'). **The ONE divergence = transTraverseR** (middle's AUTO trans-axis connect), which hopped `lift:'#18', drop:'[0-#18]'` (+15/-15 relative).
- **The center-mode arm (safeTraverseStack, probeSurface.js:143-159) already supports machineLift+returnZ** (doLift → saveMachineZNode + safeRetractNode; emitDrop('abs') → safeZParkBlock ret) — the same path the manual short-circuit uses. So the fix is a param swap, no new safeTraverseStack code.
- **#18 (clearOver) STAYS** — it still feeds the in-axis traverse-OVER (traverseOverR, a same-axis cross at the probe Z, unchanged). No orphan.

### BUILT (surgical — the declared seam + the one-line unification)
- **web/wizards/ops/safeZframe.js** — declared the CLEARANCE-MODE SEAM: `CLEAR_MODES=['max','hop','plane']`, `clearModeOf(v)` (default 'max'), `clearTraverseParams(mode,{saveVar='#95'})` → the safeTraverseStack lift/drop params for a mode. B2a wires `max` (→ `{lift:true, machineLift:true, drop:true, returnZ:'#95'}` = the #520 margin + honest return); `hop`/`plane` branch HERE in B2b (until then every mode resolves to the safe margin). Seam-before-GUI ([[declared-seam-before-declaring-gui]]) — the B2b dropdown writes this. Updated the file-scope comment (t897 "middle trans stays relative" → "t909 B2 unifies it").
- **web/wizards/middleWizard.js** — `const clearMode = clearModeOf(params.clearMode)` (default 'max', no form field yet — the seam, GUI = B2b), and transTraverseR now spreads `...clearTraverseParams(clearMode)` in place of `lift:'#18', drop:'[0-#18]'`. So middle's auto trans-axis traverse clears to the #520 machine margin + returns to the saved probe Z — the SAME as corner. Updated the transTraverseR comment block.

### VERIFIED (assert-the-value + diff-verify + real symptom)
- **ROW-4 golden (middle-reposition-refactor.spec.js) regenerated + DIFF-VERIFIED (dump-and-compare, all 6 rows):** ONLY row 4 (boss twoAxis inAxis:auto transAxis:auto) changed — the two relative lines `G0 Z#18`/`G0 Z[0-#18]` became `#95=#882 ( @saveProbeZ )` + the #520 safe-retract block + `G90`/`G53 Z#95 ( @returnProbeZ )`; the trailing error-handler label bumped N91→N92 (the traverse's retract now claims N91). The diagonal `G0 X[#22-#52-#10-#6] Y#21` + EVERY probe/feed/wcs line UNCHANGED; rows 0-3,5 byte-identical. Provenance comment updated. Both reposition-refactor tests green.
- **NEW tests/clearance-modes-909.spec.js (passes):** (1) UNIFICATION — the middle trans-traverse now SAVES the probe Z + lifts to the #520 margin (#42=#520 + G53 Z#42) + returns (@returnProbeZ); the old relative +#18/-#18 is GONE; the XY re-centre unchanged. (2) INVARIANT — corner's wall-traverse + middle's trans-traverse run the SAME clearance choreography [SAVE, MARGIN-READ, MARGIN-LIFT, RETURN] (the "same in both"). (3) PAIRING — one @saveProbeZ : one @returnProbeZ (net-zero by construction). (4) PER POST — the margin lift FOLDS (Expert #520/#42 read-only guard precise; V4.1/DM500/grbl/rs274 differ, none THREW) — inherited from safeRetractNode (the same node corner + the error handler use, already per-post tested).
- **SIM verified (real symptom):** corner-draw-anchor test (2) traces the middle boss-twoAxis emit (now with the G53) → NO NaN + per-pass SELF-anchor holds (middle anchoring is param-driven via opSimStarts, independent of the emit); lift-drop-pairing-897 (the save/return net-zero) green. SCREENSHOT of the boss-twoAxis-auto preview (viewed): both pass markers ①②in sensible positions (not collapsed to origin), the trans-axis traverse renders as a DASHED LIFTED path (traverse-clarity-893 path-style) — the new margin G53 is trace-neutral (t856 wrap; the pattern corner + middle-manual already prove). (The exact ② position is the SEPARATE B1 probe-start inversion — gated t907, NOT B2.)
- **28 middle/marker related specs green** (middle-trans-traverse/superset/diag-aim/aim-tie/data-emit/crossover/probe-z-first/datum-centre/2nd-axis-flash, marker-colour, boss-probe-collision) — no other golden captured the divergent trans-traverse lines.
- **STANDING SPLIT holds:** the mode governs the PLANNED traverse; the miss-path error handler + final park stay the machine margin (unchanged — asserted implicitly by the byte-identical rows 0-3,5).

### FULL GATE
- **1320 passed / 0 failed / 4 skipped (12.5m), GATE_EXIT=0** — CLEAN. Explicit failed-count grep = 0; the 3 log matches for "fail" are TEST NAMES (cloud-write-FAILS, corner #1505-fail byte-identical, corner-marker assert-FAILS-when-mutated, probe-MISS-fail-branch), all passing. The blocks-live-form load-flake (recurred t901/t903/t905) did NOT recur this run.
- Files (git hash-object): web/wizards/ops/safeZframe.js `5f490e7` (the clearance-mode seam + the t897 scope comment) · web/wizards/middleWizard.js `0bfc9ac` (the transTraverseR unification + clearMode read) · tests/middle-reposition-refactor.spec.js `f4ef006` (row-4 golden regen + provenance) · tests/clearance-modes-909.spec.js `b346b76` (NEW — unification + corner==middle invariant + pairing + per-post).
- No release (mid-arc; the advisor owns the release cadence + the worker is not idle). No lingering ephemeral procs from this turn (the mem-server is Playwright-managed per run).

### B2b — THE DE-RISKED FOLLOW-UP (Hop + Plane + the GUI dropdown + the net-new hazard)
- **The GUI:** replace the wizards' "Safe Z" field (m_safe_z/rc_safe_z #17, the "Safe-Z drives nothing" complaint) with the CLEARANCE dropdown (Max/Hop/Plane) + per-mode WHEN-gated fields (the t842 contourData entry/rampAngle `when:{param:'clearMode',is:'hop'|'plane'}` precedent — deriveBindings carries `when`, userOpView data-when + whenOk live show/hide; use HIDE not grey). Land the dropdown WITH the Hop/Plane emit so no inert option ships (the t903 hidden-affordance lesson). Wire params.clearMode (the seam already reads it, default 'max').
- **HOP (relative, capped) — NO native min()/max():** the cap = IF/GOTO into a scratch, EXACTLY like the #520 guard: `#95=#882 (save) / #43=[#95+hop] (proposed) / IF #43<#520 GOTO L (already below margin? keep) / #43=#520 (else clamp to margin) / N L / G53 Z#43 (lift capped) / ...work... / G53 Z#95 (return exact)`. #43 = a peer scratch to #42/#95 (documented free). SIGN caveat — home Z0=top, #520 negative-below-home; the compare direction (the lower/safer of hop-target vs margin) HAND-CHECK per post. Register-less posts (grbl/rs274/DM500) emit plain relative + the honest uncapped note.
- **PLANE (declared absolute):** a fixture-clearing absolute plane (planeZ work-Z), staged into a #var on Expert (a literal G53 fails there). THE ONE NET-NEW HAZARD (the advisor's gate-back trigger): planeZ BELOW a tall boss/clamp COLLIDES (it is the only mode NOT anchored to the safe machine margin) → Plane NEEDS a "not below stock top" sanity floor + the pre-flight "traverse passes through declared stock" violation class (folds B3's tall-stock assist forward). Do NOT ship Plane without the floor.
- Corner reads clearMode too (thread it through the per-wall lift for hop/plane; max = byte-identical, so corner is untouched until B2b). DEFAULT stays Max (safety + the twin-default byte-identity rule) — FLAG the default as the user's taste call (their morning lean was "Hop everyday"; a one-line default flip, goldens regenerated justified).
- Acceptance: Hop cap algebra numeric (round-trip nets zero on macro posts); Plane floor asserted; dropdown + per-mode field screenshots both wizards; Blocks round-trip per mode; goldens diff-verified.

## 🔨 turn 911 — TURN B2b THE USER CONTROL + Hop + Plane: GROUNDED COMPLETELY, then GATED with the full de-risked design + an honest split. B2a accepted (14/14). I grounded B2b end-to-end (2 Explore agents: the form structure + the pre-flight/Suggest patterns; + my own per-post dialect + emit analysis). The grounding revealed B2b is a genuine 2-3 TURN build (NOT the single-turn the WORK-LOG's Expert-focused sketch implied): TWO different form implementations, a NEW dialect-aware Hop atom with label-uniquification, and a NEW safety-critical pre-flight trace class. Rushing safety-critical emit-changing probe G-code (real machines) + a new safety class at this session depth is the wrong risk; the grounding FULLY de-risks a clean staged build. No code this turn.

### THE DEEPER COMPLEXITY GROUNDING SURFACED (why this is 2-3 turns, not 1)
1. **The two wizards are built DIFFERENTLY (doubles the form work).** MIDDLE = a HAND-ROLLED HTML form (index.html:324-330, field `m_safe_z`, read manually in middleView.js:137-141; when-gating is hand-rolled `classList.toggle('hidden')` per middleView.js:156-157,180,185-186; value->params is a MANUAL line in the middleView param object). CORNER = RETIRED as a built-in (2026-07-02) -> now a DATA-OP (`user_corner_data`, cornerData.js -> userOpView.js) with NATIVE `when`-gating + AUTOMATIC value->params (userOpView.js:257 Object.assign(params, read())). (NB `rc_safe_z` is ROTARY CENTER, not corner.) So the dropdown needs TWO implementations: middle (hand-rolled HTML + inputIds + view show/hide + manual param read) AND corner (declarative enum + when-gated number bindings in cornerData.js — the EASY one).
2. **The Hop cap needs a NEW dialect-aware atom + LABEL-UNIQUIFICATION (the safety-critical intricacy).** NO native min()/max(). The Expert cap = IF/GOTO into scratch #43 against the #42-guarded margin (algebra hand-checked below), which needs UNIQUE labels across multiple hops in one program (exactly like saferetract's uniquifySafeRetractLabels). The per-post FLOW diverges: Expert/V4.1/DM500/centroid = IF..GOTO (goto flow); grbl = NO flow (`ifGoto:()=>[]`) -> CANNOT cap; rs274 = O-WORD `if` that RUNS (not skips) the body -> the cap logic INVERTS. So Hop = Expert-CAPPED (the one reference post) + a UNIVERSAL RELATIVE DEGRADE elsewhere (plain `G0 Z<hop>` lift + honest uncapped note + the save/return) — the advisor's "register-less -> plain relative". That degrade is byte-safe (it IS the pre-B2a relative trans-traverse form). Machinery: a new `safehop` atom (mirror saferetractBlock, dispatch dialect.safeHop else the relative fallback) + Expert.safeHop + register in PALETTE + a label uniquifier + the round-trip SCHEMA.
3. **PLANE is the tractable mode (universal), but carries the net-new hazard.** Emit = a WORK-FRAME absolute lift: distmode('abs') + move Z<planeZ> + distmode('inc'), then the existing returnZ (G53 Z#95) — UNIVERSAL (all posts do G90 G0 Z<lit>), NO new atom, NO per-post intricacy. THE HAZARD (correctly flagged): a planeZ below the stock top plunges the traverse INTO the part; AND a subtlety I found — plane is a WORK-frame Z, so if the probe runs before the WCS-Z is set, the plane references an unset/old WCS-Z. The not-below-stock-top FLOOR + the pre-flight through-stock class are the guardrails; NOTE the WCS-Z-unset subtlety in the floor's why-tooltip.
4. **The pre-flight through-stock class is a NEW trace class (its own safety surface).** The checker is ONE fn `checkEnvelope` (web/engine/envelopeCheck.js:33-77) — reuses traceToolpath segments (seg.x1/y1/z1->x2/y2/z2, rapid, probe, line) + axisSpan bounds, tests the two ENDPOINTS, anchors a violation `{line,axis,overshoot}` to seg.line, rendered by preflightBadge.js:67-84 as `.preflight-annot`. The through-stock class DIFFERS FUNDAMENTALLY: it must test the whole SEGMENT PATH vs the stock AABB (a rapid can dive THROUGH the stock with both endpoints outside) — a segment-vs-box intersection, NOT the endpoint test. It runs in the WORK frame (stock box is work-coords -> no wcsOffset needed, a simplification). Stock box = settings.stock {x,y,z,shape,datum} (settingsPanel.js:86) placed via datumZFrac. Needs a NEW violation shape (no "overshoot" — a through-path) -> extend the renderer to branch on a kind/message.

### THE COMPLETE DE-RISKED DESIGN (ready for a fresh-context staged build)
**Emit (the shared seam, safeZframe.js clearTraverseParams — already `max`-wired from B2a):**
- clearTraverseParams(mode,{saveVar,hopDist,planeZ}): max -> {lift,machineLift,drop,returnZ} (done); hop -> {lift,clearMode:'hop',hopDist,drop,returnZ}; plane -> {lift,clearMode:'plane',planeZ,drop,returnZ}.
- safeTraverseStack doLift: clearMode 'plane' -> if returnZ saveMachineZNode(#95); distmode abs + move Z<planeZ> + distmode inc. clearMode 'hop' -> if returnZ saveMachineZNode(#95); push safeHopNode({hopDist,saveVar:#95}). emitDrop for hop/plane = the existing returnZ (G53 Z#95) — the SAME honest per-post return as max (nets zero by construction).
- HOP-CAP ALGEBRA (Expert.safeHop, hand-checked; home Z0=top, machine Z negative-down, #520 margin negative): `#43=[#95+<hopDist>] ( proposed hop target ) / #42=#520 / IF #42<0 GOTO <g> / #42=<baked margin> / N<g> / IF #43<#42 GOTO <c> ( hop below the margin -> keep it ) / #43=#42 ( else clamp to the safe machine margin ) / N<c> / G53 Z#43`. Labels <g>,<c> UNIQUE per hop (uniquifier, base e.g. 81/82 to avoid saferetract's 91). Register-less/no-flow posts (grbl/rs274/centroid/DM500) -> the atom fallback: `G0 Z<hopDist> ( clearance hop - uncapped on this post )` (relative; the return via returnZ nets it back).
- PLANE: universal (above); the return is always G53 Z#95 (honest, back to the saved probe Z).

**The Clearance dropdown (both wizards, replaces Safe-Z, no inert option):**
- MIDDLE (hand-rolled): add `<select id="m_clear_mode">` (Max safe height|Hop up|Clearance plane) mirroring m_type (index.html:229-233) + `<div id="m_hop_block" class="hidden"><input id="m_hop_dist"></div>` + `<div id="m_plane_block" class="hidden"><input id="m_plane_z"></div>`; add all ids to middleView.js inputIds; hand-roll the show/hide in middleView.update() (precedent :156-157); add `clearMode: el('m_clear_mode')?.value||'max'`, hopDist, planeZ to the param object.
- CORNER (data-op, declarative): add an enum binding `param:'clearMode'` (mirror travelShape cornerData.js:175, widget:'segmented' or a select) + two number bindings `hopDist` with `when:{param:'clearMode',is:'hop'}` and `planeZ` with `when:{param:'clearMode',is:'plane'}` (the contourData.js:68 precedent; deriveBindings.js:84 carries when; formWidgets.js:795 tags data-when; userOpView.js:284-291 live show/hide). Replace/retire the safeZ binding (cornerData.js:82).

**The Plane floor + Suggest:**
- FLOOR (not-below-stock-top): grey/clamp planeZ at the declared stock top (work-Z) with a why-tooltip (incl. the WCS-Z-unset caveat). Corner (data-op) can gate via a validator/derived clamp; middle hand-rolls the clamp in the view.
- SUGGEST (advisory, never auto): mirror feedSuggestWidget (formWidgets.js:614-657) — a button that computes stock-top(work-Z, via datumZFrac createPreviewPanel.js:71-76)+margin and fills the planeZ field via `.value` + a dispatched `input` event; advisory-only tooltip ("you own the number; clamps/fixtures are yours to add"). Corner: a `clearancesuggest` FORM_WIDGET binding next to planeZ; middle: a hand-rolled button.

**Pre-flight through-stock class (the separable safety turn):** extend checkEnvelope (or a sibling fn the badge also calls) with a rapid-segment-vs-stock-AABB intersection in the work frame; a NEW violation kind ("traverse crosses the stock") anchored at seg.line; extend preflightBadge renderAnnotations to branch on the kind (no "overshoot"); assert a planted Hop-through-tall-stock flags at the line end + clean when clear.

**corner threads clearMode (max byte-identical today); Blocks round-trip per mode (clearMode/hopDist/planeZ ride the op marker -> opSchema needs them + the middle/corner data twins); goldens for Hop/Plane regenerated justified. DEFAULT stays MAX (do NOT flip; surface the user's Hop-everyday lean as a recorded one-line taste call).**

### RECOMMENDED SPLIT (3 focused, safe, verifiable turns)
- **B2b-1 (emit):** the safehop atom (Expert cap + universal relative degrade) + the label uniquifier + Plane (universal) + clearTraverseParams(hop/plane) + safeTraverseStack doLift branches. Verify per-post NUMERICALLY (the cap algebra hand-checked; the degrade; the pairing nets zero; max byte-identical). Round-trip schema for the new params. NO form yet (the seam; default max = byte-identical everywhere).
- **B2b-2 (the control):** the dropdown on BOTH wizards (middle hand-rolled + corner data-op) + the Plane floor + the Suggest. Screenshots each mode both wizards; no inert option; Blocks round-trip per mode; goldens Hop/Plane diff-verified.
- **B2b-3 (the guardrail, the advisor's named gate-back candidate):** the pre-flight through-stock class + assert planted-flags + clean-when-clear.
- No release until all 3 land (the advisor's bundle rule holds — middle's B2a behavior ships WITH the dropdown to steer it).
...
## 🔨 turn 917 — TURN B2b-2 (MIDDLE clearance completion + retire Safe-Z): GROUNDED, then SPLIT per your "GROUND-FIRST and SPLIT if genuinely 2 turns" — it is genuinely 3 clean sub-turns. B2b-1b accepted (V4.1 cap confirmed). No code this turn (grounding + the de-risked split, exactly as B2b itself split well). Every touchpoint below has a file:line so each sub-turn is a clean build.

### WHAT THE GROUNDING FOUND (why it's 3, not 1)
1. **Safe Z was NOT inert — it drove the end PARK + the DM500 fallback.** middleWizard.js:240/246 `safeZParkBlock(safeZFrame, '#17', 'inc')` = the end-of-program PARK (frame-toggle: relative default `G0 Z#17` / machine `G53 Z#17`). #17 is declared (194) from safeZ (55) and also used by the DM500 work-frame degrade (dialect.safeRetract `G0 Z#17`). So retiring safeZ means the PARK becomes Max AND the DM500 #17 needs a new source.
2. **`safeZParkBlock` (the frame-toggle park) is SHARED by 4 wizards** — middle (240/246), alignment (alignmentWizard.js:164 `#19`), rotary_center (rotaryCenterWizard.js:166 `#17`), rotary_clock (rotaryClockWizard.js:134 `#17`). The ratified "the PARK always takes Max" applies to ALL 4 for consistency — but B2b-2 is scoped MIDDLE, so the others are a follow-up sweep (flag). **safez-frame.spec.js tests the ROTARY_CENTER park (not middle)** — so a MIDDLE-only park retirement does NOT touch that spec (de-risked). safe-z-retract-822.spec:101 asserts the DM500 error-handler `G90 / G0 Z#17` (pattern, not value) — holds if #17 stays declared.
3. **The DM500 park+return work-frame consistency (your also-confirm) needs DM500 work-position GROUNDING.** DM500 has NO work-DRO — only the machine DRO #864-866 (readMachine). Its Max traverse saves the MACHINE Z (#95=#866) then returns G53 Z#95 (ungrounded — machineMove TO-CONFIRM). Making the RETURN honestly work-frame needs either a work-Z save (DM500 can't easily) OR degrading DM500's traverse to a RELATIVE lift+drop (nets zero, no G53, no work-Z — the pre-B2a form). That is a per-post TRAVERSE change (not the park), so it belongs with the DM500-return sub-turn, not the park.
4. **The FORM** (hand-rolled HTML, from the earlier grounding): index.html:329 `m_safe_z` + middleView.js:104 inputIds + :137 param read + hand-rolled show/hide (:156-157). The dropdown + when-gated fields + floor + Suggest is the FORM sub-turn (its own review + screenshots each mode).

### THE DE-RISKED SPLIT (3 focused, safe turns)
- **B2b-2a (the MIDDLE PARK RETIREMENT — EMIT + goldens):** middleWizard park (240/246) -> `safeRetractNode({restore:'inc'})` (Max per-post: G53 posts -> #520 margin; DM500 -> work-frame degrade). Retire params.safeZ (55) + safeZFrame (56); keep `#17` declared (194) but SOURCE its value from the Settings margin magnitude `Math.abs(safeZMarginNeg())` (=5) — the "source from Settings, not the removed field" reconcile — so the DM500 degrade + repositionR gate still resolve #17. repositionR (146) `lift:'#17'` -> `lift:true` (machineLift already overrides). Import: drop safeZParkBlock/safeZFrameOf, add safeZMarginNeg. GOLDENS regen dump-and-compare (like t909): DIFF-VERIFY only the PARK block (G0 Z#17 -> the margin retract) + `#17=10`->`#17=5` + the error-handler label bump (park's safeRetract claims N91, error handler -> N92) change; probe/traverse/wcs UNTOUCHED. Per-post: DM500 park work-frame; G53 posts margin. safeZ/safeZFrame stay in opSchema (harmless — emit now ignores them; old-program round-trip safe). MIDDLE-ONLY (safez-frame.spec is rotary, unaffected).
- **B2b-2b (the MIDDLE FORM):** replace the m_safe_z field with the CLEARANCE dropdown (Max/Hop/Plane) + when-gated hopDist/planeZ (hand-rolled show/hide, the middleView precedent) + the PLANE not-below-stock-top FLOOR + WCS-Z-unset tooltip + the SUGGEST beside Plane Z (mirror feedSuggestWidget). Screenshots each mode VIEWED; the Safe-Z-drives-nothing complaint closed; round-trip clearMode/hopDist/planeZ.
- **B2b-2c (CORNER data-op, same model)** then **B2b-3 (through-stock pre-flight).** PLUS a small **park-sweep** item: alignment/rotary_center/rotary_clock parks -> Max too (the ratified decision, for consistency — currently frame-toggle) + the DM500 traverse-RETURN work-frame consistency (the flag; needs the DM500 relative-lift+drop degrade). Route these as you see fit.
...
## 🔨 turn 923 — TURN B2b-2b-cov THE COVERAGE FIX (the in-axis cross-over follows the clearance mode; retire TRAVERSE HEIGHT/#18): BUILT + verified. The ratified UNIFY decision: one clearance concept — the in-axis cross-over honours Max/Hop/Plane exactly like the trans-axis, closing the single-axis coverage gap (where the CLEARANCE dropdown drove NOTHING). DELIBERATELY breaks Max byte-identity for the in-axis cross-over (user-approved: Max = full-retract safe default; Hop restores the fast ~15mm).

### BUILT
- **web/wizards/middleWizard.js** — traverseOverR (the in-axis cross-over) now routes through safeTraverseStack `mode:'in-axis'` with `...clearTraverseParams(clearMode, {hopDist, planeZ})` (wrapped in a safetraverse block, like transTraverseR). The #19/#20 HORIZONTAL cross is PRESERVED (the move); the old relative `G0 Z#18`/`G0 Z-#18` lift/drop is replaced by the mode's lift + the honest save `#95` / `G53 Z#95` return. RETIRED `clearOver`/#18 (the const + the `#18` assign). The manual REPOSITION stays Max (operator-in-loop — NOT routed through the mode).
- **web/index.html** — retired the TRAVERSE HEIGHT (`m_clear`, #18) field; the X/Y CROSS-OVER distance fields (#19/#20) STAY.
- **web/wizards/views/middleView.js** — dropped `m_clear` from inputIds + the `clearOver` read + the `m_clear_block` show/hide.
- **web/blocks/dataOps/middleData.js** — retired the twin's `clearOver`→#18 binding (stale after #18 retired — the middle-data-emit wiring check would flag it, exactly like the safeZ binding at t919).

### VERIFIED
- **GOLDEN regen (dump-and-compare) DIFF-VERIFIED:** rows 0-3,5 (no in-axis cross-over) lose ONLY the `#18=15` config line. ROW 4's TWO in-axis cross-overs became `#95=#882 (@saveProbeZ)` + the #520 margin retract + `G0 X#19`/#20 cross (**PRESERVED, byte-identical**) + the honest `G53 Z#95 (@returnProbeZ)` return; the downstream safeRetract labels bump (the two cross-overs claim N91/N93, everything after shifts). Every PROBE / WCS / trans-axis-traverse / the trans-traverse's own @saveProbeZ-@returnProbeZ line is BYTE-IDENTICAL. (I printed the cross-over region to confirm `G0 X#19` sits between the margin lift and the honest return.)
- **COVERAGE GAP CLOSED (clearance-form-921 test 2, screenshot VIEWED):** a SINGLE-axis boss (m_both UNCHECKED) now moves the emit 3/3 across Max/Hop/Plane (was 1/3 identical — the dropdown drove nothing). Hop → the cross-over emits the capped `#43=[#95+15]`; Max → the machine-margin clear (no `G0 Z#18`). The X/Y CROSS-OVER distances stay; no TRAVERSE HEIGHT field.
- **PER-POST FOLD (direct emit):** the in-axis cross-over folds — Expert `#42=#520` + `G0 X#19`; V4.1 `#190` baked margin; DM500 work-frame degrade (no throw). Inherited from the shared safeRetractNode (already per-post tested).
- **Twin bindingCount 11→10** (retired clearOver→#18); middle-superset + clearance-modes-909 + clearance-hop-plane-913 green (9 targeted specs).
- **MIGRATED middle-center-sim.spec.js (a first-gate catch):** it asserted `#18` (the retired traverse height) == the operator-set value; migrated (the traverse-over height is superseded by the clearance mode) — kept the boss-auto centre-finding (X=30) + now asserts the cross-over clears to the machine margin (no `G0 Z#18`) + still crosses via `#19`. (knob-persist.spec flaked once in the gate — passes 3/3 in isolation, a load-sensitive FORM-persist UI test, not mine.)
- **SIM:** the cross-over's new G53 is trace-neutral (t856 wrap; the same pattern the trans-traverse + park already use); the #19/#20 XY cross is unchanged → the preview path is unchanged (the single-axis screenshot renders the boss probe cleanly).

### FULL GATE
- **1321 passed / 3 failed / 4 skipped (12.6m)** — the 3 failures are ALL load-sensitive UI flakes: blocks-mobile-drawers, preflight-badge-838 (the exec-highlight coexist test, recurred from t913), toolpath2d-pin — passing **12/12 in ISOLATION**. NONE relate to the middle cross-over emit/form change (all UI/mobile/2D tests). The heavier flake count reflects the session's sustained 4-worker load (many full gates run). The ONE real failure a first gate caught — middle-center-sim asserting the retired #18 — was MIGRATED + is green (2/2). Explicit failed-count is flakes-only.
- Files (git hash-object): web/wizards/middleWizard.js `5de585f` (cross-over → clearance mode + retire #18) · web/index.html `b12221e` (retire TRAVERSE HEIGHT) · web/wizards/views/middleView.js `311909c` (drop m_clear read/id/show-hide) · web/blocks/dataOps/middleData.js `c023c94` (retire twin clearOver binding) · tests/middle-reposition-refactor.spec.js `a335712` (golden regen) · tests/middle-data-emit.spec.js `672b276` (bindingCount 10) · tests/clearance-form-921.spec.js `33226e6` (single-axis coverage) · tests/middle-center-sim.spec.js `c5e6a85` (migrated). Commit hashes below.
- No release (bundle rule). No lingering procs.
...
## 🔨 turn 927 — TURN B2b-2c CORNER: GROUNDED COMPLETELY + PLAN (a ground-first GATE, per your dispatch — NO build this turn). B2b-2b-2 accepted (middle clearance COMPLETE). Corner is NOT parallel to middle: it is Max-only by HARDCODE (not config), and its safeZ is the PLUNGE geometry, not a park. So corner needs an EMIT re-route + a form addition. Grounded via an Explore agent + my own reads; the plan + 3 decision-points are below.

### GROUNDED (corner's clearance surface — file:line)
- **safeZ (#19) = PLUNGE/APPROACH geometry, a RELATIVE clearance — NOT a park.** cornerWizard.js:245-246 `A('#19', safeZ, ...); A('#20', scanDepth, ...); A('#17','[#19+#20]','Plunge depth'); A('#18','[0-#17]','Negative plunge')`. #19=safeZ, #20=scanDepth, #17=safeZ+scanDepth (the plunge), #18=−#17 (the descent, emitted only when probeZFirst ON, :351). safeZ is ALSO the RELATIVE lift for the Z→wall1 jog (:293 `lift:'#19'` → `G0 Z#19`). **KEEP safeZ — retiring it breaks the plunge + the Z→wall1 approach.** (travelDist drives #15/#16, separate.)
- **Corner HARDCODES the #520 machine margin (Max-only) — zero clearMode.** cornerWizard.js imports only `safeRetractNode, saveMachineZNode` (:20); grep for clearMode/clearTraverseParams/machineLift/safeHopNode = ZERO. The wall1→wall2 traverse crosses at the #520 MARGIN, set by the per-wall retreat lift (probeWallR :219 `saveMachineZNode('#95') + safeRetractNode({restore:'inc'})`); the repoTraverse (:318-326) passes NO lift + `returnZ:'#95'` (the honest G53 return). So the CROSS HEIGHT is entirely the hardcoded per-wall #520 lift.
- **TWO traverse heights:** (a) Z→wall1 (:289-298) = RELATIVE safeZ (`lift:'#19'`, probeZFirst-only); (b) wall1→wall2 (:318-326, dogleg/diagonal) = the #520 MARGIN (both shapes cross lifted; the diagonal adds zero Z). (The cornerData:82 safeZ help "travel between walls" is SEMANTICALLY STALE — the between-walls travel is the margin, not #19.)
- **NO end-of-program park.** Success path (:373-381) = "Corner found" + GOTO2 + N2 + M30, NO final move (the tool sits at wall-2's #520 retract). The only retract is the error handler `safeRetractNode()` (:379, the margin). No safeZParkBlock anywhere. So corner is ALREADY effectively always-Max on its lifts — no park to retire (simpler than middle t919).
- **cornerStack is the SINGLE emit source** (built-in `CornerWizard.generate` :392 + the data-op twin `cornerDataStack` :218 both call cornerStack) → an emit change lands in BOTH by construction.
- **cornerData descriptors:** CORNER_BINDING_SPECS (:73-95, #var-matched NUMBERS: safeZ→#19, scanDepth→#20, travelDist→#15, cross1_x/y→#23/#24, startX/Y→#21/#22 when-gated on probeZFirst) + CORNER_STRUCT_BINDINGS (:169-183, ENUMS/BOOLS, NO match — e.g. `travelShape` enum :175, widget:'segmented'). Native when-gating exists (:93-94 `when:{param:'probeZFirst',is:true}`).
- **opSchema: the `corner` schema (:98-102) LACKS clearMode/hopDist/planeZ** (middle has them at :95). No `user_corner_data` schema entry — the twin round-trips via its bindings.

### THE PLAN (for your review — I build after you rule the 3 decisions)
1. **EMIT — route the wall1→wall2 per-wall lift (cornerWizard :219) through the clearance MODE.** Extract a SHARED `clearLiftNodes(clearMode, {hopDist, planeZ, saveVar, restore})` in safeZframe.js: max → `[safeRetractNode({restore})]` (BYTE-IDENTICAL to today), hop → `[safeHopNode({hopDist, saveVar})]`, plane → `planeLiftNodes(planeZ)`. Use at :219: `saveMachineZNode('#95') + clearLiftNodes(clearMode, {...})`. The repoTraverse (:318-326) is UNCHANGED (it crosses at the lifted Z + returns to #95, honest for all modes). Import clearModeOf/clearLiftNodes in cornerWizard. **Max byte-identical (the #520 margin is exactly what safeRetractNode emits today); Hop/Plane are the new modes.** (Optionally refactor safeTraverseStack's doLift to also call clearLiftNodes — one clearance-lift source — but that's a separate cleanup, not required.)
2. **FORM — add the clearance dropdown + fields as data-op DESCRIPTORS (cornerData.js).** A `clearMode` enum in CORNER_STRUCT_BINDINGS (mirror travelShape :175; options Max safe height/Hop up/Clearance plane) + `hopDist`/`planeZ` when-gated numbers (`when:{param:'clearMode',is:'hop'|'plane'}`). **WRINKLE (resolve): hopDist/planeZ are NOT #var-backed** (they're literals consumed by safeHopNode/planeLiftNodes), so they CAN'T use the `match:{type:'assign',var:'#N'}` path every corner number binding uses today. RESOLUTION (my recommend): a NON-match "value" number binding that reads params.hopDist/planeZ directly (the enum bindings already have no match, so the descriptor path supports match-less bindings) — matching middle's literal approach, no spurious assign #var emitted. CONFIRM the userOpView value-binding path supports a match-less number in the build.
3. **KEEP safeZ #19** (the plunge/approach) — a DISTINCT field from the clearance mode; do NOT retire it (unlike middle's safeZ). Max byte-identical.
4. **Plane FLOOR + SUGGEST (the data-op path).** Corner renders via userOpView (NOT a hand-rolled form like middle), so the floor WARN + the Suggest need the WIDGET path: a FORM_WIDGET (mirror feedSuggestWidget in ui/formWidgets.js) for the ✨ Suggest beside planeZ + an inline warn (a when-gated note / a validator). REUSE the stock-top: EXTRACT `stockTopWorkZ()` (currently local in middleView.js) to a SHARED module (e.g. engine/workpiece.js or a viz helper) so both middle + the corner widget read ONE source. YES — corner's Plane floor needs the same stockTopWorkZ (the floor is stock-relative).
5. **opSchema — add clearMode/hopDist/planeZ to the `corner` schema (:98-102)** (mirror middle :95) so the BUILT-IN corner round-trips them; the twin round-trips via its new bindings.
6. **Goldens:** regen + diff-verify — the wall1→wall2 Max lines are BYTE-IDENTICAL (the #520 margin is unchanged); Hop/Plane appear only when the mode is set (no golden does). Per-post fold (Expert #520 / V4.1 #190 / DM500 work-frame). Screenshots each mode (corner data-op form). Round-trip. Full gate.

### 3 DECISIONS FOR YOU TO RULE (before I build)
- **(D1) The Z→wall1 traverse (:293, relative safeZ, probeZFirst-only).** Does it ALSO follow clearMode, or stay relative-safeZ? OPTION A (recommend): LEAVE it relative-safeZ — safeZ is explicitly the declared Z→wall1 APPROACH clearance (the user owns it), it's a distinct concern from the between-walls traverse, and it keeps Max byte-identical for that path. OPTION B: route it too (full "one clearance concept") — but that breaks its Max byte-identity (relative safeZ → the margin, a bigger lift) for probeZFirst rows. I lean A; the between-walls traverse (the true analog of middle's trans-traverse) is what the mode governs.
- **(D2) The hopDist/planeZ binding mechanism** (match-less value binding vs a new emitted assign #var). I recommend match-less (no spurious assign, matches middle). Confirm the descriptor path handles it, else emit a #var.
- **(D3) The floor/Suggest widget vs hand-roll.** Corner is a data-op → a FORM_WIDGET is the clean path (vs middle's hand-rolled button). Confirm you want the widget (reusable for other data-op wizards) — I'll extract stockTopWorkZ to share it.

### GATE
- NO gate run (no source change — only this WORK-LOG + grounding). Tree green at the B2b-2b-2 tip (1325/0 clean). Rule D1-D3 and I build B2b-2c. THEN B2b-3 (through-stock pre-flight — the runtime catch the Plane floor points at; Plane ships WITH it). No release until after B2b-3 (a clean full gate at the hold point). FLAGGED fast-follows still open: the park-sweep (alignment/rotary → Max) + the DM500 traverse-return consistency.

## 🔨 turn 929 — TURN B2b-2c (corner, split emit+dropdown per your license): the EMIT is BUILT + verified; the DATA-OP FORM surfaced a DEEPER structure than the accepted plan (a M2-template fork) — GATED for your ruling. So this turn delivers the corner clearance-mode EMIT (built-in cornerStack honours Max/Hop/Plane, Max byte-identical, per-post) + a discovery + a 2-option fork on the FORM. Rulings applied: D1=A (Z→wall1 stays safeZ), the wall2-final-stays-Max standing split, per-post fold.

### BUILT + VERIFIED (the EMIT — the concrete built-in path)
- **web/wizards/ops/safeZframe.js** — `clearLiftNodes(mode,{hopDist,planeZ,saveVar,restore})`: the shared DECLARE for an OUT-OF-BAND lift (corner's per-wall retreat, where the lift is not inside safeTraverseStack). max → `[safeRetractNode({restore})]` (BYTE-IDENTICAL to today), hop → `[safeHopNode]`, plane → `planeLiftNodes`.
- **web/wizards/cornerWizard.js** — read clearMode/hopDist/planeZ; probeWallR takes a `liftMode`; the per-wall retreat (:223) now `...clearLiftNodes(liftMode, {...})`. WALL1 (:356, between-walls, followed by repoTraverse) passes `clearMode`; WALL2 (final retract) defaults `'max'` — the STANDING SPLIT (final parks = always max). The repoTraverse (returnZ #95) is unchanged. safeZ (#19) = the PLUNGE/approach, UNCHANGED. Fixed the stale cornerData:82 help note is DEFERRED to the form turn.
- **web/blocks/dataOps/cornerData.js** — CORNER_DEFAULTS += clearMode:'max', hopDist:15, planeZ:10 (byte-identical — default max).
- VERIFIED (tests/corner-clearance-emit-929.spec.js, passes): Max byte-identical (clearMode:max === default === today); Hop → wall1 the capped `#43=[#95+15]` (EXACTLY 1 hop block — wall2 is the final Max), wall2+error stay #520, the honest `G53 Z#95` return preserved; Plane → wall1 `G0 Z13`, wall2+error Max; per-post fold Expert #42/#520, V4.1 #191/#190, DM500 no-throw. Plus the 12 existing corner tests (corner-data-emit twin byte-parity + corner-draw-anchor byte-parity + corner-data-baked-frontier) GREEN — the built-in + the data-op twin are byte-identical for Max.

### THE DISCOVERY (why the FORM is NOT just descriptors — a fork for you)
- The accepted plan said "add clearMode enum + when-gated hopDist/planeZ as data-op DESCRIPTORS." Building it, I found corner's data-op is an **M2 FIXED TEMPLATE** (cornerDataStack(CORNER_DEFAULTS), instantiated + repruned): a param changes the emit ONLY IF it (a) REPRUNES a structural guard, or (b) flows via a #var value socket. **clearMode changes the LIFT STRUCTURE** (safeRetractNode vs safeHopNode vs planeLiftNodes = DIFFERENT blocks) — so it CANNOT flow as a plain value binding; the frozen template would stay Max. And hopDist/planeZ are LITERALS (baked into `#43=[#95+hopDist]` / `G0 Z<planeZ>`), matching NEITHER CORNER_BINDING_SPECS (#var-matched, needs an assign) NOR CORNER_STRUCT_BINDINGS (structural, each derives a Blocks-tab `sc_` control — wrong for a value/number). So the data-op form needs one of:
  - **OPTION A — clearMode as a SUPERSET STRUCTURAL FORK.** cornerStack's superset emits ALL 3 lift arms guarded by clearMode; prune selects one (the wizards-as-data pattern corner already uses for travelShape). clearMode → CORNER_STRUCT_BINDINGS (structural, reprunes). hopDist/planeZ → #var sockets inside the hop/plane arms (an assign each, so they round-trip via CORNER_BINDING_SPECS). COST: 3 guarded lift arms in the superset (the hop arm carries the safehop atom's labels); hopDist/planeZ become emitted assign #vars (a small deviation from middle's pure literals, but it's how corner's data-op values round-trip).
  - **OPTION B — a `clearlift` FOLDING ATOM.** ONE leaf atom (params clearMode/hopDist/planeZ/saveVar/labels) whose EMIT dispatches to the max/hop/plane lift lines (folds like the dialect atoms fold per-post) — so the BLOCK STRUCTURE is FIXED (one clearlift block), clearMode/hopDist/planeZ are its VALUE params (round-trip + re-emit, NO structural fork, NO spurious assign — matches middle's literals). COST: a new atom + extend uniquifySafeRetractLabels to give a clearlift(hop) its 2 labels; it REPLACES clearLiftNodes' node-spreading (a cleaner shared DECLARE that middle could adopt too). This is the cleanest data-op fit (clearMode is a value, the M2 template re-emits it) and honours D2 (no assign #var).
- **I LEAN OPTION B** (the folding atom): it fits the M2 template naturally (clearMode as a value, no structural fork), honours D2 (literals, no assign #var), unifies corner+middle on one clearlift DECLARE, and round-trips cleanly. Option A works but adds a 3-arm superset fork + emitted hopDist/planeZ assigns (deviating from middle). Your call — it decides the emit block-shape (the emitted G-code is byte-identical either way for a given mode; only the block structure + round-trip differ). NOTE: if B, I'll refactor the L223 emit from clearLiftNodes(nodes) to the clearlift atom (byte-identical G-code) as part of the form turn.

### FULL GATE
- **1326 passed / 0 failed / 4 skipped (12.4m), GATE_EXIT=0** — CLEAN (explicit failed-count grep = 0; no flakes; +1 = the new corner-clearance-emit-929 spec). The corner EMIT is byte-safe (all corner byte-parity + twin tests green).
- Files (git hash-object): web/wizards/ops/safeZframe.js `f9a2552` (clearLiftNodes) · web/wizards/cornerWizard.js `356012b` (route wall1 lift, wall2 Max) · web/blocks/dataOps/cornerData.js `916fe42` (CORNER_DEFAULTS clearMode) · tests/corner-clearance-emit-929.spec.js `a2a5b9b` (NEW). Commit hashes below.
- No release (bundle rule — the corner FORM + B2b-3 remain). No lingering procs.
...
## t1347 — the switch: NOT STARTED, deliberately, and handed back intact

The dispatch is right, the envelope is earned, and the tree is clean and green at 9435c527. I am declining to
BEGIN this one rather than splitting it, which is a different thing from every previous split in this arc and worth
saying plainly.

The switch is the largest single change in the arc and the only irreversible one: re-pointing `surfacingStack`
collapses two template blocks into one, which moves every flat binding index the twin, the CAM seed, the form and
the round-trip all read. It has to land as ONE coherent act — bindings regenerated by identity, the generator
retired, the old emitter dead only after the both-paths guard, pendant mirrors proven stable, six register items,
the iron rule restated and four screenshots — and it is emit-class, with the advisor's full suite as its merge gate.

A change of that shape cannot be left half-applied. Half of it is a repository where the wizard, the twin and the
CAM slot disagree about what a surfacing op is, with a green fast tier that would not say so.

I do not have the working room left to carry it end to end, so the responsible act is to hand it back UNSTARTED on
a clean tree rather than to open it and stop somewhere in the middle. Everything the switch needs is in place and
proven: the atom covers every config the wizard can emit, the envelope is asserted empty, the bridges are green at
32/32, and the ledger entry records the one stated exception to the exactness claim.

The next session's worker starts it with a full budget, on green, against a proven emit. That is the best position
this arc can hand it over in — and it is the same judgement the arc has used throughout, applied to my own capacity
